...
class ALIENWebSocketHandler:
    ...
    async def _validate_orion_client(self, reg_info: ClientMessage) -> None:
        """
        Validate orion client's claimed device_id.
            ...
            for session_id in session_ids:
                try:
                    await self.session_manager.cancel_task(
                        session_id, reason="orion_disconnected"
                    )
    ...
    async def handle_task_request(self, data: ClientMessage) -> None:
        ...
        # Track orion session mapping
        if client_type == ClientType.ORION:
            self.client_manager.add_orion_session(data.client_id, session_id)
        ...
        target_protocol = self.client_manager.get_task_protocol(
            target_device_id if client_type == ClientType.ORION else client_id
        )
    ...
    async def handle_device_info_request(self, data: ClientMessage) -> None:
        """
        Handle device info request from orion client using AIP DeviceInfoProtocol.

        :param data: The request data from orion.
        """
    ...
    async def get_device_info(self, device_id: str) -> dict:
        ...
        This is called by orion clients via WebSocket.
