...
def parse_args():
    parser = argparse.ArgumentParser(description="ALIEN Server")
...
# Setup logger before importing other ALIEN modules
if cli_args:
...
from alien.server.services.api import create_api_router
...
ws_handler = ALIENWebSocketHandler(client_manager, session_manager, cli_args.local)
...
if __name__ == "__main__":
    ...
    logger.info(f"Starting ALIEN Server on {cli_args.host}:{cli_args.port}")
