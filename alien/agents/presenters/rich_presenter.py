...
if TYPE_CHECKING:
    ...
    from network.agents.schema import OrionAgentResponse
...
class RichPresenter(BasePresenter):
    ...
    # ============================================================================
    # OrionAgent-specific presentation methods
    # ============================================================================

    def present_orion_agent_response(
        self, response: "OrionAgentResponse", print_action: bool = False
    ) -> None:
        """
        Present OrionAgent response - matches original OrionAgent.print_response logic.

        :param response: OrionAgentResponse object
        :param print_action: Whether to print actions
        """
        # Print response header
        self._print_response_header("OrionAgent")
        ...
        # Orion (if available)
        if response.orion:
            self._present_orion_info(response.orion)
    ...
    def _present_orion_info(self, orion: Any) -> None:
        """
        Present orion information.

        :param orion: TaskOrion object
        """
        orion_name = (
            orion.name or f"Orion {orion.orion_id}"
        )
        ...
        self.console.print(
            Panel(
                orion_info,
                title=self.STYLES["orion_info"]["title"],
                style=self.STYLES["orion_info"]["style"],
            )
        )
        ...
        if orion.dependencies:
            deps_text = Text()
            for line_id, dependency in orion.dependencies.items():
    ...
    def present_orion_editing_actions(self, actions: Any) -> None:
        """
        Present orion editing actions - matches OrionEditingActionExecutionStrategy.print_actions logic.
        ...
        for idx, action in enumerate(actions.actions, 1):
            self._print_single_orion_action(idx, action)

        # Display summary
        self._print_orion_summary(success_count, failed_count, actions.status)
        self.console.print()

    def _print_single_orion_action(self, idx: int, action: Any) -> None:
        """Print a single orion editing action in compact format."""
        from rich.table import Table
        from rich.text import Text
        from aip.messages import ResultStatus
        ...
        operation = self._format_orion_operation(action)
    ...
    def _format_orion_operation(self, action: Any) -> str:
        """
        Format orion operation into human-readable text.
        ...
        elif function == "build_orion":
            config = args.get("config", {})
            if isinstance(config, dict):
                task_count = len(config.get("tasks", []))
                dep_count = len(config.get("dependencies", []))
                return f"Build Orion ({task_count} tasks, {dep_count} dependencies)"
            return "Build Orion"

        elif function == "clear_orion":
            return "Clear Orion (remove all tasks)"

        elif function == "load_orion":
            file_path = args.get("file_path", "?")
            import os

            filename = os.path.basename(file_path) if file_path else "?"
            return f"Load Orion from '{filename}'"

        elif function == "save_orion":
            file_path = args.get("file_path", "?")
            import os

            filename = os.path.basename(file_path) if file_path else "?"
            return f"Save Orion to '{filename}'"

        else:
            # Fallback for unknown operations
            return f"{function}({', '.join(f'{k}={v}' for k, v in list(args.items())[:2])})"

    def _print_orion_summary(
        self, success_count: int, failed_count: int, status: str
    ) -> None:
        """Print summary of orion editing actions."""
        from rich.panel import Panel
