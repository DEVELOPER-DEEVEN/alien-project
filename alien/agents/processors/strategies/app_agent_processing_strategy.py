...
from alien.agents.processors.context.app_agent_processing_context import (
...
from alien.agents.processors.schemas.log_schema import (
    AppAgentRequestLog,
    ControlInfoRecorder,
)
from alien.agents.processors.schemas.response_schema import AppAgentResponse
from alien.agents.processors.schemas.target import TargetInfo, TargetKind, TargetRegistry
from alien.agents.processors.strategies.processing_strategy import BaseProcessingStrategy
from alien.automator.ui_control.grounding.omniparser import OmniparserGrounding
from alien.automator.ui_control.screenshot import PhotographerFacade
...
from alien.llm.grounding_model.omniparser_service import OmniParser
...
CONTROL_BACKEND = alien_config.system.control_backend
...
class AppScreenshotCaptureStrategy(BaseProcessingStrategy):
    ...
    async def execute(
        ...
        try:
            ...
            if alien_config.system.save_full_screen:
            ...
            if alien_config.system.save_ui_tree:
...
class AppControlInfoStrategy(BaseProcessingStrategy):
    ...
    def __init__(self, fail_fast: bool = True) -> None:
        ...
        self.control_detection_backend = alien_config.system.control_backend
    ...
    def _init_omniparser_service(self) -> Optional[OmniparserGrounding]:
        ...
        """
        omniparser_config = alien_config.system.omniparser
        omniparser_endpoint = (
    ...
    async def _collect_merged_control_list(
        ...
        try:
            merged_control_list = self.photographer.merge_target_info_list(
                api_control_list,
                grounding_control_list,
                iou_overlap_threshold=alien_config.system.iou_threshold_for_merge,
            )
...
class AppLLMInteractionStrategy(BaseProcessingStrategy):
    ...
    def _collect_image_strings(
        ...
        if alien_config.system.include_last_screenshot:
        ...
        if alien_config.system.concat_screenshot:
            image_string_list += [
    ...
    def _knowledge_retrieval(self, agent: "AppAgent", subtask: str):
        ...
        offline_docs, online_docs = agent.external_knowledge_prompt_helper(
            subtask,
            alien_config.rag.offline_docs_retrieved_topk,
            alien_config.rag.online_retrieved_topk,
        )
    ...
    async def _build_app_prompt(
        ...
        try:
            ...
            prompt_message = agent.message_constructor(
                ...
                include_last_screenshot=alien_config.system.include_last_screenshot,
            )
...
class AppMemoryUpdateStrategy(BaseProcessingStrategy):
    ...
    def _create_additional_memory_data(
        ...
        try:
            # Build action lists
            from alien.agents.processors.app_agent_processor import (
                AppAgentProcessorContext,
            )
    ...
    def _update_blackboard(
        ...
        try:
            ...
            history_keys = alien_config.system.history_keys
            if history_keys:
