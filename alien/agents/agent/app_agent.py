...
# from alien.agents.processors.operator_processor import OpenAIOperatorProcessor
from alien.agents.processors.core.processor_framework import ProcessorTemplate
from alien.agents.processors.schemas.response_schema import AppAgentResponse
from alien.agents.states.app_agent_state import AppAgentStatus, ContinueAppAgentState
from alien.agents.states.operator_state import ContinueOpenAIOperatorState
    ...
    def demonstration_prompt_helper(self, request) -> Tuple[List[Dict[str, Any]]]:
        ...
        if alien_config.rag.experience:
            experience_results = self.rag_experience_retrieve(
                request, alien_config.rag.experience_retrieved_topk
            )
        else:
            experience_results = []
        ...
        if alien_config.rag.demonstration:
            demonstration_results = self.rag_demonstration_retrieve(
                request, alien_config.rag.demonstration_retrieved_topk
            )
        else:
