...
class Trajectory:
    ...
    def load_screenshot(screenshot_path: str) -> Image.Image:
        ...
        if os.path.exists(screenshot_path):
            image = alien.utils.load_image(screenshot_path)
        else:
            image = None
        return image
    ...
    def to_markdown(
        ...
        if len(self.step_log) == 0:
            ...
            with open(output_path, "w", encoding="utf-8") as file:
                ...
                file.write(
                    "- The ALIEN session was interrupted before any actions were completed\n"
                )
                file.write("- The response.log file is corrupted or empty\n")
                file.write("- The ALIEN session failed to start properly\n\n")
                file.write(
                    "To fix this, try running ALIEN again and ensure it completes successfully.\n"
                )
            return
