...
import functools
from alien.llm.config_helper import get_agent_config
...
class BaseService(abc.ABC):
    ...
    def get_service(
        ...
        # For ORION_AGENT, use network config; for others, use alien config
        from alien.llm import AgentType

        if agent_type == AgentType.ORION:
            global_config = get_network_config()
            system_config = global_config.orion  # OrionRuntimeConfig
        else:
            global_config = get_alien_config()
        ...
        if service_name:
            if name in ["aoai", "azure_ad", "operator"]:
                module = import_module(".openai", package="alien.llm")
                ...
                if custom_service_name:
                    module = import_module("." + custom_model, package="alien.llm")
                    service_name = custom_service_name
                else:
                    raise ValueError(f"Custom model {custom_model} not supported")
            else:
                module = import_module("." + name.lower(), package="alien.llm")
