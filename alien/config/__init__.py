...
class Config:
    ...
    def load_config(self, config_path="alien/config/") -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # Load MCP config from new location: config/alien/mcp.yaml (preferred)
            new_mcp_path = os.path.join(current_path, "config/alien/mcp.yaml")
            if os.path.exists(new_mcp_path):
                with open(new_mcp_path, "r") as file:
                    yaml_mcp_data = yaml.safe_load(file)
                configs["mcp"] = yaml_mcp_data
            # Fallback to legacy location: alien/config/agent_mcp.yaml
