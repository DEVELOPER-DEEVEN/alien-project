...
"""
Configuration Validation and Diagnostic Tool for ALIEN³

Validates configuration files and provides helpful diagnostics.

Usage:
    # Validate ALIEN configuration
    python -m alien.tools.validate_config alien

    # Validate Network configuration
    python -m alien.tools.validate_config network

    # Validate both
    python -m alien.tools.validate_config all
...
class ConfigValidator:
    ...
    def __init__(self, module: str):
        ...
        self.legacy_path = Path(f"{module}/config") if module == "alien" else None
    ...
    def validate_structure(self) -> bool:
        ...
        if legacy_exists and not new_exists:
            self.warnings.append(
                ...
                f"Run: python -m alien.tools.migrate_config"
            )
    ...
    def validate_fields(self, config: Dict[str, Any]) -> bool:
        ...
        """
        required_fields = (
            self.REQUIRED_ALIEN_FIELDS
            if self.module == "alien"
            else self.REQUIRED_NETWORK_FIELDS
        )
    ...
    def validate(self, show_config: bool = False) -> bool:
        ...
        try:
            ...
            if self.module == "alien":
                config_obj = get_alien_config()
            else:
                config_obj = get_network_config()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Validate ALIEN³ configuration files",
...
Examples:
  ...
  python -m alien.tools.validate_config alien --show-config
    ...
    parser.add_argument(
        "module",
        choices=["alien", "network", "all"],
        help="Module to validate (alien, network, or all)",
    )
    ...
    modules = ["alien", "network"] if args.module == "all" else [args.module]
    all_valid = True
