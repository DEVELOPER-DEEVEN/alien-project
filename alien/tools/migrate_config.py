...
"""
Configuration Migration Tool for ALIEN³

Migrates legacy configuration from alien/config/ to the new config/alien/ structure.
...
Usage:
    ...
    python -m alien.tools.migrate_config --backup

    # Force migration (skip confirmation)
    python -m alien.tools.migrate_config --force

    # Custom paths
    python -m alien.tools.migrate_config --legacy-path alien/config --new-path config/alien
"""
    ...
    def show_next_steps(self) -> None:
        ...
        panel = Panel.fit(
            ...
            + (
                ...
                f"   [cyan]cp -r {self.backup_path}/* alien/config/[/cyan]\n\n"
                if self.backup_path
                else ""
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Migrate ALIEN configuration from legacy to modern structure",
