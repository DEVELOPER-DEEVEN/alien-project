...
from alien.agents.agent.app_agent import OpenAIOperatorAgent
from alien.agents.agent.host_agent import AgentFactory
from alien.agents.states.app_agent_state import ContinueAppAgentState
from alien.agents.states.host_agent_state import ContinueHostAgentState
...
from alien.module import interactor
from alien.module.basic import BaseRound
from alien.module.context import ContextNames
from alien.module.dispatcher import LocalCommandDispatcher
from alien.module.sessions.plan_reader import PlanReader
...
class Session(WindowsBaseSession):
    ...
    def __init__(
        ...
        :param request: The user request of the session, optional. If not provided, ALIEN will ask the user to input the request.
    ...
    async def run(self) -> None:
        ...
        save_experience = alien_config.system.save_experience
    ...
    def create_new_round(self) -> Optional[BaseRound]:
        ...
        round = BaseRound(
            ...
            should_evaluate=alien_config.system.eva_round,
...
class FromFileSession(WindowsBaseSession):
    """
    A session for ALIEN from files on Windows.
    """
    ...
    def record_task_done(self) -> None:
        ...
        """
        is_record = alien_config.system.task_status
        if is_record:
            file_path = alien_config.system.get(
                "TASK_STATUS_FILE",
