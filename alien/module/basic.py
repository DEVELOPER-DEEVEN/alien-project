...
"""
This module contains the basic classes of Round and Session for the ALIEN system.

A round of a session in ALIEN manages a single user request and consists of multiple steps.

A session may consists of multiple rounds of interactions.

The session is the core class of ALIEN. It manages the state transition and handles the different states, using the state pattern.
...
from alien.agents.agent.evaluation_agent import EvaluationAgent
from alien.agents.agent.host_agent import HostAgent
from alien.agents.states.basic import AgentState, AgentStatus
from config.config_loader import get_alien_config
from aip.messages import Command
from alien.experience.summarizer import ExperienceSummarizer
from alien.module.context import Context, ContextNames
from alien.trajectory.parser import Trajectory
...
class BaseRound(ABC):
    ...
    async def run(self) -> None:
        ...
        while not self.is_finished():
            ...
            if self.state.is_subtask_end():
                time.sleep(alien_config.system.sleep_time)
    ...
    def is_finished(self) -> bool:
        ...
        return (
            ...
            or self.context.get(ContextNames.SESSION_STEP) >= alien_config.system.max_step
        )
...
class BaseSession(ABC):
    """
    A basic session in ALIEN. A session consists of multiple rounds of interactions and conversations.
    """
    ...
    async def run(self) -> List[Dict[str, str]]:
        ...
        if alien_config.system.log_to_markdown:
    ...
    def experience_saver(self) -> None:
        ...
        summarizer = ExperienceSummarizer(
            alien_config.app_agent.visual_mode,
            alien_config.system.EXPERIENCE_PROMPT,
            alien_config.system.APPAGENT_EXAMPLE_PROMPT,
            alien_config.system.API_PROMPT,
        )
        experience = summarizer.read_logs(self.log_path)
        summaries, cost = summarizer.get_summary_list(experience)

        experience_path = alien_config.system.EXPERIENCE_SAVED_PATH
    ...
    def is_finished(self) -> bool:
        ...
        if (
            self._finish
            or self.step >= alien_config.system.max_step
            or self.total_rounds >= alien_config.system.max_round
        ):
            return True
    ...
    def evaluation(self) -> None:
        ...
        evaluator = EvaluationAgent(
            name="eva_agent",
            is_visual=is_visual,
            main_prompt=alien_config.system.EVALUATION_PROMPT,
            example_prompt="",
        )
        ...
        try:
            result, cost = evaluator.evaluate(
                ...
                eva_all_screenshots=alien_config.system.eva_all_screenshots,
    ...
    def initialize_logger(log_path: str, log_filename: str, mode="a") -> logging.Logger:
        ...
        if not alien_config.system.print_log:
        ...
        logger.setLevel(alien_config.system.log_level)
