...
from alien.module.basic import BaseSession
from alien.module.sessions.session import (
    FollowerSession,
    FromFileSession,
    OpenAIOperatorSession,
    Session,
)
from alien.module.sessions.service_session import ServiceSession

if TYPE_CHECKING:
    from aip.protocol.task_execution import TaskExecutionProtocol
from alien.module.sessions.linux_session import LinuxSession, LinuxServiceSession
from alien.module.sessions.mobile_session import MobileSession, MobileServiceSession
...
class SessionPool:
    """
    The manager for the ALIEN clients.
    """

    def __init__(self, session_list: List[BaseSession]) -> None:
        """
        Initialize a batch ALIEN client.
        """
...
class SessionFactory:
    ...
    def _create_windows_session(
        ...
        if mode in ["normal", "normal_operator"]:
            ...
            return [
                Session(
                    task,
                    alien_config.system.eva_session,
                    id=kwargs.get("id", 0),
                    request=request,
                    mode=mode,
                )
            ...
            return [
                ServiceSession(
                    task=task,
                    should_evaluate=alien_config.system.eva_session,
                    id=kwargs.get("id", 0),
                    request=request,
            ...
            if self.is_folder(plan):
                ...
                return [
                    FollowerSession(task, plan, alien_config.system.eva_session, id=0)
                ]
            ...
            if self.is_folder(plan):
                ...
                return [
                    FromFileSession(task, plan, alien_config.system.eva_session, id=0)
                ]
        elif mode == "operator":
            self.logger.info(f"Creating a Windows operator session for mode: {mode}")
            return [
                OpenAIOperatorSession(
                    task, alien_config.system.eva_session, id=0, request=request
                )
            ]
        else:
    ...
    def create_sessions_in_batch(self, task: str, plan: str) -> List[BaseSession]:
        ...
        is_done_files = []
        if is_record:
            file_path = alien_config.system.task_status_file or os.path.join(
