...
if TYPE_CHECKING:
    from Alien.agents.processors.schemas.response_schema import (
        AppAgentResponse,
        HostAgentResponse,
        EvaluationAgentResponse,
    )
    from cluster.agents.schema import networkAgentResponse
...
class RichPresenter(BasePresenter):
    ...
    # ============================================================================
    # networkAgent-specific presentation methods
    # ============================================================================

    def present_network_agent_response(
        self, response: "networkAgentResponse", print_action: bool = False
    ) -> None:
        """
        Present networkAgent response - matches original networkAgent.print_response logic.

        :param response: networkAgentResponse object
        :param print_action: Whether to print actions
        """
        # Print response header
        self._print_response_header("networkAgent")
        ...
        if response.network:
            self._present_network_info(response.network)
    ...
    def _present_network_info(self, network: Any) -> None:
        """
        Present network information.

        :param network: Tasknetwork object
        """
        network_name = (
            network.name or f"network {network.network_id}"
        )
        ...
        network_info = Text()
        network_info.append(f"🆔 ID: ", style="bold cyan")
        network_info.append(f"{network.network_id}\n", style="white")
        network_info.append(f"🌟 Name: ", style="bold cyan")
        network_info.append(f"{network_name}\n", style="white")
        network_info.append(f"📊 State: ", style="bold cyan")
        network_info.append(f"{network_state}\n", style="white")
        network_info.append(f"📋 Tasks: ", style="bold cyan")
        network_info.append(f"{task_count}\n", style="white")
        network_info.append(f"🔗 Dependencies: ", style="bold cyan")
        network_info.append(f"{dependency_count}", style="white")

        self.console.print(
            Panel(
                network_info,
                title=self.STYLES["network_info"]["title"],
                style=self.STYLES["network_info"]["style"],
            )
        )
        ...
        if network.dependencies:
            deps_text = Text()
            for line_id, dependency in network.dependencies.items():
    ...
    def present_network_editing_actions(self, actions: Any) -> None:
        """
        Present network editing actions - matches networkEditingActionExecutionStrategy.print_actions logic.
        ...
        # Create header
        header = Text()
        header.append("🔧 network Editing Operations", style="bold cyan")
        header.append(
        ...
        for idx, action in enumerate(actions.actions, 1):
            self._print_single_network_action(idx, action)

        # Display summary
        self._print_network_summary(success_count, failed_count, actions.status)
        self.console.print()

    def _print_single_network_action(self, idx: int, action: Any) -> None:
        """Print a single network editing action in compact format."""
        from rich.table import Table
        from rich.text import Text
        from aip.messages import ResultStatus
        ...
        operation = self._format_network_operation(action)
    ...
    def _format_network_operation(self, action: Any) -> str:
        """
        Format network operation into human-readable text.
        ...
        elif function == "build_network":
            config = args.get("config", {})
            if isinstance(config, dict):
                task_count = len(config.get("tasks", []))
                dep_count = len(config.get("dependencies", []))
                return f"Build network ({task_count} tasks, {dep_count} dependencies)"
            return "Build network"

        elif function == "clear_network":
            return "Clear network (remove all tasks)"

        elif function == "load_network":
            file_path = args.get("file_path", "?")
            import os

            filename = os.path.basename(file_path) if file_path else "?"
            return f"Load network from '{filename}'"

        elif function == "save_network":
            file_path = args.get("file_path", "?")
            import os

            filename = os.path.basename(file_path) if file_path else "?"
            return f"Save network to '{filename}'"
    ...
    def _print_network_summary(
        ...
        """Print summary of network editing actions."""
        from rich.panel import Panel
