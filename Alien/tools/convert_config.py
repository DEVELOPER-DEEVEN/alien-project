...
"""
Configuration Conversion Tool for Alien²

Converts legacy monolithic configuration (Alien/config/*.yaml) to the new modular
structure (config/Alien/*.yaml) with proper format transformation.
...
Usage:
    ...
    # Custom paths
    python -m Alien.tools.convert_config --legacy-path Alien/config --new-path config/Alien
...
class ConfigConverter:
    """
    Converts legacy Alien configuration to new modular structure.
    ...
    FIELD_MAPPING = {
        ...
        "system.yaml": [
            ...
            "WORD_API_PROMPT",
            "EXCEL_API_PROMPT",
            "network_CREATION_PROMPT",
            "network_EDITING_PROMPT",
            "network_CREATION_EXAMPLE_PROMPT",
            "network_EDITING_EXAMPLE_PROMPT",
    ...
    def __init__(
        self,
        legacy_path: str = "Alien/config",
        new_path: str = "config/Alien",
        backup: bool = True,
    ):
        """
    ...
    def show_next_steps(self) -> None:
        ...
        panel = Panel.fit(
            ...
            "1. Review the converted files in config/Alien/\n\n"
            "2. Copy agents.yaml.template to agents.yaml and fill in your API keys:\n"
            "   [cyan]cp config/Alien/agents.yaml.template config/Alien/agents.yaml[/cyan]\n\n"
            "3. Update agents.yaml with your actual credentials\n\n"
            "4. Test the new configuration:\n"
            "   [cyan]python -m Alien --task test[/cyan]\n\n"
            "5. Once verified, you can remove legacy configs:\n"
            "   [cyan]rm -rf Alien/config/*.yaml[/cyan]\n\n"
            + (
                f"6. If needed, restore from backup:\n"
                f"   [cyan]cp -r {self.backup_path}/* config/Alien/[/cyan]\n\n"
                if self.backup_path
                else ""
            )
    ...
    def run(self, dry_run: bool = False, force: bool = False) -> bool:
        ...
        console.print(
            Panel.fit(
                "[bold blue]Alien² Configuration Conversion Tool[/bold blue]\n"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Convert Alien configuration from legacy to modern modular structure",
...
Examples:
  ...
  python -m Alien.tools.convert_config --legacy-path Alien/config --new-path config/Alien
    ...
    parser.add_argument(
        "--legacy-path",
        default="Alien/config",
        help="Legacy configuration path (default: Alien/config)",
    )

    parser.add_argument(
        "--new-path",
        default="config/Alien",
        help="New configuration path (default: config/Alien)",
    )
