...
class BaseService(abc.ABC):
    ...
    def get_service(
        ...
        # For network_AGENT, use cluster config; for others, use Alien config
        from Alien.llm import AgentType

        if agent_type == AgentType.network:
            global_config = get_cluster_config()
            system_config = global_config.network  # networkRuntimeConfig
        else:
