...
from Alien.server.services.client_connection_manager import ClientConnectionManager


class AlienWebSocketHandler:
    """
    Handles WebSocket connections for the Alien server.
    ...
    async def _validate_network_client(self, reg_info: ClientMessage) -> None:
        """
        Validate network client's claimed device_id.
    ...
    def _log_client_connection(self, client_id: str, client_type: ClientType) -> None:
        ...
        """
        if client_type == ClientType.network:
            self.logger.info(f"[WS] 🌟 network client {client_id} connected")
        else:
            ...
            for session_id in session_ids:
                try:
                    await self.session_manager.cancel_task(
                        session_id, reason="network_disconnected"
                    )
    ...
    async def handle_message(self, msg: str) -> None:
        ...
        :param client_type: The type of client ("device" or "network").
        """
        import traceback
        ...
        try:
            ...
            if client_type == ClientType.network:
                self.logger.debug(
                    f"[WS] 🌟 Handling network message from {client_id}, type: {data.type}"
                )
            else:
                self.logger.debug(
            ...
            if msg_type == ClientMessageType.TASK:
                ...
                # network requesting device info
    ...
    async def handle_task_request(self, data: ClientMessage) -> None:
        ...
        if client_type == ClientType.network:
            ...
            self.logger.info(
                f"[WS] 🌟 Handling network task request: {data.request} from {data.target_id}"
            )
        ...
        # Track network session mapping
        if client_type == ClientType.network:
            self.client_manager.add_network_session(data.client_id, session_id)
        ...
        async def send_result(sid: str, result_msg: ServerMessage):
            ...
            try:
                # Send to requesting client (network or device) using AIP
                self.logger.info(
                ...
                # If network client, also notify the target device
        ...
        target_protocol = self.client_manager.get_task_protocol(
            target_device_id if client_type == ClientType.network else client_id
        )
    ...
    async def handle_device_info_request(self, data: ClientMessage) -> None:
        """
        Handle device info request from network client using AIP DeviceInfoProtocol.

        :param data: The request data from network.
        """
        device_id = data.target_id
        request_id = data.request_id

        self.logger.info(
            f"[WS] [AIP] 🌟 network requesting device info for {device_id}"
        )
        ...
        self.logger.info(
            f"[WS] [AIP] 📤 Sent device info response for {device_id} to network"
        )

    async def get_device_info(self, device_id: str) -> dict:
        """
        Get device system information for a specific device.
        This is called by network clients via WebSocket.
