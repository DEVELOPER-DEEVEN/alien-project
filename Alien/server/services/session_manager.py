...
class SessionManager:
    """
    This class manages active sessions for the Alien service.
    ...
    async def cancel_task(
        self, session_id: str, reason: str = "network_disconnected"
    ) -> bool:
        """
        ...
        :param reason: Reason for cancellation. Options:
                      - "network_disconnected": network client disconnected (don't send callback)
                      - "device_disconnected": Target device disconnected (send callback to network)
    ...
    async def _run_session_background(
        ...
        try:
            ...
            cancellation_reason = self._cancellation_reasons.get(
                session_id, "network_disconnected"
            )
            ...
            if cancellation_reason == "device_disconnected":
                ...
                was_cancelled = False  # Still send callback to network
            else:  # network_disconnected or other reasons
