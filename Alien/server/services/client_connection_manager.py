...
class ClientConnectionManager:
    ...
    Responsibilities:
    - Track connected clients (devices and networks) with their AIP protocol instances
    - Manage session-to-client mappings for both network and device sessions
    - Store and retrieve device system information and configurations
    - Provide client registry and lookup services

    Supports both device clients and network clients.
    """
    ...
    def __init__(self, device_config_path: Optional[str] = None):
        ...
        # Track network client -> session_ids mapping
        self._network_sessions: Dict[str, List[str]] = {}

        # Track device -> session_ids mapping (for network tasks targeting this device)
    ...
    def add_network_session(self, client_id: str, session_id: str):
        """
        Track a session started by a network client.

        :param client_id: The network client ID.
    ...
    def get_network_sessions(self, client_id: str) -> List[str]:
        """
        Get all session IDs associated with a network client.
    ...
    def remove_network_sessions(self, client_id: str) -> List[str]:
        """
        Remove and return all sessions for a network client.
    ...
    def get_client_type(self, client_id: str) -> str:
        ...
        :param client_id: The ID of the client.
        :return: The client type ("device" or "network") or None if not found.
        """
        with self.lock:
            client_info = self.online_clients.get(client_id)
    ...
    def list_clients_by_type(self, client_type: ClientType) -> List[str]:
        ...
        :param client_type: The type of clients to list ("device" or "network").
