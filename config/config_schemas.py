...
class SystemConfig:
    ...
    # ========== API Usage ==========
    use_apis: bool = True
    api_prompt: str = "Alien/prompts/share/base/api.yaml"

    # ========== MCP (Model Context Protocol) ==========
    use_mcp: bool = True
    mcp_servers_config: str = "config/Alien/mcp.yaml"
    mcp_preferred_apps: List[str] = field(default_factory=list)
    mcp_fallback_to_ui: bool = True
    mcp_instructions_path: str = "Alien/config/mcp_instructions"
    ...
    # ========== Prompt Paths ==========
    hostagent_prompt: str = "Alien/prompts/share/base/host_agent.yaml"
    appagent_prompt: str = "Alien/prompts/share/base/app_agent.yaml"
    followeragent_prompt: str = "Alien/prompts/share/base/app_agent.yaml"
    evaluation_prompt: str = "Alien/prompts/evaluation/evaluate.yaml"
    hostagent_example_prompt: str = (
        "Alien/prompts/examples/{mode}/host_agent_example.yaml"
    )
    appagent_example_prompt: str = "Alien/prompts/examples/{mode}/app_agent_example.yaml"
    appagent_example_prompt_as: str = (
        "Alien/prompts/examples/{mode}/app_agent_example_as.yaml"
    )

    # ========== API and App-specific Prompts ==========
    app_api_prompt_address: Dict[str, str] = field(default_factory=dict)
    word_api_prompt: str = "Alien/prompts/apps/word/api.yaml"
    excel_api_prompt: str = "Alien/prompts/apps/excel/api.yaml"

    # ========== network Prompts ==========
    network_creation_prompt: str = (
        "cluster/prompts/network/share/network_creation.yaml"
    )
    network_editing_prompt: str = (
        "cluster/prompts/network/share/network_editing.yaml"
    )
    network_creation_example_prompt: str = (
        "cluster/prompts/network/examples/network_creation_example.yaml"
    )
    network_editing_example_prompt: str = (
        "cluster/prompts/network/examples/network_editing_example.yaml"
    )
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "SystemConfig":
        ...
        known_mappings = {
            ...
            # network Prompts
            "network_CREATION_PROMPT": "network_creation_prompt",
            "network_EDITING_PROMPT": "network_editing_prompt",
            "network_CREATION_EXAMPLE_PROMPT": "network_creation_example_prompt",
            "network_EDITING_EXAMPLE_PROMPT": "network_editing_example_prompt",
            # Third-Party Agents
...
class AlienConfig:
    """
    Complete Alien configuration with typed modules + dynamic raw access.
    ...
    def to_dict(self) -> Dict[str, Any]:
        """
        Convert AlienConfig back to dictionary format.
        Returns the raw config dictionary for backward compatibility.
        """
        return self._raw.copy()

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "AlienConfig":
        """Create AlienConfig from merged configuration dictionary"""
        return cls(
...
class networkRuntimeConfig:
    """
    network runtime configuration with fixed fields + dynamic extras.
    """

    # ========== Fixed Fields ==========
    network_id: str = "test_network"
    heartbeat_interval: float = 30.0
    reconnect_delay: float = 5.0
    max_concurrent_tasks: int = 6
    max_step: int = 15
    device_info: str = "config/cluster/devices.yaml"
    log_to_markdown: bool = True
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "networkRuntimeConfig":
        """Create networkRuntimeConfig from dictionary"""
        known_mappings = {
...
class clusterAgentConfig:
    """
    cluster agent configuration wrapper providing typed access.
    """

    network_agent: AgentConfig

    def __getattr__(self, name: str) -> Any:
        # Provide direct access to network_AGENT
        if name.upper() == "network_AGENT":
    ...
    def __getitem__(self, key: str) -> Any:
        if key == "network_AGENT":
            return self.network_agent
        raise KeyError(key)

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "clusterAgentConfig":
        """Create clusterAgentConfig from dictionary"""
        return cls(
            network_agent=AgentConfig.from_dict(
                data.get("network_AGENT", {})
            )
        )
...
class clusterConfig:
    """
    Complete cluster configuration with typed modules + dynamic raw access.

    Provides structured access:
    - config.agent.network_AGENT → typed agent config
    - config.network.MAX_STEP → typed network config
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "clusterConfig":
        """Create clusterConfig from merged configuration dictionary"""
        return cls(
            agent=clusterAgentConfig.from_dict(data),
            network=networkRuntimeConfig.from_dict(data),
            _raw=data,
        )
