...
class SystemConfig:
    ...
    # ========== Orion Prompts ==========
    orion_creation_prompt: str = (
        "network/prompts/orion/share/orion_creation.yaml"
    )
    orion_editing_prompt: str = (
        "network/prompts/orion/share/orion_editing.yaml"
    )
    orion_creation_example_prompt: str = (
        "network/prompts/orion/examples/orion_creation_example.yaml"
    )
    orion_editing_example_prompt: str = (
        "network/prompts/orion/examples/orion_editing_example.yaml"
    )
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "SystemConfig":
        ...
        known_mappings = {
            ...
            # Orion Prompts
            "ORION_CREATION_PROMPT": "orion_creation_prompt",
            "ORION_EDITING_PROMPT": "orion_editing_prompt",
            "ORION_CREATION_EXAMPLE_PROMPT": "orion_creation_example_prompt",
            "ORION_EDITING_EXAMPLE_PROMPT": "orion_editing_example_prompt",
            # Third-Party Agents
...
@dataclass
class OrionRuntimeConfig:
    """
    Orion runtime configuration with fixed fields + dynamic extras.
    """

    # ========== Fixed Fields ==========
    orion_id: str = "test_orion"
    heartbeat_interval: float = 30.0
    reconnect_delay: float = 5.0
    max_concurrent_tasks: int = 6
    max_step: int = 15
    device_info: str = "config/network/devices.yaml"
    log_to_markdown: bool = True
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "OrionRuntimeConfig":
        """Create OrionRuntimeConfig from dictionary"""
        known_mappings = {
            "ORION_ID": "orion_id",
...
class NetworkAgentConfig:
    """
    Network agent configuration wrapper providing typed access.
    """

    orion_agent: AgentConfig

    def __getattr__(self, name: str) -> Any:
        # Provide direct access to ORION_AGENT
        if name.upper() == "ORION_AGENT":
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "NetworkAgentConfig":
        """Create NetworkAgentConfig from dictionary"""
        return cls(
            orion_agent=AgentConfig.from_dict(
                data.get("ORION_AGENT", {})
            )
        )
...
class NetworkConfig:
    """
    Complete Network configuration with typed modules + dynamic raw access.

    Provides structured access:
    - config.agent.ORION_AGENT → typed agent config
    - config.orion.MAX_STEP → typed orion config
    - config["ANY_KEY"] → backward compatible dict access
    """

    # ========== Typed Module Configs ==========
    agent: NetworkAgentConfig
    orion: OrionRuntimeConfig
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "NetworkConfig":
        """Create NetworkConfig from merged configuration dictionary"""
        return cls(
            agent=NetworkAgentConfig.from_dict(data),
            orion=OrionRuntimeConfig.from_dict(data),
            _raw=data,
        )
