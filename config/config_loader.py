...
"""
Modern Configuration Loader for Alien³ and cluster
...
Configuration Structure:
    New (Recommended):
        config/Alien/          ← Alien² configurations
        config/cluster/       ← cluster configurations

    Legacy (Auto-detected):
        Alien/config/          ← Old Alien configs (still supported)
...
from config.config_schemas import AlienConfig, clusterConfig
...
class ConfigLoader:
    ...
    LEGACY_PATH_MAP = {
        "config/Alien": "Alien/config",
        "config/cluster": None,  # cluster has no legacy path
    }
    ...
    def __init__(self, base_path: str = "config"):
        ...
        self._env = os.getenv("Alien_ENV", "production")
    ...
    def _discover_yaml_files(self, directory: Path) -> List[Path]:
        ...
        which are loaded separately based on Alien_ENV.
    ...
    def _load_module_configs(
        ...
        :param module_dir: Module directory (e.g., config/Alien or config/cluster)
    ...
    def _load_with_fallback(
        ...
        :param module: Module name ("Alien" or "cluster")
    ...
    def _warn_legacy_config(self, module: str, legacy_path: str, new_path: str) -> None:
        ...
        logger.warning(
            ...
            f"  python -m Alien.tools.migrate_config\n"
            f"{'=' * 70}\n"
        )
        self._warnings_shown.add(warning_key)

    def load_Alien_config(self, env: Optional[str] = None) -> AlienConfig:
        """
        Load Alien configuration with automatic legacy fallback.

        Automatically discovers and loads all YAML files:
        - Priority 1: config/Alien/*.yaml (new structure)
        - Priority 2: Alien/config/*.yaml (legacy fallback)

        Returns AlienConfig with:
        ...
        :return: AlienConfig with typed + dynamic access
        """
        ...
        yaml_config = self._load_with_fallback("Alien", env)
    ...
    def load_cluster_config(self, env: Optional[str] = None) -> clusterConfig:
        """
        Load cluster configuration with automatic legacy fallback.

        Automatically discovers and loads all YAML files from config/cluster/.
        Returns clusterConfig with:
        - Typed fields for agent config
        - Dynamic access for any YAML field (config.client_001, etc.)

        :param env: Environment override (dev/test/prod)
        :return: clusterConfig with typed + dynamic access
        """
        env = env or self._env

        # Load configuration (cluster has no legacy fallback)
        config_data = self._load_with_fallback("cluster", env)

        # Apply legacy API base transformations
        self._apply_legacy_transforms(config_data)

        # Create typed config with dynamic fields
        return clusterConfig.from_dict(config_data)
...
_global_Alien_config: Optional[AlienConfig] = None
_global_cluster_config: Optional[clusterConfig] = None


def get_Alien_config(reload: bool = False) -> AlienConfig:
    """
    Get Alien configuration (cached).
    ...
    :return: AlienConfig instance
    """
    global _global_Alien_config

    if _global_Alien_config is None or reload:
        loader = ConfigLoader.get_instance()
        _global_Alien_config = loader.load_Alien_config()

    return _global_Alien_config


def get_cluster_config(reload: bool = False) -> clusterConfig:
    """
    Get cluster configuration (cached).

    Returns a hybrid config object with:
    - Type-safe agent config: config.network_agent.api_model
    - Dynamic YAML fields: config.client_001, config.network_id, etc.
    - Backward compatible: config["network_AGENT"]

    Usage Examples:
        config = get_cluster_config()

        # Modern typed access
        agent_model = config.network_agent.api_model

        # Dynamic access to network settings
        ...
        # Legacy dict access
        agent_old = config["network_AGENT"]
    ...
    if _global_cluster_config is None or reload:
        loader = ConfigLoader.get_instance()
        _global_cluster_config = loader.load_cluster_config()
...
def clear_config_cache():
    ...
    global _global_Alien_config, _global_cluster_config
    _global_Alien_config = None
    _global_cluster_config = None
    ConfigLoader.reset()
