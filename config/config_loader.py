...
from config.config_schemas import ALIENConfig, NetworkConfig
...
class ConfigLoader:
    ...
    LEGACY_PATH_MAP = {
        ...
        "config/network": None,  # Network has no legacy path
    }
    ...
    def _load_module_configs(
        ...
        :param module_dir: Module directory (e.g., config/alien or config/network)
    ...
    def _load_with_fallback(
        ...
        :param module: Module name ("alien" or "network")
    ...
    def load_network_config(self, env: Optional[str] = None) -> NetworkConfig:
        """
        Load Network configuration with automatic legacy fallback.

        Automatically discovers and loads all YAML files from config/network/.
        Returns NetworkConfig with:
        - Typed fields for agent config
        - Dynamic access for any YAML field (config.client_001, etc.)

        :param env: Environment override (dev/test/prod)
        :return: NetworkConfig with typed + dynamic access
        """
        env = env or self._env

        # Load configuration (Network has no legacy fallback)
        config_data = self._load_with_fallback("network", env)

        # Apply legacy API base transformations
        self._apply_legacy_transforms(config_data)

        # Create typed config with dynamic fields
        return NetworkConfig.from_dict(config_data)
...
_global_network_config: Optional[NetworkConfig] = None
...
def get_network_config(reload: bool = False) -> NetworkConfig:
    """
    Get Network configuration (cached).

    Returns a hybrid config object with:
    - Type-safe agent config: config.orion_agent.api_model
    - Dynamic YAML fields: config.client_001, config.orion_id, etc.
    - Backward compatible: config["ORION_AGENT"]

    Usage Examples:
        config = get_network_config()

        # Modern typed access
        agent_model = config.orion_agent.api_model

        # Dynamic access to orion settings
        orion_id = config.orion_id
    ...
    if _global_network_config is None or reload:
        loader = ConfigLoader.get_instance()
        _global_network_config = loader.load_network_config()
...
def clear_config_cache():
    ...
    global _global_alien_config, _global_network_config
    _global_alien_config = None
    _global_network_config = None
    ConfigLoader.reset()
