...
from cluster.visualization import DAGVisualizer, TaskDisplay
...
class TaskVisualizationHandler:
    ...
    async def handle_task_started(
        self, event: TaskEvent, network: Tasknetwork
    ) -> None:
        """
        ...
        try:
            ...
            task = network.get_task(task_id) if task_id else None
            ...
            # Show topology for smaller networks
            # if network.task_count <= 10:
    ...
    async def handle_task_completed(
        ...
        try:
            ...
            # Show execution progress for smaller networks
            if network.task_count <= 10:
                self._visualizer.display_execution_flow(network)
    ...
    async def handle_task_failed(
        ...
        try:
            ...
            # Always show failure status regardless of network size
            # self._visualizer.display_execution_flow(network)
    ...
    async def handle_task_event(
        ...
        if event.event_type == EventType.TASK_STARTED:
            await self.handle_task_started(event, network)
        elif event.event_type == EventType.TASK_COMPLETED:
            await self.handle_task_completed(event, network)
        elif event.event_type == EventType.TASK_FAILED:
            await self.handle_task_failed(event, network)
