...
class networkConfig:
    ...
    @classmethod
    def from_json(cls, config_path: str) -> "networkConfig":
        """
    ...
    @classmethod
    def from_yaml(cls, config_path: str) -> "networkConfig":
        """
    ...
    @classmethod
    def from_args(cls, args: argparse.Namespace) -> "networkConfig":
        """
    ...
    def from_env(cls) -> "networkConfig":
        ...
        config.heartbeat_interval = float(
            os.getenv("network_HEARTBEAT_INTERVAL", config.heartbeat_interval)
        )
        config.max_concurrent_tasks = int(
            os.getenv("network_MAX_CONCURRENT_TASKS", config.max_concurrent_tasks)
        )

        # Load devices from environment
        devices_json = os.getenv("network_DEVICES")
        if devices_json:
            try:
...
def setup_argument_parser() -> argparse.ArgumentParser:
    """
    Set up command line argument parser for network configuration.
...
Examples:
  ...
  python -m Alien.network.client --config config/network.json
  python -m Alien.network.client --config config/network.yaml
  
  # Add devices via command line
  python -m Alien.network.client \\
    --device laptop_001:ws://192.168.1.100:5000/ws \\
    --device workstation_002:ws://192.168.1.101:5000/ws
  
  # Create sample config
  python -m Alien.network.client --create-sample-config config/sample.json
  python -m Alien.network.client --create-sample-config config/sample.yaml
        """,
    )

    parser.add_argument(
        "--config", "-c", type=str, help="Path to network configuration file"
    )

    parser.add_argument(
        "--network-id",
        type=str,
        default="network_orchestrator",
        help="Unique identifier for this network instance",
    )
...
if __name__ == "__main__":
    ...
    if args.create_sample_config:
        ...
        if args.config:
            config = networkConfig.from_file(args.config)
        else:
            config = networkConfig.from_args(args)
