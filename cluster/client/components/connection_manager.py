...
Manages WebSocket connections to Alien servers using AIP protocols.
...
from cluster.core.types import ExecutionResult

from .types import AgentProfile, TaskRequest

if TYPE_CHECKING:
    from cluster.client.components.message_processor import MessageProcessor
...
class WebSocketConnectionManager:
    ...
    async def connect_to_device(
        ...
        try:
            ...
            success = await self._register_network_client(device_info)

            if not success:
                await transport.close()
                raise ConnectionError("Failed to register network client")
    ...
    async def _register_network_client(self, device_info: AgentProfile) -> bool:
        """
        Register this network as a client using AIP RegistrationProtocol.

        :param device_info: Device information to register with
        :return: True if registration successful, False otherwise
        """
        try:
            network_client_id = f"{self.task_name}@{device_info.device_id}"
            ...
            # Prepare metadata for network registration
            metadata = {
                "type": "network_client",
            ...
            self.logger.info(
                f"📝 Registering network client: {network_client_id}"
            )
            ...
            # (don't use register_as_network which calls receive_message)
            from aip.messages import (
                ClientMessage,
                ClientMessageType,
                ClientType,
            ...
            self.logger.info(
                f"📤 Sent network registration for {network_client_id} → {device_info.device_id}"
            )
            ...
            if not success:
                self.logger.error(
                    f"❌ Registration failed for {network_client_id}"
                )
                return False

            self.logger.info(
                f"✅ Registration successful for {network_client_id}"
            )
            return True
    ...
    async def send_task_to_device(
        ...
        try:
            ...
            network_task_id = f"{self.task_name}@{task_request.task_id}"

            # Create client message for task execution
            # Note: network sends ClientMessage.TASK to server, which is different
            ...
            task_message = ClientMessage(
                ...
                target_id=device_id,
                task_name=f"cluster/{self.task_name}/{task_request.task_name}",
            ...
            response = await asyncio.wait_for(
                self._wait_for_task_response(device_id, network_task_id),
            ...
            self._pending_tasks.pop(network_task_id, None)
            self.logger.error(
    ...
    async def request_device_info(self, device_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Note: We still use manual ClientMessage construction because network
