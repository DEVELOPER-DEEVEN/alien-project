"""
cluster Trajectory Report Generator

Quick script to generate markdown reports for cluster task execution logs.
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate Markdown report for cluster task execution logs"
    )
    parser.add_argument(
        "log_dir",
        type=str,
        help="Path to cluster log directory (e.g., logs/cluster/task_1)",
    )
    parser.add_argument(
        "-o",
        "--output",
    ...
    parser.add_argument(
        "--no-network",
        action="store_true",
        help="Exclude network evolution details",
    )
    parser.add_argument(
        "--no-tasks",
        action="store_true",
    ...
    try:
        trajectory = clusterTrajectory(args.log_dir)
    ...
    try:
        trajectory.to_markdown(
            ...
            include_network_details=not args.no_network,
