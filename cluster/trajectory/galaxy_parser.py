...
"""
cluster Trajectory Parser

Optimized parser for cluster agent logs with network visualization support.
"""
...
class clusterTrajectory:
    """
    A class to structure and visualize cluster trajectory data with network support.

    This class parses cluster response logs (JSONL format) and generates comprehensive
    Markdown documentation including:
    - network evolution (before/after states)
    ...
    def __init__(self, folder_path: str) -> None:
        """
        Initialize cluster trajectory parser.

        :param folder_path: Path to the cluster log directory (e.g., logs/cluster/task_1)
        """
    ...
    def _parse_network(
        self, network_json: Optional[str]
    ) -> Optional[Dict[str, Any]]:
        """
        Safely parse network JSON string with compatibility for string-serialized tasks.

        This method handles both:
        1. Properly formatted network JSON (new format after fix)
        2. Legacy format where tasks/dependencies are Python string representations

        :param network_json: JSON string of network data or dict
        :return: Parsed network dict (may include 'parse_error' key) or None
        """
        if not network_json:
            return None

        try:
            # Handle case where network_json might already be a dict
            ...
            if "tasks" in network and isinstance(network["tasks"], str):
                ...
                network["parse_error"] = {
                    ...
                    "raw_preview": (
                        str(network["tasks"])[:200] + "..."
                        if len(network["tasks"]) > 200
                        else network["tasks"]
                    ),
                }
                logger.warning(
                    "Detected tasks field as Python string representation (legacy format with serialization bug). "
                    "Marking with parse_error in network data."
                )
                # Keep the network with error info for display
                return network

            # Compatibility fix: Handle dependencies field as string (legacy format)
            if "dependencies" in network and isinstance(
                network["dependencies"], str
            ):
                logger.warning(
                    "Detected dependencies field as Python string representation (legacy format). "
                    "Using empty dependencies for this network."
                )
            ...
            logger.warning(f"Failed to parse network JSON: {e}")
            # Return error info instead of None
            return {
                "parse_error": {
                    "field": "network",
                    "error_type": "json_decode_error",
                    "message": f"Failed to parse network JSON: {str(e)}",
                    "raw_preview": (
                        str(network_json)[:200] + "..."
                        if len(str(network_json)) > 200
                        else str(network_json)
                    ),
                }
            }
        except Exception as e:
            logger.warning(f"Unexpected error parsing network: {e}")
            return {
                "parse_error": {
                    "field": "network",
    ...
    def _generate_topology_image(
        ...
        :param tasks: Dictionary of task_id -> task_data
        :param network_id: network ID for unique filename
        ...
        # Clean network_id for filename
        clean_id = network_id.replace(":", "_").replace("/", "_")
    ...
    def _format_dependency_graph(
        ...
        :param dependencies: Dictionary of line_id -> dependency_data
        :param tasks: Dictionary of task_id -> task_data
        :param network_id: network ID for image filename
        ...
        image_path = self._generate_topology_image(
            dependencies, tasks, network_id, step_number, state
        )
    ...
    def to_markdown(
        ...
        :param include_network_details: Include detailed network evolution
        ...
        if len(self.step_log) == 0:
            ...
            with open(output_path, "w", encoding="utf-8") as file:
                file.write("# cluster Trajectory Report\n\n")
        ...
        with open(output_path, "w", encoding="utf-8") as file:
            ...
            for idx, step in enumerate(self.step_log, 1):
                ...
                if include_network_details:
                    network_before = self._parse_network(
                        step.get("network_before")
                    )
                    network_after = self._parse_network(
                        step.get("network_after")
                    )

                    if network_before or network_after:
                        file.write("#### network Evolution\n\n")

                        # Before state
                        if network_before:
                            file.write(
                                "<details>\n<summary>network BEFORE (click to expand)</summary>\n\n"
                            )
                            file.write(
                                f"**network ID**: {network_before.get('network_id', 'N/A')}\n"
                            )
                            file.write(
                                f"**State**: {network_before.get('state', 'N/A')}\n\n"
                            )

                            # Check for parse errors
                            if "parse_error" in network_before:
                                error_info = network_before["parse_error"]
                                ...
                                file.write(
                                    "**Note**: This network cannot be fully parsed. "
                                    "Basic metadata shown above. "
                                    "This issue is fixed in newer versions.\n\n"
                                )
                            else:
                                # Normal parsing - show full details
                                tasks = network_before.get("tasks", {})
                                deps = network_before.get("dependencies", {})
                        ...
                        if network_after:
                            ...
                            file.write(
                                f"**network ID**: {network_after.get('network_id', 'N/A')}\n"
                            )
                            file.write(
                                f"**State**: {network_after.get('state', 'N/A')}\n\n"
                            )

                            # Check for parse errors
                            if "parse_error" in network_after:
                                error_info = network_after["parse_error"]
                                file.write("##### ⚠️ Parse Error\n\n")
                                file.write(
                                    f"**Error Type**: `{error_info.get('error_type', 'unknown')}`\n\n"
                                )
                                ...
                                # Normal parsing - show full details
                                tasks = network_after.get("tasks", {})
                                deps = network_after.get("dependencies", {})
            ...
            # Final network State (if available)
            if self.step_log:
                last_step = self.step_log[-1]
                final_network = self._parse_network(
                    last_step.get("network_after")
                )

                if final_network and include_task_details:
                    file.write("## Final network State\n\n")
                    file.write(
                        f"**ID**: {final_network.get('network_id', 'N/A')}\n"
                    )
                    file.write(
                        f"**State**: {final_network.get('state', 'N/A')}\n"
                    )
                    file.write(
                        f"**Created**: {final_network.get('created_at', 'N/A')}\n"
                    )
                    file.write(
                        f"**Updated**: {final_network.get('updated_at', 'N/A')}\n\n"
                    )
                    ...
                    if tasks and isinstance(tasks, dict):
                        ...
                        deps = final_network.get("dependencies", {})
                        file.write("### Final Dependency Graph\n\n")
                        file.write(
                            self._format_dependency_graph(
                                deps,
                                tasks,
                                final_network.get("network_id", "final"),
...
if __name__ == "__main__":
    ...
    console.print(
        "[BOLD BLUE] cluster Trajectory Parser - Batch Mode", style="blue bold"
    )
    ...
    if not cluster_logs_dir.exists():
        console.print(f"[FAIL] Directory not found: {cluster_logs_dir}", style="red")
        sys.exit(1)
    ...
    if not task_dirs:
        console.print(
            f"[FAIL] No task directories found in {cluster_logs_dir}", style="red"
        )
        sys.exit(1)
