...
"""
Alien-Group Framework Client Library

cluster Framework client class providing programmatic interface
for starting cluster sessions, executing DAG-based workflows, and managing
network orchestration.

This module provides the clusterClient class for integration into other applications.
For command-line usage, use cluster.py as the main entry point.
"""
...
from .session.cluster_session import clusterSession
...
class clusterClient:
    """
    Main cluster Framework client for command-line interaction.

    Provides capabilities for:
    - Starting cluster sessions
    - Processing user requests into DAG workflows
    - Managing network execution
    ...
    def __init__(
        ...
        :param session_name: Name for the cluster session (auto-generated if None)
        ...
        """
        self.session_name = (
            session_name or f"cluster_session_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
        ...
        # (cluster.py already calls setup_logger before importing clusterClient)
        ...
        self._session: Optional[clusterSession] = None
        self._current_request_task: Optional[asyncio.Task] = None
        self._is_shutting_down: bool = False

        # Load device configuration from new config system
        cluster_config = get_cluster_config()
        device_info_path = cluster_config.network.DEVICE_INFO
        self._device_config = networkConfig.from_yaml(device_info_path)
        ...
        self.display.show_cluster_banner()
        self.display.print_info(
            f"[bold cyan]🌌 cluster Client initialized:[/bold cyan] [green]{self.session_name}[/green]"
        )
        self.logger.info(f"🌌 cluster Client initialized: {self.session_name}")

    async def initialize(self) -> None:
        """
        Initialize all cluster framework components.

        Sets up agent, network client, orchestration, context,
        and cluster session with progress indication.
        """
        try:
            with self.display.show_initialization_progress() as progress:
                task = progress.add_task(
                    "[cyan]Initializing Alien-Group Framework...", total=None
                )

                self.logger.info("🚀 Initializing Alien-Group Framework components...")

                # Initialize network client
                progress.update(
                    task, description="[cyan]Setting up network Client..."
                )
                self._client = networkClient(
                    config=self._device_config, task_name=self.task_name
                )
                await self._client.initialize()
                self.display.print_success("✅ networkClient initialized")
                self.logger.info("✅ networkClient initialized")

                # cluster session will be created per request
                progress.update(
            ...
            self.display.print_success("\n🌟 Alien-Group Framework initialization complete!\n")
            self.logger.info("🌟 Alien-Group Framework initialization complete!")

        except Exception as e:
            self.display.print_error(f"❌ Failed to initialize Alien-Group Framework: {e}")
            self.logger.error(
                f"❌ Failed to initialize Alien-Group Framework: {e}", exc_info=True
            )
            raise
    ...
    async def process_request(self, request: str) -> Dict[str, Any]:
        ...
        :raises RuntimeError: If cluster client is not initialized
        """
        if not self._client:
            raise RuntimeError(
                "cluster client not initialized. Call initialize() first."
            )
        ...
        try:
            ...
            from cluster.client.components.types import DeviceStatus
            ...
            with self.display.show_initialization_progress() as progress:
                # progress.add_task("[cyan]Executing cluster session...", total=None)
            ...
            if self._session and self._session.current_network:
                network = self._session.current_network
                result["network"] = {
                    "id": network.network_id,
                    "name": network.name,
                    "task_count": len(network.tasks),
                    "dependency_count": len(network.dependencies),
    ...
    async def interactive_mode(self) -> None:
        ...
        user requests and processes them through the cluster framework.
        """
        ...
        while True:
            try:
                ...
                user_input = self.display.get_user_input(
                    f"[bold blue]Alien[{request_count}][/bold blue]"
                )
    ...
    async def reset_session(self) -> Dict[str, Any]:
        ...
        Clears the current session's network, tasks, and execution history
    ...
    async def create_next_session(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.session_name = (
                f"cluster_session_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            )
    ...
    async def shutdown(self, force: bool = False) -> None:
        ...
        Properly closes all components including the network client
        ...
        try:
            self.display.print_warning("🛑 Shutting down cluster client...")
            self.logger.info("🛑 Shutting down cluster client...")
            ...
            self.display.print_success("✅ cluster client shutdown complete")
            self.logger.info("✅ cluster client shutdown complete")
