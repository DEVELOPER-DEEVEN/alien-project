...
interface clusterStore {
  ...
  addEventToLog: (event: clusterEvent) => void;
  clearEventLog: () => void;

  networks: Record<string, networkSummary>;
  upsertnetwork: (network: Partial<networkSummary> & { id: string }) => void;
  removenetwork: (id: string) => void;
  setActivenetwork: (id: string | null) => void;

  tasks: Record<string, Task>;
  bulkUpsertTasks: (networkId: string, tasks: Array<Partial<Task> & { id: string }>, dependencies?: Record<string, string[]>) => void;
...
const computenetworkStats = (
  ...
  taskIds.forEach((taskId) => {
    ...
    if (!task || task.networkId !== networkId) {
      return;
    }
...
const defaultSessionState = (): SessionState => ({
  id: null,
  displayName: 'cluster Session',
  welcomeText: 'Launch a request to orchestrate a new Tasknetwork.',
  startedAt: null,
  debugMode: false,
  highContrast: false,
});
...
export const useclusterStore = create<clusterStore>()((set, get) => ({
  ...
  networks: mockData ? { [mockData.network.id]: mockData.network } : {},
  upsertnetwork: (network) => {
    set((state) => {
      const existing = state.networks[network.id];
      const taskIds = network.taskIds || existing?.taskIds || [];
      const updatedStats = computenetworkStats(
        network.id,
        taskIds,
        state.tasks,
      );

      const updated: networkSummary = {
        id: network.id,
        name: network.name || existing?.name || network.id,
        status: (network.status as TaskStatus) || existing?.status || 'pending',
        description: network.description ?? existing?.description,
        metadata: network.metadata ?? existing?.metadata,
        createdAt: network.createdAt ?? existing?.createdAt ?? getNow(),
        updatedAt: getNow(),
        taskIds,
        dag: {
          nodes: network.dag?.nodes ?? existing?.dag.nodes ?? [],
          edges: network.dag?.edges ?? existing?.dag.edges ?? [],
        },
      ...
      return {
        ...
        ui: {
          ...state.ui,
          activenetworkId:
            state.ui.activenetworkId || network.id,
        },
      };
    });
  },
  removenetwork: (id) =>
    set((state) => {
      const { [id]: removed, ...remaining } = state.networks;
      return {
        networks: remaining,
        ui: {
          ...state.ui,
          // If the removed network was active, clear the active selection
          activenetworkId:
            state.ui.activenetworkId === id
              ? null
              : state.ui.activenetworkId,
        },
      };
    }),
  setActivenetwork: (id) =>
  ...
  bulkUpsertTasks: (networkId, tasks, dependencies = {}) => {
    set((state) => {
      ...
      const taskIds = new Set<string>(
        state.networks[networkId]?.taskIds ?? [],
      );
      ...
      const existingnetwork = state.networks[networkId];
  ...
  updateTask: (taskId, patch) => {
    set((state) => {
      ...
      const network = state.networks[existing.networkId];

      const nextState: Partial<clusterStore> = {
        tasks: {
          ...state.tasks,
          [taskId]: updatedTask,
        },
      ...
      if (network) {
        nextState.networks = {
          ...
          [network.id]: {
            ...network,
            statistics: computenetworkStats(
              network.id,
              network.taskIds,
              {
                ...state.tasks,
      ...
      return nextState as clusterStore;
    });
  },
  ...
  ui: {
    ...defaultUIState(),
    activenetworkId: mockData?.network.id || null,
  },
  ...
  resetSessionState: (options?: { clearHistory?: boolean }) =>
    set((state) => {
      ...
      return {
        messages: [],
        eventLog: [],
        networks: clearHistory ? {} : state.networks,
