...
import { networkSummary } from '../../store/clusterStore';
...
const networkBlock: React.FC<networkBlockProps> = ({ network, onSelectTask, variant = 'standalone' }) => {
  ...
  if (!network) {
    return (
      ...
      >
        ...
        <div>No active network yet.</div>
        <div className="text-xs text-slate-500">Launch a request to generate a Tasknetwork.</div>
      </div>
    );
  }

  const statusClass = statusAccent[network.status] || 'text-slate-300';
  ...
  // Show stats button only when network is completed or failed
  const canShowStats = network.status === 'completed' || network.status === 'failed';
    ...
    <div className={containerClasses}>
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-2 text-xs text-slate-400">
          ...
          <span>{network.taskIds.length} tasks</span>
          <span className="mx-1">•</span>
          <span className={statusClass}>{network.status}</span>
        </div>
        {canShowStats && (
          <button
      ...
      <div className={previewClasses}>
        {showStats ? (
          ...
          <DagPreview
            nodes={network.dag.nodes}
            edges={network.dag.edges}
