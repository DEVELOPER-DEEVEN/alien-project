...
const networkStats: React.FC<networkStatsProps> = ({ network, onBack }) => {
  // Get statistics from metadata (sent from backend via get_statistics())
  const stats = network.metadata?.statistics || {};
  const statusCounts = stats.task_status_counts || {};

  const totalTasks = stats.total_tasks || network.statistics.total;
          ...
          <div className="space-y-2 text-xs">
            {network.description && (
              <div>
                ...
                <div className="mt-1 text-slate-200">{network.description}</div>
              </div>
            )}
            {network.metadata.display_name && (
              <div className="flex justify-between">
                <span className="text-slate-400">Name:</span>
                <span className="text-slate-200">{network.metadata.display_name}</span>
              </div>
            )}
          </div>
        </div>
