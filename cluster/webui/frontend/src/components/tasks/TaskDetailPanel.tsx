...
import { Task, useclusterStore } from '../../store/clusterStore';
...
const TaskDetailPanel: React.FC<TaskDetailPanelProps> = ({ task, onBack }) => {
  const { tasks, setActiveTask } = useclusterStore((state) => ({
    tasks: state.tasks,
  ...
  const networkTasks = useMemo(() => {
    if (!task) return [];
    return Object.values(tasks)
      .filter(t => t.networkId === task.networkId)
      .sort((a, b) => a.id.localeCompare(b.id));
  }, [task, tasks]);

  const currentTaskIndex = useMemo(() => {
    if (!task || networkTasks.length === 0) return -1;
    return networkTasks.findIndex(t => t.id === task.id);
  }, [task, networkTasks]);

  const handlePrevious = () => {
    if (currentTaskIndex > 0) {
      setActiveTask(networkTasks[currentTaskIndex - 1].id);
    }
  };

  const handleNext = () => {
    if (currentTaskIndex >= 0 && currentTaskIndex < networkTasks.length - 1) {
      setActiveTask(networkTasks[currentTaskIndex + 1].id);
    }
  };

  const canGoPrevious = currentTaskIndex > 0;
  const canGoNext = currentTaskIndex >= 0 && currentTaskIndex < networkTasks.length - 1;
  ...
  if (!task) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-3 text-center text-sm text-slate-300">
        <Zap className="h-8 w-8 text-cluster-blue/50" aria-hidden />
  ...
  return (
    <div className="flex h-full gap-4 overflow-hidden">
      ...
      <div className="flex w-[40%] flex-shrink-0 flex-col gap-3 overflow-hidden">
        {/* Task Header - Fixed */}
        <div className="flex-shrink-0 rounded-xl border border-white/10 bg-gradient-to-br from-cluster-dark/80 via-cluster-indigo/20 to-cluster-dark/90 p-3 shadow-[0_4px_20px_rgba(0,0,0,0.4),inset_0_1px_1px_rgba(255,255,255,0.08)]">
          <div className="mb-2 flex items-center gap-2">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="truncate font-mono text-xs font-semibold text-cluster-glow drop-shadow-[0_0_8px_rgba(33,240,255,0.5)]">
                {task.id}
              </div>
            </div>
          </div>
        ...
        <div className="flex-shrink-0 space-y-2 rounded-xl border border-white/10 bg-gradient-to-br from-black/60 to-black/40 p-3 shadow-[0_4px_14px_rgba(0,0,0,0.4),inset_0_1px_1px_rgba(255,255,255,0.05)]">
          <div className="mb-2 flex items-center gap-1.5 border-b border-white/10 pb-2">
            <Cpu className="h-4 w-4 text-cluster-blue" aria-hidden />
          ...
          <div className="space-y-2 font-mono text-[11px]">
            <div className="flex items-center justify-between">
              ...
              <span className="font-semibold text-cluster-teal">
                {task.deviceId || '—'}
              </span>
            </div>
        ...
        <div className="flex-shrink-0 rounded-xl border border-white/10 bg-gradient-to-br from-black/60 to-black/40 p-3 shadow-[0_4px_14px_rgba(0,0,0,0.4),inset_0_1px_1px_rgba(255,255,255,0.05)]">
          <div className="mb-2 flex items-center gap-2">
            <div className="flex h-5 w-5 items-center justify-center rounded-md bg-gradient-to-br from-cluster-teal/20 to-cluster-blue/10">
              <GitBranch className="h-3 w-3 text-cluster-teal" aria-hidden />
            </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={clsx(
                ...
                canGoPrevious
                  ? "border-white/10 bg-gradient-to-r from-white/5 to-white/3 text-slate-300 hover:border-cluster-teal/40 hover:from-cluster-teal/10 hover:to-cluster-teal/5 hover:text-slate-100 hover:shadow-[0_3px_10px_rgba(56,189,248,0.25)]"
            ...
            <button
              ...
              className={clsx(
                "group flex flex-1 items-center justify-center gap-2 rounded-lg border px-3 py-2 font-mono text-[11px] font-medium uppercase tracking-wider shadow-[0_2px_8px_rgba(0,0,0,0.3),inset_0_1px_1px_rgba(255,255,255,0.06)] transition-all",
                canGoNext
                  ? "border-white/10 bg-gradient-to-r from-white/5 to-white/3 text-slate-300 hover:border-cluster-purple/40 hover:from-cluster-purple/10 hover:to-cluster-purple/5 hover:text-slate-100 hover:shadow-[0_3px_10px_rgba(123,44,191,0.25)]"
                  : "cursor-not-allowed border-white/5 bg-white/3 text-slate-600 opacity-30"
              )}
      ...
      <div className="flex w-[60%] flex-shrink-0 flex-col gap-3 overflow-hidden">
        ...
        {task.tips && task.tips.length > 0 && (
          <div className="rounded-xl border border-cluster-purple/30 bg-gradient-to-br from-cluster-purple/10 via-cluster-indigo/5 to-black/60 p-4 shadow-[0_4px_20px_rgba(123,44,191,0.3),0_0_1px_rgba(123,44,191,0.4),inset_0_1px_1px_rgba(255,255,255,0.08)]">
            <div className="mb-3 flex items-center gap-2">
              <div className="flex h-6 w-6 items-center justify-center rounded-lg bg-gradient-to-br from-cluster-purple to-cluster-indigo shadow-[0_0_12px_rgba(123,44,191,0.5)]">
                <span className="text-[14px]">💡</span>
              </div>
              <span className="font-mono text-[11px] font-bold uppercase tracking-[0.2em] text-transparent bg-clip-text bg-gradient-to-r from-cluster-purple via-purple-300 to-cluster-purple">
                Execution Tips
              </span>
            </div>
            <ul className="space-y-2.5">
              {task.tips.map((tip, index) => (
                <li key={index} className="group flex items-start gap-3 transition-all duration-200 hover:translate-x-1">
                  <span className="mt-0.5 flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-md border border-cluster-purple/40 bg-gradient-to-br from-cluster-purple/20 to-cluster-indigo/10 font-mono text-[10px] font-extrabold text-purple-200 shadow-[0_0_8px_rgba(123,44,191,0.3)] transition-all group-hover:border-cluster-purple/60 group-hover:shadow-[0_0_12px_rgba(123,44,191,0.5)] group-hover:scale-110">
                    {index + 1}
                  </span>
        ...
        <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-cluster-blue/20 bg-gradient-to-br from-black/80 to-cluster-dark/60 shadow-[0_8px_28px_rgba(0,0,0,0.5),0_0_1px_rgba(15,123,255,0.3),inset_0_1px_1px_rgba(255,255,255,0.08)]">
          <div className="flex items-center justify-between border-b border-white/10 bg-gradient-to-r from-cluster-blue/10 to-cluster-purple/10 px-3 py-2.5">
            <div className="flex items-center gap-2">
              <div className="h-2 w-2 animate-pulse rounded-full bg-cluster-glow shadow-[0_0_6px_rgba(33,240,255,0.8)]" />
            ...
            <button
              type="button"
              className="group inline-flex items-center gap-1.5 rounded-md border border-white/10 bg-white/5 px-2.5 py-1 font-mono text-[10px] uppercase tracking-wider text-slate-400 transition-all hover:border-cluster-glow/40 hover:bg-cluster-glow/10 hover:text-cluster-glow hover:shadow-[0_0_10px_rgba(33,240,255,0.3)]"
              onClick={() => {
          ...
          <div className="flex-1 overflow-auto p-3">
            <pre className="font-mono text-[11px] leading-relaxed text-slate-200 selection:bg-cluster-blue/30">
