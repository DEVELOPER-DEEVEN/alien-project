...
import { Message } from '../../store/clusterStore';
 ...
 * Mirrors the backend's _format_network_operation logic.
 */
const formatnetworkOperation = (action: any): string => {
  ...
  switch (func) {
    ...
    case 'build_network': {
      ...
      if (args.task_count !== undefined || args.dependency_count !== undefined) {
        ...
        return `Build network (${taskCount} tasks, ${depCount} dependencies)`;
      }
    ...
    case 'load_network': {
      ...
      return `Load network from '${filename}'`;
    }
    ...
    case 'save_network': {
      ...
      // Extract filename from path
      const filename = filePath.split(/[/\\]/).pop() || filePath;
      return `Save network to '${filename}'`;
    }
...
const ActionTreeNode: React.FC<{
  ...
  const operation = formatnetworkOperation(action);
...
const MessageBubble: React.FC<MessageBubbleProps> = ({ message, nextMessage, stepNumber }) => {
  ...
  const displayName = useMemo(() => {
    ...
    return message.agentName.toLowerCase().includes('network') ? 'Alien' : message.agentName;
    ...
    >
      <div
        className={clsx(
          ...
          isUser
            ? 'rounded-br-xl border-cluster-blue/50 bg-gradient-to-br from-cluster-blue/25 via-cluster-purple/25 to-cluster-blue/15 text-slate-50 shadow-[0_0_30px_rgba(15,123,255,0.2),inset_0_1px_0_rgba(147,197,253,0.15)]'
                  ...
                  <ul className="space-y-1 text-sm">
                    {responsePayload.plan.map((step: string, index: number) => (
                      <li key={index} className="flex items-start gap-2 text-slate-200">
                        <span className="mt-[2px] h-2 w-2 rounded-full bg-cluster-blue" aria-hidden />
                        <span>{step}</span>
                      </li>
                    ))}
                  </ul>
