...
import { Message, useclusterStore } from '../../store/clusterStore';
...
const ChatWindow: React.FC = () => {
  const { messages, searchQuery, messageKind, isTaskStopped } = useclusterStore(
    (state) => ({
          ...
          {filteredMessages.length === 0 ? (
            <div className="flex h-full flex-col items-center justify-center gap-3 text-center text-slate-400">
              ...
              <p className="max-w-sm text-sm">
                Ready to launch. Describe a mission for the cluster Agent, or use quick commands below to explore diagnostics.
              </p>
            </div>
          ) : (
            <>
