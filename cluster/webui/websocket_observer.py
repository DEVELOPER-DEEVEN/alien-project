...
"""
WebSocket Observer for cluster Web UI.

This observer subscribes to all cluster events and pushes them to connected WebSocket clients.
...
class EventSerializer:
    ...
    def _initialize_type_cache(self) -> None:
        ...
        # Try to import Tasknetwork
        try:
            from cluster.network import Tasknetwork

            self._cached_types["Tasknetwork"] = Tasknetwork
        except ImportError:
            self._cached_types["Tasknetwork"] = None
            self.logger.debug("Tasknetwork not available for serialization")
    ...
    def _register_handlers(self) -> None:
        ...
        task_network_type = self._cached_types.get("Tasknetwork")
        if task_network_type:
            self._type_handlers[task_network_type] = self._serialize_network
    ...
    def serialize_event(self, event: Event) -> Dict[str, Any]:
        ...
        if isinstance(event, TaskEvent):
            ...
            base_dict.update(self._serialize_network_event_fields(event))
    ...
    def _serialize_network_event_fields(
        self, event: networkEvent
    ) -> Dict[str, Any]:
        """
        Extract network-specific fields from a networkEvent.

        :param event: The network event to serialize
        :return: Dictionary of network-specific fields
        """
        return {
    ...
    def serialize_value(self, value: Any) -> Any:
        ...
        and custom cluster objects using a chain of serialization strategies.
    ...
    def _serialize_network(self, value: Any) -> Dict[str, Any]:
        """
        Serialize a Tasknetwork object.

        :param value: Tasknetwork instance
        :return: Serialized dictionary with network details
        """
        try:
            network_dict = {
                ...
                "tasks": self._serialize_network_tasks(value.tasks),
            ...
            if hasattr(value, "get_statistics") and callable(value.get_statistics):
                try:
                    network_dict["statistics"] = value.get_statistics()
                except Exception as e:
                    self.logger.warning(f"Failed to get network statistics: {e}")

            return network_dict
        except Exception as e:
            self.logger.warning(f"Failed to serialize Tasknetwork: {e}")
            return str(value)

    def _serialize_network_tasks(
        self, tasks: Dict[str, Any]
    ) -> Dict[str, Dict[str, Any]]:
        """
        Serialize all tasks in a network.
...
class WebSocketObserver(IEventObserver):
    """
    Observer that forwards all cluster events to WebSocket clients.
