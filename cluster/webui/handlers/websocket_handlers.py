...
"""
WebSocket message handlers for cluster Web UI.
...
from cluster.webui.models.enums import WebSocketMessageType, RequestStatus
from cluster.webui.services import DeviceService, clusterService
...
class WebSocketMessageHandler:
    ...
    def __init__(self, app_state: AppState) -> None:
        ...
        self.cluster_service = clusterService(app_state)
    ...
    async def _handle_request(self, websocket: WebSocket, data: dict) -> None:
        ...
        if not self.cluster_service.is_client_available():
            await websocket.send_json(
                {
                    "type": WebSocketMessageType.ERROR,
                    "message": "cluster client not initialized",
                }
            )
            return
        ...
        async def process_in_background() -> None:
            try:
                result = await self.cluster_service.process_request(request_text)
                await websocket.send_json(
                    {
    ...
    async def _handle_reset(self, websocket: WebSocket, data: dict) -> None:
        ...
        Resets the current cluster session and clears state.
        ...
        try:
            result = await self.cluster_service.reset_session()
            await websocket.send_json(
                {
                    "type": WebSocketMessageType.RESET_ACKNOWLEDGED,
    ...
    async def _handle_next_session(self, websocket: WebSocket, data: dict) -> None:
        ...
        Creates a new cluster session while potentially maintaining some context.
        ...
        try:
            result = await self.cluster_service.create_next_session()
            await websocket.send_json(
                {
    ...
    async def _handle_stop_task(self, websocket: WebSocket, data: dict) -> None:
        ...
        Shuts down the cluster client to clean up device tasks, then reinitializes
        ...
        if not self.cluster_service.is_client_available():
            self.logger.warning("No active cluster client to stop")
            await websocket.send_json(
                {
        ...
        try:
            new_session_result = await self.cluster_service.stop_task_and_restart()
            await websocket.send_json(
                {
                    "type": WebSocketMessageType.STOP_ACKNOWLEDGED,
