...
"""
Dependency management for cluster Web UI.
...
class AppState:
    ...
    - cluster session and client instances
    ...
    def __init__(self) -> None:
        ...
        # cluster session and client instances
        self._cluster_session: Optional["clusterSession"] = None
        self._cluster_client: Optional["clusterClient"] = None
    ...
    @property
    def cluster_session(self) -> Optional["clusterSession"]:
        """
        Get the current cluster session.

        :return: cluster session or None if not initialized
        """
        return self._cluster_session

    @cluster_session.setter
    def cluster_session(self, session: "clusterSession") -> None:
        """
        Set the cluster session.

        :param session: cluster session instance
        """
        self._cluster_session = session
        self.logger.info("cluster session set")

    @property
    def cluster_client(self) -> Optional["clusterClient"]:
        """
        Get the current cluster client.

        :return: cluster client or None if not initialized
        """
        return self._cluster_client

    @cluster_client.setter
    def cluster_client(self, client: "clusterClient") -> None:
        """
        Set the cluster client.

        :param client: cluster client instance
        """
        self._cluster_client = client
        self.logger.info("cluster client set")
