...
Main editor class providing high-level interface for network manipulation
...
class networkEditor:
    """
    High-level editor for Tasknetwork manipulation.

    Provides a command pattern-based interface for comprehensive
    network editing operations with undo/redo support.
    """
    ...
    def add_observer(self, observer: callable) -> None:
        """
        Add an observer for network changes.
    ...
    def add_task(self, task: Union[TaskStar, Dict[str, Any]]) -> TaskStar:
        ...
        command = AddTaskCommand(self._network, task_data)
    ...
    def create_and_add_task(
        ...
        """
        Create and add a new task to the network.
    ...
    def remove_task(self, task_id: str) -> str:
        ...
        :raises: CommandExecutionError if operation fails
        """
        command = RemoveTaskCommand(self._network, task_id)
        result = self._invoker.execute(command)
    ...
    def update_task(self, task_id: str, **updates) -> TaskStar:
        ...
        :raises: CommandExecutionError if operation fails
        """
        command = UpdateTaskCommand(self._network, task_id, updates)
        result = self._invoker.execute(command)
    ...
    def get_task(self, task_id: str) -> Optional[TaskStar]:
        ...
        """
        return self._network.get_task(task_id)
    ...
    def list_tasks(self) -> List[TaskStar]:
        ...
        """
        return self._network.get_all_tasks()
    ...
    def add_dependency(
        ...
        command = AddDependencyCommand(self._network, dependency_data)
        result = self._invoker.execute(command)
    ...
    def create_and_add_dependency(
        ...
        **kwargs,
        ...
        """
        Create and add a new dependency to the network.
    ...
    def remove_dependency(self, dependency_id: str) -> str:
        """
        Remove a dependency from the network.

        :param dependency_id: ID of dependency to remove
        :return: The removed dependency ID
        :raises: CommandExecutionError if operation fails
        """
        command = RemoveDependencyCommand(self._network, dependency_id)
        result = self._invoker.execute(command)
    ...
    def update_dependency(self, dependency_id: str, **updates) -> TaskStarLine:
        """
        Update a dependency in the network.
        ...
        :raises: CommandExecutionError if operation fails
        """
        command = UpdateDependencyCommand(self._network, dependency_id, updates)
        result = self._invoker.execute(command)
    ...
    def get_dependency(self, dependency_id: str) -> Optional[TaskStarLine]:
        ...
        """
        return self._network.get_dependency(dependency_id)
    ...
    def list_dependencies(self) -> List[TaskStarLine]:
        ...
        """
        return self._network.get_all_dependencies()
    ...
    def get_task_dependencies(self, task_id: str) -> List[TaskStarLine]:
        ...
        """
        return self._network.get_task_dependencies(task_id)

    # Bulk Operations

    def build_network(
        self, config: TasknetworkSchema, clear_existing: bool = True
    ) -> Tasknetwork:
        """
        ...
        :raises: CommandExecutionError if operation fails
        """
        command = BuildnetworkCommand(self._network, config, clear_existing)
        result = self._invoker.execute(command)
        self._notify_observers("build_network", result)
        self._network = result  # Update reference in case of new instance
        return result
    ...
    def build_from_tasks_and_dependencies(
        ...
        """
        Build network from task and dependency lists.
        ...
        return self.build_network(config, clear_existing)

    def clear_network(self) -> Tasknetwork:
        """
        Clear all tasks and dependencies from the network.

        :return: The cleared network
        :raises: CommandExecutionError if operation fails
        """
        command = ClearnetworkCommand(self._network)
        result = self._invoker.execute(command)
        self._notify_observers("clear_network", result)
        return result

    # File Operations

    def load_network(self, file_path: str) -> Tasknetwork:
        """
        ...
        :raises: CommandExecutionError if operation fails
        """
        command = LoadnetworkCommand(self._network, file_path)
        result = self._invoker.execute(command)
        self._notify_observers("load_network", result)
        return result

    def save_network(self, file_path: str) -> str:
        """
        ...
        :raises: CommandExecutionError if operation fails
        """
        command = SavenetworkCommand(self._network, file_path)
        result = self._invoker.execute(command)
        self._notify_observers("save_network", result)
        return result

    def load_from_dict(self, data: Dict[str, Any]) -> Tasknetwork:
        """
        Load network from dictionary data.

        :param data: Dictionary representation of network
        :return: The loaded network
        """
        # Create temporary network and copy state
        temp_network = Tasknetwork.from_dict(data)

        # Use build command to apply the state
        config = temp_network.to_dict()
        return self.build_network(config, clear_existing=True)

    def load_from_json_string(self, json_string: str) -> Tasknetwork:
        """
        ...
        :return: The loaded network
        """
        temp_network = Tasknetwork.from_json(json_data=json_string)
        config = temp_network.to_dict()
        return self.build_network(config, clear_existing=True)
    ...
    def validate_network(self) -> tuple[bool, List[str]]:
        ...
        """
        return self._network.validate_dag()
    ...
    def get_topological_order(self) -> List[str]:
        ...
        :raises: ValueError if network has cycles
        """
        return self._network.get_topological_order()

    def has_cycles(self) -> bool:
        """Check if the network has any cycles."""
        return self._network.has_cycle()

    def get_ready_tasks(self) -> List[TaskStar]:
        """Get tasks that are ready to execute."""
        return self._network.get_ready_tasks()
    ...
    def create_subgraph(self, task_ids: List[str]) -> "networkEditor":
        """
        Create a new editor with a subgraph containing specified tasks.

        :param task_ids: List of task IDs to include in subgraph
        :return: New networkEditor with subgraph
        """
        subgraph_network = Tasknetwork(
            name=f"{self._network.name}_subgraph"
        )
        subgraph_editor = networkEditor(subgraph_network)
    ...
    def merge_network(
        self, other_editor: "networkEditor", prefix: str = ""
    ) -> None:
        """
        Merge another network into this one.

        :param other_editor: networkEditor to merge from
    ...
    def display_network(self, mode: str = "overview") -> None:
        """
        Display the network using visualization.

        :param mode: Display mode ('overview', 'topology', 'details', 'execution')
        """
        self._network.display_dag(mode)
    ...
    def execute_command_by_name(self, command_name: str, *args, **kwargs) -> Any:
        ...
        command = command_registry.create_command(
            command_name, self._network, *args, **kwargs
        )
        if command is None:
    ...
    def __str__(self) -> str:
        ...
        return (
            f"networkEditor("
            f"network={self._network.network_id}, "
            f"tasks={len(self._network.tasks)}, "
            f"dependencies={len(self._network.dependencies)}, "
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"networkEditor("
            f"network_id={self._network.network_id!r}, "
            f"tasks={len(self._network.tasks)}, "
            f"dependencies={len(self._network.dependencies)}, "
