...
"""
Tasknetwork - DAG management system for network V2.
...
from cluster.network.enums import networkState
from cluster.visualization.dag_visualizer import DAGVisualizer

from ..core.interfaces import Inetwork

# Use the network-specific TaskStatus instead of contracts
from .enums import networkState, TaskStatus
...
class Tasknetwork(Inetwork):
    """
    Manages a DAG of tasks (Tasknetwork) with comprehensive orchestration capabilities.
    ...
    def __init__(
        ...
        """
        Initialize a Tasknetwork.

        :param network_id: Unique identifier (auto-generated if None)
        :param name: Human-readable name for the network
        """
        self._network_id: str = (
            network_id
            or f"network_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{str(uuid.uuid4())[:8]}"
        )
        self._name: str = name or self._network_id
        self._state: networkState = networkState.CREATED
    ...
    def add_task(self, task: TaskStar) -> None:
        ...
        # Update network state as task composition changed
        self.update_state()
    ...
    def add_dependency(self, dependency: TaskStarLine) -> None:
        ...
        # Update network state as dependencies changed
        self.update_state()
    ...
    def is_complete(self) -> bool:
        """Check if the entire network has completed execution."""
        return all(task.is_terminal for task in self._tasks.values())

    def update_state(self) -> None:
        """Update the network state based on task states."""
        if not self._tasks:
        ...
        if all_terminal:
            if has_failed and has_completed:
                self._state = networkState.PARTIALLY_FAILED
            elif has_failed:
                self._state = networkState.FAILED
            else:
                self._state = networkState.COMPLETED
        elif has_running or has_completed:
            self._state = networkState.EXECUTING
        else:
    ...
    def start_task(self, task_id: str) -> None:
        ...
        # Update network state as task status changed
        self.update_state()
    ...
    def to_dict(self) -> Dict[str, Any]:
        """
        Convert the Tasknetwork to a dictionary representation.

        :return: Dictionary representation of the Tasknetwork
        """
    ...
    @staticmethod
    def _parse_network_state(state_value: Any) -> networkState:
        """
        Parse network state value (string or networkState) into networkState enum.

        :param state_value: State value to parse
        :return: networkState enum instance
        """
        if isinstance(state_value, networkState):
            return state_value
        elif isinstance(state_value, str):
            # Map string names to networkState
            state_map = {
                "CREATED": networkState.CREATED,
                "READY": networkState.READY,
                "EXECUTING": networkState.EXECUTING,
                "COMPLETED": networkState.COMPLETED,
                "FAILED": networkState.FAILED,
                "PARTIALLY_FAILED": networkState.PARTIALLY_FAILED,
            }
            return state_map.get(state_value.upper(), networkState.CREATED)
        else:
            return networkState.CREATED

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "Tasknetwork":
        """
        Create a Tasknetwork from a dictionary representation.

        :param data: Dictionary representation
        :return: Tasknetwork instance
        """
        # Create network with basic properties
        network = cls(
            network_id=data.get("network_id"), name=data.get("name")
        )

        # Restore state and metadata
        network._state = cls._parse_network_state(
    ...
    def to_json(self, save_path: Optional[str] = None) -> str:
        """
        Convert the Tasknetwork to a JSON string representation.

        :param save_path: Optional file path to save the JSON to disk
        :return: JSON string representation of the Tasknetwork
        ...
        serializable_dict = self._ensure_json_serializable(network_dict)
        ...
        if save_path:
            try:
                ...
                raise IOError(
                    f"OS error saving Tasknetwork to {save_path}: {e}"
                ) from e
            except Exception as e:
                raise IOError(
                    f"Unexpected error saving Tasknetwork to {save_path}: {e}"
                ) from e
    ...
    def from_json(
        ...
        """
        Create a Tasknetwork from a JSON string or JSON file.

        :param json_data: JSON string representation of the Tasknetwork
        :param file_path: Path to JSON file containing Tasknetwork data
        :return: Tasknetwork instance
        ...
        # Create Tasknetwork instance from dictionary
        return cls.from_dict(data)

    @classmethod
    def from_basemodel(cls, schema: "TasknetworkSchema") -> "Tasknetwork":
        """
        Create a Tasknetwork from a Pydantic BaseModel schema.

        :param schema: TasknetworkSchema instance
        :return: Tasknetwork instance
        """
        from cluster.agents.schema import TasknetworkSchema

        if not isinstance(schema, TasknetworkSchema):
            raise ValueError("Expected TasknetworkSchema instance")

        # Convert schema to dict and use existing from_dict method
        data = schema.model_dump()
        return cls.from_dict(data)

    def to_basemodel(self) -> "TasknetworkSchema":
        """
        Convert the Tasknetwork to a Pydantic BaseModel schema.

        :return: TasknetworkSchema instance
        """
        from cluster.agents.schema import TasknetworkSchema

        # Get dictionary representation and create schema
        data = self.to_dict()
        return TasknetworkSchema(**data)
    ...
    def display_dag(self, mode: str = "overview") -> None:
        ...
        try:
            ...
            if mode == "overview":
                visualizer.display_network_overview(self)
            elif mode == "topology":
    ...
    def __str__(self) -> str:
        """String representation of the Tasknetwork."""
        return f"Tasknetwork(id={self._network_id}, tasks={len(self._tasks)}, state={self._state.value})"

    def __repr__(self) -> str:
        """Detailed representation of the Tasknetwork."""
        return (
            f"Tasknetwork(network_id={self._network_id!r}, "
            f"name={self._name!r}, "
