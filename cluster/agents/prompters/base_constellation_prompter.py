...
"""
Base network Agent Prompter.

This module provides the base prompter class for network Agents with
...
from Alien.prompter.basic import BasicPrompter
...
class BasenetworkPrompter(BasicPrompter, ABC):
    """
    Base prompter for network Agent with shared functionality.
    ...
    def __init__(self, prompt_template: str, example_prompt_template: str):
        """
        Initialize base network prompter.
    ...
    def _format_network(self, network: Tasknetwork) -> str:
        """
        Format network information for prompt inclusion with modification hints.

        :param network: Task network object
        :return: Formatted network string with modification indicators
        """
        if network is None:
        ...
        try:
            ...
            return "network information unavailable due to formatting error."

        lines = []

        # Header information
        lines.append(f"Task network: {network_dict.get('name', 'Unnamed')}")
        lines.append(f"Status: {network_dict.get('state', 'unknown')}")
        lines.append(f"Total Tasks: {len(network_dict.get('tasks', {}))}")
        lines.append("")

        # Get modifiable items for reference
        try:
            modifiable_task_ids = {
                task.task_id for task in network.get_modifiable_tasks()
            }
            modifiable_dep_ids = {
                dep.line_id for dep in network.get_modifiable_dependencies()
            }
        except Exception:
        ...
        dependencies = network_dict.get("dependencies", {})
        if dependencies:
    ...
    def user_content_construction(
        ...
        prompt_text = self.user_prompt_construction(request, device_info, network)
    ...
    def user_prompt_construction(
        ...
        prompt = self.prompt_template["user"].format(
            ...
            network=self._format_network(network),
        )
...
class networkPrompterFactory:
    """
    Factory class for creating network prompters based on weaving mode.
    ...
    def create_prompter(
        ...
        if not cls._prompter_classes:
            ...
            cls._prompter_classes = {
                WeavingMode.CREATION: networkCreationPrompter,
                WeavingMode.EDITING: networkEditingPrompter,
            }
        ...
        agent_config = cluster_config.agent.network_AGENT
        if weaving_mode == WeavingMode.CREATION:
            prompt_template = agent_config.network_CREATION_PROMPT
            example_prompt_template = agent_config.network_CREATION_EXAMPLE_PROMPT
        elif weaving_mode == WeavingMode.EDITING:
            prompt_template = agent_config.network_EDITING_PROMPT
            example_prompt_template = agent_config.network_EDITING_EXAMPLE_PROMPT
