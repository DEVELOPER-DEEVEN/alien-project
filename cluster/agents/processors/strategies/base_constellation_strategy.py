...
"""
Base strategies for network Agent processing.

This module provides base classes for different types of network processing strategies,
...
from cluster.agents.processors.processor_context import networkProcessorContext
...
from Alien.agents.memory.memory import MemoryItem
from Alien.agents.processors.core.processor_framework import (
    ProcessingContext,
    ProcessingPhase,
    ProcessingResult,
)
from Alien.agents.processors.core.strategy_dependency import depends_on, provides
from Alien.agents.processors.schemas.actions import (
    ActionCommandInfo,
    ListActionCommandInfo,
)
from Alien.agents.processors.strategies.processing_strategy import BaseProcessingStrategy
from aip.messages import Command, Result
from Alien.llm import AgentType
from Alien.module.context import Context
from Alien.module.dispatcher import BasicCommandDispatcher
...
if TYPE_CHECKING:
    ...
    from Alien.module.basic import FileWriter
...
class networkLLMInteractionStrategy(BaseProcessingStrategy):
    """
    Base LLM interaction strategy for network Agent with shared logic.
    ...
    def __init__(self, fail_fast: bool = True) -> None:
        """
        Initialize base network LLM interaction strategy.
        :param fail_fast: Whether to raise exceptions immediately on errors
        """
        super().__init__(
            name=f"network_llm_interaction",
            fail_fast=fail_fast,
        )

    async def execute(
        self, agent: "networkAgent", context: ProcessingContext
    ) -> ProcessingResult:
        """
        Execute LLM interaction with comprehensive error handling and retry logic.
        :param agent: The network instance.
        ...
        try:
            ...
            network: Tasknetwork = context.get_global("network")
            ...
            self.logger.info(f"network LLM interaction completed successfully")
            ...
            error_msg = (
                f"network LLM interaction failed: {str(traceback.format_exc())}"
            )
    ...
    async def _build_comprehensive_prompt(
        ...
        try:
            ...
            network_json = network.to_json() if network else ""
    ...
    def _log_request_data(
        ...
        """
        try:
            request_data = networkRequestLog(
                step=session_step,
                device_info=device_info,
    ...
    async def _get_llm_response_with_retry(
        self, agent: "networkAgent", prompt_message: Dict[str, Any]
    ) -> tuple[str, float]:
        """
    ...
    def _parse_and_validate_response(
        self, agent: "networkAgent", response_text: str
    ) -> networkAgentResponse:
        """
    ...
    def _validate_response_fields(self, response: networkAgentResponse) -> None:
        """
...
class BasenetworkActionExecutionStrategy(BaseProcessingStrategy):
    """
    Base strategy for executing network actions with shared logic.
    ...
    def __init__(self, weaving_mode: WeavingMode, fail_fast: bool = False) -> None:
        """
        Initialize base network action execution strategy.
    ...
    async def execute(
        ...
        try:
            ...
            parsed_response: networkAgentResponse = context.get_local(
                "parsed_response"
            )
            ...
            error_msg = f"network action execution ({self.weaving_mode.value}) failed: {str(traceback.format_exc())}"
            self.logger.error(error_msg)
    ...
    async def _execute_network_action(
        ...
        try:
            ...
            raise Exception(f"Failed to execute network action: {str(e)}")
...
class networkMemoryUpdateStrategy(BaseProcessingStrategy):
    """
    Memory update strategy for network Agent - shared across all modes.
    ...
    def __init__(self, fail_fast: bool = False) -> None:
        """
        Initialize network Agent memory update strategy.
        :param fail_fast: Whether to raise exceptions immediately on errors
        """
        super().__init__(name="network_memory_update", fail_fast=fail_fast)
    ...
    async def execute(
        ...
        try:
            ...
            error_msg = f"network Agent memory update failed: {str(e)}"
            self.logger.error(error_msg)
    ...
    def _create_and_populate_memory_item(
        ...
        additional_memory: "networkProcessorContext",
    ) -> MemoryItem:
        """
