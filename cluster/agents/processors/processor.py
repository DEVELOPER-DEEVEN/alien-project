...
"""
network Agent Processor - Processor for network Agent using the new framework.
"""
...
class networkAgentProcessor(ProcessorTemplate):
    """
    Enhanced processor for cluster Creator Agent with comprehensive functionality.

    This processor manages the complete workflow of a cluster Creator Agent including:
    ...
    # Override the processor context class to use networkProcessorContext
    processor_context_class: Type[networkProcessorContext] = (
        networkProcessorContext
    )

    def __init__(self, agent: "networkAgent", global_context: Context) -> None:
        """
        Initialize the cluster Creator Agent Processor with enhanced capabilities.
        :param agent: The cluster Creator Agent instance to be processed
    ...
    def _setup_middleware(self) -> None:
        ...
        - networkLoggingMiddleware: Specialized logging for network Agent operations
        """
    ...
    def _get_processor_specific_context_data(self) -> Dict[str, Any]:
        ...
        before_network: Tasknetwork = self.global_context.get(
            ContextNames.network
        )

        if before_network:
            network_before_json = before_network.to_json()
        else:
            network_before_json = None

        return {
            "weaving_mode": self.global_context.get(ContextNames.WEAVING_MODE),
            "device_info": self.global_context.get(ContextNames.DEVICE_INFO),
            "network_before": network_before_json,
        }
...
class networkLoggingMiddleware(EnhancedLoggingMiddleware):
    """
    Specialized logging middleware for network Agent with enhanced contextual information.

    This middleware provides:
    - network Agent specific progress messages with color coding
    - Detailed step information and context logging
    - Performance metrics and execution summaries
    - Enhanced error reporting with network Agent context
    """

    def __init__(self) -> None:
        """Initialize network Agent logging middleware with appropriate log level."""
    ...
    async def before_process(
        ...
        """
        Log network Agent processing start with detailed context information.
        :param processor: network Agent processor instance
        ...
        # Add network Agent specific logging
        ...
        self.logger.info(
            f"network Agent Processing Context - "
    ...
    async def on_error(self, processor: ProcessorTemplate, error: Exception) -> None:
        """
        Enhanced error handling for network Agent with contextual information.
        :param processor: network Agent processor instance
        ...
        self.logger.error(
            f"networkAgent: Encountered error - {str(tb_str)}", "red"
        )
