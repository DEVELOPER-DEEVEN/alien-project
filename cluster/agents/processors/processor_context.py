...
class networkProcessorContext(BasicProcessorContext):
    ...
    This extends the basic context with network specific data including
    ...
    def to_dict(self, selective: bool = True) -> Dict[str, Any]:
        ...
        network_before and network_after from JSON strings
        ...
        if "network_before" in result and isinstance(
            result["network_before"], str
        ):
            try:
                result["network_before"] = json.loads(
                    result["network_before"]
                )
            except (json.JSONDecodeError, TypeError):
                # Keep as string if parsing fails
                pass

        if "network_after" in result and isinstance(
            result["network_after"], str
        ):
            try:
                result["network_after"] = json.loads(
                    result["network_after"]
                )
            except (json.JSONDecodeError, TypeError):
                # Keep as string if parsing fails
                pass
