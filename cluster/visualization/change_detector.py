...
from ..network.task_star_line import TaskStarLine


class VisualizationChangeDetector:
    """
    Utility class for detecting and analyzing changes between network states.
    ...
    @staticmethod
    def calculate_network_changes(
        old_network: Optional["Tasknetwork"],
        new_network: "Tasknetwork",
    ) -> Dict[str, Any]:
        """
        Calculate detailed changes between old and new network by comparing their structure.

        :param old_network: Previous network state (can be None for new network)
        :param new_network: Current network state
        :return: Dictionary containing detailed changes
        """
        changes = {
            "modification_type": "network_created",
            "added_tasks": [],
            "removed_tasks": [],
            "modified_tasks": [],
            "added_dependencies": [],
        ...
        if not old_network:
            # New network - all tasks and dependencies are "added"
            changes["modification_type"] = "network_created"
            changes["added_tasks"] = [
                task.task_id for task in new_network.tasks.values()
            ]
            changes["added_dependencies"] = [
                f"{dep.from_task_id}->{dep.to_task_id}"
                for dep in new_network.dependencies.values()
            ]
            return changes

        # Get task IDs for comparison
        old_task_ids = set(old_network.tasks.keys())
        new_task_ids = set(new_network.tasks.keys())
        ...
        for task_id in common_task_ids:
            old_task = old_network.tasks[task_id]
            new_task = new_network.tasks[task_id]
        ...
        for dep in old_network.dependencies.values():
