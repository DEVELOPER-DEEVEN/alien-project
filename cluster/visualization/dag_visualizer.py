...
"""
DAG Visualization Module for cluster Framework

This module provides DAG topology visualization capabilities for Tasknetwork
...
from ..network.enums import DependencyType, TaskStatus
from ..network.task_star import TaskStar
...
class DAGVisualizer:
    """
    DAG topology visualization for Tasknetwork.
    ...
    def __init__(self, console: Optional[Console] = None):
        ...
        self.network_display = networkDisplay(console)
    ...
    def display_network_overview(
        self,
        network: "Tasknetwork",
        title: str = "Task network Overview",
    ) -> None:
        """
        Display comprehensive network overview using specialized display components.

        :param network: The Tasknetwork to visualize
        :param title: Custom title for the display
        """
        self.console.print()
        self.console.rule(f"[bold cyan]{title}[/bold cyan]")

        # Use network display for basic info and stats
        info_panel = self.network_display._create_basic_info_panel(
            network, title
        )
        stats_panel = self.network_display._create_basic_stats_panel(
            network
        )
        ...
        # DAG topology
        self.display_dag_topology(network)

        # Task details if not too many
        if network.task_count <= 20:
            self.display_task_details(network)

        # Dependency summary
        self.display_dependency_summary(network)

        self.console.print()

    def display_dag_topology(self, network: "Tasknetwork") -> None:
        """
        ...
        if network.task_count == 0:
            self.console.print("[dim]No tasks in network[/dim]")
            return

        # Build topology layers
        layers = self._build_topology_layers(network)
        ...
        tree = Tree("🌌 [bold cyan]Task network[/bold cyan]")
        ...
        for layer_idx, layer_tasks in enumerate(layers):
            ...
            for task in layer_tasks:
                ...
                deps = network.get_task_dependencies(task.task_id)
                if deps:
                    dep_branch = task_branch.add("[dim]Dependencies:[/dim]")
                    for dep in deps:
                        dep_task = network.get_task(dep.from_task_id)
                        if dep_task:
    ...
    def display_task_details(self, network: "Tasknetwork") -> None:
        """
        ...
        tasks = list(network.get_all_tasks())
    ...
    def display_dependency_summary(self, network: "Tasknetwork") -> None:
        """
        ...
        dependencies = network.get_all_dependencies()
        if not dependencies:
        ...
        for dep_type, deps in dep_by_type.items():
            ...
            for dep in deps[:10]:  # Limit to first 10 for readability
                from_task = network.get_task(dep.from_task_id)
                to_task = network.get_task(dep.to_task_id)
    ...
    def display_execution_flow(self, network: "Tasknetwork") -> None:
        """
        ...
        running_tasks = network.get_running_tasks()
        completed_tasks = network.get_completed_tasks()
        failed_tasks = network.get_failed_tasks()
    ...
    def _build_topology_layers(
        ...
        :param network: The Tasknetwork to build layers from
        :return: List of task layers in topological order
        """
        tasks = {task.task_id: task for task in network.get_all_tasks()}
        dependencies = network.get_all_dependencies()
...
def display_network_creation(
    network: "Tasknetwork", console: Optional[Console] = None
) -> None:
    """
    Display network when first created.

    :param network: Newly created Tasknetwork
    :param console: Optional console for output
    """
    display = networkDisplay(console)
    display.display_network_started(
        network, {"status": "New network created"}
    )


def display_network_update(
    network: "Tasknetwork",
    change_description: str = "",
    console: Optional[Console] = None,
) -> None:
    """
    Display network after updates/modifications.

    :param network: Updated Tasknetwork
    ...
    title = "🔄 Task network Updated"
    if change_description:
        title += f" - {change_description}"

    visualizer.display_network_overview(network, title)
...
def display_execution_progress(
    ...
    """
    Display network execution progress.

    :param network: Tasknetwork in execution
    :param console: Optional console for output
    """
    visualizer = DAGVisualizer(console)
    visualizer.display_execution_flow(network)
...
def visualize_dag(
    ...
    :param network: Tasknetwork to visualize
    ...
    if mode == "overview":
        visualizer.display_network_overview(network)
    elif mode == "topology":
        visualizer.display_dag_topology(network)
    elif mode == "details":
        visualizer.display_task_details(network)
    elif mode == "execution":
        visualizer.display_execution_flow(network)
    else:
        visualizer.display_network_overview(network)
