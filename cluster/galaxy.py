...
Primary command-line interface for cluster Framework with comprehensive functionality.
This script provides both simple and advanced interfaces for cluster sessions.

Simple Usage:
    python -m cluster "Create a machine learning pipeline"
    python -m cluster --interactive
    python -m cluster --demo

Advanced Usage:
    python -m cluster --request "Task description" --session-name "my_session"
    python -m cluster --request "Task" --output-dir "./results" --log-level DEBUG
...
# Add Alien-Unis to path to enable imports
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="cluster Framework - AI-powered DAG workflow orchestration",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  Simple Usage:
    python -m cluster "Create a data analysis pipeline"
    python -m cluster --demo
    python -m cluster --interactive

  Advanced Usage:
    python -m cluster --request "Build ML pipeline" --session-name "ml_session"
    python -m cluster --interactive --max-rounds 20 --log-level DEBUG
    python -m cluster --request "Task" --output-dir "./results" --mock
        """,
    )
    ...
    parser.add_argument(
        "--session-name", dest="session_name", help="Custom name for the cluster session"
    )
...
# Parse args and setup logger BEFORE importing clusterClient
# This ensures config warnings are displayed with correct color
args = parse_args()
setup_logger(args.log_level)

# Now import clusterClient after logger is configured
...
async def cluster_quick_start(
    request: str,
    session_name: str = "cluster_quick",
    ...
    :param session_name: Name for the cluster session (default: "cluster_quick")
...
async def cluster_interactive(
    session_name: str = "cluster_interactive",
    log_level: str = "WARNING",
    max_rounds: int = 10,
    output_dir: str = "./logs",
):
    ...
    :param session_name: Name for the cluster session (default: "cluster_interactive")
    :param log_level: Logging level (default: "WARNING")
...
async def main():
    ...
    try:
        ...
        if "client" in locals():
            client.display.print_error(f"❌ cluster Framework error: {e}")
        else:
            # Fallback display for when client is not yet initialized
            from cluster.visualization.client_display import ClientDisplay

            display = ClientDisplay(console=console)
            display.print_error(f"❌ cluster Framework error: {e}")
        logging.error(f"cluster Framework error: {e}", exc_info=True)
        sys.exit(1)
    finally:
...
async def run_demo_with_client(client: clusterClient):
    """
    Run demo mode with initialized client.

    :param client: Initialized clusterClient instance
    """
...
async def run_webui_mode(client: clusterClient):
    """
    ...
    import webbrowser
    import uvicorn
    from cluster.webui.server import app, set_cluster_client

    # Set the cluster client for the WebUI server
    set_cluster_client(client)
    ...
    if frontend_dir.exists():
        ...
        try:
            with open(env_file, "w", encoding="utf-8") as f:
                f.write(f"# Auto-generated by cluster backend\n")
    ...
    # Display banner
    client.display.print_info("🌌 cluster WebUI Starting...")
