...
- **LLM configured** in `config/Alien/agents.yaml` (same as AppAgent)
...
```console
2024-11-06 10:30:22 - Alien.server.app - INFO - Starting Alien Server on 0.0.0.0:5001
...
**Specific IP Binding:**
```bash
python -m Alien.server.app --host 192.168.1.100 --port 5001
```

**Debug Mode:**
```bash
python -m Alien.server.app --port 5001 --log-level DEBUG
```
...
INFO - Alien Client initialized for platform: linux
...
- `user@203.0.113.50`: SSH server address (where the Alien server runs)
...
## 🌌 Step 6: Configure as Alien³ cluster Device

To use the Linux Agent as a managed device within the **Alien³ cluster** multi-tier framework, you need to register it in the `devices.yaml` configuration file.
...
Edit `config/cluster/devices.yaml` and add your Linux agent under the `devices` section:
...
### Using cluster to Control Linux Agents

Once configured, you can launch cluster and it will automatically manage the Linux agents:
...
**cluster will:**
1. ✅ Automatically load device configuration from `config/cluster/devices.yaml`
...
> **ℹ️ cluster Documentation:** For detailed cluster configuration and usage, see:
> 
...
python -m Alien.client.client --ws --client-id linux_agent_1
...
### Issue 5: cluster Cannot Find Device
...
| **cluster Integration** | Multi-device orchestration | [cluster Overview](../cluster/overview.md) |
...
✅ (Optional) Registered the device in cluster configuration  
...
- 🌌 Integrate with Alien³ cluster for multi-device workflows
