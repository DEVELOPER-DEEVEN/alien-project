...
> - **cluster Integration:** [As cluster Device](../mobile/as_cluster_device.md)
...
- **LLM configured** in `config/Alien/agents.yaml` (see [Model Configuration](../configuration/models/overview.md))
...
```bash
python -m Alien.server.app --port 5001 --platform mobile
```

**Expected Output:**

```console
INFO - Starting Alien Server on 0.0.0.0:5001
...
```bash
# Different port
python -m Alien.server.app --port 8080 --platform mobile

# Localhost only
python -m Alien.server.app --host 127.0.0.1 --port 5001 --platform mobile

# Debug mode
python -m Alien.server.app --port 5001 --platform mobile --log-level DEBUG
```
...
```console
====================================================================
Alien Mobile MCP Servers (Android)
...
INFO - Alien Client initialized for platform: mobile
...
## 🌌 Step 7: Configure as Alien³ cluster Device

To use the Mobile Agent as a managed device within the **Alien³ cluster** multi-tier framework, you need to register it in the `devices.yaml` configuration file.

> **📖 Detailed Guide:** For comprehensive information on using Mobile Agent in cluster, including multi-device workflows and advanced configuration, see [Using Mobile Agent as cluster Device](../mobile/as_cluster_device.md).
...
```
config/cluster/devices.yaml
```

### Add Mobile Agent Configuration

Edit `config/cluster/devices.yaml` and add your Mobile agent:
...
> 
> **If these don't match, cluster cannot control the device!**

### Using cluster to Control Mobile Agents

Once configured, launch cluster:
...
**cluster will:**
1. ✅ Load device configuration from `config/cluster/devices.yaml`
2. ✅ Connect to all configured Android devices
3. ✅ Orchestrate multi-device tasks
4. ✅ Route tasks based on capabilities

> **ℹ️ cluster Documentation:** For detailed cluster usage, see:
> 
> - [cluster Overview](../cluster/overview.md)
> - [cluster Quick Start](quick_start_cluster.md)
> - [Mobile Agent as cluster Device](../mobile/as_cluster_device.md)
...
3. **Check firewall settings:**
   ...
   netsh advfirewall firewall add rule name="Alien Server" dir=in action=allow protocol=TCP localport=5001
...
python -m Alien.client.client --ws --client-id mobile_phone_1
...
3. **Run separate MCP servers:**
   ```bash
   # Server for emulator
   ANDROID_SERIAL=emulator-5554 python -m Alien.client.mcp.http_servers.mobile_mcp_server --data-port 8020 --action-port 8021 --server both
   
   # Server for physical device
   ANDROID_SERIAL=192.168.1.100:5555 python -m Alien.client.mcp.http_servers.mobile_mcp_server --data-port 8022 --action-port 8023 --server both
   ```
...
| **cluster Integration** | Multi-device orchestration with Alien³ | [As cluster Device](../mobile/as_cluster_device.md) |
...
✅ (Optional) Configured device in cluster  
...
- 🌌 Integrate with Alien³ cluster for cross-platform workflows
...
python -m Alien.client.client --ws --ws-server ws://localhost:5001/ws --client-id mobile_phone_1 --platform mobile
...
Start-Process powershell -ArgumentList "-NoExit", "-Command", "python -m Alien.server.app --port 5001 --platform mobile"
Start-Sleep 2
Start-Process powershell -ArgumentList "-NoExit", "-Command", "python -m Alien.client.mcp.http_servers.mobile_mcp_server --server both"
Start-Sleep 2
Start-Process powershell -ArgumentList "-NoExit", "-Command", "python -m Alien.client.client --ws --ws-server ws://localhost:5001/ws --client-id mobile_phone_1 --platform mobile"
```
