...
- Using as Sub-Agent in Network? See [Using Linux Agent as Network Device](as_network_device.md)

## Architecture Overview

LinuxAgent operates as a single-agent instance that interacts with Linux systems through command-line interface (CLI) commands. Unlike the two-tier architecture of ALIEN (HostAgent + AppAgent), LinuxAgent uses a simplified single-agent model optimized for shell-based automation.
...
- **Cross-Device Workflows**: As a sub-agent in Network multi-device orchestration

!!!tip "Network Integration"
    LinuxAgent can serve as a device agent in Network's multi-device orchestration framework, executing Linux-specific tasks as part of cross-platform workflows alongside Windows and other devices.
    
    See [Using Linux Agent as Network Device](as_network_device.md) for configuration details.
