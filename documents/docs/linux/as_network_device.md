# Using Linux Agent as Network Device

Configure Linux Agent as a sub-agent in ALIEN's Network framework to enable cross-platform, multi-device task orchestration. Network can coordinate Linux agents alongside Windows devices to execute complex workflows spanning multiple systems.

## Overview

The **Network framework** provides multi-tier orchestration capabilities, allowing you to manage multiple device agents (Windows, Linux, etc.) from a central OrionAgent. When configured as a Network device, LinuxAgent becomes a **sub-agent** that can:

- Execute Linux-specific subtasks assigned by Network
...
## Network Architecture with Linux Agent
...
graph TB
    ...
    Network -->|Windows Subtask| Win2
    Network -->|Linux Subtask| Linux1
    Network -->|Linux Subtask| Linux2
    Network -->|Linux Subtask| Linux3
...
Network orchestrates task decomposition, device selection based on capabilities, parallel execution, and result aggregation across all devices.

## Configuration Guide

### Step 1: Configure Device in `devices.yaml`

Add your Linux agent(s) to the device list in `config/network/devices.yaml`:
...
Network uses the `capabilities` field to intelligently route subtasks to appropriate devices. Define capabilities based on server roles, task types, installed software, or data access requirements.
...
## Starting Network with Linux Agents
...
4. LLM configured in `config/alien/agents.yaml` (for ALIEN) or `config/network/agent.yaml` (for Network)
...
sequenceDiagram
    ...
    Note over Network,WinDesktop: Subtask 2: Create Excel report
    Network->>WinDesktop: "Create Excel report from CSV"
    WinDesktop->>WinDesktop: Open Excel
    WinDesktop->>WinDesktop: Import CSV
    WinDesktop->>WinDesktop: Format report
    WinDesktop-->>Network: Excel file created
    
    Note over Network,WinDesktop: Subtask 3: Send email
    Network->>WinDesktop: "Email report to team"
    WinDesktop->>WinDesktop: Open Outlook
    WinDesktop->>WinDesktop: Attach file
    WinDesktop->>WinDesktop: Send email
    WinDesktop-->>Network: Email sent
...
4. **Network**: Aggregate results and generate summary report
...
### How Network Routes Tasks to Linux Agents

Network's OrionAgent uses several factors to select the appropriate device for each subtask:
...
INFO - [Network] Subtask 1 → linux_web_server (capability match: web_server)
INFO - [Network] Subtask 2 → linux_db_server (capability match: database_server)
INFO - [Network] Subtask 3 → windows_desktop_1 (capability match: email)
```

### Troubleshooting Device Connection

**Issue**: Linux agent not appearing in Network device pool
...
Using Linux Agent as a Network device enables multi-device orchestration with capability-based routing, metadata context for LLM-aware command generation, parallel execution, and seamless cross-platform workflows between Linux and Windows agents.
