...
python -m alien --task {task_name} --mode batch_normal --plan {plan_file}
```
...
ALIEN can automatically evaluate whether the task was completed successfully. To enable evaluation, ensure `EVA_SESSION` is set to `True` in the `config/alien/system.yaml` file.
...
The `FromFileSession` is located at `alien/module/sessions/session.py`.
