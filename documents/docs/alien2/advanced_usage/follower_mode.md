...
Follower mode enables ALIEN to execute a predefined list of steps in natural language. Unlike normal mode where the agent generates its own plan, follower mode creates an `AppAgent` that follows user-provided steps to interact with applications. This mode is particularly useful for debugging, software testing, and verification.
...
python -m alien --task {task_name} --mode follower --plan {plan_file}
```
...
```bash
# Assume you are in the cloned ALIEN folder
python -m alien --task {task_name} --mode follower --plan {plan_folder}
``` 

ALIEN will automatically detect and execute all plan files in the folder sequentially.
...
ALIEN can automatically evaluate task completion. To enable evaluation, ensure `EVA_SESSION` is set to `True` in `config/alien/system.yaml`.
...
The `PlanReader` is located at `alien/module/sessions/plan_reader.py`.

:::module.sessions.plan_reader.PlanReader

### FollowerSession

The `FollowerSession` is located at `alien/module/sessions/session.py`.
