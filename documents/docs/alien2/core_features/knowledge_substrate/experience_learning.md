...
When ALIEN successfully completes a task, users can save the successful experience to enhance the AppAgent's future performance. The AppAgent learns from its own successful experiences to improve task execution.
...
1. **Complete a Session**: ALIEN finishes executing a task successfully
...
The `ExperienceSummarizer` class in `alien/experience/summarizer.py` handles experience summarization:

:::experience.summarizer.ExperienceSummarizer

### Experience Retriever

The `ExperienceRetriever` class in `alien/rag/retriever.py` handles experience retrieval:
