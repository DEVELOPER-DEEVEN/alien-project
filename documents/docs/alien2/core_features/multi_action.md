...
ALIEN² introduces **Speculative Multi-Action Execution**, a feature that allows agents to bundle multiple predicted steps into a single LLM call and validate them against the live application state. This approach can reduce LLM queries by up to **51%** compared to inferring each action separately.
...
Enable speculative multi-action execution in `config/alien/system.yaml`:
...
**Configuration Location**: `config/alien/system.yaml` (migrated from legacy `config_dev.yaml`)

For configuration migration details, see [Configuration Migration Guide](../../configuration/system/migration.md).

## Implementation Details

The multi-action system is implemented through two core classes in `alien/agents/processors/schemas/actions.py`:
