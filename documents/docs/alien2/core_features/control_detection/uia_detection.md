...
UIA is the default control detection backend. Configure it in `config/alien/system.yaml`:
...
ALIEN² uses the `ControlInspectorFacade` class to interact with the UIA framework. The facade pattern provides a simplified interface to:
