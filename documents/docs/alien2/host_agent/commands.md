...
!!!note "Dynamic Commands"
    HostAgent commands are **not hardcoded**. They are dynamically discovered from configured MCP servers. Available commands depend on MCP server configuration in `config/alien/mcp.yaml`, installed MCP servers, and active MCP connections.
...
HostAgent commands are configured in **`config/alien/mcp.yaml`**:
