...
|-----------|------|---------|
| **AlienWebSocketClient** | Main Class | Orchestrates all WebSocket communication |
...
sequenceDiagram
    ...
    Main->>WSC: Create AlienWebSocketClient(ws_url, Alien_client)
...
# Create Alien client (execution engine)
...
| `Alien_client` | `AlienClient` | Required | Alien client instance for command execution |
...
Alien uses a **push model** for device information: clients proactively send their profile during registration, rather than waiting for the server to request it. This reduces latency for network (multi-client) scenarios.
...
async def register_client(self):
    ...
    success = await self.registration_protocol.register_as_device(
        device_id=self.Alien_client.client_id,
        metadata=metadata,
...
sequenceDiagram
    participant Server
    participant WSC as WebSocket Client
    participant UFC as Alien Client
...
async def start_task(self, request_text: str, task_name: str | None):
    ...
    async def task_loop():
        try:
            async with self.Alien_client.task_lock:
                self.Alien_client.reset()  # Clear previous session state
                
                # Build metadata with platform info
                metadata = {}
                if self.Alien_client.platform:
                    metadata["platform"] = self.Alien_client.platform
                
                # Send task request via AIP
                await self.task_protocol.send_task_request(
                    request=request_text,
                    task_name=task_name if task_name else str(uuid4()),
                    session_id=self.Alien_client.session_id,
                    client_id=self.Alien_client.client_id,
                    metadata=metadata if metadata else None
                )
                
                self.logger.info(
                    f"[WS] \[AIP] Sent task request with platform: {self.Alien_client.platform}"
                )
        except Exception as e:
...
async def handle_commands(self, server_response: ServerMessage):
    ...
    # Execute commands via Alien Client
    action_results = await self.Alien_client.execute_step(server_response)
...
2. **Execute Commands**: Delegate to `Alien_client.execute_step()`
...
ws_client = AlienWebSocketClient(ws_url, Alien_client)
...
ws_client = AlienWebSocketClient(
    ws_url="wss://Alien-server.company.com/ws",  # WSS, not WS
    Alien_client=Alien_client
)
```
...
```python
async with self.Alien_client.task_lock:
    self.Alien_client.reset()  # Clears session state
...
The WebSocket client delegates all command execution to the Alien Client.
...
| WebSocket Client Role | Alien Client Role |
|----------------------|-----------------|
...
See [Alien Client](./Alien_client.md) for execution details.
...
2. **Understand Command Execution** - Learn how the Alien Client executes commands: [Alien Client Documentation](./Alien_client.md)
