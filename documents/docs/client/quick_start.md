...
```bash
python -m alien.client.client --ws --client-id my_device
```
...
curl -X POST http://localhost:5000/api/dispatch \
  ...
  -d '{
    "client_id": "device_windows_001",
    "request": "Open Notepad and type Hello from ALIEN"
  }'
```
...
```log
INFO - [WS] Starting task: Open Notepad and type Hello from ALIEN
...
python -m alien.client.client --ws --client-id NEW_UNIQUE_ID
```
...
Create `/etc/systemd/system/alien-client.service`:

```ini
[Unit]
Description=ALIEN Device Client - Execution Agent
Documentation=https://github.com/DEVELOPER-DEEVEN/alien-project
...
User=alienuser
Group=alienuser
WorkingDirectory=/home/alienuser/ALIEN2

# Environment variables (if needed)
Environment="PYTHONUNBUFFERED=1"

# Main command
ExecStart=/usr/bin/python3 -m alien.client.client \
  --ws \
  --ws-server ws://192.168.1.100:5000/ws \
...
sudo systemctl enable alien-client

# Start service now
sudo systemctl start alien-client

# Check status
sudo systemctl status alien-client

# View logs
sudo journalctl -u alien-client -f
```
...
| Command | Purpose |
|---------|---------|
| `systemctl start alien-client` | Start the service |
| `systemctl stop alien-client` | Stop the service |
| `systemctl restart alien-client` | Restart the service |
| `systemctl status alien-client` | Check service status |
| `journalctl -u alien-client -f` | Follow logs in real-time |
| `systemctl disable alien-client` | Disable auto-start |
...
```powershell
# Install as service
nssm install ALIENClient "C:\Python310\python.exe" `
  "-m" "alien.client.client" `
  "--ws" `
  "--ws-server" "ws://192.168.1.100:5000/ws" `
  "--client-id" "device_windows_prod_01" `
  "--log-level" "INFO"
...
nssm set ALIENClient AppDirectory "C:\ALIEN2"

# Set restart policy
nssm set ALIENClient AppExit Default Restart
nssm set ALIENClient AppRestartDelay 10000

# Set logging
nssm set ALIENClient AppStdout "C:\ALIEN2\logs\client-stdout.log"
nssm set ALIENClient AppStderr "C:\ALIEN2\logs\client-stderr.log"
```
...
```powershell
# Start service
nssm start ALIENClient

# Check status
nssm status ALIENClient

# Stop service
nssm stop ALIENClient

# Remove service
nssm remove ALIENClient confirm
```
...
$action = New-ScheduledTaskAction -Execute "python.exe" `
  -Argument "-m alien.client.client --ws --ws-server ws://server:5000/ws --client-id device_win_01"
$trigger = New-ScheduledTaskTrigger -AtStartup
$settings = New-ScheduledTaskSettingsSet -RestartCount 3 -RestartInterval (New-TimeSpan -Minutes 1)

Register-ScheduledTask -TaskName "ALIENClient" `
  -Action $action `
  -Trigger $trigger `
  -Settings $settings `
  -User "System" `
...
# Or start directly
pm2 start "python -m alien.client.client --ws --ws-server ws://192.168.1.100:5000/ws --client-id device_001" \
  --name alien-client
...
| `pm2 start alien-client` | Start process |
| `pm2 stop alien-client` | Stop process |
| `pm2 restart alien-client` | Restart process |
| `pm2 delete alien-client` | Remove process |
| `pm2 logs alien-client` | View logs |
...
# View logs:
journalctl -u alien-client -f --since "1 hour ago"
```
...
*/5 * * * * /usr/local/bin/check-alien-client.sh
```
...
--ws-server wss://alien-server.company.com/ws
...
```bash
# Only show errors
python -m alien.client.client --ws --client-id my_device 2>&1 | grep ERROR
    
# Only show connection events
python -m alien.client.client --ws --client-id my_device 2>&1 | grep -E "Connect|Register"
```
