...
graph TB
    ...
    Client[ALIEN Client<br/>Session Orchestration]
...
|-----------|----------|----------------|
| **Computer** | `alien.client.computer.Computer` | Manages MCP servers, routes tool calls, executes in thread pool |
| **MCP Server Manager** | `alien.client.mcp.mcp_server_manager.MCPServerManager` | Creates/manages server instances (local/http/stdio) |
| **Command Router** | `alien.client.computer.CommandRouter` | Routes commands to appropriate Computer instances |
...
|-------|-----------|-------------|
| **1. Command Reception** | ALIEN Client | Receives AIP Command from server |
...
| **5. Result Return** | ALIEN Client | Packages result for server |
...
```python
from alien.client.computer import Computer
...
MCP servers are configured in `config/alien/mcp.yaml`:
...
```python
from alien.client.computer import ComputerManager, CommandRouter
from alien.client.mcp.mcp_server_manager import MCPServerManager
...
# Register in config/alien/mcp.yaml:
# action:
...
See [ALIEN Client](./alien_client.md) and [Computer](./computer.md) for integration details.
...
| **ALIEN Client** | Session orchestration | [ALIEN Client](./alien_client.md) |
...
- Servers configured in `config/alien/mcp.yaml`
