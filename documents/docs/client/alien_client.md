...
The ALIEN Client follows a stateless execution philosophy:
...
computer_manager = ComputerManager(
    alien_config.to_dict(),
    mcp_server_manager
)

# 3. Create ALIEN Client
client = ALIENClient(
...
|------|--------|-----------|
| 1 | Receive commands | ALIEN Client |
| 2 | Log command count | ALIEN Client |
| 3 | Call CommandRouter | ALIEN Client |
| 4 | Route to Computer | CommandRouter |
| 5 | Execute via MCP | Computer |
| 6 | Collect results | CommandRouter |
| 7 | Return results | ALIEN Client |
...
async with self.alien_client.task_lock:
    self.alien_client.reset()  # Automatic
...
This example shows how to use the ALIEN Client in a typical workflow.
...
The WebSocket client uses ALIEN Client for all command execution.
...
The ALIEN Client delegates all execution to the CommandRouter.
...
1. **Understand Network Communication** - Learn how the WebSocket client uses ALIEN Client: [WebSocket Client](./websocket_client.md)
