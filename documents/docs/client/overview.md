# ALIEN Client Overview

The **ALIEN Client** runs on target devices and serves as the **execution layer** of ALIEN's distributed agent system. It manages MCP (Model Context Protocol) servers, executes commands deterministically, and communicates with the Agent Server through the Agent Interaction Protocol (AIP).
...
**The ALIEN Client is a stateless execution agent that:**
...
**Architecture:** The ALIEN Client is part of ALIEN's distributed **server-client architecture**, where it handles command execution and resource access while the [Agent Server](../server/overview.md) handles orchestration and decision-making. See [Server-Client Architecture](../infrastructure/agents/server_client_architecture.md) for the complete design rationale, communication protocols, and deployment patterns.
...
| **ALIEN Client** | Execution orchestration | • Command execution<br/>• Result aggregation<br/>• Error handling<br/>• Session management | [Details →](./alien_client.md) |
...
sequenceDiagram
    participant Server
    participant Client as ALIEN Client
...
| 2️⃣ **Load Config** | Load ALIEN configuration | MCP servers, tools, settings |
...
Start the ALIEN client with:

```bash
python -m alien.client.client [OPTIONS]
```
...
python -m alien.client.client \
  ...
  --ws-server ws://alien-server.company.com:5000/ws \
  --max-retries 10 \
  --log-level INFO
```
...
```ini
[Unit]
Description=ALIEN Agent Client
...
ExecStart=/usr/bin/python3 -m alien.client.client \
  ...
  --ws-server ws://alien-server.internal:5000/ws \
  --log-level INFO
Restart=always
RestartSec=10
...
| [ALIEN Client](./alien_client.md) | Orchestration | Session tracking, command execution |
...
| [Configuration](../configuration/system/overview.md) | ALIEN configuration | Customize behavior |
