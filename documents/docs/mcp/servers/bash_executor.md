...
```ini
# /etc/systemd/system/Alien-bash-mcp.service
[Unit]
Description=Alien Bash MCP Server
After=network.target

[Service]
Type=simple
User=Alien
WorkingDirectory=/home/Alien/Alien-Unis
ExecStart=/usr/bin/python3 -m Alien.client.mcp.http_servers.linux_mcp_server --host 0.0.0.0 --port 8010
Restart=on-failure
...
Enable and start:
```bash
sudo systemctl enable Alien-bash-mcp
sudo systemctl start Alien-bash-mcp
sudo systemctl status Alien-bash-mcp
```
...
2. **Run as limited user**: Don't run server as root
   ...
   sudo -u Alien python3 -m Alien.client.mcp.http_servers.linux_mcp_server
   ```
