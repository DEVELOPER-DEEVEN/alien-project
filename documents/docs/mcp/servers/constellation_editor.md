# networkEditor Server

## Overview

**networkEditor** provides multi-device task coordination and dependency management for distributed workflows in Alien².
...
| **Server Name** | `Alien network Editor MCP Server` |
...
| **Bulk Operations** | `build_network` | Build complete network from config |
...
`str` - JSON representation of complete Tasknetwork after adding task
...
Remove a task from the network (also removes all dependencies involving this task).
...
`str` - JSON representation of network after removal
...
`str` - JSON representation of network after update
...
`str` - JSON representation of network after adding dependency
...
Build a complete network from configuration data (batch creation).

#### Parameters

| Parameter | Type | Required | Default | Description |
|-----------|------|----------|---------|-------------|
| `config` | `TasknetworkSchema` | ✅ Yes | - | Complete network configuration |
...
`str` - JSON representation of built network
...
- [Configuration Guide](../configuration.md) - network setup
