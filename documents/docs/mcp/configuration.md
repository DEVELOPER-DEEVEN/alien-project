...
MCP configuration in Alien² uses a **hierarchical YAML structure** that maps agents to their MCP servers. The configuration file is located at:
...
config_path = Path("config/Alien/mcp.yaml")
...
Alien² validates MCP configuration on load:

```python
from Alien.config.config_schemas import MCPConfigSchema
...
If you're migrating from an older Alien configuration:

**Old Format** (config.yaml):
```yaml
MCP_SERVERS:
  - name: ui_collector
    module: Alien.mcp.ui_server
```
...
- [HostAgent Overview](../Alien-Unis/host_agent/overview.md) - HostAgent configuration examples
- [AppAgent Overview](../Alien-Unis/app_agent/overview.md) - AppAgent configuration examples
