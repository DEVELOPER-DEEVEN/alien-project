...
Remote MCP servers run as separate processes or on different machines, communicating with ALIEN² over HTTP or stdio. This enables **cross-platform automation**, process isolation, and distributed workflows.

**Cross-Platform Automation:** Remote servers enable **Windows ALIEN² agents to control Linux systems, mobile devices, and hardware** through HTTP MCP servers running on those platforms.
...
**Server Start:**
```bash
python -m alien.client.mcp.http_servers.hardware_mcp_server --host 0.0.0.0 --port 8006
```
...
**Server Start:**
```bash
python -m alien.client.mcp.http_servers.linux_mcp_server --host 0.0.0.0 --port 8010
```
...
python -m alien.client.mcp.http_servers.mobile_mcp_server --server both --host 0.0.0.0 --data-port 8020 --action-port 8021
```
