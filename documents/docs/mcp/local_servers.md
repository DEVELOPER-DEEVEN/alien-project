...
Local MCP servers run in-process with the Alien² agent, providing fast and efficient access to tools without network overhead. They are the most common server type for built-in functionality.

**For remote HTTP servers** (BashExecutor, HardwareExecutor, MobileExecutor), see [Remote Servers](./remote_servers.md).

## Overview

Alien² includes several built-in local MCP servers organized by functionality. This page provides a quick reference - click each server name for complete documentation.
...
| **networkEditor** | Action | Multi-device task orchestration | **[→ Full Docs](servers/network_editor.md)** |
...
**[→ See complete networkEditor documentation](servers/network_editor.md)** for multi-device workflow tools.

---

## Configuration

All local servers are configured in `config/Alien/mcp.yaml`. For detailed configuration options, see:
...
- [HostAgent Overview](../Alien-Unis/host_agent/overview.md) - HostAgent architecture
- [AppAgent Overview](../Alien-Unis/app_agent/overview.md) - AppAgent architecture
- [Hybrid Actions](../Alien-Unis/core_features/hybrid_actions.md) - GUI + API dual-mode automation
