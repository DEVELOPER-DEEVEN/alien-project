...
The **Network WebUI** is a modern, interactive web interface for the ALIEN³ Network Framework. It provides real-time visualization of task orions, device status, agent interactions, and execution flow through an elegant, space-themed interface.

<div align="center">
  <img src="../img/webui.png" alt="Network WebUI Interface" width="90%">
  <p><em>Network WebUI - Interactive orion visualization and real-time monitoring</em></p>
</div>
...
!!!tip "Custom Session Name"
    ```powershell
    python -m network --webui --session-name "data_pipeline_demo"
    ```
...
3. **Watch the orion form** in the DAG visualization panel
...
The Network WebUI backend follows **software engineering best practices**:
...
The Network WebUI follows a modern client-server architecture with real-time event streaming:

```mermaid
graph TB
    subgraph "Network WebUI Stack"
        ...
        subgraph Backend["Backend (FastAPI + WebSocket)"]
            ...
            subgraph Business["Business Logic Layer"]
                B3[Services<br/>Config/Device/Network]
...
The Network WebUI backend follows a **modular architecture** with clear separation of concerns:

| Component | File/Directory | Responsibility |
|-----------|----------------|----------------|
| **FastAPI Server** | `network/webui/server.py` | Application initialization, middleware, router registration, lifespan management |
| **Models** | `network/webui/models/` | Pydantic models for requests/responses, enums for type safety |
| **Services** | `network/webui/services/` | Business logic layer (config, device, network operations) |
| **Handlers** | `network/webui/handlers/` | WebSocket message processing and routing |
| **Routers** | `network/webui/routers/` | FastAPI endpoint definitions organized by feature |
| **Dependencies** | `network/webui/dependencies.py` | Dependency injection for state management (AppState) |
| **WebSocket Observer** | `network/webui/websocket_observer.py` | Event subscription and broadcasting to WebSocket clients |
...
│   └── network_service.py    # Network client interactions
├── handlers/
│   ├── __init__.py
...
| **DagPreview** | `src/components/orion/DagPreview.tsx` | Interactive orion graph visualization |
...
| **RightPanel** | `src/components/layout/RightPanel.tsx` | Tabbed panel for orion, tasks, details |
| **EventLog** | `src/components/EventLog.tsx` | Real-time event stream display |
| **NetworkStore** | `src/store/networkStore.ts` | Zustand state management |
...
The WebUI maintains a persistent WebSocket connection to the Network backend for bidirectional real-time communication.
...
{
  "type": "welcome",
  "message": "Connected to Network Web UI",
  "timestamp": 1234567890
}
```
...
All Network events are forwarded to the WebUI in real-time:

```json
{
  "event_type": "agent_response",
  "source_id": "OrionAgent",
  "timestamp": 1234567890,
  "agent_name": "OrionAgent",
  "agent_type": "orion",
  "output_type": "response",
  "output_data": {
    "thought": "I need to decompose this task...",
    "plan": ["Analyze requirements", "Create DAG", "Assign devices"],
    "response": "Creating orion with 3 tasks"
  }
}
```
...
```json
{
  "event_type": "orion_created",
  "source_id": "TaskOrion",
  "timestamp": 1234567890,
  "orion_id": "orion_123",
  "orion_state": "planning",
  "data": {
    "orion": {
      "orion_id": "orion_123",
...
**Message Types:**
- **User Messages**: Your requests to Network
- **Agent Responses**: OrionAgent thoughts, plans, and responses
...
2. **Configuration**: Device is saved to `config/network/devices.yaml`
3. **Registration**: Device is registered with the Network Device Manager
...
The Network WebUI uses a **space-themed design** with a dark color palette and vibrant accents.

#### Color Palette

```typescript
// Primary Colors
network-dark:   #0a0e27  // Deep space background
network-blue:   #00d4ff  // Cyan accent (primary actions)
network-purple: #7b2cbf  // Purple accent (secondary)
network-pink:   #ff006e  // Pink accent (tertiary)
...
```powershell
python -m network --webui [OPTIONS]
```
...
| `--session-name` | Session display name | `"Network Session"` |
...
Builds production-ready frontend to `network/webui/frontend/dist/`
...
```mermaid
flowchart TD
    A[Network Core Event] --> B[Event Bus publish]
...
**Features:**
- **Type Handler Registry**: Pre-registered handlers for Network-specific types (TaskStarLine, TaskOrion)
...
- **Polymorphic Event Handling**: Different serialization logic for TaskEvent, OrionEvent, AgentEvent, DeviceEvent
...
4. Check registered type handlers (TaskStarLine, TaskOrion)
...
The WebUI subscribes to all Network event types:

| Event Type | Source | Description |
|------------|--------|-------------|
| `agent_request` | OrionAgent | User request received |
| `agent_response` | OrionAgent | Agent thought/plan/response |
| `orion_created` | TaskOrion | New orion formed |
| `orion_updated` | TaskOrion | Orion modified |
| `orion_completed` | TaskOrion | All tasks finished |
...
interface NetworkStore {
  ...
  // Data
  messages: Message[];
  orions: Record<string, OrionSummary>;
  ...
  // UI State
  ui: {
    activeOrionId: string | null;
    activeTaskId: string | null;
    activeDeviceId: string | null;
    rightPanelTab: 'orion' | 'tasks' | 'details';
...
4. **Try different port:**
   ```powershell
   python -m network --webui --port 8080
   ```
...
2. **Check build output:**
   - Verify `network/webui/frontend/dist/` exists
...
3. **Enable debug mode:**
   ```powershell
   python -m network --webui --log-level DEBUG
   ```
...
2. **Reduce DAG complexity:**
   - Large orions (>50 tasks) may be slow
...
1. **Check `devices.yaml` exists:**
   ...
   Test-Path config/network/devices.yaml
   ```
...
3. **Check device logs:**
   ...
   - Verify device can reach the Network server
...
# 3. In another terminal, start Network backend
cd ../../..
python -m network --webui
```
...
│   ├── config_service.py       # Configuration management
│   ├── device_service.py       # Device operations and snapshots
│   └── network_service.py       # Network client interaction
...
Output: `network/webui/frontend/dist/`
...
import pytest
from network.webui.websocket_observer import EventSerializer
from network.core.events import TaskEvent
...
def test_build_device_snapshot():
    ...
    app_state = AppState()
    # Setup mock network_client with devices
...
from network.webui.models.enums import WebSocketMessageType
...
The modular architecture makes it easy to extend the Network WebUI backend:
...
```python
# network/webui/models/requests.py
...
```python
# network/webui/models/responses.py
...
```python
# network/webui/services/task_service.py
...
class TaskService:
    ...
    def get_task_details(self, task_id: str, include_history: bool) -> Dict[str, Any]:
        ...
        network_session = self.app_state.network_session
        if not network_session:
            raise ValueError("No active Network session")
        
        # Your business logic here
        task = network_session.get_task(task_id)
        return {
...
```python
# network/webui/routers/tasks.py
from fastapi import APIRouter, Depends
from network.webui.dependencies import get_app_state
from network.webui.models.requests import TaskQueryRequest
from network.webui.models.responses import TaskQueryResponse
from network.webui.services.task_service import TaskService
...
```python
# network/webui/server.py
from network.webui.routers import tasks_router
...
```python
# network/webui/handlers/websocket_handlers.py
...
```python
# network/webui/websocket_observer.py
...
import { NetworkEvent } from './websocket';

export function handleCustomEvent(event: NetworkEvent) {
...
export const MyVisualization: React.FC = () => {
  const orion = useNetworkStore(s => 
    s.orions[s.ui.activeOrionId || '']
  );
...
- [Task Orchestrator](orion_orchestrator/overview.md)
- [Device Manager](client/device_manager.md)

### Community
- [GitHub Issues](https://github.com/DEVELOPER-DEEVEN/alien-project/issues)
- [GitHub Discussions](https://github.com/DEVELOPER-DEEVEN/alien-project/discussions)
...
1. **[Quick Start Guide](../getting_started/quick_start_network.md)** - Set up your first Network session
2. **[Orion Agent](orion_agent/overview.md)** - Learn about task decomposition
