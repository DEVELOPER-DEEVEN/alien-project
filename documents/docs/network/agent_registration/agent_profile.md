...
For a complete understanding of how agents work in the orion system, see:

- [Orion Overview](../orion/overview.md) - Architecture and multi-device coordination
- [Orion Agent](../orion_agent/overview.md) - Agent behavior and lifecycle
...
The `device_id` must be unique across the entire orion. Attempting to register a duplicate `device_id` will fail.
...
```python
from network.client.components import DeviceRegistry, AgentProfile, DeviceStatus
