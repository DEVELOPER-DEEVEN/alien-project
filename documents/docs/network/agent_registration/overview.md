...
At the core of AIP's initialization process is the **OrionClient** (implemented as `OrionDeviceManager`), which maintains a global registry of active agents. Any device agent service that exposes a WebSocket endpoint and implements the AIP task dispatch and result-return protocol can be seamlessly integrated into ALIEN, providing remarkable **extensibility**.
...
For a complete understanding of the orion system, see:

- [Orion Overview](../orion/overview.md) - Multi-device coordination architecture
- [Orion Agent Overview](../orion_agent/overview.md) - Agent behavior and patterns
...
The agent registry is a centralized store that tracks all active orion agents. Each registered agent is represented by an **AgentProfile** that consolidates comprehensive information about the agent's capabilities, system resources, and operational status.

| Component | Responsibility | Location |
|-----------|---------------|----------|
| **OrionDeviceManager** | Central coordinator for device management | `network/client/device_manager.py` |
| **DeviceRegistry** | Device registration and information storage | `network/client/components/device_registry.py` |
| **AgentProfile** | Multi-source agent metadata representation | `network/client/components/types.py` |
...
|--------|----------|------------------|------------------|
| **1. User Configuration** | Administrator (devices.yaml + orion.yaml) | Endpoint identity, user preferences, capabilities | Static (config load) |
...
**Note:** While orion.yaml contains runtime settings like heartbeat intervals, the device-specific configuration is in devices.yaml.
...
sequenceDiagram
    ...
    participant CDM as OrionDeviceManager
...
|-------|-------------|---------------------|--------|
| **1. User Configuration** | Administrator registers device with endpoint and capabilities | OrionDeviceManager, DeviceRegistry | AgentProfile created with user-specified data |
...
| **5. Activation** | Set device to IDLE state, ready for task assignment | DeviceRegistry | Agent ready for orion tasks |
...
The **AgentProfile** is the primary data structure representing a registered orion agent:
...
The agent lifecycle is managed through a state machine that tracks connection, registration, and task execution states. For more details on agent behavior and state management, see [Orion Agent State Management](../orion_agent/state.md).
...
**File:** `network/client/device_manager.py`

The central coordinator for all device management operations in the orion system.
...
**File:** `network/client/components/device_registry.py`
...
Handles AIP registration message exchange for both device and orion clients.
...
- Track connected clients (devices and orions)
...
- [Orion Task Distribution](../orion/overview.md) - Task routing strategies
...
| **Network Devices Config** | [Network Devices Configuration](../../configuration/system/network_devices.md) | YAML configuration reference |
| **Device Registry** | [Device Registry](./device_registry.md) | Registry component details |
| **Orion System** | [Orion Overview](../orion/overview.md) | Multi-device coordination |
...
- **Orion Agent Overview**: `documents/docs/img/orion_agent.png`
...
3. **Configure Your Devices**: See [Network Devices Configuration](../../configuration/system/network_devices.md)
...
- **Configuration**: `config/network/devices.yaml`
