...
- Identify client type (DEVICE vs ORION)
...
async def connect(self, websocket: WebSocket) -> str:
    ...
    client_type = reg_info.client_type  # DEVICE or ORION
...
For orion clients (not device agents), the registration differs:

```python
# Orion client registration
ClientMessage(
    type=ClientMessageType.REGISTER,
    client_id="orion_orchestrator",
    client_type=ClientType.ORION,
    target_id="windowsagent",  # Target device for this orion
    status=TaskStatus.OK,
    timestamp="2025-11-06T10:30:00.000Z",
    metadata={
        "type": "orion_client",
...
> **Note:** Device clients register as `ClientType.DEVICE`, while orion orchestrators register as `ClientType.ORION` with a `target_id` pointing to the device they want to control.

### Output

- Client registered in server's `ClientConnectionManager`
- Client type (DEVICE/ORION) recorded
...
| **Network Devices Config** | [Network Devices Configuration](../../configuration/system/network_devices.md) | YAML config reference |
