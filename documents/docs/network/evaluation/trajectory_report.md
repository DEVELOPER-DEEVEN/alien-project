# Network Trajectory Report

## Overview

The **Network Trajectory Report** (`output.md`) is an automatically generated comprehensive execution log that documents the complete lifecycle of a multi-device task execution session in Network. This human-readable Markdown report provides step-by-step visualization of orion evolution, task execution, and device coordination.

### Report Location

After each Network session completes, the trajectory report is automatically generated:

```
logs/network/<session_name>/output.md
logs/network/<session_name>/topology_images/  # DAG visualizations
```

**Example:**
```
logs/network/request_20251111_140216_1/
...
└── topology_images/             # Generated DAG topology graphs
    ├── step1_after_orion_xxx.png
    ├── step2_after_orion_xxx.png
    └── step999_final_orion_xxx.png
```
...
**Fields:**
- **Agent**: Agent name and type (OrionAgent for orchestration)
...
**Common Functions:**
- `build_orion`: Initial DAG creation
- `edit_orion`: Dynamic DAG modification
- `execute_orion`: Trigger task execution
...
<details>
<summary>Orion AFTER (click to expand)</summary>

**Orion ID**: orion_bcd1726e_20251105_134526
**State**: created

##### Dependency Graph (Topology)

<img src="topology_images/step2_after_orion_xxx.png" width="600">
...
**ID**: orion_bcd1726e_20251105_134526
**State**: completed
...
The trajectory report is generated automatically by `NetworkSession` upon completion:

```python
# network/session/network_session.py
async def close_session(self):
    """Generate trajectory report on session close"""
    trajectory = NetworkTrajectory(self.log_path)
    trajectory.to_markdown(self.log_path + "output.md")
```

**Trigger Points:**
1. Normal session completion (`NetworkClient.shutdown()`)
...
python -m network.trajectory.generate_report logs/network/test1

# Custom output path
python -m network.trajectory.generate_report logs/network/test1 -o custom_report.md

# Minimal report (exclude details)
python -m network.trajectory.generate_report logs/network/test1 \
  --no-orion --no-tasks --no-devices
```

**CLI Options:**
- `--no-orion`: Exclude orion evolution details
...
```python
# network/trajectory/network_parser.py
if __name__ == "__main__":
    """Process all Network task logs and generate markdown reports."""
    
    network_logs_dir = Path("logs/network")
    task_dirs = sorted([d for d in network_logs_dir.iterdir() if d.is_dir()])
    
    for task_dir in task_dirs:
        trajectory = NetworkTrajectory(str(task_dir))
...
```bash
cd c:\Users\chaoyunzhang\OneDrive - Microsoft\Desktop\research\GPTV\ALIEN-windows\github\saber\ALIEN2
python -m network.trajectory.network_parser
```

**Output:**
```
[BOLD BLUE] Network Trajectory Parser - Batch Mode
...
```python
from network.trajectory import NetworkTrajectory

# Load trajectory from log directory
trajectory = NetworkTrajectory("logs/network/test1")
...
### Extracting Orion Data

```python
# Get final orion state
last_step = trajectory.step_log[-1]
final_orion = trajectory._parse_orion(
    last_step.get("orion_after")
)

if final_orion:
    orion_id = final_orion.get("orion_id")
    state = final_orion.get("state")
    tasks = final_orion.get("tasks", {})
    
    print(f"Orion {orion_id}: {state}")
...
trajectory.to_markdown(
    ...
    include_orion_details=True,  # Show DAG evolution
...
```
topology_images/
├── step1_after_orion_7b3c0f47_20251104_182305.png
├── step2_before_orion_bcd1726e_20251105_134526.png
├── step2_after_orion_bcd1726e_20251105_134526.png
├── step3_before_orion_bcd1726e_20251105_134526.png
├── step3_after_orion_bcd1726e_20251105_134526.png
└── step999_final_orion_bcd1726e_20251105_134526.png
```

**Naming Convention:**
- `step{N}_{state}_{orion_id}.png`
...
```python
trajectory = NetworkTrajectory("logs/network/failed_session")

for step in trajectory.step_log:
    orion = trajectory._parse_orion(step.get("orion_after"))
    if not orion:
        continue
...
trajectory = NetworkTrajectory("logs/network/task_32")

# Load metrics for performance data
with open("logs/network/task_32/result.json") as f:
    result = json.load(f)
...
slow_tasks = [
    ...
    for step in trajectory.step_log
    for tid, task in trajectory._parse_orion(
        step.get("orion_after")
    ).get("tasks", {}).items()
]
...
### 3. Orion Evolution Analysis

Track DAG modifications across steps:

```python
trajectory = NetworkTrajectory("logs/network/adaptive_session")

for idx, step in enumerate(trajectory.step_log, 1):
    before = trajectory._parse_orion(step.get("orion_before"))
    after = trajectory._parse_orion(step.get("orion_after"))
...
```python
trajectory = NetworkTrajectory("logs/network/multi_device")
...
for step in trajectory.step_log:
    ...
    for task in orion.get("tasks", {}).values():
...
def compare_sessions(session1_path, session2_path):
    t1 = NetworkTrajectory(session1_path)
    t2 = NetworkTrajectory(session2_path)
...
compare_sessions("logs/network/test_v1", "logs/network/test_v2")
```
...
trajectory.to_markdown(
    ...
    include_orion_details=True,  # DAG evolution (default: True)
...
for dir in logs/network/*/; do
    python -m network.trajectory.generate_report "$dir"
done

# Open reports in browser for visual inspection
start logs/network/test1/output.md
```
...
cp logs/network/*/output.md trajectory_archives/$(date +%Y-%m-%d)/
cp logs/network/*/result.json trajectory_archives/$(date +%Y-%m-%d)/
```
...
def validate_trajectory(log_dir):
    trajectory = NetworkTrajectory(log_dir)
...
Use orion evolution to verify correctness:

```python
# Verify DAG grows monotonically (no premature task deletion)
trajectory = NetworkTrajectory("logs/network/session")

prev_task_count = 0
for step in trajectory.step_log:
    orion = trajectory._parse_orion(step.get("orion_after"))
    if orion:
        task_count = len(orion.get("tasks", {}))
...
- **[Orion Orchestrator](../orion_orchestrator/overview.md)** - DAG execution engine
- **[Task Orion](../orion/overview.md)** - DAG data structure and validation
...
1. Check for `response.log` existence:
   ```bash
   ls logs/network/<session_name>/response.log
   ```

2. Manually trigger generation:
   ```bash
   python -m network.trajectory.generate_report logs/network/<session_name>
   ```
...
1. Check `topology_images/` directory exists:
   ```bash
   ls logs/network/<session_name>/topology_images/
   ```
...
2. Reduce topology image quality (edit `network_parser.py`):
   ```python
...
3. Archive and compress:
   ```bash
   gzip logs/network/<session_name>/output.md
   ```

## API Reference

### NetworkTrajectory Class

```python
class NetworkTrajectory:
    """Parser for Network agent logs with orion visualization"""
    ...
    def __init__(self, folder_path: str) -> None:
        ...
        Args:
            folder_path: Path to Network log directory (e.g., logs/network/task_1)
    ...
    def to_markdown(
        self,
        output_path: str,
        include_orion_details: bool = True,
        ...
        Args:
            ...
            include_orion_details: Include DAG evolution details
