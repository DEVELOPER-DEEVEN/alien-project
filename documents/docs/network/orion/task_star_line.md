...
**TaskStarLine** represents a directed dependency relationship between two TaskStars, forming an edge in the task orion DAG. Each TaskStarLine defines how tasks depend on each other, with support for conditional logic, success-only execution, and custom condition evaluation.
...
from network.orion.enums import DependencyType
...
!!! warning "Modification During Execution"
    Changing `dependency_type` or `condition_evaluator` resets the satisfaction status. Be cautious when modifying dependencies during active orion execution.
...
```python
# TaskOrion validates dependencies automatically
try:
    # This would fail if it creates a cycle
    orion.add_dependency(cyclic_dep)
except ValueError as e:
...
try:
    ...
    orion.add_dependency(invalid_dep)
except ValueError as e:
    print(f"Validation error: {e}")
    # TaskOrion will detect cycle
```
...
5. **Avoid cycles**: TaskOrion validates, but design carefully to avoid attempts
...
!!! warning "Common Pitfalls"
    ...
    - **Missing tasks**: Ensure both `from_task_id` and `to_task_id` exist in orion
...
- **[TaskOrion](task_orion.md)** — DAG manager that validates and executes dependencies
- **[OrionEditor](orion_editor.md)** — Safe dependency editing with undo/redo
