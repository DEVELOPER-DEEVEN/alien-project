# TaskOrion — DAG Orchestrator

## Overview

**TaskOrion** is the complete DAG (Directed Acyclic Graph) orchestration system that manages distributed workflows across heterogeneous devices. It provides comprehensive task management, dependency validation, execution scheduling, and runtime dynamism for complex cross-device orchestration.

**Formal Definition:** A TaskOrion $\mathcal{C}$ is a DAG defined as:
...
|-----------|------|-------------|
| **orion_id** | `str` | Unique identifier for the orion |
| **name** | `str` | Human-readable orion name |
| **state** | `OrionState` | Current execution state |
| **tasks** | `Dict[str, TaskStar]` | All tasks in the orion |
| **dependencies** | `Dict[str, TaskStarLine]` | All dependency relationships |
| **metadata** | `Dict[str, Any]` | Additional orion metadata |
...
| **created_at** | `datetime` | Orion creation timestamp |
...
|-------|-------------|-------------------|
| **CREATED** | Empty orion, no tasks added | Initialization |
...
# Output: orion_20251106_143052_a1b2c3d4

# Create with custom name
orion = TaskOrion(
    name="ml_training_pipeline",
    orion_id="pipeline_001"
)
```
...
```python
from network.orion import TaskStar
...
print(f"Total tasks: {orion.task_count}")
...
print(f"Total dependencies: {orion.dependency_count}")
...
orion.remove_task("preprocess")

# Remove a dependency
orion.remove_dependency(dep1.line_id)

# Get specific task or dependency
task = orion.get_task("fetch_data")
dep = orion.get_dependency(dep1.line_id)
```
...
```python
# Start orion execution
orion.start_execution()

# Start a specific task
orion.start_task("fetch_data")
...
```python
# Get tasks by status
running = orion.get_running_tasks()
completed = orion.get_completed_tasks()
failed = orion.get_failed_tasks()
pending = orion.get_pending_tasks()
...
# Check if entire orion is complete
if orion.is_complete():
    orion.complete_execution()
    print(f"State: {orion.state}")
```
...
longest_path_length, longest_path = orion.get_longest_path()
...
critical_time, critical_path_tasks = orion.get_critical_path_length_with_time()

print(f"Critical path duration: {critical_time:.2f} seconds")
print(f"Tasks on critical path: {critical_path_tasks}")

# Get total work
total_work = orion.get_total_work()
...
can_modify_task = orion.is_task_modifiable("task_a")
can_modify_dep = orion.is_dependency_modifiable("dep_1")
```
...
orion.add_task(diagnostic_task)
...
orion.add_dependency(fallback_dep)
...
!!! warning "Modification Safety"
    The orion enforces safe modification:
...
json_string = orion.to_json()

# Save to file
orion.to_json(save_path="orion_backup.json")

# Load from JSON string
restored = TaskOrion.from_json(json_data=json_string)

# Load from file
loaded = TaskOrion.from_json(file_path="orion_backup.json")
```
...
orion_dict = orion.to_dict()

# Create from dictionary
new_orion = TaskOrion.from_dict(orion_dict)
...
# Create from schema
orion_from_schema = TaskOrion.from_basemodel(schema)
```
...
orion.display_dag(mode="overview")

# Topology mode - detailed DAG graph
orion.display_dag(mode="topology")

# Details mode - task execution details
orion.display_dag(mode="details")

# Execution mode - real-time flow
orion.display_dag(mode="execution")
```
...
task_deps = orion.get_task_dependencies("train_model")

for dep in task_deps:
    print(f"{dep.from_task_id} → {dep.to_task_id} ({dep.dependency_type.value})")

# Get all dependencies in orion
all_deps = orion.get_all_dependencies()
```
...
```python
# Create: A → B → C
orion = TaskOrion(name="linear_pipeline")
...
# Fan-out: A → B, A → C
orion.add_dependency(TaskStarLine.create_success_only("a", "b"))
orion.add_dependency(TaskStarLine.create_success_only("a", "c"))

# Fan-in: B → D, C → D
orion.add_dependency(TaskStarLine.create_success_only("b", "d"))
orion.add_dependency(TaskStarLine.create_success_only("c", "d"))

# Analyze
order = orion.get_topological_order()
print(f"Possible order: {order}")  # ['a', 'b', 'c', 'd'] or ['a', 'c', 'b', 'd']

longest_path_length, path = orion.get_longest_path()
...
!!! warning "Common Pitfalls"
    ...
    - **Ignoring state**: Check orion state before modifications
...
The TaskOrion enforces **acyclicity** through:
...
The orion provides **safe concurrent execution**:
...
- **[OrionEditor](orion_editor.md)** — Safe editing with undo/redo
...
| Method | Description |
|--------|-------------|
| `add_task(task)` | Add task to orion |
...
| Method | Description |
|--------|-------------|
| `start_execution()` | Mark orion as started |
...
| `complete_execution()` | Mark orion as completed |
| `is_complete()` | Check if all tasks are terminal (returns `bool`) |
| `update_state()` | Update orion state based on task states |
...
| `get_statistics()` | Get all orion statistics (returns `Dict[str, Any]`) |
...
| Method | Description |
|--------|-------------|
| `display_dag(mode)` | Display orion (modes: overview, topology, details, execution) |

---

*TaskOrion — Orchestrating distributed workflows across the digital network*
