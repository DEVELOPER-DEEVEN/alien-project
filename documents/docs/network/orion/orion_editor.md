# OrionEditor — Interactive DAG Editor
...
**Usage in Network**: The OrionEditor is primarily used by the [Orion Agent](../orion_agent/overview.md) to programmatically build task workflows, but can also be used directly for manual orion creation and debugging.
...
```mermaid
graph TD
    A[OrionEditor] -->|manages| B[TaskOrion]
...
| Component | Purpose |
|-----------|---------|
| **OrionEditor** | High-level interface for orion editing |
...
# Create editor with new orion
editor = OrionEditor()

# Create editor with existing orion
existing = TaskOrion(name="my_workflow")
editor = OrionEditor(
    orion=existing,
...
# Access orion
print(f"Editing: {editor.orion.name}")
```
...
orion = editor.build_orion(
    config=config,
    clear_existing=True  # Clear current orion first
)
```
...
orion = editor.build_from_tasks_and_dependencies(
    tasks=tasks,
...
print(f"Orion cleared: {cleared.task_count == 0}")
```
...
```python
# Save to JSON file
file_path = editor.save_orion("my_workflow.json")
...
```python
# Load from JSON file
loaded = editor.load_orion("my_workflow.json")
...
orion = editor.load_from_dict(data)

# Load from JSON string
json_string = '{"name": "workflow", "tasks": {...}}'
orion = editor.load_from_json_string(json_string)
```
...
def on_change(editor, command, result):
    ...
    print(f"Orion state: {editor.orion.state.value}")
...
def log_observer(editor, command, result):
    with open("orion_log.txt", "a") as f:
...
print(f"Subgraph tasks: {subgraph_editor.orion.task_count}")
print(f"Subgraph deps: {subgraph_editor.orion.dependency_count}")
...
# Create editor
editor = OrionEditor(enable_history=True)
...
# Step 5: Save
editor.save_orion("ml_training_pipeline.json")

# Step 6: Execute (via orchestrator)
orion = editor.orion
# Pass to OrionOrchestrator for distributed execution
# See: ../orion_orchestrator/overview.md for execution details
```

For details on executing the built orion, see the [Orion Orchestrator documentation](../orion_orchestrator/overview.md).
...
2. **Validate frequently**: Run `validate_orion()` after major structural changes
...
5. **Save incrementally**: Create orion checkpoints during complex editing workflows
...
!!!warning "Common Pitfalls"
    ...
    - **Modifying running orions**: Editor operations will fail if orion is currently executing
...
- **[TaskOrion](task_orion.md)** — The orion DAG being edited
- **[Overview](overview.md)** — Task Orion framework overview

### Related Documentation

- **[Orion Orchestrator](../orion_orchestrator/overview.md)** — Learn how edited orions are scheduled and executed
- **[Orion Agent](../orion_agent/overview.md)** — Understand how agents use the editor to build orions
...
| Method | Description |
|--------|-------------|
| `build_orion(config, clear_existing)` | Build orion from TaskOrionSchema |
| `build_from_tasks_and_dependencies(tasks, deps, ...)` | Build orion from task and dependency lists (returns TaskOrion) |
| `clear_orion()` | Remove all tasks and dependencies from orion |
...
| Method | Description |
|--------|-------------|
| `save_orion(file_path)` | Save orion to JSON file, returns file path |
| `load_orion(file_path)` | Load orion from JSON file, returns TaskOrion |
| `load_from_dict(data)` | Load orion from dictionary, returns TaskOrion |
| `load_from_json_string(json_string)` | Load orion from JSON string, returns TaskOrion |
...
| Method | Description |
|--------|-------------|
| `validate_orion()` | Validate DAG structure, returns tuple of (is_valid: bool, errors: List[str]) |
...
| `get_statistics()` | Get comprehensive orion and editor statistics, returns Dict[str, Any] |
...
| `merge_orion(other_editor, prefix)` | Merge another orion with optional ID prefix |
| `display_orion(mode)` | Display visualization (modes: 'overview', 'topology', 'details', 'execution') |

For interactive web-based visualization and editing, see the [Network WebUI](../webui.md).

---

**OrionEditor** — Safe, interactive, and reversible orion manipulation
