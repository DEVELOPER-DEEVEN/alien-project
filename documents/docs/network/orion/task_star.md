...
**TaskStar** represents the atomic unit of computation in ALIEN Network—the smallest indivisible task scheduled on a device agent. Each TaskStar encapsulates complete context necessary for autonomous execution, including semantic description, assigned device, execution state, and dependency relationships.
...
from network.orion.enums import DeviceType, TaskPriority
...
```python
# Add dependency (internal use by TaskOrion)
...
!!! note "Managed by TaskOrion"
    Dependency management methods are primarily used internally by `TaskOrion`. Direct manipulation is not recommended—use `OrionEditor` for safe editing with undo/redo support.
...
!!! warning "Common Pitfalls"
    ...
    - **Don't forget validation**: Always validate tasks before adding to orion
...
- **[TaskOrion](task_orion.md)** — DAG orchestration and execution
- **[OrionEditor](orion_editor.md)** — Safe task editing with undo/redo
- **[OrionDeviceManager](../client/device_manager.md)** — Device management and task assignment
