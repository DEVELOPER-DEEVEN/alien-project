...
The Event System Core provides the foundational infrastructure for event-driven communication in the Network framework. It implements the Observer pattern through a central event bus, type-safe event classes, and well-defined interfaces.

**Location:** `network/core/events.py`
...
class EventType(Enum):
    ...
    ORION_STARTED = "orion_started"
    ORION_COMPLETED = "orion_completed"
    ORION_FAILED = "orion_failed"
    
    # Structure modification events
    ORION_MODIFIED = "orion_modified"
...
| `OrionEvent` | `Event` | `orion_id`, `orion_state`, `new_ready_tasks` | Orion lifecycle events |
...
from network.core.events import Event
...
class MyCustomObserver(IEventObserver):
    ...
    async def _handle_orion_event(self, event: OrionEvent) -> None:
        """Process orion events."""
        if event.event_type == EventType.ORION_STARTED:
            print(f"Orion {event.orion_id} started")
```
...
# Subscribe to orion events only
event_bus.subscribe(orion_observer, {
...
### Pattern 2: Orion Modification Flow
...
```python
from network.core.events import (
    get_event_bus, TaskEvent, OrionEvent, 
...
from network.core.events import (
    TaskEvent, OrionEvent, EventType, IEventObserver
)
...
class CustomMetricsCollector(IEventObserver):
    ...
    def __init__(self):
        ...
        self.orion_tasks: Dict[str, List[str]] = {}
    ...
    async def _handle_orion_event(self, event: OrionEvent) -> None:
        if event.event_type == EventType.ORION_STARTED:
            const_id = event.orion_id
            self.orion_tasks[const_id] = []
...
!!! note "Additional Observer Documentation"
    For documentation on `OrionProgressObserver`, `DAGVisualizationObserver`, `OrionModificationSynchronizer`, and `AgentOutputObserver`, refer to their implementation in `network/session/observers/`.
...
This foundation enables the Network framework to implement a loosely coupled, extensible event-driven architecture.
