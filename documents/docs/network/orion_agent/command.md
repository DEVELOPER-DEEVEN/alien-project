# Orion MCP Server — Structured Task Management

## Overview

The **Orion MCP Server** provides a standardized, idempotent interface for manipulating Task Orions. Through Model Context Protocol (MCP), it exposes task and dependency management primitives that bridge LLM-level reasoning and concrete execution state, ensuring reproducibility and auditability.

The Orion MCP Server is a lightweight component that operationalizes dynamic graph construction for the Orion Agent. It serves as the **structured manipulation layer** between LLM reasoning and the Task Orion data structure.
...
| **Consistency** | Returns globally valid orion snapshots after each operation |
...
Add a new atomic task (TaskStar) to the orion.
...
{
  "type": "string",
  "description": "JSON string of complete updated TaskOrion after adding task"
}
```
...
Remove a task and all associated dependencies from the orion.
...
{
  "type": "string",
  "description": "JSON string of complete updated TaskOrion after removing task"
}
```
...
# Returns updated orion without the task
orion = json.loads(result)
```
...
- **Task exists**: `task_id` must exist in orion
...
{
  "type": "string",
  "description": "JSON string of complete updated TaskOrion after updating task"
}
```
...
{
  "type": "string",
  "description": "JSON string of complete updated TaskOrion after adding dependency"
}
```
...
- **Both tasks exist**: `from_task_id` and `to_task_id` must exist in orion
...
{
  "type": "string",
  "description": "JSON string of complete updated TaskOrion after removing dependency"
}
```
...
{
  "type": "string",
  "description": "JSON string of complete updated TaskOrion after updating dependency"
}
```
...
Batch-create a complete orion from structured configuration.
...
{
  "type": "string",
  "description": "JSON string of built TaskOrion with all tasks, dependencies, and metadata"
}
```
...
**Creation Mode Usage**: In creation mode, the Orion Agent uses `build_orion` to generate the initial orion in a single operation, which is more efficient than incremental `add_task` calls.
...
```python
# Agent creates initial orion via build_orion
config = {
    "tasks": [...],
    "dependencies": [...]
}

orion_json = await mcp_client.call_tool(
    "build_orion",
...
```python
# Agent edits orion incrementally based on events
...
|-------|-------|----------|
| `Task not found` | Invalid `task_id` | Verify task exists in orion |
...
| `build_orion` | $O(n + e)$ | $O(n + e)$ | Full orion rebuild |
...
| build_orion latency | 50-200ms | 1s |
| add_task latency | 10-50ms | 100ms |
| Orion JSON size | 5-50 KB | 500 KB |
...
**Creation Mode:** Use `build_orion` for initial synthesis

**Editing Mode:** Use granular tools (`add_task`, `update_task`, etc.)

**Bulk Edits:** Accumulate changes and apply via `build_orion` with `clear_existing=False`
...
```python
modifiable = orion.get_modifiable_tasks()
...
- [Orion Agent Strategy Pattern](strategy.md) — Processing strategies and prompters
- [Orion Editor MCP Server](../../mcp/servers/orion_editor.md) — Detailed MCP server reference
- [Task Orion Overview](../orion/overview.md) — DAG model and data structures
...
**Orion MCP Server — Structured, idempotent task manipulation for adaptive orchestration**
