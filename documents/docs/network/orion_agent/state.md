# Orion Agent State Machine

The Orion Agent's finite-state machine provides deterministic lifecycle management while enabling dynamic orion evolution. This FSM governs how the agent transitions between creation, monitoring, success, and failure states—ensuring predictable behavior in complex distributed workflows.
...
The Orion Agent implements a **4-state finite-state machine (FSM)** that provides clear, enforceable structure for task lifecycle management. This design separates LLM reasoning from deterministic control logic, improving safety and debuggability.
...
stateDiagram-v2
    ...
    START --> CONTINUE: Orion Created Successfully
...
|-------|------|-------------|------------------|
| **START** | Initial | Initialize and create orion | Agent instantiation, restart after completion |
| **CONTINUE** | Steady-State | Monitor events and process feedback | Orion created successfully |
...
1. Generates the initial Task Orion from user request
...
sequenceDiagram
    ...
    Agent->>Agent: Check if orion exists
    
    alt No Orion
        Agent->>Creation: process_creation(context)
        Creation->>Agent: Return orion + timing
...
|----------|--------|------------|
| **First Execution** | Generate orion via LLM | `CONTINUE` (success) / `FAIL` (error) |
| **Restart Trigger** | Use existing orion | `CONTINUE` |
| **Creation Failure** | Log error, no orion created | `FAIL` |
...
3. Merges orion state with latest modifications
...
class ContinueOrionAgentState(OrionAgentState):
    ...
    async def handle(self, agent: "OrionAgent", context=None) -> None:
        ...
        # Get latest orion and merge states
...
sequenceDiagram
    ...
    Agent->>Sync: Merge orion states
    Sync-->>Agent: Merged orion
    
    Agent->>Editing: process_editing(events, orion)
    Editing->>Agent: Updated orion
...
The **state synchronizer** merges the orchestrator's orion with agent modifications:
...
> 
> 1. Task A completes → Agent edits orion (adds Task C)
...
| **Restart Needed** | New orion required | `START` |
...
if orion.is_complete() and no_more_edits_needed:
    agent.status = OrionAgentStatus.FINISH.value
elif critical_error_occurred:
    agent.status = OrionAgentStatus.FAIL.value
elif new_orion_needed:
    agent.status = OrionAgentStatus.START.value
else:
...
- All tasks in the orion have completed successfully
...
```python
# LLM decides to finish based on orion state
{
...
# Example error chain:
try:
    orion = await agent.process_creation(context)
except Exception as e:
...
The state manager uses the **@register decorator** pattern to automatically register state classes. For more details on the overall agent architecture, see [Orion Agent Overview](overview.md).
...
```mermaid
gantt
    title Orion Agent State Timeline
...
try:
    orion, timing = await agent.process_creation(context)
except AttributeError as e:
