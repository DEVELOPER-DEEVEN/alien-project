# Orion Agent — The Centralized Orion Weaver

The **Orion Agent** serves as the central intelligence of ALIEN³ Network, acting as both a planner and replanner. It interprets user intent, constructs executable Task Orions, and dynamically steers their evolution across heterogeneous devices. By bridging high-level natural-language goals and concrete multi-agent execution, the Orion Agent provides unified orchestration through a feedback-driven control loop.
...
The Orion Agent extends the abstract [Task Orion](../orion/overview.md) model into runtime execution. Residing within the **OrionClient** (see [Network Client](../client/overview.md)), it transforms user requests into structured DAG workflows and continuously refines them as distributed agents provide feedback.

Unlike traditional static DAG schedulers, the Orion Agent operates as a **dynamic orchestrator** powered by an LLM-driven architecture and governed by a finite-state machine (FSM). This design enables it to alternate between two complementary operating modes:

- **Creation Mode**: Synthesizes initial Task Orions from user instructions
- **Editing Mode**: Incrementally refines orions based on runtime feedback
...
| **DAG Synthesis** | Decompose requests into structured Task Orions with dependencies | Creation |
| **Device Assignment** | Map tasks to appropriate devices based on AgentProfile capabilities | Creation |
| **Runtime Monitoring** | Track task completion events and orion state | Editing |
...
The Orion Agent implements a **dual-mode control pattern** that separates planning from replanning:
...
sequenceDiagram
    ...
    participant Editor as Orion Editor MCP
    participant Orchestrator
    
    User->>Agent: Submit Request
    Agent->>Prompter: Format Creation Prompt
    Prompter->>LLM: Send Prompt + Examples
    LLM->>Agent: Return Orion JSON
    Agent->>Dispatcher: Execute build_orion
    Dispatcher->>MCP: Route Command
    MCP->>Editor: Call build_orion
    Editor->>MCP: Return Built Orion
    MCP->>Dispatcher: Return Result
    Dispatcher->>Agent: Orion Ready
    Agent->>Orchestrator: Start Execution
    Orchestrator-->>Agent: Orion Started
...
|--------|------|-------------|
| **Task Orion** | `TaskOrion` | Structured DAG with tasks and dependencies |
| **Observation** | `str` | Analysis of input context and device profiles |
| **Thought** | `str` | Reasoning trace explaining decomposition logic |
| **State** | `OrionAgentStatus` | Next FSM state (typically `CONTINUE`) |
...
| **Current Orion** | `TaskOrion` | Serialized orion snapshot |
...
sequenceDiagram
    ...
    Agent->>Dispatcher: Sync Orion State
    Dispatcher->>MCP: build_orion (sync)
    ...
    Editor->>MCP: Return Updated Orion
    MCP->>Dispatcher: Return Results
    Dispatcher->>Agent: Orion Updated
...
The agent can perform the following modifications through the MCP-based Orion Editor:
...
| Output | Type | Description |
|--------|------|-------------|
| **Updated Orion** | `TaskOrion` | Modified DAG with new tasks/dependencies |
...
The Orion Agent's behavior is governed by a **4-state finite-state machine**:

| State | Description | Triggers |
|-------|-------------|----------|
| **START** | Initialize orion, begin orchestration | Agent instantiation, restart after completion |
| **CONTINUE** | Monitor events, process feedback, update orion | Task completion/failure events |
...
- **Orion Editor MCP Server**: Implements the actual orion manipulation operations
...
The Orion Editor MCP Server exposes the following operations:

| Operation | Purpose | Inputs | Output |
|------|---------|--------|--------|
| `build_orion` | Batch-create orion from config | Configuration dict, clear flag | Built orion |
| `add_task` | Add atomic task node | Task ID, name, description, device, tips | Updated orion |
| `remove_task` | Remove task and dependencies | Task ID | Updated orion |
| `update_task` | Modify task fields | Task ID + updated fields | Updated orion |
| `add_dependency` | Create dependency edge | From/to task IDs, type, condition | Updated orion |
| `remove_dependency` | Delete dependency | Dependency ID | Updated orion |
| `update_dependency` | Update dependency logic | Dependency ID, condition | Updated orion |
...
```python
# Load available MCP tools from Orion Editor
...
# Returns: OrionAgentResponse with thought, status, actions
```
...
context.set(ContextNames.ORION, updated_orion)
...
classDiagram
    ...
    class OrionCreationPrompter {
        +user_prompt_construction()
        +examples_prompt_helper()
    }
    
    class OrionEditingPrompter {
        +user_prompt_construction()
        +examples_prompt_helper()
    }
    
    class OrionPrompterFactory {
        +create_prompter(mode)
        +get_supported_modes()
    }
    
    BaseOrionPrompter <|-- OrionCreationPrompter
    BaseOrionPrompter <|-- OrionEditingPrompter
    OrionPrompterFactory --> BaseOrionPrompter
```
...
High-level task synthesis and low-level execution coordination are decoupled yet tightly synchronized through the Task Orion abstraction. The agent focuses on semantic reasoning while the orchestrator handles distributed execution.

### 2. Dynamic Adaptability

The editable orion enables:
...
- **Re-validation**: $O(n + e)$ for modified orion

### Scalability

| Metric | Typical | Maximum Tested |
|--------|---------|----------------|
| Tasks per Orion | 5-20 | 100+ |
| Dependencies per Orion | 4-30 | 200+ |
...
- **[TaskStarLine](../orion/task_star_line.md)** — Dependency relationships
- **[Orion Orchestrator](../orion_orchestrator/overview.md)** — Distributed executor
