...
The `OrionManager` is a companion component to the `TaskOrionOrchestrator` that handles device assignment, resource management, and orion lifecycle tracking. While the orchestrator focuses on execution flow and coordination, the manager provides the infrastructure for device operations and state management.
...
```mermaid
graph TB
    O[TaskOrionOrchestrator] -->|uses| CM[OrionManager]
    CM -->|communicates| DM[OrionDeviceManager]
    DM -->|manages| D1[Device 1]
    DM -->|manages| D2[Device 2]
    DM -->|manages| D3[Device N]
    
    CM -->|tracks| MD[(Orion Metadata)]
...
The OrionManager handles four primary responsibilities:
...
Tracks and manages orion resources:

- Device availability and status
- Orion registration and metadata
- Device utilization statistics
- Assignment validation

### 3. Lifecycle Management

Manages orion lifecycle:
...
- Best for: Orions with varying task complexity
...
**Purpose**: Track active orions and their metadata
...
async def get_orion_status(
    ...
    return {
        ...
        "running_tasks": [task.task_id for task in orion.get_running_tasks()],
        "completed_tasks": [task.task_id for task in orion.get_completed_tasks()],
...
```json
{
  "orion_id": "orion_20251106_143052_a1b2c3d4",
...
Remove a orion from management:
...
def validate_orion_assignments(
    ...
    if self._logger:
        if is_valid:
            self._logger.info(
                f"All tasks in orion '{orion.name}' have "
                f"valid assignments"
            )
        else:
            self._logger.warning(
                f"Orion '{orion.name}' has {len(errors)} "
                f"assignment errors"
            )
...
Get device utilization across orion:
...
def list_orions(self) -> List[Dict[str, Any]]:
    """List all managed orions with basic information."""
...
```python
from network.orion.orchestrator import OrionManager
from network.client.device_manager import OrionDeviceManager

# Create device manager
device_manager = OrionDeviceManager()

# Create orion manager
manager = OrionManager(device_manager, enable_logging=True)
...
```python
# Check orion status during execution
...
class TaskOrionOrchestrator:
    ...
    async def orchestrate_orion(self, orion, ...):
