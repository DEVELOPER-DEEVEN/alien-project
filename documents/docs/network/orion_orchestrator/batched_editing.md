...
- Orion state synchronization
- Event publishing and notification

To balance **responsiveness** with **efficiency**, the orchestrator supports **batched orion editing**: during a reasoning round, multiple task completion events are aggregated and their resulting modifications applied atomically in a single cycle.
...
while system is running:
    ...
    publish(ORION_MODIFIED, all_task_ids)
    C ← synchronize(C, T_C)
...
The `OrionModificationSynchronizer` batches pending modifications:
...
The Orion Agent receives multiple task IDs and processes them together:
...
async def process_editing(
    ...
    """Process task completion events and update orion."""
    ...
    after_orion = await self._create_and_process(context)
    
    # Publish single ORION_MODIFIED event for entire batch
...
t=350ms: Agent completes editing
    → Publishes ORION_MODIFIED with on_task_id = ['A', 'B', 'C']
...
foreach event in batch:
    Δ ← invoke(OrionAgent, edit(C, event))
    C ← apply(C, Δ)
end

validate(C)  # ← Validates combined result
publish(ORION_MODIFIED, batch_task_ids)
...
Batching is a pure **performance optimization** - it doesn't change the semantics of orion evolution.
...
Design orions with tasks that complete around the same time:
...
!!!tip "Next Steps"
    To understand device assignment and resource management, continue to [Orion Manager](orion_manager.md).
