...
Since the TaskOrion may be dynamically rewritten by an LLM-based agent, the orchestrator must enforce runtime invariants to preserve correctness even under partial or invalid updates. Without these guarantees, the system could execute invalid DAGs, violate dependencies, or enter inconsistent states.

The Orion Orchestrator enforces three critical invariants (I1-I3) that together ensure safety, consistency, and semantic validity throughout execution.
...
**Invariant**: Edits must preserve DAG acyclicity - the orion remains a valid directed acyclic graph after all modifications.
...
**Rationale**: Modifying tasks that have started or finished execution could invalidate already-collected results, create inconsistencies between device state and orion state, or violate causal dependencies.
...
async def _validate_and_prepare_orion(
    ...
    is_valid, errors = self._orion_manager \
        .validate_orion_assignments(orion)
    if not is_valid:
...
async def _run_execution_loop(self, orion: TaskOrion) -> None:
    ...
    while not orion.is_complete():
        ...
        # I2 checked implicitly by TaskOrion.add_dependency()
...
After the agent modifies the orion, validation occurs before releasing the lock:

```python
# In safe assignment lock algorithm
while queue not empty:
    e ← dequeue()
    Δ ← invoke(OrionAgent, edit(C, e))
    C ← apply(C, Δ)
...
Concurrent task execution and orion editing create multiple consistency challenges:
...
4. **Consistency**: Orchestrator only sees valid orion states

During the lock period (modification + validation + sync), the orchestrator has a strongly consistent view of the orion. Learn more about [safe assignment locking](safe_assignment_locking.md).

## Formal Invariant Definitions

### Mathematical Formulation

Let C = (T, D) be a orion with tasks T and dependencies D, where:
...
**Response**: Reject dependency addition, log error, orion remains acyclic
...
try:
    ...
    # Continue with existing orion structure
    # Don't block orchestration on agent errors
```

The orchestrator continues execution with the last valid orion state.
...
async def test_invariants_during_orchestration():
    ...
    # Create orion with potential for violations
    orion = create_complex_orion()
    ...
    for task in orion.get_completed_tasks() + orion.get_failed_tasks():
        with pytest.raises(ValueError):
