# OrionClient Reference

OrionClient is the device coordination layer in Network Client. It provides a clean API for registering devices, managing connections, and assigning tasks. Most applications interact with OrionClient rather than the lower-level DeviceManager.
...
- [Configuration](../../configuration/system/network_orion.md) - Device configuration
- [NetworkClient](./network_client.md) - Session wrapper on top of OrionClient

## What OrionClient Does

OrionClient implements the Facade pattern, providing a simplified interface to the complex device management system underneath. Think of it as the "device management API" for Network.

**Core Responsibilities:**

**Device Lifecycle Management**: OrionClient handles the complete lifecycle of device connections. When you register a device, it stores the device information (ID, server URL, capabilities) in DeviceRegistry. When you connect, it coordinates with DeviceManager to establish WebSocket connections, perform AIP registration, and start health monitoring. When you disconnect, it cleanly tears down all resources.

**Task Assignment**: When you have a task to execute, OrionClient determines which device should run it (based on capabilities), checks if the device is available, and delegates to DeviceManager for actual execution. It abstracts away details like task queuing when devices are busy or handling connection failures during execution.

**Configuration Management**: OrionClient loads device configurations from YAML files or programmatic APIs, validates settings, and maintains the runtime configuration. This centralizes all configuration logic so other components don't need to worry about it.

**Status Reporting**: Applications need to know what's happening with devices. OrionClient provides methods to query device status, get health summaries, and retrieve execution statistics. This information is aggregated from multiple components (DeviceRegistry, DeviceManager, TaskQueueManager) and presented in a unified format.

**What OrionClient Does NOT Do:**

- **DAG Planning**: Task decomposition is handled by OrionAgent
- **DAG Execution**: Coordinating task dependencies is handled by TaskOrionOrchestrator  
- **Session Management**: Multi-round interactions are handled by NetworkSession
- **Low-Level Connection Management**: WebSocket lifecycle is handled by DeviceManager

This separation of concerns keeps OrionClient focused on device-level operations.
...
def __init__(
    ...
    """
    Initialize OrionClient with configuration.
...
When you create a OrionClient, it performs these initialization steps:

1. **Load or Create Configuration**: If you provide a `config` parameter, it uses that. Otherwise, it creates a default `OrionConfig` object. This config contains device information, heartbeat settings, and other parameters.

2. **Override Task Name**: If you provide `task_name`, it overrides the task name from the configuration. The task name identifies this orion instance in logs and messages.

3. **Create DeviceManager**: OrionClient creates an internal DeviceManager instance, passing the task name and connection settings (heartbeat interval, reconnect delay). DeviceManager is the component that actually manages connections.
...
config = OrionConfig.from_yaml("config/devices.yaml")
client = OrionClient(config=config)
...
The task name appears in logs and helps identify which orion instance generated which messages, which is useful when running multiple orions simultaneously.
...
After creating a OrionClient, you must call `initialize()` before using it. This method processes all devices defined in the configuration:
...
This diagram shows the initialization sequence. For each configured device, OrionClient delegates to DeviceManager, which handles the low-level connection setup if auto-connect is enabled.
...
While OrionClient doesn't expose a direct `assign_task_to_device()` method in its public API (that's internal to DeviceManager), it's used by higher-level orchestrators like TaskOrionOrchestrator. Understanding how task assignment works helps you understand the system:
...
OrionClient focuses on device management, not task orchestration. Task assignment is exposed through higher-level APIs:

- TaskOrionOrchestrator assigns tasks based on DAG dependencies
- NetworkSession coordinates multi-round task execution
...
```python
def get_orion_info(self) -> Dict[str, Any]:
    """Get overall orion status and configuration."""
```

Returns orion-level information:
...
This provides a high-level view of the entire orion, useful for monitoring overall system health.
...
```python
def validate_config(self, config: Optional[OrionConfig] = None) -> Dict[str, Any]:
    """
...
async def shutdown(self) -> None:
    """
    Gracefully shutdown the orion client.
...
client = OrionClient(base_config)
await client.initialize()
...
NetworkClient wraps OrionClient for session management:

```python
class NetworkClient:
    def __init__(self, ...):
        # Create internal OrionClient
        self._client = OrionClient(config, task_name)
    
    async def initialize(self):
        # Initialize OrionClient
        await self._client.initialize()
        
    async def process_request(self, request):
        # Use OrionClient for device coordination
        # while NetworkSession handles task orchestration
        session = NetworkSession(client=self._client, ...)
        await session.run()
```

### Used by TaskOrionOrchestrator

TaskOrionOrchestrator uses OrionClient's DeviceManager for task assignment:
...
for task in dag.tasks:
    ...
    # Assign through DeviceManager (internal to OrionClient)
    result = await orion_client.device_manager.assign_task_to_device(
        task_id=task.id,
        device_id=device_id,
...
OrionClient is the primary interface for device management in Network Client. It provides:
...
For most applications, OrionClient (or NetworkClient which wraps it) is all you need. Only advanced scenarios require working directly with DeviceManager or its components.
...
- See [NetworkClient](./network_client.md) for session-level API
