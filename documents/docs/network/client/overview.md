...
Network Client is the client-side layer responsible for multi-device coordination in the ALIEN³ framework. At its core is **OrionClient**, which manages device registration, connection, and task assignment. **NetworkClient** provides a lightweight wrapper offering convenient session management interfaces.

## Related Documentation

- [OrionClient](./orion_client.md) - Core device coordination component
...
- [NetworkClient](./network_client.md) - Session wrapper API
- [Configuration](../../configuration/system/network_orion.md) - Device configuration guide

## The Complete Path: From User Request to Device Execution

To understand Network Client, we first need to see the entire system workflow. When a user submits a task request, the system processes it through several layers:

### 1. User Interaction Layer (Optional)

Users can interact with the Network system in two ways:

**Interactive Mode**: Users input natural language requests through a command-line interface (CLI), which are received and processed by NetworkClient. This mode is primarily used for rapid prototyping and manual testing.

**Programmatic Mode**: Developers directly call the Python API of OrionClient or NetworkClient, integrating Network into their applications. This is the recommended approach for production environments.

### 2. Session Management Layer (NetworkClient)

NetworkClient's role is to manage the lifecycle of task sessions. It doesn't handle specific device operations but instead:

- Initializes and holds a OrionClient instance
- Creates a NetworkSession for each user request
- Passes requests to OrionAgent for DAG planning (task decomposition)
- Coordinates TaskOrionOrchestrator to execute the DAG
- Collects and aggregates execution results

**NetworkClient is optional**. If your application doesn't need session management, you can use OrionClient directly.

### 3. Device Coordination Layer (OrionClient)

OrionClient is the heart of Network Client. It is responsible for:
...
**Task Assignment**: Receiving task requests from upper layers (TaskOrionOrchestrator), selecting appropriate devices based on capabilities, sending tasks to devices via the AIP protocol, and waiting for and collecting task execution results.

OrionClient doesn't concern itself with how tasks are decomposed (that's OrionAgent's responsibility) or how DAGs are executed (that's TaskOrionOrchestrator's responsibility). It focuses on "device-level matters."

### 4. Connection Management Layer (DeviceManager)

DeviceManager is the core internal component of OrionClient, responsible for all low-level connection management:
...
All communication with devices goes through the [Agent Interaction Protocol (AIP)](../../aip/overview.md). AIP is a WebSocket-based messaging protocol that defines standard message types and interaction flows. Main message types used by Network Client include:
...
### OrionClient: The Device Coordination Facade

OrionClient implements the Facade pattern. It provides simple device management APIs externally while delegating actual work to DeviceManager internally.
...
- DAG planning (handled by OrionAgent)
- DAG execution (handled by TaskOrionOrchestrator)
- Session management (handled by NetworkSession)

See [OrionClient documentation](./orion_client.md) for detailed API reference.

### DeviceManager: The Connection Management Engine

DeviceManager is the "engine" of OrionClient. It uses 5 modular components to accomplish connection management:
...
### NetworkClient: Session Management Wrapper

NetworkClient provides a higher-level abstraction on top of OrionClient:

```python
client = NetworkClient()
await client.initialize()  # Initialize OrionClient and connect devices

# Process user request (internally creates NetworkSession, calls OrionAgent for DAG planning)
result = await client.process_request("Open Excel and create a sales chart")

await client.shutdown()  # Cleanup resources
```

NetworkClient's main value lies in:

- Simplifying initialization flow (automatically loads device info from config)
- Providing session management (creates independent NetworkSession for each request)
- Integrating display components (Rich console output, progress bars, etc.)
- Supporting interactive mode (command-line interface)

If your application already has its own session management logic, you can skip NetworkClient and use OrionClient directly. See [NetworkClient documentation](./network_client.md) for detailed API.
...
**Step 1: Initialize NetworkClient**
...
1. NetworkClient loads device information from config file (`device_info.yaml`)
2. Creates OrionClient instance and passes configuration
3. OrionClient calls `device_manager.register_device()` to register each device
...
1. NetworkClient creates a NetworkSession
2. NetworkSession calls OrionAgent for task planning
3. OrionAgent (LLM-powered) decomposes task into DAG:
...
4. TaskOrionOrchestrator executes DAG:
...
**Step 4: Task Assignment and Execution (OrionClient/DeviceManager)**

For each task, OrionClient calls:
...
1. TaskOrionOrchestrator aggregates all task results
2. NetworkSession generates session results (including execution time, rounds, DAG statistics)
3. NetworkClient returns results to user
...
Network Client is not an isolated system—it closely collaborates with other ALIEN³ components:

### Depends on Agent Server for Message Routing

Network Client doesn't connect directly to devices but routes through [Agent Server](../../server/overview.md). Agent Server's role is to:
...
**Route Messages**: Forwards TASK messages from Network Client to the correct device based on device_id. The server acts as a message broker, decoupling clients from devices.
...
### Used by OrionAgent for Task Planning

When NetworkClient receives a user request, it calls [OrionAgent](../orion_agent/overview.md) to decompose the request into a DAG (Directed Acyclic Graph). OrionAgent is LLM-powered and can:
...
**Dynamically Adjust DAG**: If issues arise during execution (e.g., a device fails), OrionAgent can replan and modify the DAG to adapt to the new situation.

For more details, see [OrionAgent Documentation](../orion_agent/overview.md).

### Coordinates with TaskOrionOrchestrator for DAG Execution

Once OrionAgent creates the DAG, [TaskOrionOrchestrator](../orion_orchestrator/overview.md) executes it across devices. The orchestrator:
...
For more details, see [TaskOrionOrchestrator Documentation](../orion_orchestrator/overview.md).
...
Device information is defined through configuration files. See [Network Configuration](../../configuration/system/network_orion.md) for complete configuration options.

A typical configuration example:

```yaml
# config/network/orion.yaml
task_name: "production_orion"
...
- **task_name**: Unique identifier for this orion, used in logs and debugging
...
- Use interactive mode for quick testing: `python -m network --interactive`
...
- [OrionClient API Reference](./orion_client.md) - Complete device coordination API
...
- [NetworkClient Session Wrapper](./network_client.md) - Session management API

## Summary

Network Client provides the core multi-device coordination capabilities in ALIEN³. Through layered design, it simplifies complex distributed system management into clear APIs:

- **OrionClient** is the core of device management, handling device registration, connection, and task assignment
- **DeviceManager** is the underlying engine, processing WebSocket, heartbeat, message routing, and task queuing
- **NetworkClient** is an optional session wrapper, providing more convenient high-level APIs

If you're new to Network Client, we recommend reading the documentation in this order:

1. This Overview (understand overall architecture and workflow)
2. [OrionClient](./orion_client.md) (learn core API)
...
If you need to get started quickly, jump directly to [NetworkClient](./network_client.md) example code.
