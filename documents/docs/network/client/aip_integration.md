...
The Agent Interaction Protocol (AIP) is the communication protocol used throughout Network Client for device coordination. This document explains how Network Client integrates with AIP, the message flow patterns, and how different components use the protocol.

## Related Documentation

- [Overview](./overview.md) - Overall Network Client architecture
...
**Transport Agnostic**: AIP runs over WebSocket in Network Client, but the protocol itself is transport-independent. You could implement AIP over HTTP, gRPC, or any other transport.
...
## Protocol Architecture in Network Client

Network Client uses AIP at multiple levels:
...
### Layer 4: Application Logic (DeviceManager, OrionClient)
...
```json
{
  ...
  "task_name": "network/production/excel_task",
...
- Handled by: DeviceManager, OrionClient
...
## Complete Message Flow: OrionClient to Device Agent

This section shows the complete end-to-end message flow from when a OrionClient assigns a task through the Agent Server to the final execution on a Device Agent.
...
When OrionClient assigns a task to a device, the message passes through multiple layers:
...
sequenceDiagram
    ...
    MP->>MP: Create ClientMessage(type=TASK, client_type=ORION)
    ...
    Server->>Server: client_type=ORION, extract target_id
    Server->>Server: Create session_id, register orion session
    ...
    Server->>WS1: TASK_END to OrionClient
...
- `client_type: "orion"`: Identifies this as a orion client (not device)
- `target_id`: The device that should execute this task (e.g., "windows_pc")
- `session_id`: Orion session identifier for tracking
...
1. **Orion Sessions**: Maps orion_client_id → [session_ids]
2. **Device Sessions**: Maps device_id → [session_ids]

This allows the server to:

- Cancel all sessions when a orion client disconnects
- Cancel all sessions when a device disconnects
- Send results to both orion client AND device
...
**Orion Client Disconnects:**

```python
# Cancel all sessions started by this orion
session_ids = self.client_manager.get_orion_sessions(client_id)
for session_id in session_ids:
    await self.session_manager.cancel_task(
        session_id, 
        reason="orion_disconnected"
    )
```
...
On the OrionClient side, DeviceManager detects disconnection via:
...
| Aspect | ORION Client | DEVICE Client |
|--------|---------------------|---------------|
| Task Request | Includes `target_id` field | No `target_id`, executes locally |
| Session Tracking | Tracked in orion_sessions | Tracked in device_sessions |
...
This allows the same server to support both direct device connections and orion-mediated connections.

---

## Summary

AIP integration in Network Client follows a layered architecture:
...
4. **Application**: DeviceManager and OrionClient use messages for coordination
5. **Server Routing**: ALIENWebSocketHandler routes messages between orion clients and devices
...
- **Task Execution (Orion)**: OrionClient TASK → Server routes → Device executes → Server routes → OrionClient TASK_END
...
User Request
    ↓
NetworkClient (session management)
    ↓
OrionClient (device coordination)
    ↓
...
AIP provides a robust, extensible protocol for agent communication with strong typing, clear message flows, comprehensive error handling, and intelligent routing between orion clients and devices.
...
- See [OrionClient](./orion_client.md) for device coordination API
