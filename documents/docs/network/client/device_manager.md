...
DeviceManager is the connection orchestration layer in Network Client. While OrionClient provides the high-level device management API, DeviceManager handles the low-level details of WebSocket connections, health monitoring, message routing, and task queuing.

## Related Documentation

- [Overview](./overview.md) - Overall Network Client architecture and workflow
- [OrionClient](./orion_client.md) - High-level device management API
...
def __init__(
    ...
    Args:
        task_name: Identifier for this orion instance (default "test_task")
...
**task_name**: This identifier appears in log messages and helps distinguish between multiple orion instances running simultaneously. For example, "production_orion" vs "test_orion".
...
Most applications should use OrionClient, which wraps DeviceManager. Use DeviceManager directly only for:
...
- See [OrionClient](./orion_client.md) for high-level API
- See [AIP Integration](./aip_integration.md) for protocol details and message flows
- See [Overview](./overview.md) for overall Network Client architecture
