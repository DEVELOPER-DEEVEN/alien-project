...
| **networkEndpoint** | Orchestrator | networkClient | ✅ Multi-device coordination<br>✅ Task distribution<br>✅ Device info querying<br>✅ Connection pooling |
...
graph TB
    ...
    Base --> network[networkEndpoint<br/>Orchestrator]
...
Wraps Alien's server-side WebSocket handler with AIP protocol support for managing multiple device connections simultaneously.
...
The Device Server Endpoint maintains full compatibility with Alien's existing WebSocket handler.
...
Wraps Alien's client-side WebSocket client with AIP protocol support, automatic reconnection, and heartbeat management.
...
| **Message Routing** | Auto-routes to Alien client | Handled internally |
...
# (routed to underlying Alien client)
...
Enables the networkClient to communicate with multiple devices simultaneously, managing connections, tasks, and queries.

### Configuration

```python
from aip.endpoints import networkEndpoint
...
Learn more about [AgentProfile configuration](../cluster/client/device_manager.md) in the cluster documentation.
...
```python
# 1. Initialize
endpoint = DeviceClientEndpoint(ws_url, Alien_client)

# 2. Connect
await endpoint.start()

# 3. Handle messages (automatic)
# Alien client receives and processes messages
...
**network Lifecycle Code:**

```python
# 1. Initialize
endpoint = networkEndpoint(task_name)
...
| Multi-device orchestrator | `networkEndpoint` |
...
from aip.endpoints import (
    ...
    networkEndpoint, # Orchestrator-side
)
```
...
- [cluster Client](../cluster/client/overview.md) - Multi-device orchestration with networkClient
- [Alien Server](../server/websocket_handler.md) - WebSocket server implementation
- [Alien Client](../client/websocket_client.md) - WebSocket client implementation
