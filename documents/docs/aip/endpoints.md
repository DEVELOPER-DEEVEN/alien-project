...
graph TB
    ...
    Base --> Orion[OrionEndpoint<br/>Orchestrator]
...
Enables the OrionClient to communicate with multiple devices simultaneously, managing connections, tasks, and queries.

### Configuration

```python
from aip.endpoints import OrionEndpoint

endpoint = OrionEndpoint(
...
Learn more about [AgentProfile configuration](../network/client/device_manager.md) in the Network documentation.
...
**Orion Lifecycle Code:**

```python
# 1. Initialize
endpoint = OrionEndpoint(task_name)
...
| Multi-device orchestrator | `OrionEndpoint` |
...
from aip.endpoints import (
    ...
    OrionEndpoint, # Orchestrator-side
)
```
...
- [Network Client](../network/client/overview.md) - Multi-device orchestration with OrionClient
