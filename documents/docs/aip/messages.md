...
| `ORION` | Multi-device orchestrator | • Manages multiple devices<br>• Coordinates tasks<br>• Requires `target_id` |
...
Devices and orion clients use `ClientMessage` to communicate with the server.
...
| `target_id` | str? | Target device (for orion) |
