...
|----------|------|------------------|
| `networkEndpoint` | Orchestrator | Global agent registry, task assignment, DAG coordination |
...
Together, these layers form a vertically integrated stack that enables Alien² to maintain **correctness and availability** under challenging conditions:
...
|--------|----------|-------------|
| **User Config** | networkClient | Endpoint URLs, user preferences, device identity |
...
sequenceDiagram
    ...
    CC->>CC: Update Tasknetwork
    CC->>CC: Notify networkAgent
```
...
- [Tasknetwork documentation](../cluster/network/task_network.md)
- [TaskStar (task nodes) documentation](../cluster/network/task_star.md)
...
| **Disconnect during task** | Mark tasks as `FAILED`<br>Propagate to networkAgent<br>Trigger DAG edit | N/A |

### networkClient Disconnection

!!!danger "Bidirectional Fault Handling"
    When the **networkClient** disconnects, all Device Agent Services:
...
- ✅ Consistent Tasknetwork state (G4)
...
## Integration with Alien² Ecosystem
...
| **Tasknetwork** | Real-time state synchronization via AIP messages | Planning DAG always reflects distributed execution state |
| **Configuration System** | Agent endpoints, capabilities managed via Alien² config | Centralized management, type-safe validation |
...
- [Tasknetwork (DAG orchestrator)](../cluster/network/task_network.md)
- [networkAgent (orchestration agent)](../cluster/network_agent/overview.md)
...
By decomposing orchestration into five logical layers—each addressing specific reliability and adaptability concerns—AIP enables Alien² to maintain **correctness and availability** under DAG evolution (G4, G5), agent churn (G3, G6), and heterogeneous execution environments (G1, G2).
