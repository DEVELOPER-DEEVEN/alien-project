...
AIP serves as the **nervous system** of ALIEN², connecting the OrionClient, device agent services, and device clients under a unified, event-driven control plane. It is designed as a lightweight yet evolution-tolerant protocol to satisfy six goals:
...
|----------|------|------------------|
| `OrionEndpoint` | Orchestrator | Global agent registry, task assignment, DAG coordination |
...
|--------|----------|-------------|
| **User Config** | OrionClient | Endpoint URLs, user preferences, device identity |
...
sequenceDiagram
    ...
    CC->>CC: Update TaskOrion
    CC->>CC: Notify OrionAgent
```
...
- [TaskOrion documentation](../network/orion/task_orion.md)
- [TaskStar (task nodes) documentation](../network/orion/task_star.md)
...
| **Disconnect during task** | Mark tasks as `FAILED`<br>Propagate to OrionAgent<br>Trigger DAG edit | N/A |

### OrionClient Disconnection

!!!danger "Bidirectional Fault Handling"
    When the **OrionClient** disconnects, all Device Agent Services:
...
| **TaskOrion** | Real-time state synchronization via AIP messages | Planning DAG always reflects distributed execution state |
...
- [TaskOrion (DAG orchestrator)](../network/orion/task_orion.md)
- [OrionAgent (orchestration agent)](../network/orion_agent/overview.md)
