...
AIP's resilience layer ensures stable communication and consistent orchestration across distributed agent orions through automatic reconnection, heartbeat monitoring, and timeout management.
...
The Resilient Connection Protocol governs how connection disruptions are detected, handled, and recovered between OrionClient and Device Agents.
...
!!!danger "Impact on Running Tasks"
    All tasks running on a disconnected device are **immediately marked as FAILED** to maintain TaskOrion consistency.
...
| **3. Task Failure** | Mark tasks as `TASK_FAILED` | Propagate to OrionAgent |
...
When OrionClient disconnects, Device Agent Servers proactively clean up to prevent orphaned tasks.
