...
|----------|---------|--------------|----------|
| **RegistrationProtocol** | Agent capability advertisement | `REGISTER`, `HEARTBEAT(OK)` | Device joins orion |
...
Handles initial registration and capability advertisement when agents join the orion.
...
```python
success = await reg_protocol.register_as_orion(
    orion_id="orchestrator_001",
...
!!!warning "Target Device Required"
    Orion clients **must** specify `target_device` to indicate which device they coordinate.
