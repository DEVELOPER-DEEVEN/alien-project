...
|----------|---------|--------------|----------|
| **RegistrationProtocol** | Agent capability advertisement | `REGISTER`, `HEARTBEAT(OK)` | Device joins network |
...
Handles initial registration and capability advertisement when agents join the network.
...
```python
success = await reg_protocol.register_as_network(
    network_id="orchestrator_001",
...
!!!warning "Target Device Required"
    network clients **must** specify `target_device` to indicate which device they coordinate.
...
sequenceDiagram
    ...
    participant CA as networkAgent
