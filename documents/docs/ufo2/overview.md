...
<figure markdown>
  ...
  <figcaption><b>Figure 1:</b> Comparison between (a) traditional CUAs that rely on screenshots and simulated inputs, and (b) Alien² AgentOS that deeply integrates with OS APIs, application internals, and hybrid GUI–API execution.</figcaption>
</figure>
...
Alien² implements a **hierarchical multi-agent system** optimized for Windows desktop automation:

<figure markdown>
  ![Alien² System Architecture](../img/framework2.png)
  <figcaption><b>Figure 2:</b> Alien² system architecture featuring the two-tier agent hierarchy (HostAgent + AppAgents), hybrid control detection pipeline, continuous knowledge substrate integration, and unified GUI–API action layer coordinated through MCP servers.</figcaption>
</figure>
...
**Alien² Solution:** Nested virtual desktop via Windows Remote Desktop loopback:
...
!!!success "User Experience"
    Users can continue email, browsing, or coding while Alien² automates Excel reports in the background PiP desktop.
...
Alien² integrates with a centralized YAML-based configuration system:

```yaml
# config/Alien/host_agent_config.yaml
host_agent:
...
# config/Alien/app_agent_config.yaml
app_agent:
...
Alien² is designed to be run from the command line:

**Interactive Mode:**
```powershell
# Start Alien² in interactive mode
python -m Alien --task <your_task_name>
```

**Example:**
```powershell
python -m Alien --task excel_demo
```
...
**Example:**
```powershell
python -m Alien --task excel_demo -r "Open Excel and create a chart from Sheet1 data"
```

!!!tip "Complete Setup Guide"
    For detailed installation, configuration, and advanced usage options, see the **[Quick Start Guide](../getting_started/quick_start_Alien-Unis.md)**.
...
!!!tip "Next Steps"
    📖 [Getting Started Guide](../getting_started/quick_start_Alien-Unis.md)  
...
Alien² demonstrates that **system-level integration** and **architectural design** matter more than model size alone:
...
!!!info "Learning Paths"
    **🚀 New Users:** Start with [Quick Start Guide](../getting_started/quick_start_Alien-Unis.md)  
