...
|----------------|-------------|---------------|
| **Client Registration** | Validate and register new device/orion connections | AIP Registration Protocol |
...
```mermaid
sequenceDiagram
    participant C as Client<br/>(Device/Orion)
...
# Orion-specific validation
if client_type == ClientType.ORION:
    await self._validate_orion_client(reg_info)
```

**Orion Validation:**

```python
async def _validate_orion_client(self, reg_info: ClientMessage) -> None:
    """Validate orion's claimed target_id."""
    ...
    if not self.client_manager.is_device_connected(claimed_device_id):
        ...
        self.logger.warning(f"[WS] Orion registration failed: {error_msg}")
...
| **Target Device (Orion)** | If `target_id` specified, device must be online | `"Target device '<id>' is not connected"` | Send error + close |
| **Client ID Uniqueness** | No existing client with same ID | Handled by ClientConnectionManager | Disconnect old connection |

**Orion Dependency:**

Orions **must** specify a valid `target_id` that refers to an already-connected device. If the device is offline or doesn't exist, registration fails immediately.

**Workaround:** Connect devices first, then orions.
...
| `handle_device_info_request` | `DEVICE_INFO_REQUEST` | Orion queries device capabilities | `DEVICE_INFO_RESPONSE` |
...
The handler supports task requests from **both device clients** (self-execution) and **orion clients** (orchestrated execution on target devices).
...
**Orion Orchestrated Execution:**

When a **orion** dispatches a task to a target device:
...
async def handle_task_request(self, data: ClientMessage, websocket: WebSocket):
    ...
    if client_type == ClientType.ORION:
        # Orion dispatching to target device
...
| **Orion** | `client_manager.add_orion_session(orion_id, session_id)` | Track which orion requested the session |
...
Orions can query device capabilities (screen resolution, installed apps, OS version) to make intelligent task routing decisions.
...
sequenceDiagram
    ...
    Note over WH,C: 6️⃣ Forward to Orion
...
async def handle_device_info_request(
    ...
    """Handle device info requests from orions."""
    device_id = data.target_id
    request_id = data.request_id
    
    self.logger.info(
        f"[WS] Orion {data.client_id} requesting info for device {device_id}"
    )
...
graph TD
    ...
    C -->|Orion| E[Get Orion Sessions]
    
    D --> F[Cancel Each Session<br/>reason='device_disconnected']
    E --> G[Cancel Each Session<br/>reason='orion_disconnected']
    
    F --> H[Remove Device Session Mappings]
    G --> I[Remove Orion Session Mappings]
...
**Orion Client Cleanup:**

```python
if client_info and client_info.client_type == ClientType.ORION:
    # Get all sessions initiated by orion
    ...
    # Clean up mappings
    self.client_manager.remove_orion_sessions(client_id)
```
...
|----------|---------------------|----------------|------|
| **Device Disconnects** | `device_disconnected` | Yes Orion | Notify orchestrator to reassign task |
| **Orion Disconnects** | `orion_disconnected` | No | Requester is gone, no one to notify |
...
```python
async def _validate_orion_client(self, reg_info: ClientMessage) -> None:
    """Validate orion's target device."""
    claimed_device_id = reg_info.target_id
    
    if not self.client_manager.is_device_connected(claimed_device_id):
        error_msg = f"Target device '{claimed_device_id}' is not connected"
        self.logger.warning(f"Orion registration failed: {error_msg}")
...
- **Authorization failures** (target device not connected for orions)
...
async def handle_task_request(self, data: ClientMessage, websocket: WebSocket):
    ...
    if data.client_type == ClientType.ORION and not data.target_id:
        raise ValueError("Orion must specify target_id")
...
async def disconnect(self, client_id: str):
    ...
    if client_info.client_type == ClientType.ORION:
        reason = "orion_disconnected"  # Don't send callback
    else:
        reason = "device_disconnected"  # Send callback to orion
...
- **Dual Client Support** - Devices (self-execution) vs. Orions (orchestration)
