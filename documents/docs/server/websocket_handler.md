...
The **AlienWebSocketHandler** is the central nervous system of the server, implementing the Agent Interaction Protocol (AIP) to manage structured, reliable communication between the server and all connected clients.
...
|----------------|-------------|---------------|
| **Client Registration** | Validate and register new device/network connections | AIP Registration Protocol |
...
```mermaid
sequenceDiagram
    participant C as Client<br/>(Device/network)
...
if client_type == ClientType.network:
    await self._validate_network_client(reg_info)
```

**network Validation:**

```python
async def _validate_network_client(self, reg_info: ClientMessage) -> None:
    """Validate network's claimed target_id."""
    ...
    if not self.client_manager.is_device_connected(claimed_device_id):
        ...
        self.logger.warning(f"[WS] network registration failed: {error_msg}")
...
def _log_client_connection(self, client_id: str, client_type: ClientType) -> None:
    ...
    if client_type == ClientType.DEVICE:
        ...
        self.logger.info(f"[WS] 🌟 Registered network client: {client_id}")
```
...
| **Target Device (network)** | If `target_id` specified, device must be online | `"Target device '<id>' is not connected"` | Send error + close |
| **Client ID Uniqueness** | No existing client with same ID | Handled by ClientConnectionManager | Disconnect old connection |

**network Dependency:**

networks **must** specify a valid `target_id` that refers to an already-connected device. If the device is offline or doesn't exist, registration fails immediately.

**Workaround:** Connect devices first, then networks.
...
| `handle_device_info_request` | `DEVICE_INFO_REQUEST` | network queries device capabilities | `DEVICE_INFO_RESPONSE` |
...
The handler supports task requests from **both device clients** (self-execution) and **network clients** (orchestrated execution on target devices).
...
**network Orchestrated Execution:**

When a **network** dispatches a task to a target device:
...
async def handle_task_request(self, data: ClientMessage, websocket: WebSocket):
    ...
    if client_type == ClientType.network:
        # network dispatching to target device
...
| **network** | `client_manager.add_network_session(network_id, session_id)` | Track which network requested the session |
...
networks can query device capabilities (screen resolution, installed apps, OS version) to make intelligent task routing decisions.
...
sequenceDiagram
    ...
    Note over WH,C: 6️⃣ Forward to network
...
async def handle_device_info_request(
    ...
    """Handle device info requests from networks."""
    device_id = data.target_id
    request_id = data.request_id
    
    self.logger.info(
        f"[WS] network {data.client_id} requesting info for device {device_id}"
    )
...
graph TD
    ...
    C -->|network| E[Get network Sessions]
    
    D --> F[Cancel Each Session<br/>reason='device_disconnected']
    E --> G[Cancel Each Session<br/>reason='network_disconnected']
    
    F --> H[Remove Device Session Mappings]
    G --> I[Remove network Session Mappings]
...
**network Client Cleanup:**

```python
if client_info and client_info.client_type == ClientType.network:
    # Get all sessions initiated by network
    session_ids = self.client_manager.get_network_sessions(client_id)
    
    if session_ids:
        self.logger.info(
            f"[WS] 🌟 network {client_id} disconnected, "
            f"cancelling {len(session_ids)} active session(s)"
        )
    ...
    # Clean up mappings
    self.client_manager.remove_network_sessions(client_id)
```
...
|----------|---------------------|----------------|------|
| **Device Disconnects** | `device_disconnected` | Yes network | Notify orchestrator to reassign task |
| **network Disconnects** | `network_disconnected` | No | Requester is gone, no one to notify |
...
```python
async def _validate_network_client(self, reg_info: ClientMessage) -> None:
    """Validate network's target device."""
    claimed_device_id = reg_info.target_id
    
    if not self.client_manager.is_device_connected(claimed_device_id):
        error_msg = f"Target device '{claimed_device_id}' is not connected"
        self.logger.warning(f"network registration failed: {error_msg}")
...
- **Authorization failures** (target device not connected for networks)
...
async def handle_task_request(self, data: ClientMessage, websocket: WebSocket):
    ...
    if data.client_type == ClientType.network and not data.target_id:
        raise ValueError("network must specify target_id")
...
async def disconnect(self, client_id: str):
    ...
    if client_info.client_type == ClientType.network:
        reason = "network_disconnected"  # Don't send callback
    else:
        reason = "device_disconnected"  # Send callback to network
...
if client_type == ClientType.network:
    self.logger.info(
        f"[WS] 🌟 network {client_id} requesting task on {target_id}"
    )
else:
    self.logger.debug(
...
- **Dual Client Support** - Devices (self-execution) vs. networks (orchestration)
