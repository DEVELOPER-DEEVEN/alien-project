...
The **SessionManager** orchestrates agent session lifecycles, coordinates background task execution, and maintains execution state across the server. It serves as the "execution engine" that powers Alien's autonomous task capabilities.
...
await session_manager.cancel_task(
    session_id="session_abc123",
    reason="device_disconnected"  # or "network_disconnected"
)
```
...
|--------|----------|-------------------|----------|
| `network_disconnected` | network client lost connection | **No callback** (client is gone) | Task requester disconnected, no one to notify |
| `device_disconnected` | Target device lost connection | **Send callback** to network | Notify orchestrator to reassign task |
...
```mermaid
sequenceDiagram
    participant C as Client (network)
    participant WH as WebSocket Handler
    participant SM as Session Manager
    participant BT as Background Task
    ...
    Note over C,SM: Scenario 2: network Disconnects During Task
    C->>WH: Task dispatched
    WH->>SM: execute_task_async()
    SM->>BT: Background task running
    
    Note over C: network disconnects
    C--xWH: WebSocket closed
    WH->>SM: cancel_task(session_id, reason="network_disconnected")
    SM->>BT: task.cancel()
    BT->>BT: Catch asyncio.CancelledError
...
Session timeouts should match task complexity:
    ...
    ```python
    # Configure in Alien config
    Alien_config.system.timeout = 300  # 5 minutes for medium tasks
    ```
...
async def handle_client_disconnect(client_id, client_type, session_manager, client_manager):
    ...
    if client_type == ClientType.network:
        # network disconnected - cancel all its tasks
        ...
        # Device disconnected - notify networks to reassign
...
async def handle_task_dispatch(self, session_id, request, client_id):
    """Handle incoming task from network."""
...
The Client Connection Manager tracks which clients own which sessions:
    ...
    client_manager.add_network_session(
    ...
    session_ids = client_manager.get_network_sessions("network_001")
...
```python
# In API router (Alien/server/services/api.py)
...
| `reason` | `str` | No | `"network_disconnected"` | Cancellation reason |

**Valid Reasons:**

- `"network_disconnected"` - Don't send callback
- `"device_disconnected"` - Send callback to network
