...
await session_manager.cancel_task(
    session_id="session_abc123",
    reason="device_disconnected"  # or "orion_disconnected"
)
```
...
|--------|----------|-------------------|----------|
| `orion_disconnected` | Orion client lost connection | **No callback** (client is gone) | Task requester disconnected, no one to notify |
| `device_disconnected` | Target device lost connection | **Send callback** to orion | Notify orchestrator to reassign task |
...
sequenceDiagram
    ...
    Note over C,SM: Scenario 2: Orion Disconnects During Task
    C->>WH: Task dispatched
    WH->>SM: execute_task_async()
    SM->>BT: Background task running
    
    Note over C: Orion disconnects
    C--xWH: WebSocket closed
    WH->>SM: cancel_task(session_id, reason="orion_disconnected")
    SM->>BT: task.cancel()
    BT->>BT: Catch asyncio.CancelledError
...
async def _run_session_background(...):
    try:
        ...
        if reason == "device_disconnected":
            # Send callback to orion
        elif reason == "orion_disconnected":
            # Skip callback
```
...
async def handle_client_disconnect(client_id, client_type, session_manager, client_manager):
    ...
    if client_type == ClientType.ORION:
        # Orion disconnected - cancel all its tasks
        ...
        # Device disconnected - notify orions to reassign
...
async def handle_task_dispatch(self, session_id, request, client_id):
    """Handle incoming task from orion."""
...
The Client Connection Manager tracks which clients own which sessions:
    ...
    ```python
    # Track orion sessions
    client_manager.add_orion_session(
        orion_id="orion_001",
        session_id="session_abc123"
    )
    ...
    session_ids = client_manager.get_orion_sessions("orion_001")
...
| `reason` | `str` | No | `"orion_disconnected"` | Cancellation reason |

**Valid Reasons:**

- `"orion_disconnected"` - Don't send callback
- `"device_disconnected"` - Send callback to orion
