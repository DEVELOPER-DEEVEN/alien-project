...
graph LR
    ...
    subgraph "Alien Server HTTP API"
...
| **Client Monitoring** | `GET /api/clients` | Query connected devices and networks |
...
- **External Integration**: Trigger Alien tasks from web apps, scripts, or CI/CD pipelines
...
|-------|------|----------|---------|-------------|
| `client_id` | `string` | ✅ **Yes** | - | Target client identifier (device or network) |
...
**Source Code** (verified from `Alien/server/services/api.py`):
...
Query all currently connected clients (devices and networks) to determine which targets are available for task dispatch.
...
# Example: devices start with "device_", networks with "network_"
    
clients = response.json()["online_clients"]
    
devices = [c for c in clients if c.startswith("device_")]
networks = [c for c in clients if c.startswith("network_")]
...
```python
    ...
    dispatch_response = requests.post(
        ...
        json={
            ...
            "request": "Open Notepad and type 'Hello from Alien API'",
...
1. **Web Applications**: Trigger Alien tasks from web frontends
2. **Automation Scripts**: Integrate Alien into CI/CD pipelines
3. **External Tools**: Connect third-party systems to Alien
