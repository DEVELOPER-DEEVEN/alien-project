...
| **Client Monitoring** | `GET /api/clients` | Query connected devices and orions |
...
Query all currently connected clients (devices and orions) to determine which targets are available for task dispatch.
...
# Example: devices start with "device_", orions with "orion_"
    
clients = response.json()["online_clients"]
    
devices = [c for c in clients if c.startswith("device_")]
orions = [c for c in clients if c.startswith("orion_")]
