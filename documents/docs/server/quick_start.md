...
This hands-on guide walks you through starting the Alien Agent Server, connecting clients, and dispatching your first task. Perfect for first-time users.
...
- **Alien dependencies** installed (`pip install -r requirements.txt`)
...
```console
2024-11-04 14:30:22 - Alien.server.app - INFO - Starting Alien Server on 0.0.0.0:5000
2024-11-04 14:30:22 - Alien.server.app - INFO - Platform: auto-detected
2024-11-04 14:30:22 - Alien.server.app - INFO - Log level: WARNING
...
**Custom Port:**
```bash
python -m Alien.server.app --port 8080
```
...
**Production (Linux):**
```bash
python -m Alien.server.app --port 5000 --platform linux --log-level WARNING
```
...
**Windows Device:**
```bash
python -m Alien.client.client --ws --ws-server ws://127.0.0.1:5000/ws --client-id my_windows_device
```

**Linux Device:**
```bash
python -m Alien.client.client --ws --ws-server ws://127.0.0.1:5000/ws --client-id my_linux_device --platform linux
```
...
A network Client is an orchestrator that coordinates multi-device tasks. It connects to the server and can dispatch work across multiple registered device clients.

### Basic network Connection

```bash
python -m cluster.network.network --ws --ws-server ws://127.0.0.1:5000/ws --target-id my_windows_device
```
...
A network can dynamically dispatch tasks to different devices, not just the `target-id`. For more on multi-device orchestration, see [network Documentation](../cluster/overview.md).
...
curl -X POST http://localhost:5000/api/dispatch \
  ...
  -d '{
    "client_id": "my_windows_device",
    "request": "Open Chrome, navigate to GitHub.com, and search for Alien framework",
...
### Issue 6: Target Device Not Connected (network)
...
3. Then connect network

!!!tip "Debug Mode"
    For maximum verbosity, start the server with: `python -m Alien.server.app --log-level DEBUG`
...
| **network Mode** | Multi-device orchestration | Coming Soon |
...
!!!example "Systemd Service (Linux)"
    Create `/etc/systemd/system/Alien-server.service`:
    
    ```ini
    [Unit]
    Description=Alien Agent Server
    After=network.target
    
    [Service]
    Type=simple
    User=Alien
    WorkingDirectory=/opt/Alien
    Environment="PATH=/opt/Alien/venv/bin"
    ExecStart=/opt/Alien/venv/bin/python -m Alien.server.app --port 5000 --log-level INFO
    Restart=always
    RestartSec=10
    ...
    **Enable and start:**
    ...
    sudo systemctl enable Alien-server
    sudo systemctl start Alien-server
    sudo systemctl status Alien-server
    ```
...
pm2 start "python -m Alien.server.app --port 5000" --name Alien-server
...
- Started the Alien Agent Server with custom configurations
- Connected device and network clients via WebSocket
...
- **Multi-Device Coordination**: [network Overview](../cluster/overview.md)
