...
### Basic Orion Connection

```bash
python -m network.orion.orion --ws --ws-server ws://127.0.0.1:5000/ws --target-id my_windows_device
```
...
A orion can dynamically dispatch tasks to different devices, not just the `target-id`. For more on multi-device orchestration, see [Orion Documentation](../network/overview.md).
...
### Issue 6: Target Device Not Connected (Orion)
...
| **Orion Mode** | Multi-device orchestration | Coming Soon |
...
- Connected device and orion clients via WebSocket
...
- **Multi-Device Coordination**: [Orion Overview](../network/overview.md)
