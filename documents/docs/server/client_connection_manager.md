...
|----------------|-------------|---------|
| **Client Registry** | Store all connected device and orion clients | Fast O(1) client lookup by ID |
| **Session Tracking** | Map sessions to their orion orchestrators | Enable proper cleanup on disconnection |
...
| **System Info Caching** | Store device capabilities and configuration | Optimize orion decision-making |
...
graph TB
    ...
    subgraph "Server - ClientConnectionManager"
        ...
        subgraph "Storage"
            ...
            CS[Orion Sessions<br/>_orion_sessions]
...
class ClientInfo:
    ...
    client_type: ClientType            # DEVICE or ORION
...
| `client_type` | `ClientType` | Whether DEVICE or ORION | `ClientType.DEVICE` |
...
**Example - Adding a Orion Client:**

```python
client_manager.add_client(
    client_id="orion_orchestrator_001",
    platform="linux",  # Platform of the orion server
    ws=websocket,
...
orions = client_manager.list_clients_by_type(ClientType.ORION)
    
print(f"Devices online: {len(devices)}")
print(f"Orions online: {len(orions)}")
```
...
!!!danger "Cleanup Required"
    ...
    - Session mappings (`_orion_sessions`, `_device_sessions`)
...
def is_device_connected(self, device_id: str) -> bool:
    ...
    with self.lock:
        ...
        # Verify it's a DEVICE client (not orion)
...
```python
# In WebSocket Handler - orion requesting task on device
...
!!! warning "Type Check is Critical"
    The method returns `False` if the client exists but is **not a device** (e.g., it's a orion). This prevents accidentally dispatching device tasks to orion clients.
...
def is_online(self, client_id: str) -> bool:
    """Check if any client (device or orion) is currently online."""
    with self.lock:
...
1. **Orion → Sessions**: Which sessions did a orion initiate?
2. **Device → Sessions**: Which sessions is a device currently executing?

This dual tracking enables proper cleanup when either orion or device disconnects.

```mermaid
graph TB
    subgraph "Orion Perspective"
        C[Orion_001]
        CS[_orion_sessions]
        CS --> S1[session_abc]
        CS --> S2[session_def]
...
### Orion Session Mapping

Orion clients initiate tasks on remote devices. Track these sessions to enable cleanup when the orchestrator disconnects.

**Add Orion Session:**

```python
def add_orion_session(self, client_id: str, session_id: str):
    """Map a session to its orion orchestrator."""
    
    with self.lock:
        if client_id not in self._orion_sessions:
            self._orion_sessions[client_id] = []
        self._orion_sessions[client_id].append(session_id)
```

**Get Orion Sessions:**

```python
def get_orion_sessions(self, client_id: str) -> List[str]:
    """Get all sessions initiated by a orion client."""
    
    with self.lock:
        return self._orion_sessions.get(client_id, []).copy()
        # .copy() prevents external modification of internal list
```

**Remove Orion Sessions:**

```python
def remove_orion_sessions(self, client_id: str) -> List[str]:
    """Remove and return all sessions for a orion."""
    
    with self.lock:
        return self._orion_sessions.pop(client_id, [])
        # Returns removed sessions for cleanup
```

**Example - Orion Disconnect Cleanup:**

```python
# In WebSocket Handler - when orion disconnects
orion_id = "orion_001"

# Get all sessions this orion initiated
session_ids = client_manager.get_orion_sessions(orion_id)

logger.info(
    f"Orion {orion_id} disconnected, "
...
for session_id in session_ids:
    await session_manager.cancel_task(
        session_id,
        reason="orion_disconnected"  # Don't send callback
    )

# Remove mappings
client_manager.remove_orion_sessions(orion_id)
```

### Device Session Mapping

Device clients execute tasks sent by orions (or themselves). Track these sessions to enable cleanup when the device disconnects.
...
!!!example "Device Disconnect Cleanup"
    ...
    for session_id in session_ids:
        await session_manager.cancel_task(
            session_id,
            reason="device_disconnected"  # Send callback to orion
        )
...
sequenceDiagram
    ...
    WH->>WSM: add_orion_session("orion_001", "session_abc")
    WH->>WSM: add_device_session("device_001", "session_abc")
    
    Note over WSM: Session Mappings
    WSM->>WSM: _orion_sessions["orion_001"] = ["session_abc"]
...
!!!warning "Sessions Persist Until Cleanup"
    ...
    1. The orion disconnects (removes all its sessions)
    2. The device disconnects (removes all its sessions)
    3. Manual cleanup (future feature)
    
    **Implication:** Over time, `_orion_sessions` and `_device_sessions` can grow large. Consider implementing periodic cleanup for completed sessions.

### Dual Mapping Example

!!!example "Single Session, Dual Mapping"
    When a orion dispatches a task to a device:
    
    ```python
    orion_id = "orion_orchestrator_001"
    device_id = "device_windows_001"
    session_id = "session_abc123"
    
    # Session is mapped to BOTH the orion and the device
    client_manager.add_orion_session(orion_id, session_id)
    client_manager.add_device_session(device_id, session_id)
    
    # Later retrieval
    orion_sessions = client_manager.get_orion_sessions(orion_id)
    ...
    **Why dual mapping?**
    ...
    - If **orion disconnects**: Cancel all its sessions (notify devices)
    - If **device disconnects**: Cancel all sessions on that device (notify orions)
...
def get_stats(self) -> Dict[str, int]:
    ...
    with self.lock:
        ...
        orion_count = sum(
            1
            for info in self.online_clients.values()
            if info.client_type == ClientType.ORION
        )
        return {
            "total": len(self.online_clients),
            "device_clients": device_count,
            "orion_clients": orion_count
        }
```
...
async def dispatch_task_to_device(
    ...
    # Step 1: Validate orion is connected
    if not client_manager.is_online(orion_id):
        raise ValueError(f"Orion {orion_id} not connected")
...
async def handle_client_disconnect(
    ...
    if client_type == ClientType.ORION:
            session_ids = client_manager.get_orion_sessions(client_id)
            cancel_reason = "orion_disconnected"
        else:  # DEVICE
        ...
        if client_type == ClientType.ORION:
            client_manager.remove_orion_sessions(client_id)
        else:
...
- [x] Remove session mappings (orion/device)
...
**Case 3: Orion and device both disconnect**
...
async def cleanup_completed_sessions(client_manager, session_manager):
    ...
    all_sessions.update(
        session_id
        for sessions in client_manager._orion_sessions.values()
...
async def on_task_complete(session_id, orion_id, device_id):
    # Remove specific session from mappings
    client_manager.remove_session_from_orion(orion_id, session_id)
...
sequenceDiagram
    ...
    WSM-->>WH: WebSocket
    WH->>WSM: add_orion_session(const_id, session_id)
    ...
    WH->>WSM: get_orion_sessions(client_id)
    WSM-->>WH: [session_ids...]
    WH->>SM: cancel_task(session_id, reason)
    WH->>WSM: remove_orion_sessions(client_id)
...
| `get_client_type()` | `client_id: str` | `Optional[ClientType]` | Get client type (DEVICE/ORION) |
...
| Method | Parameters | Returns | Description |
|--------|------------|---------|-------------|
| `add_orion_session()` | `client_id: str`<br/>`session_id: str` | `None` | Map session to orion |
| `get_orion_sessions()` | `client_id: str` | `List[str]` | Get orion's sessions |
| `remove_orion_sessions()` | `client_id: str` | `List[str]` | Remove and return sessions |
...
class ClientType(Enum):
    ...
    ORION = "orion"  # Orchestrator client
```
...
|------------|-------------|-------------|
| **Client Registry** | Track connected devices and orion clients | Single source of truth for client state |
...
1. **Dual Session Mapping**: Track sessions from both orion and device perspectives for comprehensive cleanup
...
4. **Type-Safe Validation**: Always verify client type (DEVICE vs ORION) before operations
