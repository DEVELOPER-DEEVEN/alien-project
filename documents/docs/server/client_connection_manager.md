...
|----------------|-------------|---------|
| **Client Registry** | Store all connected device and network clients | Fast O(1) client lookup by ID |
| **Session Tracking** | Map sessions to their network orchestrators | Enable proper cleanup on disconnection |
...
| **System Info Caching** | Store device capabilities and configuration | Optimize network decision-making |
...
graph TB
    ...
    subgraph "Server - ClientConnectionManager"
        ...
        subgraph "Storage"
            ...
            CS[network Sessions<br/>_network_sessions]
...
class ClientInfo:
    ...
    client_type: ClientType            # DEVICE or network
...
| `client_type` | `ClientType` | Whether DEVICE or network | `ClientType.DEVICE` |
...
**Example - Adding a network Client:**

```python
client_manager.add_client(
    client_id="network_orchestrator_001",
    platform="linux",  # Platform of the network server
    ws=websocket,
...
elif client_type == ClientType.network:
...
networks = client_manager.list_clients_by_type(ClientType.network)
    
print(f"Devices online: {len(devices)}")
print(f"networks online: {len(networks)}")
```
...
!!!danger "Cleanup Required"
    ...
    - Session mappings (`_network_sessions`, `_device_sessions`)
...
def is_device_connected(self, device_id: str) -> bool:
    ...
    with self.lock:
        ...
        # Verify it's a DEVICE client (not network)
...
```python
# In WebSocket Handler - network requesting task on device
...
!!! warning "Type Check is Critical"
    The method returns `False` if the client exists but is **not a device** (e.g., it's a network). This prevents accidentally dispatching device tasks to network clients.
...
def is_online(self, client_id: str) -> bool:
    """Check if any client (device or network) is currently online."""
    with self.lock:
...
1. **network → Sessions**: Which sessions did a network initiate?
2. **Device → Sessions**: Which sessions is a device currently executing?

This dual tracking enables proper cleanup when either network or device disconnects.

```mermaid
graph TB
    subgraph "network Perspective"
        C[network_001]
        CS[_network_sessions]
        CS --> S1[session_abc]
        CS --> S2[session_def]
        CS --> S3[session_ghi]
    end
...
network clients initiate tasks on remote devices. Track these sessions to enable cleanup when the orchestrator disconnects.

**Add network Session:**

```python
def add_network_session(self, client_id: str, session_id: str):
    """Map a session to its network orchestrator."""
    
    with self.lock:
        if client_id not in self._network_sessions:
            self._network_sessions[client_id] = []
        self._network_sessions[client_id].append(session_id)
```

**Get network Sessions:**

```python
def get_network_sessions(self, client_id: str) -> List[str]:
    """Get all sessions initiated by a network client."""
    
    with self.lock:
        return self._network_sessions.get(client_id, []).copy()
        # .copy() prevents external modification of internal list
```

**Remove network Sessions:**

```python
def remove_network_sessions(self, client_id: str) -> List[str]:
    """Remove and return all sessions for a network."""
    
    with self.lock:
        return self._network_sessions.pop(client_id, [])
        # Returns removed sessions for cleanup
```

**Example - network Disconnect Cleanup:**

```python
# In WebSocket Handler - when network disconnects
network_id = "network_001"

# Get all sessions this network initiated
session_ids = client_manager.get_network_sessions(network_id)

logger.info(
    f"network {network_id} disconnected, "
...
for session_id in session_ids:
    await session_manager.cancel_task(
        session_id,
        reason="network_disconnected"  # Don't send callback
    )

# Remove mappings
client_manager.remove_network_sessions(network_id)
```

### Device Session Mapping

Device clients execute tasks sent by networks (or themselves). Track these sessions to enable cleanup when the device disconnects.
...
!!!example "Device Disconnect Cleanup"
    ...
    for session_id in session_ids:
        await session_manager.cancel_task(
            session_id,
            reason="device_disconnected"  # Send callback to network
        )
...
sequenceDiagram
    ...
    WH->>WSM: add_network_session("network_001", "session_abc")
    WH->>WSM: add_device_session("device_001", "session_abc")
    
    Note over WSM: Session Mappings
    WSM->>WSM: _network_sessions["network_001"] = ["session_abc"]
...
!!!warning "Sessions Persist Until Cleanup"
    ...
    1. The network disconnects (removes all its sessions)
    2. The device disconnects (removes all its sessions)
    3. Manual cleanup (future feature)
    
    **Implication:** Over time, `_network_sessions` and `_device_sessions` can grow large. Consider implementing periodic cleanup for completed sessions.

### Dual Mapping Example

!!!example "Single Session, Dual Mapping"
    When a network dispatches a task to a device:
    
    ```python
    network_id = "network_orchestrator_001"
    device_id = "device_windows_001"
    session_id = "session_abc123"
    
    # Session is mapped to BOTH the network and the device
    client_manager.add_network_session(network_id, session_id)
    client_manager.add_device_session(device_id, session_id)
    
    # Later retrieval
    network_sessions = client_manager.get_network_sessions(network_id)
    ...
    **Why dual mapping?**
    ...
    - If **network disconnects**: Cancel all its sessions (notify devices)
    - If **device disconnects**: Cancel all sessions on that device (notify networks)

---

## 💻 System Information Management

The ClientConnectionManager caches device system information to enable intelligent task routing by networks without repeatedly querying devices.
...
def get_stats(self) -> Dict[str, int]:
    ...
    with self.lock:
        ...
        network_count = sum(
            1
            for info in self.online_clients.values()
            if info.client_type == ClientType.network
        )
        return {
            "total": len(self.online_clients),
            "device_clients": device_count,
            "network_clients": network_count
        }
```
...
print(f"  networks: {stats['network_clients']}")
...
async def dispatch_task_to_device(
    ...
    # Step 1: Validate network is connected
    if not client_manager.is_online(network_id):
        raise ValueError(f"network {network_id} not connected")
...
async def handle_client_disconnect(
    ...
    if client_type == ClientType.network:
            session_ids = client_manager.get_network_sessions(client_id)
            cancel_reason = "network_disconnected"
        else:  # DEVICE
        ...
        if client_type == ClientType.network:
            client_manager.remove_network_sessions(client_id)
        else:
...
- [x] Remove session mappings (network/device)
...
**Case 3: network and device both disconnect**
...
async def cleanup_completed_sessions(client_manager, session_manager):
    ...
    all_sessions.update(
        session_id
        for sessions in client_manager._network_sessions.values()
...
async def on_task_complete(session_id, network_id, device_id):
    # Remove specific session from mappings
    client_manager.remove_session_from_network(network_id, session_id)
...
sequenceDiagram
    ...
    WSM-->>WH: WebSocket
    WH->>WSM: add_network_session(const_id, session_id)
    ...
    WH->>WSM: get_network_sessions(client_id)
    WSM-->>WH: [session_ids...]
    WH->>SM: cancel_task(session_id, reason)
    WH->>WSM: remove_network_sessions(client_id)
...
| `get_client_type()` | `client_id: str` | `Optional[ClientType]` | Get client type (DEVICE/network) |
...
| Method | Parameters | Returns | Description |
|--------|------------|---------|-------------|
| `add_network_session()` | `client_id: str`<br/>`session_id: str` | `None` | Map session to network |
| `get_network_sessions()` | `client_id: str` | `List[str]` | Get network's sessions |
| `remove_network_sessions()` | `client_id: str` | `List[str]` | Remove and return sessions |
...
class ClientType(Enum):
    ...
    network = "network"  # Orchestrator client
```
...
|------------|-------------|-------------|
| **Client Registry** | Track connected devices and network clients | Single source of truth for client state |
...
1. **Dual Session Mapping**: Track sessions from both network and device perspectives for comprehensive cleanup
...
4. **Type-Safe Validation**: Always verify client type (DEVICE vs network) before operations

**Integration with Alien Server:**
...
- [Server Overview](./overview.md) - Alien server architecture and components
...
- [Quick Start](./quick_start.md) - Get started with Alien server
