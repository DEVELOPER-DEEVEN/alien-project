...
The Agent Server is a **FastAPI-based asynchronous WebSocket server** that serves as the communication hub for ALIEN's distributed architecture. It bridges orion orchestrators, device agents, and external systems through a unified protocol interface.
...
!!!danger "Important: Session Cancellation on Disconnect"
    When a client disconnects (device or orion), **all associated sessions are immediately cancelled** to prevent orphaned tasks and resource leaks.
...
### Multi-Device Orion Workflow

**Orion Client Coordinating Multiple Devices:**

```mermaid
sequenceDiagram
    participant CC as Orion Client
    participant Server as Agent Server
    participant D1 as Device 1 (GPU)
    participant D2 as Device 2 (CPU)
    
    CC->>Server: REGISTER (orion)
...
The server acts as a message router, forwarding tasks to target devices and routing results back to the orion orchestrator. See [Orion Documentation](../network/overview.md) for more details on multi-device orchestration.
...
|----------|-----------------|------------------|---------------|
| **Device Disconnect** | Heartbeat timeout / WebSocket close | Cancel device sessions, notify orion | Task fails, orion retries |
| **Orion Disconnect** | Heartbeat timeout / WebSocket close | Continue device execution, skip callbacks | Device completes but results not delivered |
