...
The **Agent Server** is the central orchestration engine that transforms Alien into a distributed multi-agent system, enabling seamless task coordination across heterogeneous devices through persistent WebSocket connections and robust state management.
...
The Agent Server is a **FastAPI-based asynchronous WebSocket server** that serves as the communication hub for Alien's distributed architecture. It bridges network orchestrators, device agents, and external systems through a unified protocol interface.

### Core Responsibilities

| Capability | Description | Key Benefit |
|------------|-------------|-------------|
| **🔌 Connection Management** | Tracks device & network client lifecycles | Real-time device availability awareness |
...
!!!danger "Important: Session Cancellation on Disconnect"
    When a client disconnects (device or network), **all associated sessions are immediately cancelled** to prevent orphaned tasks and resource leaks.
...
### Multi-Device network Workflow

**network Client Coordinating Multiple Devices:**

```mermaid
sequenceDiagram
    participant CC as network Client
    participant Server as Agent Server
    participant D1 as Device 1 (GPU)
    participant D2 as Device 2 (CPU)
    
    CC->>Server: REGISTER (network)
...
The server acts as a message router, forwarding tasks to target devices and routing results back to the network orchestrator. See [network Documentation](../cluster/overview.md) for more details on multi-device orchestration.
...
```bash
python -m Alien.server.app --platform windows  # Force Windows sessions
python -m Alien.server.app --platform linux    # Force Linux sessions
python -m Alien.server.app                     # Auto-detect (default)
```
...
The server runs out-of-the-box with sensible defaults. Advanced configuration inherits from Alien's central config system.

### Command-Line Arguments

```bash
python -m Alien.server.app [OPTIONS]
```
...
!!!example "Configuration Examples"
    ...
    python -m Alien.server.app --local --log-level DEBUG --port 8000
    
    # Production: External access, info logging
    python -m Alien.server.app --host 0.0.0.0 --port 5000 --log-level INFO
    
    # Testing: Force Linux sessions
    python -m Alien.server.app --platform linux --port 9000
    ```

### Alien Configuration Inheritance

The server uses Alien's central configuration from `config_dev.yaml`:
...
|----------|-----------------|------------------|---------------|
| **Device Disconnect** | Heartbeat timeout / WebSocket close | Cancel device sessions, notify network | Task fails, network retries |
| **network Disconnect** | Heartbeat timeout / WebSocket close | Continue device execution, skip callbacks | Device completes but results not delivered |
...
- Monitor logs in separate terminal: `tail -f logs/Alien_server.log`
...
!!!example "Development Testing Pattern"
    ...
    python -m Alien.server.app --local --log-level DEBUG
    
    # Terminal 2: Connect device client
    python -m Alien.client.client --ws --ws-server ws://127.0.0.1:5000/ws
...
```nginx
upstream Alien_server {
    server localhost:5000;
}

server {
    listen 443 ssl;
    server_name Alien-server.example.com;
...
python -m Alien.server.app --host 0.0.0.0 --port 5000
```
