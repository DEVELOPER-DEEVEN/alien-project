...
Configure the `HOST_AGENT` and `APP_AGENT` in the `config/Alien/agents.yaml` file to use Ollama.
...
Edit `config/Alien/agents.yaml` with your Ollama configuration:
...
Alien requires at least 20,000 tokens to function properly. Ollama's default context length is 2048 tokens, which is insufficient. You must create a custom model with increased context:
...
## Step 4: Start Using Alien

After configuration, you can start using Alien with Ollama. Refer to the [Quick Start Guide](../../getting_started/quick_start_Alien-Unis.md) for detailed instructions on running your first tasks.
