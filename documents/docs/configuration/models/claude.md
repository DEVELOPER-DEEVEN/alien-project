...
Configure the `HOST_AGENT` and `APP_AGENT` in the `config/Alien/agents.yaml` file to use the Claude API.
...
Edit `config/Alien/agents.yaml` with your Claude configuration:
...
After configuration, you can start using Alien with the Claude API. Refer to the [Quick Start Guide](../../getting_started/quick_start_Alien-Unis.md) for detailed instructions on running your first tasks.