...
Alien supports and welcomes the integration of custom LLM models. If you have a custom LLM model that you would like to use with Alien, follow the steps below to configure it.
...
Create a Python script under the `Alien/llm` directory and implement your own LLM model class by inheriting the `BaseService` class from `Alien/llm/base.py`.

**Reference Example:** See `PlaceHolderService` in `Alien/llm/placeholder.py` as a template.
...
Configure the `HOST_AGENT` and `APP_AGENT` in the `config/Alien/agents.yaml` file to use your custom model.
...
Edit `config/Alien/agents.yaml` with your custom model configuration:
...
Update the model factory in `Alien/llm/__init__.py` to include your custom model class:

```python
from Alien.llm.your_model import YourModelService
...
## Step 5: Start Using Alien

After configuration, you can start using Alien with your custom model. Refer to the [Quick Start Guide](../../getting_started/quick_start_Alien-Unis.md) for detailed instructions on running your first tasks.
