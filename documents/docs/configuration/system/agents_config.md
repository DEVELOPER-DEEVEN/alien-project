...
Configure all LLM models and agent-specific settings for Alien². Each agent type can use different models and API configurations for optimal performance.

## Overview

The `agents.yaml` file defines LLM settings for all agents in Alien². This is the **most important configuration file** as it contains your API keys and model selections.

**File Location**: `config/Alien/agents.yaml`
...
2. **Edit `config/Alien/agents.yaml`** with your API keys and settings
...
Choose your LLM provider and edit `config/Alien/agents.yaml`:
...
Alien² uses different agents for different purposes. Each can be configured with different models.
...
```yaml
HOST_AGENT:
  PROMPT: "Alien/prompts/share/base/host_agent.yaml"
  EXAMPLE_PROMPT: "Alien/prompts/examples/{mode}/host_agent_example.yaml"

APP_AGENT:
  PROMPT: "Alien/prompts/share/base/app_agent.yaml"
  EXAMPLE_PROMPT: "Alien/prompts/examples/{mode}/app_agent_example.yaml"
  API_PROMPT: "Alien/prompts/share/base/api.yaml"
```

You can customize prompts by creating your own YAML files and updating these paths. See the [Customization Guide](../../Alien-Unis/advanced_usage/customization.md) for details.
