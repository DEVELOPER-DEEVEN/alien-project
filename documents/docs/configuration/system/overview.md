...
│   │   ├── agent.yaml              # network agent settings
│   │   ├── agent.yaml.template     # Template for setup
│   │   ├── network.yaml      # network runtime settings
...
The cluster network system has its own set of configuration files in `config/cluster/`:

| File | Purpose | Template | Documentation |
|------|---------|----------|---------------|
| **network.yaml** | network runtime settings (heartbeat, concurrency, step limits) | No | [cluster network Config](./cluster_network.md) |
| **devices.yaml** | Device agent definitions (device_id, server_url, capabilities, metadata) | No | [cluster Devices Config](./cluster_devices.md) |
| **agent.yaml** | network agent LLM configuration (API settings, prompts) | **Yes** (.template) | [cluster Agent Config](./cluster_agent.md) |

### cluster Configuration Structure

```
config/cluster/
├── network.yaml          # Runtime settings for orchestrator
...
│
└── agent.yaml                  # network agent LLM config
    └── network_AGENT:
...
# Load cluster configuration (includes agent and network settings)
cluster_config = get_cluster_config()

# Access agent configuration (LLM settings)
agent_config = cluster_config.agent.network_agent

# Access network runtime settings
network_settings = cluster_config.network

# Or use raw dict access for backward compatibility
network_id = cluster_config["network_ID"]
```

**cluster vs Alien Configuration:**

- **Alien Configurations** (`config/Alien/`) - Single-agent automation settings
- **cluster Configurations** (`config/cluster/`) - Multi-device network settings
...
- **[cluster network Configuration](./cluster_network.md)** - Runtime settings for network orchestrator
- **[cluster Devices Configuration](./cluster_devices.md)** - Device definitions and capabilities
- **[cluster Agent Configuration](./cluster_agent.md)** - LLM configuration for network agent
...
- `config.config_schemas.clusterConfig` - cluster configuration schema
- `config.config_loader.get_Alien_config()` - Get Alien configuration instance
- `config.config_loader.get_cluster_config()` - Get cluster configuration instance
