...
Configure Retrieval-Augmented Generation (RAG) to enhance ALIEN² with external knowledge sources, online search, experience learning, and demonstration-based learning.

---

## Overview

The `rag.yaml` file configures knowledge retrieval systems that augment ALIEN²'s capabilities beyond its base LLM knowledge. RAG helps ALIEN² make better decisions by providing:
...
**File Location**: `config/alien/rag.yaml`

**Optional Configuration:** RAG features are **optional**. ALIEN² works without them, but they can significantly improve performance on complex or domain-specific tasks.
...
Learn from ALIEN²'s own past successful task executions.
...
!!!info "How It Works"
    1. ALIEN² completes a task successfully
...
EXPERIENCE_PROMPT: "alien/prompts/share/experience/experience.yaml"
```
...
Learn from user demonstrations (you show ALIEN² how to do a task).
...
!!!info "How It Works"
    1. User demonstrates a task (ALIEN² records it)
...
1. Record Demonstration:
   python -m alien --mode demonstration
   → Perform task manually
   → ALIEN² records your actions
...
DEMONSTRATION_PROMPT: "alien/prompts/share/demonstration/demonstration.yaml"
```
...
```powershell
python -m alien --request "Create a sales report"
```
...
python -m alien --request "Create a quarterly report"
```
...
```powershell
python -m alien --mode demonstration --task "format_monthly_report"
```

**Step 2**: Perform task manually
- ALIEN² records your every action
...
```powershell
python -m alien --request "Format the report like I showed you"
```
...
!!!bug "Symptom"
    ALIEN² doesn't seem to learn from past tasks
...
| **New to ALIEN²** | Online Search only |
