...
Configure ALIEN²'s runtime behavior, execution limits, control backends, logging, and operational parameters. This file controls how ALIEN² interacts with the Windows environment.

## Overview

The `system.yaml` file defines runtime settings that control ALIEN²'s behavior during task execution. Unlike `agents.yaml` (which configures LLMs), this file configures **how** ALIEN² operates on Windows.

**File Location**: `config/alien/system.yaml`
...
These settings control how ALIEN² communicates with LLM APIs.
...
Control how long and how many attempts ALIEN² makes for tasks.
...
Example: If `MAX_ROUND: 3`, ALIEN² will retry the entire task up to 3 times if it fails.

## Control Backend

Configure how ALIEN² detects and interacts with UI elements.
...
Configure how ALIEN² performs actions on UI elements.
...
Control ALIEN²'s logging output and debugging information.
...
| `MCP_SERVERS_CONFIG` | String | `"config/alien/mcp.yaml"` | Path to MCP servers config |
...
| `MCP_INSTRUCTIONS_PATH` | String | `"alien/config/mcp_instructions"` | MCP instruction templates path |
...
# Recommended settings
USE_MCP: True
MCP_SERVERS_CONFIG: "config/alien/mcp.yaml"
...
!!!danger "Safety Warning"
    When `SAFE_GUARD: True`, ALIEN² will only interact with control types in `CONTROL_LIST`. This prevents accidental dangerous operations but may limit functionality.
...
| `API_PROMPT` | String | `"alien/prompts/share/base/api.yaml"` | API prompt template |
...
APP_API_PROMPT_ADDRESS:
  "WINWORD.EXE": "alien/prompts/apps/word/api.yaml"
  "EXCEL.EXE": "alien/prompts/apps/excel/api.yaml"
  "msedge.exe": "alien/prompts/apps/web/api.yaml"
  "chrome.exe": "alien/prompts/apps/web/api.yaml"
  "POWERPNT.EXE": "alien/prompts/apps/powerpoint/api.yaml"
```
...
**Symptom:** ALIEN² can't find UI elements
