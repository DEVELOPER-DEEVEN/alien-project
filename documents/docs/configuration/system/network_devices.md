...
Device configuration in **devices.yaml** defines the orion's device agents, providing device identity, capabilities, metadata, and connection parameters for each agent in the orion.

---

## Overview

The **devices.yaml** configuration file defines the **devices array** for the Network orion system. It provides:
...
**Orion vs Device Configuration:**

- **devices.yaml** - Defines individual device agents (this document)
- **orion.yaml** - Defines orion-wide runtime settings
- See [Network Orion Configuration](./network_orion.md) for runtime settings

**Configuration Flow:**

```mermaid
graph LR
    A[devices.yaml] -->|Load| B[OrionDeviceManager]
...
with open("config/network/devices.yaml", "r", encoding="utf-8") as f:
...
# Initialize manager with orion settings
manager = OrionDeviceManager(
    task_name=orion_config.get("ORION_ID", "default"),
    heartbeat_interval=orion_config.get("HEARTBEAT_INTERVAL", 30.0),
    reconnect_delay=orion_config.get("RECONNECT_DELAY", 5.0)
)
...
# Device Configuration - YAML Format
# Defines devices for the orion
# Runtime settings (orion_id, heartbeat_interval, etc.) are configured in orion.yaml
...
See [AgentProfile Documentation](../../network/agent_registration/agent_profile.md#multi-source-construction) for merging details.
...
# Access orion-level settings
orion_id = config.get("orion_id", "default")
...
| **AgentProfile** | [AgentProfile](../../network/agent_registration/agent_profile.md) | Profile structure and merging |
| **Registration Flow** | [Registration Flow](../../network/agent_registration/registration_flow.md) | Registration process |
| **Device Registry** | [Device Registry](../../network/agent_registration/device_registry.md) | Registry component |
