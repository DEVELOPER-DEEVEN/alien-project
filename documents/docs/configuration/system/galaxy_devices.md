...
Device configuration in **devices.yaml** defines the network's device agents, providing device identity, capabilities, metadata, and connection parameters for each agent in the network.

---

## Overview

The **devices.yaml** configuration file defines the **devices array** for the cluster network system. It provides:
...
**network vs Device Configuration:**

- **devices.yaml** - Defines individual device agents (this document)
- **network.yaml** - Defines network-wide runtime settings
- See [cluster network Configuration](./cluster_network.md) for runtime settings

**Configuration Flow:**

```mermaid
graph LR
    A[devices.yaml] -->|Load| B[networkDeviceManager]
...
├── config/
    └── cluster/
        ...
        ├── network.yaml     # ⚙️ Runtime settings
...
with open("config/cluster/devices.yaml", "r", encoding="utf-8") as f:
...
# Initialize manager with network settings
manager = networkDeviceManager(
    task_name=network_config.get("network_ID", "default"),
    heartbeat_interval=network_config.get("HEARTBEAT_INTERVAL", 30.0),
    reconnect_delay=network_config.get("RECONNECT_DELAY", 5.0)
)
...
# Defines devices for the network
...
### Example 1: Multi-Device network

```yaml
# Device Configuration - YAML Format
# Defines devices for the network
# Runtime settings (network_id, heartbeat_interval, etc.) are configured in network.yaml
...
See [AgentProfile Documentation](../../cluster/agent_registration/agent_profile.md#multi-source-construction) for merging details.
...
# Access network-level settings
network_id = config.get("network_id", "default")
...
| **AgentProfile** | [AgentProfile](../../cluster/agent_registration/agent_profile.md) | Profile structure and merging |
| **Registration Flow** | [Registration Flow](../../cluster/agent_registration/registration_flow.md) | Registration process |
| **Device Registry** | [Device Registry](../../cluster/agent_registration/device_registry.md) | Registry component |
...
- **Example Config**: `config/cluster/devices.yaml`
- **Loading Logic**: `cluster/client/device_manager.py`
- **DeviceRegistry**: `cluster/client/components/device_registry.py`
- **AgentProfile**: `cluster/client/components/types.py`
