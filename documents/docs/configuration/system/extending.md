...
This guide shows you how to add custom configuration options to Alien-Unis.
...
```yaml
    # config/Alien/analytics.yaml
    ANALYTICS:
      enabled: true
      backend: "influxdb"
      endpoint: "http://localhost:8086"
      database: "Alien_metrics"
      retention: "30d"
...
The config loader automatically discovers and loads all YAML files in `config/Alien/`:
...
### Step 2: Integrate into AlienConfig
...
```yaml
    ...
    # config/Alien/system.dev.yaml (development override)
    LOG_LEVEL: "DEBUG"
    DEBUG_MODE: true
    PROFILING_ENABLED: true

    # config/Alien/system.prod.yaml (production override)
    LOG_LEVEL: "WARNING"
    CACHE_SIZE: 10000
...
```python
    ...
    from config.Alien.schemas.analytics_config import AnalyticsConfig
    ...
    def test_analytics_config_defaults():
        ...
        assert analytics.database == 'Alien_metrics'  # Default
    ...
    def test_config_loading():
        ...
        loader = ConfigLoader()
        config = loader.load_Alien_config('config/Alien')
