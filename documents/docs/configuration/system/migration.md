...
This guide helps you migrate from the legacy configuration system (`Alien/config/config.yaml`) to the new modular configuration system (`config/Alien/`).
...
| Feature | Legacy (`Alien/config/`) | New (`config/Alien/`) |
|---------|----------------------|-------------------|
...
```bash
# From Alien-Unis root directory
python -m Alien.tools.migrate_config

# Or with options
python -m Alien.tools.migrate_config --backup --validate
```
...
mkdir -p config/cluster  # If using cluster
```
...
cp config/cluster/agent.yaml.template config/cluster/agent.yaml  # If using cluster
```

#### Step 3: Split Configuration

Split your `Alien/config/config.yaml` into modular files:

**Legacy config.yaml**:
```yaml
# Alien/config/config.yaml (OLD - Monolithic)
HOST_AGENT:
  API_TYPE: "openai"
  API_KEY: "sk-..."
  API_MODEL: "gpt-4o"
...
cp Alien/config/config.yaml Alien/config/config.yaml.backup

# Remove legacy config (after verifying new config works)
rm Alien/config/config.yaml
```
...
|----------------|--------------|-------|
| `HOST_AGENT.*` | `config/Alien/agents.yaml` → `HOST_AGENT.*` | Same structure |
| `APP_AGENT.*` | `config/Alien/agents.yaml` → `APP_AGENT.*` | Same structure |
| `BACKUP_AGENT.*` | `config/Alien/agents.yaml` → `BACKUP_AGENT.*` | Same structure |
| `EVALUATION_AGENT.*` | `config/Alien/agents.yaml` → `EVALUATION_AGENT.*` | Same structure |
| `OPERATOR.*` | `config/Alien/agents.yaml` → `OPERATOR.*` | New in Alien² |

### System Configurations

| Legacy Field | New Location | New Access Pattern |
|-------------|--------------|-------------------|
| `MAX_STEP` | `config/Alien/system.yaml` | `config.system.max_step` |
| `MAX_RETRY` | `config/Alien/system.yaml` | `config.system.max_retry` |
| `TEMPERATURE` | `config/Alien/system.yaml` | `config.system.temperature` |
| `CONTROL_BACKEND` | `config/Alien/system.yaml` | `config.system.control_backend` |
| `ACTION_SEQUENCE` | `config/Alien/system.yaml` | `config.system.action_sequence` |

### RAG Configurations

| Legacy Field | New Location | New Access Pattern |
|-------------|--------------|-------------------|
| `RAG_OFFLINE_DOCS` | `config/Alien/rag.yaml` | `config.rag.offline_docs` |
| `RAG_EXPERIENCE` | `config/Alien/rag.yaml` | `config.rag.experience` |
| `RAG_DEMONSTRATION` | `config/Alien/rag.yaml` | `config.rag.demonstration` |
| `BING_API_KEY` | `config/Alien/rag.yaml` | `config.rag.BING_API_KEY` |

### MCP Configurations

| Legacy Field | New Location | Notes |
|-------------|--------------|-------|
| `USE_MCP` | `config/Alien/system.yaml` | Keep in system config |
| `MCP_SERVERS_CONFIG` | `config/Alien/system.yaml` | Points to `config/Alien/mcp.yaml` |
| MCP server definitions | `config/Alien/mcp.yaml` | New dedicated file |
...
```yaml
# config/Alien/system.yaml (base)
MAX_STEP: 50

# config/Alien/system_dev.yaml (development override)
MAX_STEP: 10
LOG_LEVEL: "DEBUG"
```
...
```bash
# Set environment
export Alien_ENV=dev  # Automatically uses system_dev.yaml overrides
```
...
```yaml
# config/Alien/custom.yaml (new file)
...
python -m Alien --task "Open Notepad"
...
ls config/Alien/agents.yaml
ls config/Alien/system.yaml

# Check file permissions
chmod 644 config/Alien/*.yaml
```
...
mv Alien/config/config.yaml Alien/config/config.yaml.backup

# Option 2: Disable automatic fallback (in code)
config = get_Alien_config()  # Will warn but use new path
```
...
cp Alien/config/config.yaml.backup Alien/config/config.yaml

# 2. Remove new config files
rm -rf config/Alien/*.yaml
