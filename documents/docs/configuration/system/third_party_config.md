...
!!!info "Alien³ Integration"
    LinuxAgent is used by **Alien³ cluster** to orchestrate Linux devices as sub-agents in multi-device workflows. When cluster routes a task to a Linux device, it uses LinuxAgent to execute commands via CLI.
...
### For Alien³ cluster (Recommended)

```yaml
# Enable LinuxAgent for Alien³ cluster multi-device orchestration
ENABLED_THIRD_PARTY_AGENTS: ["LinuxAgent"]
...
## Alien³ cluster Integration

When using Alien³ cluster for multi-device orchestration, LinuxAgent must be enabled to support Linux devices.
...
**Step 2**: Configure Linux devices in `config/cluster/devices.yaml`
...
See [Linux Agent as cluster Device](../../linux/as_cluster_device.md) for complete setup.
...
!!!tip "Recommendations"
    ...
    - ✅ **For Alien³ cluster** - Always enable LinuxAgent when using Linux devices
...
- **[Linux Agent as cluster Device](../../linux/as_cluster_device.md)** - Using LinuxAgent in Alien³
...
- **[cluster Devices Configuration](./cluster_devices.md)** - Multi-device setup
...
!!!success "Key Takeaways"
    ...
    ✅ **LinuxAgent for Alien³** - Required when using Linux devices in cluster  
