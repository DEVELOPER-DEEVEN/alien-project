# cluster Trajectory Report

## Overview

The **cluster Trajectory Report** (`output.md`) is an automatically generated comprehensive execution log that documents the complete lifecycle of a multi-device task execution session in cluster. This human-readable Markdown report provides step-by-step visualization of network evolution, task execution, and device coordination.

### Report Location

After each cluster session completes, the trajectory report is automatically generated:

```
logs/cluster/<session_name>/output.md
logs/cluster/<session_name>/topology_images/  # DAG visualizations
```

**Example:**
```
logs/cluster/request_20251111_140216_1/
...
└── topology_images/             # Generated DAG topology graphs
    ├── step1_after_network_xxx.png
    ├── step2_after_network_xxx.png
    └── step999_final_network_xxx.png
```
...
- **Agent**: network_agent (networkAgent)
- **Status**: CONTINUE
...
**Fields:**
- **Agent**: Agent name and type (networkAgent for orchestration)
...
**Common Functions:**
- `build_network`: Initial DAG creation
- `edit_network`: Dynamic DAG modification
- `execute_network`: Trigger task execution
...
<details>
<summary>network AFTER (click to expand)</summary>

**network ID**: network_bcd1726e_20251105_134526
**State**: created

##### Dependency Graph (Topology)

<img src="topology_images/step2_after_network_xxx.png" width="600">
...
**ID**: network_bcd1726e_20251105_134526
**State**: completed
...
The trajectory report is generated automatically by `clusterSession` upon completion:

```python
# cluster/session/cluster_session.py
async def close_session(self):
    """Generate trajectory report on session close"""
    trajectory = clusterTrajectory(self.log_path)
    trajectory.to_markdown(self.log_path + "output.md")
```

**Trigger Points:**
1. Normal session completion (`clusterClient.shutdown()`)
...
python -m cluster.trajectory.generate_report logs/cluster/test1

# Custom output path
python -m cluster.trajectory.generate_report logs/cluster/test1 -o custom_report.md

# Minimal report (exclude details)
python -m cluster.trajectory.generate_report logs/cluster/test1 \
  --no-network --no-tasks --no-devices
```

**CLI Options:**
- `--no-network`: Exclude network evolution details
...
```python
# cluster/trajectory/cluster_parser.py
if __name__ == "__main__":
    """Process all cluster task logs and generate markdown reports."""
    
    cluster_logs_dir = Path("logs/cluster")
    task_dirs = sorted([d for d in cluster_logs_dir.iterdir() if d.is_dir()])
    
    for task_dir in task_dirs:
        trajectory = clusterTrajectory(str(task_dir))
...
```bash
cd c:\Users\chaoyunzhang\OneDrive - Microsoft\Desktop\research\GPTV\Alien-windows\github\saber\Alien-Unis
python -m cluster.trajectory.cluster_parser
```

**Output:**
```
[BOLD BLUE] cluster Trajectory Parser - Batch Mode
...
```python
from cluster.trajectory import clusterTrajectory

# Load trajectory from log directory
trajectory = clusterTrajectory("logs/cluster/test1")
...
final_network = trajectory._parse_network(
    last_step.get("network_after")
)

if final_network:
    network_id = final_network.get("network_id")
    state = final_network.get("state")
    tasks = final_network.get("tasks", {})
    
    print(f"network {network_id}: {state}")
...
trajectory.to_markdown(
    ...
    include_network_details=True,  # Show DAG evolution
...
```
topology_images/
├── step1_after_network_7b3c0f47_20251104_182305.png
├── step2_before_network_bcd1726e_20251105_134526.png
├── step2_after_network_bcd1726e_20251105_134526.png
├── step3_before_network_bcd1726e_20251105_134526.png
├── step3_after_network_bcd1726e_20251105_134526.png
└── step999_final_network_bcd1726e_20251105_134526.png
```

**Naming Convention:**
- `step{N}_{state}_{network_id}.png`
...
```python
trajectory = clusterTrajectory("logs/cluster/failed_session")

for step in trajectory.step_log:
    network = trajectory._parse_network(step.get("network_after"))
    if not network:
        continue
...
trajectory = clusterTrajectory("logs/cluster/task_32")

# Load metrics for performance data
with open("logs/cluster/task_32/result.json") as f:
    result = json.load(f)
...
slow_tasks = [
    ...
    for step in trajectory.step_log
    for tid, task in trajectory._parse_network(
        step.get("network_after")
    ).get("tasks", {}).items()
]
...
```python
trajectory = clusterTrajectory("logs/cluster/adaptive_session")

for idx, step in enumerate(trajectory.step_log, 1):
    before = trajectory._parse_network(step.get("network_before"))
    after = trajectory._parse_network(step.get("network_after"))
...
```python
trajectory = clusterTrajectory("logs/cluster/multi_device")
...
for step in trajectory.step_log:
    ...
    for task in network.get("tasks", {}).values():
...
def compare_sessions(session1_path, session2_path):
    t1 = clusterTrajectory(session1_path)
    t2 = clusterTrajectory(session2_path)
...
compare_sessions("logs/cluster/test_v1", "logs/cluster/test_v2")
```
...
trajectory.to_markdown(
    ...
    include_network_details=True,  # DAG evolution (default: True)
...
for dir in logs/cluster/*/; do
    python -m cluster.trajectory.generate_report "$dir"
done

# Open reports in browser for visual inspection
start logs/cluster/test1/output.md
```
...
cp logs/cluster/*/output.md trajectory_archives/$(date +%Y-%m-%d)/
cp logs/cluster/*/result.json trajectory_archives/$(date +%Y-%m-%d)/
```
...
def validate_trajectory(log_dir):
    trajectory = clusterTrajectory(log_dir)
...
Use network evolution to verify correctness:

```python
# Verify DAG grows monotonically (no premature task deletion)
trajectory = clusterTrajectory("logs/cluster/session")

prev_task_count = 0
for step in trajectory.step_log:
    network = trajectory._parse_network(step.get("network_after"))
    if network:
        task_count = len(network.get("tasks", {}))
...
- **[cluster Overview](../overview.md)** - Main cluster framework documentation
- **[network Orchestrator](../network_orchestrator/overview.md)** - DAG execution engine
- **[Task network](../network/overview.md)** - DAG data structure and validation
...
1. Check for `response.log` existence:
   ```bash
   ls logs/cluster/<session_name>/response.log
   ```

2. Manually trigger generation:
   ```bash
   python -m cluster.trajectory.generate_report logs/cluster/<session_name>
   ```
...
1. Check `topology_images/` directory exists:
   ```bash
   ls logs/cluster/<session_name>/topology_images/
   ```
...
2. Reduce topology image quality (edit `cluster_parser.py`):
   ```python
...
3. Archive and compress:
   ```bash
   gzip logs/cluster/<session_name>/output.md
   ```

## API Reference

### clusterTrajectory Class

```python
class clusterTrajectory:
    """Parser for cluster agent logs with network visualization"""
    ...
    def __init__(self, folder_path: str) -> None:
        ...
        Args:
            folder_path: Path to cluster log directory (e.g., logs/cluster/task_1)
    ...
    def to_markdown(
        ...
        Args:
            ...
            include_network_details: Include DAG evolution details
