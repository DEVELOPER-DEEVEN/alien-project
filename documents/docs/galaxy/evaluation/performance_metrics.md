...
cluster provides comprehensive performance monitoring and metrics collection throughout multi-device workflow execution. The system tracks task execution times, network modifications, and overall session metrics to enable analysis and optimization of distributed workflows.

## Overview

cluster uses an **event-driven observer pattern** to collect real-time performance metrics without impacting execution flow. The `SessionMetricsObserver` automatically captures timing data, task statistics, network modifications, and parallelism metrics.
...
| **network Metrics** | DAG-level statistics and parallelism analysis | Optimize workflow structure |
| **Modification Metrics** | Dynamic network editing during execution | Understand adaptability patterns |
...
The `SessionMetricsObserver` is automatically initialized for every cluster session and listens to events from the orchestration system.
...
graph LR
    ...
    C[network Operations] -->|network Events| B
...
| `network_STARTED` | New DAG created | Initial statistics, task count |
| `network_COMPLETED` | DAG fully executed | Final statistics, total duration |
| `network_MODIFIED` | DAG edited during execution | Changes, modification type |
...
{
    "network_timings": {
        "network_b0864385_20251025_183508": {
...
| Field | Type | Description | Example |
|-------|------|-------------|---------|
| `total_networks` | int | Number of DAGs created | `1` |
| `completed_networks` | int | Successfully completed DAGs | `1` |
| `failed_networks` | int | Failed DAGs | `0` |
| `success_rate` | float | Completion rate | `1.0` |
| `average_network_duration` | float | Mean DAG execution time | `659.98` |
| `min_network_duration` | float | Fastest DAG completion | `659.98` |
| `max_network_duration` | float | Slowest DAG completion | `659.98` |
| `average_tasks_per_network` | float | Mean tasks per DAG | `5.0` |
...
### 3. network Modification Metrics
        ...
        "network_b0864385_20251025_183508": [
            {
                ...
                "modification_type": "Edited by network_agent",
                "on_task_id": ["t1"],
                "changes": {
...
| Field | Type | Description | Example |
|-------|------|-------------|---------|
| `total_modifications` | int | Total network edits | `4` |
| `networks_modified` | int | Number of DAGs modified | `1` |
| `average_modifications_per_network` | float | Mean edits per DAG | `4.0` |
| `max_modifications_for_single_network` | int | Most edits to one DAG | `4` |
| `most_modified_network` | str | network ID with most edits | `network_...` |
| `modifications_per_network` | dict | Edit count per DAG | `{"network_...": 4}` |
| `modification_types_breakdown` | dict | Count by modification type | `{"Edited by network_agent": 4}` |

**Modification Types:**

| Type | Description | Trigger |
|------|-------------|---------|
| `Edited by network_agent` | networkAgent refined DAG | Task completion, feedback |
| `task_properties_updated` | Task details modified | Result refinement |
| `network_updated` | DAG structure changed | Dependency updates |
| `tasks_added` | New tasks inserted | Workflow expansion |
| `tasks_removed` | Tasks deleted | Optimization |

## Session Results Structure

The complete session results are saved to `logs/cluster/<task_name>/result.json` with the following structure:

```json
{
    "session_name": "cluster_session_20251025_183449",
    ...
    "trajectory_path": "logs/cluster/task_32/",
    ...
    "network": {
        "id": "network_b0864385_20251025_183508",
        "name": "network_b0864385_20251025_183508",
        "task_count": 5,
        "dependency_count": 4,
        "state": "completed"
    }
...
def analyze_session_performance(result_path: str):
    """
    Analyze cluster session performance from result.json.
    ...
    # network performance
    const_stats = metrics["network_statistics"]
    print(f"\n🌌 networks: {const_stats['completed_networks']}/{const_stats['total_networks']}")
    print(f"⏱️  Average network duration: {const_stats['average_network_duration']:.2f}s")
    
    # Parallelism analysis
    final_stats = result["session_results"]["final_network_stats"]
    ...
    print(f"   Average per network: {mod_stats['average_modifications_per_network']:.1f}")
    
    return metrics

# Example usage
metrics = analyze_session_performance("logs/cluster/task_32/result.json")
```
...
# Example usage
bottlenecks = identify_bottlenecks("logs/cluster/task_32/result.json")
```
...
# Example usage
visualize_task_timeline("logs/cluster/task_32/result.json")
```
...
# Example usage
analyze_dependencies("logs/cluster/task_32/result.json")
```
...
# Example usage
generate_optimization_report("logs/cluster/task_32/result.json")
```

### 3. Reduce network Modifications
...
def analyze_modification_overhead(result_path: str):
    ...
    metrics = result["session_results"]["metrics"]
    modifications = metrics["network_modifications"]
...
# Example usage
analyze_modification_overhead("logs/cluster/task_32/result.json")
```
...
# Analyze every session to identify trends
for session_dir in Path("logs/cluster").iterdir():
        ...
        if result_file.exists():
            with open(result_file, 'r') as f:
                ...
                results.append({
                    ...
                    "parallelism": data["session_results"]["final_network_stats"].get("parallelism_ratio", 1.0)
                })
...
# Example usage
track_performance_trends("logs/cluster")
```
...
- **[Task network](../network/task_network.md)** - DAG-based task planning and parallelism metrics
- **[network Orchestrator](../network_orchestrator/overview.md)** - Execution coordination and event handling

## Summary

cluster's performance metrics system provides comprehensive monitoring capabilities:

- **Real-time monitoring** - Event-driven metrics collection through `SessionMetricsObserver`
- **Comprehensive coverage** - Tasks, networks, and modifications tracking
