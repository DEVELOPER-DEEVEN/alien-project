# network Agent — The Centralized network Weaver

The **network Agent** serves as the central intelligence of Alien³ cluster, acting as both a planner and replanner. It interprets user intent, constructs executable Task networks, and dynamically steers their evolution across heterogeneous devices. By bridging high-level natural-language goals and concrete multi-agent execution, the network Agent provides unified orchestration through a feedback-driven control loop.

For an overview of the cluster system architecture, see [cluster Overview](../overview.md).

## 🌟 Introduction

![network Agent Architecture](../../img/network_agent.png)
**Figure:** An overview of the network Agent showing the dual-mode control cycle between creation and editing phases.

The network Agent extends the abstract [Task network](../network/overview.md) model into runtime execution. Residing within the **networkClient** (see [cluster Client](../client/overview.md)), it transforms user requests into structured DAG workflows and continuously refines them as distributed agents provide feedback.

Unlike traditional static DAG schedulers, the network Agent operates as a **dynamic orchestrator** powered by an LLM-driven architecture and governed by a finite-state machine (FSM). This design enables it to alternate between two complementary operating modes:

- **Creation Mode**: Synthesizes initial Task networks from user instructions
- **Editing Mode**: Incrementally refines networks based on runtime feedback
...
The network Agent orchestrates distributed workflows through structured feedback loops, alternating between creation and editing phases with explicit operational boundaries. For details on task execution, see [network Orchestrator](../network_orchestrator/overview.md).
...
| **DAG Synthesis** | Decompose requests into structured Task networks with dependencies | Creation |
| **Device Assignment** | Map tasks to appropriate devices based on AgentProfile capabilities | Creation |
| **Runtime Monitoring** | Track task completion events and network state | Editing |
...
The network Agent implements a **dual-mode control pattern** that separates planning from replanning:
...
In creation mode, the network Agent receives a user request and generates the initial Task network.
...
|--------|------|-------------|
| **Task network** | `Tasknetwork` | Structured DAG with tasks and dependencies |
| **Observation** | `str` | Analysis of input context and device profiles |
| **Thought** | `str` | Reasoning trace explaining decomposition logic |
| **State** | `networkAgentStatus` | Next FSM state (typically `CONTINUE`) |
...
During execution, the network Agent enters editing mode to process task completion events and adapt the network.
...
| **Current network** | `Tasknetwork` | Serialized network snapshot |
...
sequenceDiagram
    ...
    Agent->>Dispatcher: Sync network State
    Dispatcher->>MCP: build_network (sync)
    ...
    Editor->>MCP: Return Updated network
    MCP->>Dispatcher: Return Results
    Dispatcher->>Agent: network Updated
...
The agent can perform the following modifications through the MCP-based network Editor:
...
| Output | Type | Description |
|--------|------|-------------|
| **Updated network** | `Tasknetwork` | Modified DAG with new tasks/dependencies |
| **Thought** | `str` | Reasoning explaining modifications or no-op |
| **State** | `networkAgentStatus` | Next FSM state (`CONTINUE`, `FINISH`, or `FAIL`) |
...
**Figure:** Lifecycle state transitions of the network Agent FSM.

The network Agent's behavior is governed by a **4-state finite-state machine**:

| State | Description | Triggers |
|-------|-------------|----------|
| **START** | Initialize network, begin orchestration | Agent instantiation, restart after completion |
| **CONTINUE** | Monitor events, process feedback, update network | Task completion/failure events |
...
## 🛠️ MCP-Based network Editor

The network Agent interacts with the **network Editor** through the **Model Context Protocol (MCP)** layer. The architecture uses:
...
- **network Editor MCP Server**: Implements the actual network manipulation operations
...
The network Editor MCP Server exposes the following operations:

| Operation | Purpose | Inputs | Output |
|------|---------|--------|--------|
| `build_network` | Batch-create network from config | Configuration dict, clear flag | Built network |
| `add_task` | Add atomic task node | Task ID, name, description, device, tips | Updated network |
| `remove_task` | Remove task and dependencies | Task ID | Updated network |
| `update_task` | Modify task fields | Task ID + updated fields | Updated network |
| `add_dependency` | Create dependency edge | From/to task IDs, type, condition | Updated network |
| `remove_dependency` | Delete dependency | Dependency ID | Updated network |
| `update_dependency` | Update dependency logic | Dependency ID, condition | Updated network |
...
For complete MCP command specifications and examples, see [Command Reference](command.md). For details on the underlying Task network structure, see [Task network Overview](../network/overview.md).

## 📋 Processing Pipeline

The network Agent follows a **4-phase processing pipeline** for both creation and editing modes:

### Phase 1: Context Provision

```python
# Load available MCP tools from network Editor
...
# Returns: networkAgentResponse with thought, status, actions
```
...
context.set(ContextNames.network, updated_network)
...
The network Agent uses the **Factory Pattern** to create appropriate prompters for different weaving modes (creation and editing).

### Prompter Hierarchy

```mermaid
classDiagram
    class BasenetworkPrompter {
        <<abstract>>
    ...
    class networkCreationPrompter {
        +user_prompt_construction()
        +examples_prompt_helper()
    }
    
    class networkEditingPrompter {
        +user_prompt_construction()
        +examples_prompt_helper()
    }
    
    class networkPrompterFactory {
        +create_prompter(mode)
        +get_supported_modes()
    }
    
    BasenetworkPrompter <|-- networkCreationPrompter
    BasenetworkPrompter <|-- networkEditingPrompter
    networkPrompterFactory --> BasenetworkPrompter
```
...
High-level task synthesis and low-level execution coordination are decoupled yet tightly synchronized through the Task network abstraction. The agent focuses on semantic reasoning while the orchestrator handles distributed execution.

### 2. Dynamic Adaptability

The editable network enables:
...
- **Re-validation**: $O(n + e)$ for modified network
...
- **[Task network](../network/overview.md)** — Abstract DAG model
- **[TaskStar](../network/task_star.md)** — Atomic execution units
- **[TaskStarLine](../network/task_star_line.md)** — Dependency relationships
- **[network Orchestrator](../network_orchestrator/overview.md)** — Distributed executor
...
The network Agent serves as the **central weaver** of distributed intelligence in Alien³ cluster. Through its dual-mode control loop, finite-state machine governance, and MCP-based network manipulation, it transforms abstract user goals into live, evolving networks—maintaining both rigor and adaptability across the complete lifecycle of multi-device orchestration.
