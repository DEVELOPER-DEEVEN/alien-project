...
DeviceManager is the connection orchestration layer in cluster Client. While networkClient provides the high-level device management API, DeviceManager handles the low-level details of WebSocket connections, health monitoring, message routing, and task queuing.

## Related Documentation

- [Overview](./overview.md) - Overall cluster Client architecture and workflow
- [networkClient](./network_client.md) - High-level device management API
...
def __init__(
    ...
    Args:
        task_name: Identifier for this network instance (default "test_task")
...
**task_name**: This identifier appears in log messages and helps distinguish between multiple network instances running simultaneously. For example, "production_network" vs "test_network".
...
Most applications should use networkClient, which wraps DeviceManager. Use DeviceManager directly only for:
...
- See [networkClient](./network_client.md) for high-level API
- See [AIP Integration](./aip_integration.md) for protocol details and message flows
- See [Overview](./overview.md) for overall cluster Client architecture
