...
The Agent Interaction Protocol (AIP) is the communication protocol used throughout cluster Client for device coordination. This document explains how cluster Client integrates with AIP, the message flow patterns, and how different components use the protocol.

## Related Documentation

- [Overview](./overview.md) - Overall cluster Client architecture
...
**Transport Agnostic**: AIP runs over WebSocket in cluster Client, but the protocol itself is transport-independent. You could implement AIP over HTTP, gRPC, or any other transport.
...
## Protocol Architecture in cluster Client

cluster Client uses AIP at multiple levels:
...
### Layer 4: Application Logic (DeviceManager, networkClient)
...
```json
{
  ...
  "task_name": "cluster/production/excel_task",
...
- Handled by: DeviceManager, networkClient
...
## Complete Message Flow: networkClient to Device Agent

This section shows the complete end-to-end message flow from when a networkClient assigns a task through the Agent Server to the final execution on a Device Agent.
...
```
networkClient (cluster Client)
        ↓ WebSocket + AIP
AlienWebSocketHandler (Agent Server)
        ↓ WebSocket + AIP  
Device Agent Client
```
...
When networkClient assigns a task to a device, the message passes through multiple layers:
...
sequenceDiagram
    ...
    participant Server as AlienWebSocketHandler
    ...
    MP->>MP: Create ClientMessage(type=TASK, client_type=network)
    ...
    Server->>Server: client_type=network, extract target_id
    Server->>Server: Create session_id, register network session
    ...
    Server->>WS1: TASK_END to networkClient
...
#### Layer 1: networkClient to Server
...
- `client_type: "network"`: Identifies this as a network client (not device)
- `target_id`: The device that should execute this task (e.g., "windows_pc")
- `session_id`: network session identifier for tracking
- `task_name`: Human-readable task identifier

#### Layer 2: Server Processing

The `AlienWebSocketHandler` receives the message and processes it:
...
async def handle_task_request(self, data: ClientMessage) -> None:
    ...
    if client_type == ClientType.network:
        # Extract target device from network request
        target_device_id = data.target_id
        self.logger.info(f"🌟 network task for device {target_device_id}")
        
        # Track session mapping
        self.client_manager.add_network_session(client_id, session_id)
...
1. **network Sessions**: Maps network_client_id → [session_ids]
2. **Device Sessions**: Maps device_id → [session_ids]

This allows the server to:

- Cancel all sessions when a network client disconnects
- Cancel all sessions when a device disconnects
- Send results to both network client AND device
...
```python
if client_type == ClientType.network and target_device_id:
    target_protocol = self.client_manager.get_task_protocol(target_device_id)
...
```python
# Cancel all sessions started by this network
session_ids = self.client_manager.get_network_sessions(client_id)
...
On the networkClient side, DeviceManager detects disconnection via:
...
| Aspect | network Client | DEVICE Client |
|--------|---------------------|---------------|
| Task Request | Includes `target_id` field | No `target_id`, executes locally |
| Session Tracking | Tracked in network_sessions | Tracked in device_sessions |
...
This allows the same server to support both direct device connections and network-mediated connections.

---

## Summary

AIP integration in cluster Client follows a layered architecture:
...
4. **Application**: DeviceManager and networkClient use messages for coordination
5. **Server Routing**: AlienWebSocketHandler routes messages between network clients and devices
...
- **Task Execution (network)**: networkClient TASK → Server routes → Device executes → Server routes → networkClient TASK_END
...
User Request
    ↓
clusterClient (session management)
    ↓
networkClient (device coordination)
    ↓
DeviceManager (connection orchestration)
    ↓
MessageProcessor (AIP messaging)
    ↓
WebSocket → AlienWebSocketHandler (server routing)
    ↓
WebSocket → Device Agent (task execution)
```

AIP provides a robust, extensible protocol for agent communication with strong typing, clear message flows, comprehensive error handling, and intelligent routing between network clients and devices.
...
- See [networkClient](./network_client.md) for device coordination API
