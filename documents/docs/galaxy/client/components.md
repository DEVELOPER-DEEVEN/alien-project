# cluster Client Components

cluster Client is built from focused, single-responsibility components that work together to provide device management capabilities. This document explains how these components interact and what each one does.
...
- [networkClient](./network_client.md) - How components are used in the coordination layer
...
cluster Client uses 8 modular components divided into three categories: **Device Management**, **Display & UI**, and **Support Components**. Understanding how these components work together is key to understanding cluster Client's design.
...
async def send_task_to_device(device_id, task_request):
    ...
    task_message = ClientMessage(
        ...
        target_id=device_id,
        task_name=f"cluster/{task_name}/{task_request.task_name}",
        request=task_request.request,
        session_id=network_task_id,
    ...
    result = await self._wait_for_task_response(device_id, network_task_id)
...
**Purpose**: Provides Rich-based console output for interactive mode and status reporting. This component is only used by clusterClient, not by networkClient or DeviceManager.

**Key Features**:

**Banner and Branding**: Shows ASCII art banner when clusterClient starts, creating a visual identity for the framework.
...
```python
user_input = display.get_user_input("Alien[0]")
```
...
# Register as network client using AIP RegistrationProtocol
await self._register_network_client(device_info)
```
...
cluster Client's component architecture demonstrates several important design principles:
...
This design makes cluster Client maintainable, extensible, and testable. When you understand how components collaborate, you can confidently modify or extend the system.

## Related Documentation

- [DeviceManager Reference](./device_manager.md) - See how DeviceManager orchestrates these components
- [networkClient](./network_client.md) - Learn how components are used in the coordination layer
- [Overview](./overview.md) - Understand the broader cluster Client architecture
