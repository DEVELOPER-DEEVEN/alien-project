...
cluster Client is the client-side layer responsible for multi-device coordination in the Alien³ framework. At its core is **networkClient**, which manages device registration, connection, and task assignment. **clusterClient** provides a lightweight wrapper offering convenient session management interfaces.

## Related Documentation

- [networkClient](./network_client.md) - Core device coordination component
...
- [clusterClient](./cluster_client.md) - Session wrapper API
- [Configuration](../../configuration/system/cluster_network.md) - Device configuration guide

## The Complete Path: From User Request to Device Execution

To understand cluster Client, we first need to see the entire system workflow. When a user submits a task request, the system processes it through several layers:

### 1. User Interaction Layer (Optional)

Users can interact with the cluster system in two ways:

**Interactive Mode**: Users input natural language requests through a command-line interface (CLI), which are received and processed by clusterClient. This mode is primarily used for rapid prototyping and manual testing.

**Programmatic Mode**: Developers directly call the Python API of networkClient or clusterClient, integrating cluster into their applications. This is the recommended approach for production environments.

### 2. Session Management Layer (clusterClient)

clusterClient's role is to manage the lifecycle of task sessions. It doesn't handle specific device operations but instead:

- Initializes and holds a networkClient instance
- Creates a clusterSession for each user request
- Passes requests to networkAgent for DAG planning (task decomposition)
- Coordinates TasknetworkOrchestrator to execute the DAG
- Collects and aggregates execution results

**clusterClient is optional**. If your application doesn't need session management, you can use networkClient directly.

### 3. Device Coordination Layer (networkClient)

networkClient is the heart of cluster Client. It is responsible for:
...
**Task Assignment**: Receiving task requests from upper layers (TasknetworkOrchestrator), selecting appropriate devices based on capabilities, sending tasks to devices via the AIP protocol, and waiting for and collecting task execution results.

networkClient doesn't concern itself with how tasks are decomposed (that's networkAgent's responsibility) or how DAGs are executed (that's TasknetworkOrchestrator's responsibility). It focuses on "device-level matters."

### 4. Connection Management Layer (DeviceManager)

DeviceManager is the core internal component of networkClient, responsible for all low-level connection management:
...
All communication with devices goes through the [Agent Interaction Protocol (AIP)](../../aip/overview.md). AIP is a WebSocket-based messaging protocol that defines standard message types and interaction flows. Main message types used by cluster Client include:
...
### networkClient: The Device Coordination Facade

networkClient implements the Facade pattern. It provides simple device management APIs externally while delegating actual work to DeviceManager internally.
...
- DAG planning (handled by networkAgent)
- DAG execution (handled by TasknetworkOrchestrator)
- Session management (handled by clusterSession)

See [networkClient documentation](./network_client.md) for detailed API reference.

### DeviceManager: The Connection Management Engine

DeviceManager is the "engine" of networkClient. It uses 5 modular components to accomplish connection management:
...
### clusterClient: Session Management Wrapper

clusterClient provides a higher-level abstraction on top of networkClient:

```python
client = clusterClient()
await client.initialize()  # Initialize networkClient and connect devices

# Process user request (internally creates clusterSession, calls networkAgent for DAG planning)
result = await client.process_request("Open Excel and create a sales chart")

await client.shutdown()  # Cleanup resources
```

clusterClient's main value lies in:

- Simplifying initialization flow (automatically loads device info from config)
- Providing session management (creates independent clusterSession for each request)
- Integrating display components (Rich console output, progress bars, etc.)
- Supporting interactive mode (command-line interface)

If your application already has its own session management logic, you can skip clusterClient and use networkClient directly. See [clusterClient documentation](./cluster_client.md) for detailed API.
...
**Step 1: Initialize clusterClient**
...
1. clusterClient loads device information from config file (`device_info.yaml`)
2. Creates networkClient instance and passes configuration
3. networkClient calls `device_manager.register_device()` to register each device
...
1. clusterClient creates a clusterSession
2. clusterSession calls networkAgent for task planning
3. networkAgent (LLM-powered) decomposes task into DAG:
...
4. TasknetworkOrchestrator executes DAG:
...
**Step 4: Task Assignment and Execution (networkClient/DeviceManager)**

For each task, networkClient calls:
...
1. TasknetworkOrchestrator aggregates all task results
2. clusterSession generates session results (including execution time, rounds, DAG statistics)
3. clusterClient returns results to user
...
cluster Client is not an isolated system—it closely collaborates with other Alien³ components:

### Depends on Agent Server for Message Routing

cluster Client doesn't connect directly to devices but routes through [Agent Server](../../server/overview.md). Agent Server's role is to:
...
**Route Messages**: Forwards TASK messages from cluster Client to the correct device based on device_id. The server acts as a message broker, decoupling clients from devices.
...
### Used by networkAgent for Task Planning

When clusterClient receives a user request, it calls [networkAgent](../network_agent/overview.md) to decompose the request into a DAG (Directed Acyclic Graph). networkAgent is LLM-powered and can:
...
**Dynamically Adjust DAG**: If issues arise during execution (e.g., a device fails), networkAgent can replan and modify the DAG to adapt to the new situation.

For more details, see [networkAgent Documentation](../network_agent/overview.md).

### Coordinates with TasknetworkOrchestrator for DAG Execution

Once networkAgent creates the DAG, [TasknetworkOrchestrator](../network_orchestrator/overview.md) executes it across devices. The orchestrator:
...
For more details, see [TasknetworkOrchestrator Documentation](../network_orchestrator/overview.md).

### Collaborates with Device Agents for Task Execution

The actual task execution happens on [Device Agents](../../client/overview.md) running on each device (such as Alien² Desktop Agent, Linux Agent, etc.). Device Agents are responsible for:
...
Device information is defined through configuration files. See [cluster Configuration](../../configuration/system/cluster_network.md) for complete configuration options.

A typical configuration example:

```yaml
# config/cluster/network.yaml
task_name: "production_network"
...
- **task_name**: Unique identifier for this network, used in logs and debugging
...
- Use interactive mode for quick testing: `python -m cluster --interactive`
...
- [networkClient API Reference](./network_client.md) - Complete device coordination API
...
- [clusterClient Session Wrapper](./cluster_client.md) - Session management API

## Summary

cluster Client provides the core multi-device coordination capabilities in Alien³. Through layered design, it simplifies complex distributed system management into clear APIs:

- **networkClient** is the core of device management, handling device registration, connection, and task assignment
- **DeviceManager** is the underlying engine, processing WebSocket, heartbeat, message routing, and task queuing
- **clusterClient** is an optional session wrapper, providing more convenient high-level APIs

If you're new to cluster Client, we recommend reading the documentation in this order:

1. This Overview (understand overall architecture and workflow)
2. [networkClient](./network_client.md) (learn core API)
...
If you need to get started quickly, jump directly to [clusterClient](./cluster_client.md) example code.
