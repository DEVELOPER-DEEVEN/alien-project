...
networkClient is the device coordination layer in cluster Client. It provides a clean API for registering devices, managing connections, and assigning tasks. Most applications interact with networkClient rather than the lower-level DeviceManager.
...
- [Configuration](../../configuration/system/cluster_network.md) - Device configuration
- [clusterClient](./cluster_client.md) - Session wrapper on top of networkClient

## What networkClient Does

networkClient implements the Facade pattern, providing a simplified interface to the complex device management system underneath. Think of it as the "device management API" for cluster.

**Core Responsibilities:**

**Device Lifecycle Management**: networkClient handles the complete lifecycle of device connections. When you register a device, it stores the device information (ID, server URL, capabilities) in DeviceRegistry. When you connect, it coordinates with DeviceManager to establish WebSocket connections, perform AIP registration, and start health monitoring. When you disconnect, it cleanly tears down all resources.

**Task Assignment**: When you have a task to execute, networkClient determines which device should run it (based on capabilities), checks if the device is available, and delegates to DeviceManager for actual execution. It abstracts away details like task queuing when devices are busy or handling connection failures during execution.

**Configuration Management**: networkClient loads device configurations from YAML files or programmatic APIs, validates settings, and maintains the runtime configuration. This centralizes all configuration logic so other components don't need to worry about it.

**Status Reporting**: Applications need to know what's happening with devices. networkClient provides methods to query device status, get health summaries, and retrieve execution statistics. This information is aggregated from multiple components (DeviceRegistry, DeviceManager, TaskQueueManager) and presented in a unified format.

**What networkClient Does NOT Do:**

- **DAG Planning**: Task decomposition is handled by networkAgent
- **DAG Execution**: Coordinating task dependencies is handled by TasknetworkOrchestrator  
- **Session Management**: Multi-round interactions are handled by clusterSession
- **Low-Level Connection Management**: WebSocket lifecycle is handled by DeviceManager

This separation of concerns keeps networkClient focused on device-level operations.
...
When you create a networkClient, it performs these initialization steps:

1. **Load or Create Configuration**: If you provide a `config` parameter, it uses that. Otherwise, it creates a default `networkConfig` object. This config contains device information, heartbeat settings, and other parameters.

2. **Override Task Name**: If you provide `task_name`, it overrides the task name from the configuration. The task name identifies this network instance in logs and messages.

3. **Create DeviceManager**: networkClient creates an internal DeviceManager instance, passing the task name and connection settings (heartbeat interval, reconnect delay). DeviceManager is the component that actually manages connections.
...
config = networkConfig.from_yaml("config/devices.yaml")
...
The task name appears in logs and helps identify which network instance generated which messages, which is useful when running multiple networks simultaneously.
...
After creating a networkClient, you must call `initialize()` before using it. This method processes all devices defined in the configuration:
...
This diagram shows the initialization sequence. For each configured device, networkClient delegates to DeviceManager, which handles the low-level connection setup if auto-connect is enabled.
...
While networkClient doesn't expose a direct `assign_task_to_device()` method in its public API (that's internal to DeviceManager), it's used by higher-level orchestrators like TasknetworkOrchestrator. Understanding how task assignment works helps you understand the system:
...
networkClient focuses on device management, not task orchestration. Task assignment is exposed through higher-level APIs:

- TasknetworkOrchestrator assigns tasks based on DAG dependencies
- clusterSession coordinates multi-round task execution
...
def get_network_info(self) -> Dict[str, Any]:
    """Get overall network status and configuration."""
```

Returns network-level information:
...
This provides a high-level view of the entire network, useful for monitoring overall system health.
...
```python
def validate_config(self, config: Optional[networkConfig] = None) -> Dict[str, Any]:
    """
...
client = networkClient(base_config)
await client.initialize()
...
### Used by clusterClient

clusterClient wraps networkClient for session management:

```python
class clusterClient:
    def __init__(self, ...):
        # Create internal networkClient
        self._client = networkClient(config, task_name)
    ...
    async def process_request(self, request):
        # Use networkClient for device coordination
        # while clusterSession handles task orchestration
        session = clusterSession(client=self._client, ...)
        await session.run()
```

### Used by TasknetworkOrchestrator

TasknetworkOrchestrator uses networkClient's DeviceManager for task assignment:
...
for task in dag.tasks:
    ...
    # Assign through DeviceManager (internal to networkClient)
    result = await network_client.device_manager.assign_task_to_device(
        task_id=task.id,
        device_id=device_id,
...
networkClient is the primary interface for device management in cluster Client. It provides:
...
For most applications, networkClient (or clusterClient which wraps it) is all you need. Only advanced scenarios require working directly with DeviceManager or its components.
...
- See [clusterClient](./cluster_client.md) for session-level API
