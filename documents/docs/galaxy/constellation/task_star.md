...
**TaskStar** represents the atomic unit of computation in Alien cluster—the smallest indivisible task scheduled on a device agent. Each TaskStar encapsulates complete context necessary for autonomous execution, including semantic description, assigned device, execution state, and dependency relationships.
...
from cluster.network.enums import DeviceType, TaskPriority
...
```python
# Add dependency (internal use by Tasknetwork)
...
!!! note "Managed by Tasknetwork"
    Dependency management methods are primarily used internally by `Tasknetwork`. Direct manipulation is not recommended—use `networkEditor` for safe editing with undo/redo support.
...
# Task is now managed by network
...
!!! warning "Common Pitfalls"
    ...
    - **Don't forget validation**: Always validate tasks before adding to network
...
- **[Tasknetwork](task_network.md)** — DAG orchestration and execution
- **[networkEditor](network_editor.md)** — Safe task editing with undo/redo
- **[networkDeviceManager](../client/device_manager.md)** — Device management and task assignment
