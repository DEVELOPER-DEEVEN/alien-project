...
<div align="center">
  <img src="/img/task_network.png" alt="Task network DAG Structure" style="max-width: 90%; height: auto; margin: 20px 0;">
  <p><em>Example of a Task network illustrating both sequential and parallel dependencies</em></p>
</div>
...
| **[Tasknetwork](task_network.md)** | DAG orchestrator | Complete workflow graph with validation, scheduling, and dynamic modification |
| **[networkEditor](network_editor.md)** | Interactive editor | Command pattern-based interface with undo/redo for safe network manipulation |
...
A Task network $\mathcal{C}$ is formally defined as a directed acyclic graph (DAG):
...
### 3. Runtime Dynamism
Unlike static DAG schedulers, Task networks are **mutable objects**. Tasks and dependency edges can be:
...
The Task network progresses through several states during its lifecycle:
...
| State | Description |
|-------|-------------|
| **CREATED** | network initialized, no tasks added |
...
The Task network provides several metrics to analyze workflow parallelism:

#### Critical Path Length ($L$)
The longest serial dependency chain in the network:
...
!!!info "Calculation Modes"
    The network supports two calculation modes:
...
# Create network
network = Tasknetwork(name="my_workflow")
...
### Safe Editing with networkEditor
...
The Task network provides multiple visualization modes for monitoring and debugging:

### Overview Mode
High-level network structure with task counts and state
...
```python
# Display network
network.display_dag(mode="overview")  # or "topology", "details", "execution"
```

For interactive web-based visualization, check out the [cluster WebUI](../webui.md).
...
- **[TaskStar](task_star.md)** — Atomic execution units representing individual tasks in the network
- **[TaskStarLine](task_star_line.md)** — Dependency relationships connecting tasks with conditional logic
- **[Tasknetwork](task_network.md)** — Complete DAG orchestrator managing workflow execution and coordination
- **[networkEditor](network_editor.md)** — Interactive editor with command pattern and undo/redo capabilities

### Related Documentation

- **[network Orchestrator](../network_orchestrator/overview.md)** — Learn how networks are scheduled and executed across devices
- **[network Agent](../network_agent/overview.md)** — Understand how agents plan and manage network lifecycles
- **[Evaluation & Metrics](../evaluation/performance_metrics.md)** — Monitor network performance and analyze execution patterns

---

## 🔬 Research Background

The Task network model is grounded in formal DAG theory and distributed systems research. Key properties include:
...
For more on cluster's architecture and design principles, see the [cluster Overview](../overview.md).
...
- Master **[Tasknetwork](task_network.md)** — DAG orchestration
- Try **[networkEditor](network_editor.md)** — Interactive editing
