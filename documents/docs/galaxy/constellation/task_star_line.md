...
**TaskStarLine** represents a directed dependency relationship between two TaskStars, forming an edge in the task network DAG. Each TaskStarLine defines how tasks depend on each other, with support for conditional logic, success-only execution, and custom condition evaluation.
...
from cluster.network.enums import DependencyType
...
!!! warning "Modification During Execution"
    Changing `dependency_type` or `condition_evaluator` resets the satisfaction status. Be cautious when modifying dependencies during active network execution.
...
```python
# Tasknetwork validates dependencies automatically
try:
    # This would fail if it creates a cycle
    network.add_dependency(cyclic_dep)
except ValueError as e:
...
try:
    ...
    network.add_dependency(invalid_dep)
except ValueError as e:
    print(f"Validation error: {e}")
    # Tasknetwork will detect cycle
```
...
5. **Avoid cycles**: Tasknetwork validates, but design carefully to avoid attempts
...
!!! warning "Common Pitfalls"
    ...
    - **Missing tasks**: Ensure both `from_task_id` and `to_task_id` exist in network
...
- **[Tasknetwork](task_network.md)** — DAG manager that validates and executes dependencies
- **[networkEditor](network_editor.md)** — Safe dependency editing with undo/redo
