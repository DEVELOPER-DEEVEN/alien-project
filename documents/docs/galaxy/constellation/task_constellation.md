# Tasknetwork — DAG Orchestrator

## Overview

**Tasknetwork** is the complete DAG (Directed Acyclic Graph) orchestration system that manages distributed workflows across heterogeneous devices. It provides comprehensive task management, dependency validation, execution scheduling, and runtime dynamism for complex cross-device orchestration.

**Formal Definition:** A Tasknetwork $\mathcal{C}$ is a DAG defined as:
...
| **state** | `networkState` | Current execution state |
| **tasks** | `Dict[str, TaskStar]` | All tasks in the network |
| **dependencies** | `Dict[str, TaskStarLine]` | All dependency relationships |
| **metadata** | `Dict[str, Any]` | Additional network metadata |
...
| **created_at** | `datetime` | network creation timestamp |
...
|-------|-------------|-------------------|
| **CREATED** | Empty network, no tasks added | Initialization |
...
# Output: network_20251106_143052_a1b2c3d4

# Create with custom name
network = Tasknetwork(
    name="ml_training_pipeline",
    network_id="pipeline_001"
)
```
...
print(f"Total tasks: {network.task_count}")
...
print(f"Total dependencies: {network.dependency_count}")
...
network.remove_task("preprocess")

# Remove a dependency
network.remove_dependency(dep1.line_id)

# Get specific task or dependency
task = network.get_task("fetch_data")
dep = network.get_dependency(dep1.line_id)
```
...
```python
# Get tasks by status
running = network.get_running_tasks()
completed = network.get_completed_tasks()
failed = network.get_failed_tasks()
pending = network.get_pending_tasks()
...
# Check if entire network is complete
if network.is_complete():
    network.complete_execution()
    print(f"State: {network.state}")
```
...
longest_path_length, longest_path = network.get_longest_path()
...
max_width = network.get_max_width()
...
metrics = network.get_parallelism_metrics()
...
critical_time, critical_path_tasks = network.get_critical_path_length_with_time()

print(f"Critical path duration: {critical_time:.2f} seconds")
print(f"Tasks on critical path: {critical_path_tasks}")

# Get total work
total_work = network.get_total_work()
...
modifiable_deps = network.get_modifiable_dependencies()
# Dependencies whose target task hasn't started

# Check specific task/dependency
can_modify_task = network.is_task_modifiable("task_a")
can_modify_dep = network.is_dependency_modifiable("dep_1")
```
...
network.add_task(diagnostic_task)
...
network.add_dependency(fallback_dep)
...
!!! warning "Modification Safety"
    The network enforces safe modification:
...
# Save to file
network.to_json(save_path="network_backup.json")

# Load from JSON string
restored = Tasknetwork.from_json(json_data=json_string)

# Load from file
loaded = Tasknetwork.from_json(file_path="network_backup.json")
```
...
new_network = Tasknetwork.from_dict(network_dict)

# Dictionary structure includes:
# - network_id, name, state
...
# Create from schema
network_from_schema = Tasknetwork.from_basemodel(schema)
```
...
network.display_dag(mode="overview")

# Topology mode - detailed DAG graph
network.display_dag(mode="topology")

# Details mode - task execution details
network.display_dag(mode="details")

# Execution mode - real-time flow
network.display_dag(mode="execution")
```
...
task_deps = network.get_task_dependencies("train_model")

for dep in task_deps:
    print(f"{dep.from_task_id} → {dep.to_task_id} ({dep.dependency_type.value})")

# Get all dependencies in network
all_deps = network.get_all_dependencies()
```
...
```python
# Create: A → B → C
network = Tasknetwork(name="linear_pipeline")
...
network.add_dependency(dep_ab)
network.add_dependency(dep_bc)
...
```python
# Create: A → [B, C, D]
network = Tasknetwork(name="fan_out")
...
```python
# Create: A → [B, C] → D
network = Tasknetwork(name="diamond")
...
# Fan-out: A → B, A → C
network.add_dependency(TaskStarLine.create_success_only("a", "b"))
network.add_dependency(TaskStarLine.create_success_only("a", "c"))

# Fan-in: B → D, C → D
network.add_dependency(TaskStarLine.create_success_only("b", "d"))
network.add_dependency(TaskStarLine.create_success_only("c", "d"))

# Analyze
order = network.get_topological_order()
print(f"Possible order: {order}")  # ['a', 'b', 'c', 'd'] or ['a', 'c', 'b', 'd']

longest_path_length, path = network.get_longest_path()
...
try:
    network.remove_task(task.task_id)
except ValueError as e:
    print(f"❌ {e}")
...
!!! warning "Common Pitfalls"
    ...
    - **Ignoring state**: Check network state before modifications
...
The Tasknetwork enforces **acyclicity** through:
...
The network provides **safe concurrent execution**:
...
- **[networkEditor](network_editor.md)** — Safe editing with undo/redo
...
| Method | Description |
|--------|-------------|
| `add_task(task)` | Add task to network |
...
| Method | Description |
|--------|-------------|
| `start_execution()` | Mark network as started |
...
| `complete_execution()` | Mark network as completed |
| `is_complete()` | Check if all tasks are terminal (returns `bool`) |
| `update_state()` | Update network state based on task states |
...
| `get_statistics()` | Get all network statistics (returns `Dict[str, Any]`) |
...
| Method | Description |
|--------|-------------|
| `display_dag(mode)` | Display network (modes: overview, topology, details, execution) |

---

*Tasknetwork — Orchestrating distributed workflows across the digital cluster*
