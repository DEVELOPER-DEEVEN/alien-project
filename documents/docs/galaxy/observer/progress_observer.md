# network Progress Observer

The **networkProgressObserver** is responsible for tracking task execution progress and coordinating between the orchestrator and the agent. It acts as the bridge that enables the agent to react to task completion events and make necessary network modifications.
...
- **network Event Handling** — Notifies the agent when network execution completes
...
| **Agent** | Processes completions, modifies network | ← Agent Queue |
...
Handles network lifecycle events:

| Event Type | Action | Effect |
|------------|--------|--------|
| `network_COMPLETED` | Queue completion event for agent | Wakes up agent's Continue state to process final results |
...
agent = networkAgent(orchestrator=orchestrator)
progress_observer = networkProgressObserver(agent=agent)
...
|-----------|------|-------------|
| `agent` | `networkAgent` | The agent that will process queued events |
...
async def _handle_network_event(self, event: networkEvent) -> None:
    ...
    try:
        ...
        self.logger.error(
            f"Error handling network event: {e}", 
            exc_info=True
        )
```
...
- `agent` — `networkAgent` instance that will process queued events
...
Handle network-related events (TaskEvent or networkEvent).

**Parameters:**

- `event` — Event instance (TaskEvent or networkEvent)

**Behavior:**

- Filters events by type (TaskEvent vs networkEvent)
...
async def setup_progress_tracking():
    """Set up progress tracking for network execution."""
...
```python
async def monitor_task_progress(observer: networkProgressObserver):
...
```python
from cluster.core.events import IEventObserver, TaskEvent, EventType
...
The Progress Observer integrates tightly with the networkAgent's state machine:
...
# Queue network completion event
await self.agent.add_network_completion_event(network_event)
```
...
stateDiagram-v2
    ...
    ProcessEvent --> Updatenetwork: Event is TASK_COMPLETED
    ProcessEvent --> HandleFailure: Event is TASK_FAILED
    Updatenetwork --> Continue: More tasks pending
    Updatenetwork --> Finish: All tasks done
...
| **Updatenetwork** | Modify network based on result | Task completed successfully |
| **HandleFailure** | Handle task failure, retry if needed | Task failed |
| **Finish** | Complete network execution | All tasks done or unrecoverable error |
...
- **Clear results** after network completion to free memory
...
Clear task results after network execution:

```python
async def execute_with_cleanup(orchestrator, network, progress_observer):
    """Execute network and clean up observer."""
...
- **[network Agent](../network_agent/overview.md)** — Agent state machine and event processing
- **[network Modification Synchronizer](synchronizer.md)** — Coordination between agent and orchestrator

## Summary

The network Progress Observer:
...
- **Enables** event-driven network modification

This observer is essential for the agent-orchestrator coordination pattern in cluster, replacing complex callback mechanisms with a clean event-driven interface.
