...
**Location:** `cluster/session/observers/agent_output_observer.py`
...
- **Debugging** — Understand agent behavior during network execution
...
Triggered when agent executes actions (network editing):
...
```python
from cluster.session.observers import AgentOutputObserver
...
When the agent performs network editing:

```
╭─────────────────────────────────────────────────────────────╮
│ 🛠️ Agent Actions: network Editing                     │
├─────────────────────────────────────────────────────────────┤
│ Action 1: add_task                                          │
...
sequenceDiagram
    ...
    AOO->>AOO: Reconstruct networkAgentResponse
    AOO->>P: present_network_agent_response()
    ...
    AOO->>P: present_network_editing_actions()
...
async def execute_with_agent_feedback(show_agent_output: bool = True):
    """Execute network with optional agent output display."""
...
The observer reconstructs `networkAgentResponse` from event data:
...
async def _handle_agent_response(self, event: AgentEvent) -> None:
    ...
    try:
        ...
        if event.agent_type == "network":
            # Reconstruct networkAgentResponse from output data
            response = networkAgentResponse.model_validate(output_data)
            print_action = output_data.get("print_action", False)
            
            # Use presenter to display the response
            self.presenter.present_network_agent_response(
                response, 
...
async def _handle_agent_action(self, event: AgentEvent) -> None:
    ...
    try:
        ...
        if output_data.get("action_type") == "network_editing":
            ...
            self.presenter.present_network_editing_actions(actions)
...
The observer integrates with the networkAgent's state machine:
...
class networkAgent:
    ...
    async def execute_actions(self, actions):
        ...
        result = await self._execute_network_editing(actions)
...
- **[network Agent](../network_agent/overview.md)** — Agent implementation and state machine
...
This observer is essential for understanding agent behavior during network execution, providing visibility into the AI's thought process and actions.
