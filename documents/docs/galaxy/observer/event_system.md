...
The Event System Core provides the foundational infrastructure for event-driven communication in the cluster framework. It implements the Observer pattern through a central event bus, type-safe event classes, and well-defined interfaces.

**Location:** `cluster/core/events.py`
...
| `networkEvent` | `Event` | `network_id`, `network_state`, `new_ready_tasks` | network lifecycle events |
...
from cluster.core.events import Event
...
class MyCustomObserver(IEventObserver):
    ...
    async def _handle_network_event(self, event: networkEvent) -> None:
        ...
        if event.event_type == EventType.network_STARTED:
            print(f"network {event.network_id} started")
```
...
event_bus.subscribe(network_observer, {
...
### Pattern 2: network Modification Flow
...
```python
from cluster.core.events import (
    get_event_bus, TaskEvent, networkEvent, 
...
from cluster.core.events import (
    TaskEvent, networkEvent, EventType, IEventObserver
)
...
class CustomMetricsCollector(IEventObserver):
    ...
    def __init__(self):
        ...
        self.network_tasks: Dict[str, List[str]] = {}
    ...
    async def _handle_network_event(self, event: networkEvent) -> None:
        if event.event_type == EventType.network_STARTED:
            const_id = event.network_id
            self.network_tasks[const_id] = []
...
!!! note "Additional Observer Documentation"
    For documentation on `networkProgressObserver`, `DAGVisualizationObserver`, `networkModificationSynchronizer`, and `AgentOutputObserver`, refer to their implementation in `cluster/session/observers/`.
...
This foundation enables the cluster framework to implement a loosely coupled, extensible event-driven architecture.
