...
This document provides comprehensive API documentation for the network Orchestrator system. The API is organized into three main components:

- **TasknetworkOrchestrator** - Main orchestration engine
- **networkManager** - Device assignment and resource management  
- **networkModificationSynchronizer** - Safe concurrent editing

## TasknetworkOrchestrator

The main orchestration engine that coordinates task execution across devices.

**Module**: `cluster.network.orchestrator.orchestrator`
...
|-----------|------|-------------|---------|
| `device_manager` | `networkDeviceManager` or `None` | Device manager for communication | `None` |
...
Main entry point for orchestrating a network's execution.
...
|-----------|------|-------------|----------|
| `network` | `Tasknetwork` | The network to orchestrate | Yes |
...
results = await orchestrator.orchestrate_network(
    network=my_network,
...
Execute a single task independently (without network context).
...
Get detailed status of a network during execution.
...
| Parameter | Type | Description | Required |
|-----------|------|-------------|----------|
| `network` | `Tasknetwork` | network to query | Yes |

**Returns**: Status dictionary from networkManager

**Note**: This method delegates to `networkManager.get_network_status()` using the network's ID.
...
| Parameter | Type | Description | Required |
|-----------|------|-------------|----------|
| `device_manager` | `networkDeviceManager` | Device manager instance | Yes |

**Example**:
```python
new_device_manager = networkDeviceManager()
...
| Parameter | Type | Description | Required |
|-----------|------|-------------|----------|
| `synchronizer` | `networkModificationSynchronizer` | Synchronizer instance | Yes |

**Example**:
```python
from cluster.session.observers.network_sync_observer import (
...
Manages device assignments, resource allocation, and network lifecycle.

**Module**: `cluster.network.orchestrator.network_manager`
...
| Parameter | Type | Description | Default |
|-----------|------|-------------|---------|
| `device_manager` | `networkDeviceManager` or `None` | Device manager instance | `None` |
...
| Parameter | Type | Description | Default |
|-----------|------|-------------|---------|
| `network` | `Tasknetwork` | network to assign | Required |
...
For more details on device assignment strategies, see [network Manager](network_manager.md).
...
| Parameter | Type | Description | Required |
|-----------|------|-------------|----------|
| `network` | `Tasknetwork` | network containing task | Yes |
...
Register a network for management tracking.
...
Unregister and clean up a network.

```python
def unregister_network(
    self, 
    network_id: str
) -> bool
```
...
**Returns**: List of network info dictionaries
...
Synchronizes network modifications with orchestrator execution to prevent race conditions.

**Module**: `cluster.session.observers.network_sync_observer`
...
| Parameter | Type | Description | Required |
|-----------|------|-------------|----------|
| `orchestrator` | `TasknetworkOrchestrator` | Orchestrator instance | Yes |
...
- `network_MODIFIED`: Complete pending modifications
...
| Parameter | Type | Description | Required |
|-----------|------|-------------|----------|
| `orchestrator_network` | `Tasknetwork` | Orchestrator's network | Yes |

**Returns**: Merged network with consistent state

**Example**:
```python
merged = synchronizer.merge_and_sync_network_states(
    orchestrator_network=current_network
)
```
...
Get the network currently being modified.
...
# Setup
device_manager = networkDeviceManager()
orchestrator = TasknetworkOrchestrator(device_manager)

# Create network
network = Tasknetwork(name="MyWorkflow")
...
results = await orchestrator.orchestrate_network(network)
```
...
for task in network.get_all_tasks():
...
See [Tasknetwork documentation](../network/task_network.md)

### TaskStar

See [TaskStar documentation](../network/task_star.md)
...
EventType.network_STARTED # Orchestration begins
EventType.network_COMPLETED  # All tasks finished
EventType.network_FAILED     # Orchestration failed
EventType.network_MODIFIED   # DAG structure updated
```
...
- **[network Manager](network_manager.md)** - Resource management
