...
At the core of the network Orchestrator lies a fully **asynchronous scheduling loop** that maximizes parallelism across heterogeneous devices. Unlike traditional schedulers that alternate between discrete planning and execution phases, the orchestrator continuously monitors the evolving DAG to identify ready tasks and dispatches them concurrently.

Most critically, **task execution and network editing can proceed concurrently**, allowing the system to adapt in real-time as results stream in while computation continues uninterrupted.

For more on the DAG structure being scheduled, see the [Tasknetwork documentation](../network/task_network.md).

![Asynchronous Timeline](../../img/async_timeline.png)

*Illustration of asynchronous scheduling and concurrent network editing. Task execution overlaps with DAG modifications, reducing end-to-end latency.*

## Core Scheduling Loop

The orchestration workflow is driven by a continuous asynchronous loop that coordinates task execution, network synchronization, and event handling:
...
async def _run_execution_loop(self, network: Tasknetwork) -> None:
    ...
    while not network.is_complete():
        # 1. Wait for pending modifications and refresh network
...
- Checks for network modifications from the agent
...
The network can be modified during execution:
...
The orchestrator treats the Tasknetwork as a **living data structure** that evolves during execution, not a static plan fixed at the start.
...
The `Tasknetwork` determines readiness by checking:
...
2. **Event publishing**: Trigger network modifications as soon as possible
...
## Concurrent network Editing

### The Challenge

Traditional schedulers treat DAG structure as **immutable** during execution. But in Alien, the LLM-based network Agent can modify the DAG based on task results:
...
This creates a **race condition**: tasks may be executing while the agent modifies the network.

### The Solution: Overlapping Execution and Editing

The orchestrator allows task execution and network editing to **proceed concurrently**:
...
- **Task C** starts at t=135 based on updated network
...
The orchestrator synchronizes network state at the start of each scheduling iteration:
...
async def _sync_network_modifications(
    ...
    if self._modification_synchronizer:
        ...
        network = self._modification_synchronizer \
...
The `merge_and_sync_network_states` method ensures:

- Agent's network has latest structural modifications
...
[Learn more about synchronization →](safe_assignment_locking.md#network-state-merging)
...
```python
# Agent may modify multiple tasks before publishing network_MODIFIED
...
sequenceDiagram
    ...
    O->>O: sync_network_modifications()
    
    par Agent editing
        A->>A: Modify network
        A-->>O: Publish network_MODIFIED
    ...
    Note over O: network complete
```

**Key observations:**

1. **Iteration 1**: Tasks A and B scheduled concurrently
2. **Concurrent editing**: Agent modifies network while Task B executes
...
2. Marks task as failed in network
...
| **Sync overhead** | 10-50ms | Per network modification |
