...
While asynchronous execution maximizes efficiency, it introduces correctness challenges when task execution overlaps with DAG updates. The orchestrator must prevent race conditions where the network Agent dynamically adds, removes, or rewires tasks during execution.
...
To ensure atomicity, the orchestrator employs a safe assignment lock protocol combined with network state synchronization.
...
3. **Inconsistent state**: network doesn't reflect actual execution
...
while not network.is_complete():
    ready_tasks = network.get_ready_tasks()  # ← May see stale state
    await schedule_ready_tasks(ready_tasks)        # ← Dispatch based on stale view
    await wait_for_task_completion()
```

Meanwhile, the agent modifies the same network object concurrently.
...
2. **End**: `network_MODIFIED` event published
...
stateDiagram-v2
    ...
    Editing_In_Progress --> Edit_Cycle_Complete: network_MODIFIED
...
The complete protocol ensures atomic network updates:

```
Algorithm: Safe Assignment Locking and Asynchronous Rescheduling Protocol

Input: Event stream E, current Tasknetwork C
...
while system is running do
    ...
    while queue not empty do
        e ← dequeue()
        Δ ← invoke(networkAgent, edit(C, e))  // Propose DAG edits
        C ← apply(C, Δ)                             // Update structure
        validate(C)                                  // Ensure invariants I1-I3
        publish(network_MODIFIED, t)
...
- **Validation**: network consistency checked before releasing
...
The `networkModificationSynchronizer` component implements the locking protocol by coordinating between the orchestrator and agent.
...
Each future represents an edit cycle that will be completed when `network_MODIFIED` is received.

### Completing Modifications

When the agent publishes `network_MODIFIED`, the synchronizer completes the future:

```python
async def _handle_network_event(self, event: networkEvent) -> None:
    """Handle network modification events."""
    
    if event.event_type != EventType.network_MODIFIED:
        return
...
To prevent deadlocks if the agent fails to publish `network_MODIFIED`:
...
## network State Merging

After modifications complete, the synchronizer must merge two potentially conflicting views:

1. **Agent's network**: Has latest structural changes (new tasks, modified dependencies)
2. **Orchestrator's network**: Has latest execution state (task statuses, results)
...
t1: Agent modifies network (Task A still RUNNING in agent's copy)
t2: Task B completes (orchestrator marks as COMPLETED)
t3: Agent publishes network_MODIFIED
...
def merge_and_sync_network_states(
    self, orchestrator_network: Tasknetwork
) -> Tasknetwork:
    """Merge network states: structural changes + execution state."""
...
**2. Consistency**: network always satisfies invariants after edits. Validation performed before releasing. See [consistency guarantees](consistency_guarantees.md) for details.
...
The lock does not block task execution - while the lock is held for network modification, already-dispatched tasks continue executing concurrently.
