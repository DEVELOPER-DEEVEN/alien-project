...
Since the Tasknetwork may be dynamically rewritten by an LLM-based agent, the orchestrator must enforce runtime invariants to preserve correctness even under partial or invalid updates. Without these guarantees, the system could execute invalid DAGs, violate dependencies, or enter inconsistent states.

The network Orchestrator enforces three critical invariants (I1-I3) that together ensure safety, consistency, and semantic validity throughout execution.
...
**Invariant**: Edits must preserve DAG acyclicity - the network remains a valid directed acyclic graph after all modifications.
...
**Rationale**: Modifying tasks that have started or finished execution could invalidate already-collected results, create inconsistencies between device state and network state, or violate causal dependencies.
...
async def _run_execution_loop(self, network: Tasknetwork) -> None:
    ...
    while not network.is_complete():
        ...
        # I2 checked implicitly by Tasknetwork.add_dependency()
...
After the agent modifies the network, validation occurs before releasing the lock:

```python
# In safe assignment lock algorithm
while queue not empty:
    e ← dequeue()
    Δ ← invoke(networkAgent, edit(C, e))
    C ← apply(C, Δ)
...
Concurrent task execution and network editing create multiple consistency challenges:
...
4. **Consistency**: Orchestrator only sees valid network states

During the lock period (modification + validation + sync), the orchestrator has a strongly consistent view of the network. Learn more about [safe assignment locking](safe_assignment_locking.md).

## Formal Invariant Definitions

### Mathematical Formulation

Let C = (T, D) be a network with tasks T and dependencies D, where:
...
**Response**: Reject dependency addition, log error, network remains acyclic
...
try:
    ...
    # Continue with existing network structure
    # Don't block orchestration on agent errors
```

The orchestrator continues execution with the last valid network state.
...
async def test_invariants_during_orchestration():
    ...
    # Create network with potential for violations
    ...
    for task in network.get_completed_tasks() + network.get_failed_tasks():
        with pytest.raises(ValueError):
