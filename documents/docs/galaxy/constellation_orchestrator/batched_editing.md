...
To balance **responsiveness** with **efficiency**, the orchestrator supports **batched network editing**: during a reasoning round, multiple task completion events are aggregated and their resulting modifications applied atomically in a single cycle.
...
while system is running:
    ...
    publish(network_MODIFIED, all_task_ids)
    C ← synchronize(C, T_C)
...
The `networkModificationSynchronizer` batches pending modifications:
...
The network Agent receives multiple task IDs and processes them together:
...
async def process_editing(
    ...
    """Process task completion events and update network."""
    ...
    after_network = await self._create_and_process(context)
    
    # Publish single network_MODIFIED event for entire batch
...
t=350ms: Agent completes editing
    → Publishes network_MODIFIED with on_task_id = ['A', 'B', 'C']
...
foreach event in batch:
    Δ ← invoke(networkAgent, edit(C, event))
    C ← apply(C, Δ)
end

validate(C)  # ← Validates combined result
publish(network_MODIFIED, batch_task_ids)
...
Batching is a pure **performance optimization** - it doesn't change the semantics of network evolution.
...
Design networks with tasks that complete around the same time:
...
**Alien's approach**: Dynamic batching with no artificial delays
...
**Alien's approach**: Event-driven batching without fixed windows
...
**Alien's approach**: Automatic batching when beneficial
...
| **Dynamic (Alien)** | Low-Medium | High (best) | Low |
...
!!!tip "Next Steps"
    To understand device assignment and resource management, continue to [network Manager](network_manager.md).
