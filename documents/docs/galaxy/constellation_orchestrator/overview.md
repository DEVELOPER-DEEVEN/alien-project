# network Orchestrator Overview

## Introduction

The **network Orchestrator** is the execution engine at the heart of Alien's multi-device orchestration system. While the network Agent handles reasoning and task graph evolution, the orchestrator transforms these declarative plans into concrete execution across heterogeneous devices.

Unlike traditional DAG schedulers that execute static task graphs, the network Orchestrator operates as a **living execution fabric** where tasks evolve concurrently, react to runtime signals, and adapt to new decisions from the reasoning agent in real-time.

![Orchestrator Architecture](../../img/orchestrator.png)

*The network Orchestrator bridges Tasknetwork and execution, enabling asynchronous, adaptive task orchestration across devices.*
...
The network Orchestrator is built on five fundamental design pillars:
...
- `network_MODIFIED` - DAG structure updated by agent
...
A continuous scheduling loop monitors the evolving Tasknetwork, identifies ready tasks (dependencies satisfied), and dispatches them concurrently to available devices. Critically, **task execution and network editing proceed in parallel**, overlapping computation with orchestration.
...
These invariants ensure **structural integrity** and **semantic validity** of the network.

[Learn more →](consistency_guarantees.md)

### 5. Batched network Editing
...
Handles device assignment, resource management, and network lifecycle. It provides:

- Multiple assignment strategies (round-robin, capability-match, load-balance)
- Device validation and status tracking
- network registration and metadata management

[Learn more →](network_manager.md)
...
- **[Batched Editing](batched_editing.md)** - Efficient network updates
- **[network Manager](network_manager.md)** - Device and resource management
- **[API Reference](api_reference.md)** - Complete API documentation

## Further Reading

- [Tasknetwork Documentation](../network/overview.md) - Understand the DAG structure
- [network Agent](../network_agent/overview.md) - LLM-based reasoning
