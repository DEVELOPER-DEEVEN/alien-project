...
The **AgentProfile** is a multi-source data structure that consolidates administrator configuration, service-level capabilities, and real-time client telemetry into a unified, dynamically updated representation of each network agent.

---

## 📋 Overview

The **AgentProfile** is the primary data structure representing a registered network agent. It aggregates information from **three distinct sources** to provide a comprehensive view of each agent's identity, capabilities, operational status, and hardware characteristics.

For a complete understanding of how agents work in the network system, see:

- [network Overview](../network/overview.md) - Architecture and multi-device coordination
- [network Agent](../network_agent/overview.md) - Agent behavior and lifecycle
...
The `device_id` must be unique across the entire network. Attempting to register a duplicate `device_id` will fail.
...
```python
from cluster.client.components import DeviceRegistry, AgentProfile, DeviceStatus
...
The following patterns demonstrate how AgentProfile is used for intelligent task routing and device management. For more details on task network concepts, see [network Overview](../network/overview.md).
...
2. **Configure Devices**: See [cluster Devices Configuration](../../configuration/system/cluster_devices.md)
...
- **AgentProfile Definition**: `cluster/client/components/types.py`
- **DeviceRegistry**: `cluster/client/components/device_registry.py`
- **networkDeviceManager**: `cluster/client/device_manager.py`
- **DeviceInfoProvider**: `Alien/client/device_info_provider.py`
