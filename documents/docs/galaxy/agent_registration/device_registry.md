...
The **DeviceRegistry** is a focused component that manages device registration and information storage, providing a clean separation of concerns in the network architecture. It is responsible for **device data management only** - storing, retrieving, and updating AgentProfile instances without handling networking, task execution, or protocol logic.
...
def register_device(
    ...
    :param server_url: Alien WebSocket server URL
...
```python
from cluster.client.components import DeviceRegistry, DeviceStatus
...
DeviceRegistry is used internally by other components in the network system. See [Components Overview](../client/components.md) for details on the component architecture.

### With networkDeviceManager

```python
# networkDeviceManager uses DeviceRegistry internally
...
2. **Learn Configuration**: See [cluster Devices Configuration](../../configuration/system/cluster_devices.md)
3. **Study Registration**: Check [Registration Flow](./registration_flow.md)
4. **Explore Components**: See networkDeviceManager implementation
