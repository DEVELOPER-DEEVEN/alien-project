...
The registration flow transforms a device configuration entry into a fully profiled, connected, and task-ready network agent through a coordinated **5-phase process**:
...
Establish a persistent WebSocket connection to the device agent's Alien server. This connection is managed by the `WebSocketConnectionManager` component.
...
- Identify client type (DEVICE vs network)
...
sequenceDiagram
    ...
    participant Server as Alien Server Handler
...
```python
# In AlienWebSocketHandler.connect()
...
async def connect(self, websocket: WebSocket) -> str:
    ...
    client_type = reg_info.client_type  # DEVICE or network
...
For network clients (not device agents), the registration differs:

```python
# network client registration
ClientMessage(
    type=ClientMessageType.REGISTER,
    client_id="network_orchestrator",
    client_type=ClientType.network,
    target_id="windowsagent",  # Target device for this network
    status=TaskStatus.OK,
    timestamp="2025-11-06T10:30:00.000Z",
    metadata={
        "type": "network_client",
...
> **Note:** Device clients register as `ClientType.DEVICE`, while network orchestrators register as `ClientType.network` with a `target_id` pointing to the device they want to control.

### Output

- Client registered in server's `ClientConnectionManager`
- Client type (DEVICE/network) recorded
...
| **cluster Devices Config** | [cluster Devices Configuration](../../configuration/system/cluster_devices.md) | YAML config reference |
...
1. **Configure Devices**: Read [cluster Devices Configuration](../../configuration/system/cluster_devices.md)
2. **Understand DeviceRegistry**: Check [Device Registry](./device_registry.md)
3. **Learn Task Assignment**: See [Task Execution Documentation](../network_orchestrator/overview.md)
...
- **AlienWebSocketHandler**: `Alien/server/ws/handler.py`
