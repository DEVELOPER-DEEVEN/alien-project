...
**Agent Registration** is the cornerstone of the AIP (Agent Interaction Protocol) initialization process. It enables dynamic discovery, capability advertisement, and intelligent task allocation across distributed network agents.
...
*An overview of the network Agent architecture showing the registration and profiling system.*

At the core of AIP's initialization process is the **networkClient** (implemented as `networkDeviceManager`), which maintains a global registry of active agents. Any device agent service that exposes a WebSocket endpoint and implements the AIP task dispatch and result-return protocol can be seamlessly integrated into Alien, providing remarkable **extensibility**.
...
For a complete understanding of the network system, see:

- [network Overview](../network/overview.md) - Multi-device coordination architecture
- [network Agent Overview](../network_agent/overview.md) - Agent behavior and patterns
...
The agent registry is a centralized store that tracks all active network agents. Each registered agent is represented by an **AgentProfile** that consolidates comprehensive information about the agent's capabilities, system resources, and operational status.

| Component | Responsibility | Location |
|-----------|---------------|----------|
| **networkDeviceManager** | Central coordinator for device management | `cluster/client/device_manager.py` |
| **DeviceRegistry** | Device registration and information storage | `cluster/client/components/device_registry.py` |
| **AgentProfile** | Multi-source agent metadata representation | `cluster/client/components/types.py` |
| **ClientConnectionManager** | Server-side client connection tracking | `Alien/server/services/client_connection_manager.py` |
...
|--------|----------|------------------|------------------|
| **1. User Configuration** | Administrator (devices.yaml + network.yaml) | Endpoint identity, user preferences, capabilities | Static (config load) |
...
**Note:** While network.yaml contains runtime settings like heartbeat intervals, the device-specific configuration is in devices.yaml.
...
sequenceDiagram
    ...
    participant CDM as networkDeviceManager
    participant Server as Alien Server
...
|-------|-------------|---------------------|--------|
| **1. User Configuration** | Administrator registers device with endpoint and capabilities | networkDeviceManager, DeviceRegistry | AgentProfile created with user-specified data |
...
| **3. Service Registration** | AIP registration protocol exchange with capability advertisement | RegistrationProtocol, AlienWebSocketHandler | Client type and platform recorded |
...
| **5. Activation** | Set device to IDLE state, ready for task assignment | DeviceRegistry | Agent ready for network tasks |
...
The **AgentProfile** is the primary data structure representing a registered network agent. For detailed information about the AgentProfile and its lifecycle operations, see [Agent Profile Documentation](./agent_profile.md).

### Core Fields

The **AgentProfile** is the primary data structure representing a registered network agent:
...
*Lifecycle state transitions of the network Agent.*

The agent lifecycle is managed through a state machine that tracks connection, registration, and task execution states. For more details on agent behavior and state management, see [network Agent State Management](../network_agent/state.md).
...
### 1. networkDeviceManager

**File:** `cluster/client/device_manager.py`

The central coordinator for all device management operations in the network system.
...
**File:** `cluster/client/components/device_registry.py`
...
Handles AIP registration message exchange for both device and network clients.
...
**File:** `Alien/client/device_info_provider.py`
...
**File:** `Alien/server/services/client_connection_manager.py`
...
- Track connected clients (devices and networks)
...
**1. `config/cluster/devices.yaml`** - Device definitions:
- Device endpoints and identities
- User-specified capabilities and metadata
- Connection parameters (max retries, auto-connect)

**2. `config/cluster/network.yaml`** - Runtime settings:
- network identification and logging
- Heartbeat interval and reconnection delay
- Task concurrency and step limits

See [cluster Devices Configuration](../../configuration/system/cluster_devices.md) and [cluster network Configuration](../../configuration/system/cluster_network.md) for details.

**Example Device Configuration (devices.yaml):**

```yaml
# Device Configuration - YAML Format
# Runtime settings are configured in network.yaml
...
👉 **[cluster Devices Configuration Guide](../../configuration/system/cluster_devices.md)**
...
- [network Task Distribution](../network/overview.md) - Task routing strategies
...
| **cluster Devices Config** | [cluster Devices Configuration](../../configuration/system/cluster_devices.md) | YAML configuration reference |
| **Device Registry** | [Device Registry](./device_registry.md) | Registry component details |
| **network System** | [network Overview](../network/overview.md) | Multi-device coordination |
...
- **network Agent Overview**: `documents/docs/img/network_agent.png`
...
3. **Configure Your Devices**: See [cluster Devices Configuration](../../configuration/system/cluster_devices.md)
...
- **Source Code**: `cluster/client/device_manager.py`
- **AIP Protocol**: `aip/protocol/registration.py`
- **Device Info**: `Alien/client/device_info_provider.py`
- **Configuration**: `config/cluster/devices.yaml`
