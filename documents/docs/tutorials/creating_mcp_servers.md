...
This tutorial teaches you how to create, register, and deploy custom MCP servers for ALIEN² agents. You'll learn to build **local**, **HTTP**, and **stdio** MCP servers, and how to register them with different agents.
...
ALIEN² supports three deployment models:
...
Local servers run **in-process** with the ALIEN² agent, providing the fastest tool access.

### Step 1: Create Your Server

Create a Python file in `alien/client/mcp/local_servers/` (or your custom location):

```python
# File: alien/client/mcp/local_servers/my_custom_server.py
...
Add your server to `alien/client/mcp/local_servers/__init__.py`:

```python
# File: alien/client/mcp/local_servers/__init__.py
...
Add your server to the appropriate agent in `config/alien/mcp.yaml`:
...
from alien.client.mcp.local_servers.my_custom_server import create_my_custom_server
...
```python
# File: alien/client/mcp/local_servers/chrome_executor.py

from typing import Annotated, Optional
from fastmcp import FastMCP
from pydantic import Field
from alien.client.mcp.mcp_registry import MCPRegistry
from alien.automator.puppeteer import AppPuppeteer
from alien.automator.action_execution import ActionExecutor
...
```python
# File: alien/client/mcp/http_servers/my_http_server.py
...
```bash
# Start on localhost
python -m alien.client.mcp.http_servers.my_http_server --host localhost --port 8020

# Start on all interfaces (for remote access)
python -m alien.client.mcp.http_servers.my_http_server --host 0.0.0.0 --port 8020
```
...
**Linux/macOS:**
```bash
nohup python -m alien.client.mcp.http_servers.my_http_server --host 0.0.0.0 --port 8020 &
```

**Windows:**
```powershell
Start-Process python -ArgumentList "-m", "alien.client.mcp.http_servers.my_http_server", "--host", "0.0.0.0", "--port", "8020" -WindowStyle Hidden
```
...
```python
# File: alien/client/mcp/http_servers/linux_executor.py
...
python -m alien.client.mcp.http_servers.linux_executor --host 0.0.0.0 --port 8010
```

**Configure on Windows ALIEN²:**
...
**Cross-Platform Workflow**: Now your Windows ALIEN² agent can execute Linux commands remotely! The LLM will select `execute_command` or `get_system_info` as needed.
...
Set environment variables before running ALIEN²:
...
logger = logging.getLogger("alien.client.mcp")
```
