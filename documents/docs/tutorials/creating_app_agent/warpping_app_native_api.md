...
Alien² uses **MCP (Model Context Protocol) servers** to expose application native APIs to the AppAgent. This document shows you how to create custom MCP action servers that wrap your application's COM APIs, REST APIs, or other programmable interfaces.
...
3. **Review Examples**: Study existing servers in `Alien/client/mcp/local_servers/`

## Step-by-Step Guide

### Step 1: Create Your MCP Server File

Create a new Python file in `Alien/client/mcp/local_servers/` for your application's MCP server:

```python
# File: Alien/client/mcp/local_servers/your_app_executor.py
...
The receiver class executes the actual COM API calls. Create it in `Alien/automator/app_apis/`:

```python
# File: Alien/automator/app_apis/your_app/your_app_client.py

import win32com.client
from typing import Dict, Any, List, Optional
from Alien.automator.app_apis.basic import WinCOMReceiverBasic
from Alien.automator.basic import CommandBasic
...
```python
# In the same file: Alien/automator/app_apis/your_app/your_app_client.py
...
Add your receiver to the COM receiver factory in `Alien/automator/app_apis/factory.py`:
...
Configure the MCP server for your application in `config/Alien/mcp.yaml`:
...
from Alien.client.mcp.local_servers.your_app_executor import create_your_app_executor
...
See the complete implementation in Alien²'s codebase:

- **MCP Server**: `Alien/client/mcp/local_servers/excel_wincom_mcp_server.py`
- **COM Receiver**: `Alien/automator/app_apis/excel/excel_client.py`
- **Configuration**: `config/Alien/mcp.yaml` (under `AppAgent.EXCEL.EXE`)
...
**Solution**:
- Run Alien² with administrator privileges
...
- [Hybrid GUI–API Actions](../../Alien-Unis/core_features/hybrid_actions.md) - How AppAgent chooses tools
- [Knowledge Substrate Overview](../../Alien-Unis/core_features/knowledge_substrate/overview.md) - Understanding the RAG architecture
