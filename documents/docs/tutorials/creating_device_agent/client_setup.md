...
This tutorial teaches you how to set up the **ALIEN device client** that runs on the target device, manages MCP servers, and communicates with the agent server via WebSocket. We'll use the existing client implementation as reference.
...
3. [ALIEN Client Implementation](#alien-client-implementation)
...
graph TB
    ...
    subgraph "Device Client (Your Implementation)"
        ...
        ALIENClient[ALIENClient<br/>Core Logic]
        ...
        ALIENClient --> MCPMgr
        ALIENClient --> CompMgr
        ALIENClient --> CmdRouter
        WSClient --> ALIENClient
    end
...
| **ALIEN Client** | Execute commands, route actions | `ALIENClient.execute_actions()` |
| **WebSocket Client** | Bidirectional communication | `ALIENWebSocketClient.handle_messages()` |
...
## ALIEN Client Implementation

### File Location

**Path**: `alien/client/alien_client.py`

### Core ALIEN Client Class

```python
# alien/client/alien_client.py

import asyncio
import logging
from typing import List, Optional

from alien.client.computer import CommandRouter, ComputerManager
...
class ALIENClient:
    """
    Client for interacting with the ALIEN web service.
...
**Path**: `alien/client/websocket.py`

### WebSocket Client Implementation

```python
# alien/client/websocket.py (simplified)
...
class ALIENWebSocketClient:
    """
    WebSocket client for ALIEN device agents.
    ...
    async def register_client(self):
        ...
        """
        from alien.client.device_info_provider import DeviceInfoProvider

        # Collect device system information
        system_info = DeviceInfoProvider.collect_system_info(
            self.alien_client.client_id,
...
```mermaid
sequenceDiagram
    participant Client as ALIENWebSocketClient
    participant Server as Agent Server
    participant ALIEN as ALIENClient
    ...
    Client->>ALIEN: execute_step(message)
    ALIEN->>MCP: execute(commands)
    MCP->>ALIEN: results
    ALIEN->>Client: results
...
```python
# alien/client/mcp/mcp_server_manager.py (simplified)
...
```python
# alien/client/client.py (platform detection)
...
**File**: `alien/client/client.py`

```python
#!/usr/bin/env python
# alien/client/client.py
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description="ALIEN Device Client")
...
async def main():
    ...
    logger.info(f"ALIEN Client initialized: {args.client_id}")
...
```python
# tests/unit/test_alien_client.py
...
class TestALIENClient:
    """Unit tests for ALIEN Client."""
...
import pytest
import asyncio
from alien.client.client import main
...
```bash
# 1. Start MCP server
python -m alien.client.mcp.http_servers.mobile_mcp_server --port 8020
...
|-----------|---------|-------------|
| **ALIENClient** | Execute commands | `execute_step()`, `execute_actions()` |
| **ALIENWebSocketClient** | Server communication | `connect_and_listen()`, `handle_messages()` |
