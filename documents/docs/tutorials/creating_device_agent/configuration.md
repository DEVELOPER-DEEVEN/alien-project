...
This tutorial covers the **configuration files and deployment procedures** needed to integrate your device agent into Alien³. You'll learn to configure `third_party.yaml`, register devices in `devices.yaml`, create prompt templates, and deploy the complete system.
...
6. [cluster Multi-Device Integration](#cluster-multi-device-integration)
...
```mermaid
graph TB
    subgraph "Alien Configuration"
        AlienConfig[config/Alien/<br/>Alien Framework Config]
        ThirdParty[third_party.yaml<br/>Agent Registration]
        
        AlienConfig --> ThirdParty
    end
    
    subgraph "cluster Configuration"
        clusterConfig[config/cluster/<br/>Multi-Device Config]
        Devices[devices.yaml<br/>Device Registry]
        network[network.yaml<br/>Orchestration]
    ...
    subgraph "Prompt Templates"
        MainPrompt[Alien/prompts/third_party/<br/>agent_name.yaml]
        ExamplePrompt[Alien/prompts/third_party/<br/>agent_name_example.yaml]
    end
...
| `config/cluster/devices.yaml` | Register device instances | ✅ Yes (for cluster) |
| `config/cluster/network.yaml` | Multi-device orchestration | Optional |
...
**Path**: `config/Alien/third_party.yaml`

### Configuration Structure

```yaml
# Third-Party Agent Integration Configuration
# This file configures external/third-party agents that extend Alien's capabilities
...
THIRD_PARTY_AGENT_CONFIG:
  ...
  MobileAgent:
    ...
    APPAGENT_PROMPT: "Alien/prompts/third_party/mobile_agent.yaml"
    APPAGENT_EXAMPLE_PROMPT: "Alien/prompts/third_party/mobile_agent_example.yaml"
    
    # Optional: API prompt template (for custom tool descriptions)
    # API_PROMPT: "Alien/prompts/third_party/mobile_agent_api.yaml"
...
!!! warning "Configuration Checklist"
    ...
    - ✅ Write descriptive `INTRODUCTION` for cluster orchestration
...
**Path**: `config/cluster/devices.yaml`

### Device Configuration Structure

```yaml
# Device Configuration - YAML Format
# This configuration defines device instances for cluster network
...
devices:
  ...
  - device_id: "mobile_agent_1"
    ...
    # Device capabilities (used by cluster for task routing)
    capabilities:
      - "ui_automation"
      - "mobile_app_testing"
      - "touch_gestures"
...
!!! tip "Device Configuration Best Practices"
    ...
    - ✅ Add comprehensive `capabilities` for cluster task routing
...
**File**: `Alien/prompts/third_party/mobile_agent.yaml`

```yaml
version: 1.0

system: |-
  You are **MobileAgent**, the Alien framework's intelligent agent for mobile device automation.
...
**File**: `Alien/prompts/third_party/mobile_agent_example.yaml`
...
nano config/Alien/third_party.yaml
```
...
```bash
# Edit config/cluster/devices.yaml
nano config/cluster/devices.yaml
```
...
```bash
# Create main prompt
touch Alien/prompts/third_party/mobile_agent.yaml

# Create example prompt
touch Alien/prompts/third_party/mobile_agent_example.yaml
```
...
```bash
# Agent class
# Edit: Alien/agents/agent/customized_agent.py

# Processor
# Edit: Alien/agents/processors/customized/customized_agent_processor.py

# States
# Create: Alien/agents/states/mobile_agent_state.py

# Strategies
# Create: Alien/agents/processors/strategies/mobile_agent_strategy.py

# Prompter
# Create: Alien/prompter/customized/mobile_agent_prompter.py
```
...
```bash
# Create MCP server
touch Alien/client/mcp/http_servers/mobile_mcp_server.py
```
...
```bash
# Terminal 1: Start Alien agent server
python -m Alien.server.app --port 5010
```
...
```
==================================================
Alien Mobile MCP Server (Android)
...
## cluster Multi-Device Integration

### network Configuration

**File**: `config/cluster/network.yaml`
...
network:
  ...
  network_id: "mobile_test_network"
...
```bash
# ========================================
# Start cluster with Multiple Devices
# ========================================

# Terminal 1: cluster orchestrator
python -m cluster \
  --network-id mobile_test_network \
  --config config/cluster/network.yaml

# Terminal 2-4: Device clients
python -m Alien.client.client --ws --ws-server ws://localhost:5010/ws --client-id mobile_agent_1 --platform android &
python -m Alien.client.client --ws --ws-server ws://localhost:5011/ws --client-id mobile_agent_2 --platform ios &
python -m Alien.client.client --ws --ws-server ws://localhost:5001/ws --client-id linux_agent_1 --platform linux &

# Terminal 5: Submit multi-device task
python -m cluster.client.submit_task \
  --network mobile_test_network \
...
# config/cluster/devices.yaml (dev)
devices:
...
# config/cluster/devices.yaml (prod)
devices:
...
- ✅ cluster multi-device integration (optional)

**Key Takeaways**:

| Configuration | Purpose | File |
|---------------|---------|------|
| **Agent Registration** | Enable agent in Alien | `config/Alien/third_party.yaml` |
| **Device Registry** | Register device instances | `config/cluster/devices.yaml` |
| **Prompt Templates** | Define LLM prompts | `Alien/prompts/third_party/*.yaml` |
...
- **[cluster Overview](../../cluster/overview.md)** - Multi-device orchestration
