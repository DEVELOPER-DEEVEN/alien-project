...
6. [Network Multi-Device Integration](#network-multi-device-integration)
...
graph TB
    ...
    subgraph "Network Configuration"
        NetworkConfig[config/network/<br/>Multi-Device Config]
        Devices[devices.yaml<br/>Device Registry]
        Orion[orion.yaml<br/>Orchestration]
...
**Path**: `config/network/devices.yaml`

### Device Configuration Structure

```yaml
# Device Configuration - YAML Format
# This configuration defines device instances for Network orion
...
devices:
  ...
  - device_id: "mobile_agent_1"
    ...
    # Device capabilities (used by Network for task routing)
    capabilities:
      - "ui_automation"
      - "mobile_app_testing"
      - "touch_gestures"
...
```bash
# Edit config/network/devices.yaml
nano config/network/devices.yaml
```
...
## Network Multi-Device Integration

### Orion Configuration

**File**: `config/network/orion.yaml`
...
```bash
# ========================================
# Start Network with Multiple Devices
# ========================================

# Terminal 1: Network orchestrator
python -m network \
  --orion-id mobile_test_orion \
  --config config/network/orion.yaml
...
python -m network.client.submit_task \
  --orion mobile_test_orion \
...
# config/network/devices.yaml (dev)
devices:
...
# config/network/devices.yaml (prod)
devices:
...
| **Device Registry** | Register device instances | `config/network/devices.yaml` |
...
- **[Network Overview](../../network/overview.md)** - Multi-device orchestration
