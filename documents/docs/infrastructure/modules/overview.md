...
The **Module System** is the core execution engine of ALIEN, orchestrating the complete lifecycle of user interactions from initial request to final completion. It manages sessions, rounds, context state, and command dispatch across both Windows and Linux platforms.
...
A **Session** represents a complete conversation between the user and ALIEN, potentially spanning multiple requests and rounds.
...
!!!example "Session Creation"
    ```python
    from alien.module.session_pool import SessionFactory
...
!!!warning "Round Termination Conditions"
    ...
    - Session step exceeds `alien_config.system.max_step`
...
Key configuration options from `alien_config`:
