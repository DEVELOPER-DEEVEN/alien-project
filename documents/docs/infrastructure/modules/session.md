...
A **Session** is a continuous conversation instance between the user and ALIEN, managing multiple rounds of interaction from initial request to task completion across different execution modes and platforms.
...
ALIEN supports **7 session types** across Windows and Linux platforms:
...
```python
from alien.module.sessions.session import FollowerSession
...
```python
from alien.module.sessions.linux_session import LinuxSession
