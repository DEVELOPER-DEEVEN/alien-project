...
The `Blackboard` is a shared memory space visible to all agents in the ALIEN framework. It stores information required for agents to interact with the user and applications at every step. The `Blackboard` enables agents to share information and collaborate to fulfill user requests.
...
| --- | --- |
| `questions` | A list of questions that ALIEN asks the user, along with their corresponding answers. |
...
The keys stored in the `trajectories` are configured as `HISTORY_KEYS` in the `config/alien/system.yaml` file. You can customize the keys based on your requirements and the agent's logic.

Whether to save the screenshots is determined by the `AppAgent`. You can enable or disable screenshot capture by setting the `SCREENSHOT_TO_MEMORY` flag in the `config/alien/system.yaml` file.
