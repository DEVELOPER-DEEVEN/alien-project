...
ALIEN provides two dispatcher implementations for different deployment scenarios:
...
graph TB
    ...
    subgraph "MCP Servers"
        Win[Windows MCP Server<br/>alien_windows]
        Linux[Linux MCP Server<br/>alien_linux]
...
tools_cmd = Command(
    ...
    arguments={"server_name": "alien_windows"}
)
...
```python
# Location: alien/module/dispatcher.py
class BasicCommandDispatcher(ABC):
