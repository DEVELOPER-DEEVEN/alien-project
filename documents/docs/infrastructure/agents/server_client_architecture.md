...
Device agents in Alien are partitioned into **server** and **client** components, separating high-level orchestration from low-level execution. This architecture enables safe, scalable, and flexible task execution across heterogeneous devices through the Agent Interaction Protocol (AIP).
...
The **agent server** is responsible for managing the agent's state machine lifecycle, executing high-level strategies, and interacting with the network Agent or orchestrator. It handles task decomposition, prompt construction, decision-making, and command sequencing.
...
graph TB
    ...
    subgraph "External Interfaces"
        ORCHESTRATOR[network Agent/<br/>Orchestrator]
...
Note: The AgentProfile concept is part of the design for multi-agent coordination in cluster (network-level orchestration). In Alien-Group's current implementation, agent metadata is managed through the session context and WebSocket handler registration.
...
Device clients can be **rapidly deployed** on new devices with minimal configuration, immediately becoming execution endpoints within Alien.

```bash
# Deploy client on new device (example)
# 1. Install client package
pip install Alien-client
...
Alien-client start --config client_config.yaml
```
...
The server-client architecture is a foundational design pattern in Alien's distributed agent system:
...
By decoupling high-level reasoning from low-level execution, the server-client architecture enables Alien to safely orchestrate complex workflows across diverse computing environments while maintaining flexibility, reliability, and ease of maintenance.
