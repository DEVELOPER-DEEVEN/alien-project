...
The **agent server** is responsible for managing the agent's state machine lifecycle, executing high-level strategies, and interacting with the Orion Agent or orchestrator. It handles task decomposition, prompt construction, decision-making, and command sequencing.
...
graph TB
    ...
    subgraph "External Interfaces"
        ORCHESTRATOR[Orion Agent/<br/>Orchestrator]
...
Note: The AgentProfile concept is part of the design for multi-agent coordination in Network (orion-level orchestration). In ALIEN3's current implementation, agent metadata is managed through the session context and WebSocket handler registration.
