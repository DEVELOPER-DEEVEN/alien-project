# Using Mobile Agent as cluster Device

Configure Mobile Agent as a sub-agent in Alien's cluster framework to enable cross-platform, multi-device task orchestration. cluster can coordinate Mobile agents alongside Windows and Linux devices to execute complex workflows spanning multiple systems and platforms.

> **📖 Prerequisites:**
> 
> Before configuring Mobile Agent in cluster, ensure you have:
> 
...
> - Reviewed the [cluster Overview](../cluster/overview.md) - Understand multi-device orchestration

## Overview

The **cluster framework** provides multi-tier orchestration capabilities, allowing you to manage multiple device agents (Windows, Linux, Android, etc.) from a central networkAgent. When configured as a cluster device, MobileAgent becomes a **sub-agent** that can:

- Execute Android-specific subtasks assigned by cluster
...
## cluster Architecture with Mobile Agent
...
graph TB
    ...
    cluster -->|Mobile Subtask| Mobile1
    cluster -->|Mobile Subtask| Mobile2
    cluster -->|Mobile Subtask| Mobile3
...
Add your Mobile agent(s) to the device list in `config/cluster/devices.yaml`:
...
cluster uses the `capabilities` field to intelligently route subtasks to appropriate devices. Define capabilities based on device features, installed apps, or task types.
...
## Starting cluster with Mobile Agents
...
6. ✅ LLM configured in `config/Alien/agents.yaml` or `config/cluster/agent.yaml`
...
```bash
python -m cluster "Your cross-device task description here"
```
...
sequenceDiagram
    ...
    User->>cluster: Request meeting workflow
    cluster->>cluster: Decompose task
    
    Note over cluster,WorkPhone: Subtask 1: Get notes from email
    cluster->>WorkPhone: "Open Outlook and find meeting notes"
    ...
    WorkPhone-->>cluster: Notes content
    
    Note over cluster,Desktop: Subtask 2: Summarize on desktop
    cluster->>Desktop: "Summarize meeting notes"
    Desktop->>Desktop: Open Word
    Desktop->>Desktop: Paste notes
    Desktop->>Desktop: Generate summary
    Desktop-->>cluster: Summary document
    
    Note over cluster,PersonalPhone: Subtask 3: Send via WhatsApp
    cluster->>PersonalPhone: "Send summary to team on WhatsApp"
    ...
    PersonalPhone-->>cluster: Message sent
    
    cluster-->>User: Workflow completed
```
...
4. **cluster**: Aggregate confirmations and notify user

---

## Task Assignment Behavior

### How cluster Routes Tasks to Mobile Agents

cluster's networkAgent uses several factors to select the appropriate mobile device for each subtask:
...
INFO - [cluster] Subtask 1 → mobile_phone_personal (capability match: messaging)
INFO - [cluster] Subtask 2 → mobile_phone_work (capability match: calendar)
INFO - [cluster] Subtask 3 → mobile_tablet_home (capability match: media)
```

### Troubleshooting Device Connection

**Issue**: Mobile agent not appearing in cluster device pool
...
4. **Review cluster logs** for connection errors
...
### Screenshot Capture for cluster

Mobile agents automatically capture screenshots during execution, which cluster can:
...
Different Android devices have different screen sizes and densities. cluster handles this by:
...
cluster can query `installed_apps` from metadata to:
...
Using Mobile Agent as a cluster device enables sophisticated multi-device orchestration:

- **Cross-Platform Workflows**: Seamlessly combine Android, Windows, and Linux tasks
- **Capability-Based Routing**: cluster selects the right device for each subtask
...
With Mobile Agent in cluster, you can orchestrate complex workflows spanning mobile apps, desktop applications, and server systems from a single unified interface.
