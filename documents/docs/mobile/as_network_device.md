# Using Mobile Agent as Network Device
...
> - Reviewed the [Network Overview](../network/overview.md) - Understand multi-device orchestration

## Overview

The **Network framework** provides multi-tier orchestration capabilities, allowing you to manage multiple device agents (Windows, Linux, Android, etc.) from a central OrionAgent. When configured as a Network device, MobileAgent becomes a **sub-agent** that can:

- Execute Android-specific subtasks assigned by Network
...
## Network Architecture with Mobile Agent

```mermaid
graph TB
    User[User Request]
    Network[Network OrionAgent<br/>Orchestrator]
    ...
    User -->|Complex Task| Network
    Network -->|Windows Subtask| Win1
    Network -->|Linux Subtask| Linux1
    Network -->|Mobile Subtask| Mobile1
    Network -->|Mobile Subtask| Mobile2
    Network -->|Mobile Subtask| Mobile3
...
Add your Mobile agent(s) to the device list in `config/network/devices.yaml`:
...
Network uses the `capabilities` field to intelligently route subtasks to appropriate devices. Define capabilities based on device features, installed apps, or task types.
...
## Starting Network with Mobile Agents
...
python -m alien.server.app --port 5002 --platform mobile

# On machine hosting work phone agent (192.168.1.104)
python -m alien.server.app --port 5003 --platform mobile

# On machine hosting tablet agent (192.168.1.105)
python -m alien.server.app --port 5004 --platform mobile
```
...
python -m alien.client.mcp.http_servers.mobile_mcp_server \
  --host localhost \
  --data-port 8020 \
  --action-port 8021 \
  --server both
...
```bash
python -m network "Your cross-device task description here"
```

Network will automatically connect to all configured devices and display the orchestration interface.
...
sequenceDiagram
    ...
    User->>Network: Request meeting workflow
    Network->>Network: Decompose task
    
    Note over Network,WorkPhone: Subtask 1: Get notes from email
    Network->>WorkPhone: "Open Outlook and find meeting notes"
    ...
    WorkPhone-->>Network: Notes content
    
    Note over Network,Desktop: Subtask 2: Summarize on desktop
    Network->>Desktop: "Summarize meeting notes"
    Desktop->>Desktop: Open Word
    Desktop->>Desktop: Paste notes
    Desktop->>Desktop: Generate summary
    Desktop-->>Network: Summary document
    
    Note over Network,PersonalPhone: Subtask 3: Send via WhatsApp
    Network->>PersonalPhone: "Send summary to team on WhatsApp"
    ...
    PersonalPhone-->>Network: Message sent
    
    Network-->>User: Workflow completed
```
...
4. **Network**: Aggregate confirmations and notify user

---

## Task Assignment Behavior

### How Network Routes Tasks to Mobile Agents

Network's OrionAgent uses several factors to select the appropriate mobile device for each subtask:
...
!!!danger "Configuration Validation"
    Ensure these match exactly or Network cannot control the device:
...
**Check Network device pool:**

```bash
curl http://<network-server>:5000/api/devices
```
...
INFO - [Network] Subtask 1 → mobile_phone_personal (capability match: messaging)
INFO - [Network] Subtask 2 → mobile_phone_work (capability match: calendar)
INFO - [Network] Subtask 3 → mobile_tablet_home (capability match: media)
```

### Troubleshooting Device Connection

**Issue**: Mobile agent not appearing in Network device pool
...
### Screenshot Capture for Network

Mobile agents automatically capture screenshots during execution, which Network can:
...
Different Android devices have different screen sizes and densities. Network handles this by:
...
Network can query `installed_apps` from metadata to:
...
- [Network Overview](../network/overview.md) - Multi-device orchestration framework
- [Network Quick Start](../getting_started/quick_start_network.md) - Network deployment guide
- [Orion Orchestrator](../network/orion_orchestrator/overview.md) - Task orchestration
- [Network Devices Configuration](../configuration/system/network_devices.md) - Complete device configuration reference

---

## Summary

Using Mobile Agent as a Network device enables sophisticated multi-device orchestration:

- **Cross-Platform Workflows**: Seamlessly combine Android, Windows, and Linux tasks
- **Capability-Based Routing**: Network selects the right device for each subtask
...
With Mobile Agent in Network, you can orchestrate complex workflows spanning mobile apps, desktop applications, and server systems from a single unified interface.
