...
class MockNetworkWeaverAgent:
    """Mock NetworkWeaverAgent for testing orion continuation"""
    ...
    async def _add_new_tasks(self):
        """模拟添加新任务到orion"""
        # 在实际实现中，这里会向orion添加新任务
        # 并触发新的任务事件
        self.new_tasks_added = True
        # 可以模拟发送新的TASK_STARTED事件


class TestOrionContinuation:
    """测试orion完成后的继续执行"""
    ...
    async def test_continuation_after_completion(self):
        ...
        mock_orion.is_complete.return_value = True  # 初始完成
    ...
    @pytest.mark.asyncio 
    async def test_orion_continuation_with_new_tasks(self):
        """测试orion完成后添加新任务并正确执行"""
        monitoring_state = MonitoringNetworkAgentState()
        agent = MockNetworkWeaverAgent()
        context = Context()
