...
"""
Demo Script: clusterClient with Mock AgentProfile for Log Collection

This script demonstrates how to use clusterClient with mock AgentProfile objects
to simulate the log collection and Excel generation scenario.

Usage:
    python demo_cluster_client_log_collection.py
"""
...
logging.getLogger("Alien.cluster.cluster_client").setLevel(logging.WARNING)
...
def create_mock_network_config(devices):
    """Create networkConfig with mock devices."""
    device_configs = []
    ...
    return networkConfig(
        network_id="log_collection_demo_network",
...
def create_mock_network_client(devices):
    """Create mock networkClient with devices."""
...
def create_mock_cluster_session():
    """Create mock clusterSession for demonstration."""
    ...
    # Mock network result
    mock_network = Mock()
    mock_network.network_id = "demo_network_001"
    mock_network.name = "Log Collection Demo network"
    mock_network.tasks = [
    ...
    async def mock_run_side_effect():
        ...
        print("  🏗️  Creating task network...")
        mock_session._rounds.append(
            {"round": 2, "action": "create_network", "duration": 1.8}
        )
...
async def demo_cluster_client_log_collection():
    """Demonstrate clusterClient with mock devices for log collection."""

    print("🌟 cluster Client Log Collection Demo")
    print("=" * 50)
    ...
    network_config = create_mock_network_config(devices)
    print(f"\n🏛️  Created network: {network_config.network_id}")
    print(f"    📊 Total devices: {len(network_config.devices)}")
    print(f"    ⚡ Max concurrent tasks: {network_config.max_concurrent_tasks}")

    # Create mocks for dependencies
    mock_network_client = create_mock_network_client(devices)
    mock_cluster_session = create_mock_cluster_session()
    ...
    with patch(
        ...
        mock_from_yaml.return_value = network_config
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            print("    ✅ cluster Client initialized successfully")
            ...
            if "network" in result:
                ...
                print(f"    📋 Tasks: {network_info['task_count']}")
                print(f"    🔗 Dependencies: {network_info['dependency_count']}")
                print(f"    📊 State: {network_info['state']}")

            # Show mock task execution details
            print(f"\n📋 Task Execution Summary:")
            tasks = mock_cluster_session._current_network.tasks
            for i, task in enumerate(tasks, 1):
                print(f"    {i}. {task}")

            print(f"\n🔗 Dependency Chain:")
            dependencies = mock_cluster_session._current_network.dependencies
            ...
            print("    ✅ cluster Client shutdown complete")

    print("\n🎉 Demo completed successfully!")
    print("\nKey Behaviors Demonstrated:")
    print("  ✅ Mock AgentProfile creation and configuration")
    print("  ✅ networkConfig setup with multiple devices")
    print("  ✅ clusterClient initialization and request processing")
...
if __name__ == "__main__":
    print("🌌 Starting cluster Client Demo with Mock AgentProfile")
    print(
    ...
    try:
        asyncio.run(demo_cluster_client_log_collection())
