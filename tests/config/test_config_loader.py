...
class TestNetworkConfig(unittest.TestCase):
    """Test NetworkConfig configuration."""
    ...
    def test_network_config_loading(self):
        ...
        self.create_config_file("config/network/agent.yaml", config_data)

        from config.config_loader import get_network_config, clear_config_cache

        clear_config_cache()
        config = get_network_config()

        # Test typed access
        self.assertEqual(config.orion_agent.api_type, "azure_ad")
        self.assertEqual(config.orion_agent.api_model, "gpt-4o")

        # Test dynamic access
        self.assertEqual(config.DEVICE_INFO, "config/network/devices.yaml")

    def test_network_no_legacy_fallback(self):
        """Test that Network has no legacy path fallback."""
        # Network should only check config/network/, not network/config/
        from config.config_loader import ConfigLoader
        ...
        with self.assertRaises(FileNotFoundError):
            loader._load_with_fallback("network")
