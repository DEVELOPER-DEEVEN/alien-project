...
class TestConfigLoader(unittest.TestCase):
    ...
    def test_load_new_config_only(self):
        ...
        self.create_config_file("config/Alien/test.yaml", new_config)

        # Load config
        from config.config_loader import ConfigLoader

        loader = ConfigLoader()
        config_data = loader._load_with_fallback("Alien")
    ...
    def test_load_legacy_config_only(self):
        ...
        self.create_config_file("Alien/config/test.yaml", legacy_config)
    ...
    def test_multiple_yaml_files_merge(self):
        ...
        self.create_config_file("config/Alien/agents.yaml", config1)
        self.create_config_file("config/Alien/system.yaml", config3)
        self.create_config_file("config/Alien/backup.yaml", config2)
    ...
    def test_environment_overrides(self):
        ...
        self.create_config_file("config/Alien/config.yaml", base_config)

        # Create dev override
        dev_config = {"MAX_STEP": 100}  # Override MAX_STEP
        self.create_config_file("config/Alien/config_dev.yaml", dev_config)

        # Load with dev environment
        from config.config_loader import ConfigLoader

        loader = ConfigLoader()
        config_data = loader._load_with_fallback("Alien", env="dev")
    ...
    def test_no_config_found_error(self):
        ...
        with self.assertRaises(FileNotFoundError) as context:
            loader._load_with_fallback("Alien")
    ...
    def test_yaml_parsing_error_handling(self):
        ...
        valid_path = Path(self.test_dir) / "config/Alien/valid.yaml"
        valid_path.parent.mkdir(parents=True, exist_ok=True)
        with open(valid_path, "w") as f:
            yaml.dump({"VALID_KEY": "valid_value"}, f)

        invalid_path = Path(self.test_dir) / "config/Alien/invalid.yaml"
    ...
    def test_cache_mechanism(self):
        ...
        # Create config
        config = {"MAX_STEP": 50}
        config_path = self.create_config_file("config/Alien/test.yaml", config)
    ...
    def test_warning_on_duplicate_configs(self):
        ...
        # Create both configs
        self.create_config_file("config/Alien/test.yaml", {"MAX_STEP": 50})
        self.create_config_file("Alien/config/test.yaml", {"MAX_STEP": 30})
...
class TestAlienConfig(unittest.TestCase):
    """Test AlienConfig typed configuration."""
    ...
    def test_typed_access(self):
        ...
        self.create_config_file("config/Alien/test.yaml", config_data)

        from config.config_loader import get_Alien_config, clear_config_cache
...
class TestclusterConfig(unittest.TestCase):
    """Test clusterConfig configuration."""
    ...
    def test_cluster_config_loading(self):
        ...
        self.create_config_file("config/cluster/agent.yaml", config_data)

        from config.config_loader import get_cluster_config, clear_config_cache

        clear_config_cache()
        config = get_cluster_config()

        # Test typed access
        self.assertEqual(config.network_agent.api_type, "azure_ad")
        self.assertEqual(config.network_agent.api_model, "gpt-4o")

        # Test dynamic access
        self.assertEqual(config.DEVICE_INFO, "config/cluster/devices.yaml")

    def test_cluster_no_legacy_fallback(self):
        """Test that cluster has no legacy path fallback."""
        # cluster should only check config/cluster/, not cluster/config/
        from config.config_loader import ConfigLoader
        ...
        with self.assertRaises(FileNotFoundError):
            loader._load_with_fallback("cluster")
...
class TestConfigCaching(unittest.TestCase):
    ...
    def test_global_config_cache(self):
        ...
        # First load
        config1 = get_Alien_config()

        # Second load should return same instance
        config2 = get_Alien_config()
    ...
    def test_cache_reload(self):
        ...
        # Force reload
        config2 = get_Alien_config(reload=True)
