...
class AttributeAccessValidator:
    ...
    def test_network_orion_config(self):
        """测试 Network OrionRuntimeConfig 的属性访问"""
        console.print(
            "\n[bold cyan]测试 Network OrionRuntimeConfig 属性访问[/bold cyan]"
        )
        ...
        try:
            ...
            for upper_name, lower_name in orion_fields:
                new_val_upper = getattr(network_config.orion, upper_name, None)
                new_val_lower = getattr(network_config.orion, lower_name, None)

                # Network 没有旧配置，只验证大小写访问一致性
                result = {
                    "name": f"orion.{upper_name}",
                    "new_upper": new_val_upper,
                    "new_lower": new_val_lower,
                    "status": (
                        ""
            ...
            console.print(f"[yellow]Network 配置测试跳过: {e}[/yellow]")

    def test_network_agent_config(self):
        """测试 Network AgentConfig 的属性访问"""
        console.print(
            "\n[bold cyan]测试 Network ORION_AGENT 属性访问[/bold cyan]"
        )

        try:
            network_config = get_network_config()
            agent = network_config.agent.ORION_AGENT
            ...
            console.print(f"[yellow]Network Agent 配置测试跳过: {e}[/yellow]")
...
def main():
    ...
    try:
        ...
        # Network 配置测试
        validator.test_network_orion_config()
        validator.test_network_agent_config()
