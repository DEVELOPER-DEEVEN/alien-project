...
class TestclusterValidator(unittest.TestCase):
    ...
    def test_valid_cluster_config(self):
        ...
        self.create_config_file("config/cluster/agent.yaml", config)

        validator = ConfigValidator("cluster")
        result = validator.validate_structure()

        self.assertTrue(result)

    def test_cluster_no_legacy_path(self):
        """Test that cluster validator doesn't check legacy path."""
        from Alien.tools.validate_config import ConfigValidator

        validator = ConfigValidator("cluster")

        # cluster should not have legacy path
