...
"""
Real clusterSession Integration Test with Mock AgentProfile

This test uses REAL clusterSession.run() (not mocked) to test the complete
...
from cluster.core.types import ExecutionResult, TaskStatus


class TestRealclusterSessionWithMockDevices:
    """Test real clusterSession execution with mock AgentProfile to find bugs."""
    ...
    class NoComputerRunActionFilter(logging.Filter):
        ...
        def filter(self, record):
            ...
            if record.name == "Computer" and record.funcName == "_run_action":
                return False
            # Also filter out any Alien.agents.agent.basic logs which might be noisy
            if "Alien.agents" in record.name and record.funcName == "_run_action":
                return False
            return True

    def _setup_comprehensive_logging(self):
        """Setup comprehensive logging for all cluster components."""
        ...
        loggers_to_configure = [
            "Alien.cluster.session",
            "Alien.cluster.agents",
            "Alien.cluster.network",
            "Alien.cluster.client",
            "Alien.cluster.core",
            "cluster.session",
            "cluster.agents",
            "cluster.network",
    ...
    def mock_network_client(
        ...
        """Create real networkClient with mocked device manager."""
        # Create a real networkClient but mock its device manager
        ...
        mock_client.get_network_info.return_value = {
    ...
    @pytest.mark.asyncio
    async def test_real_cluster_session_execution_with_mock_devices(
        self,
        mock_network_client,
        mock_linux_server_1: AgentProfile,
        mock_linux_server_2: AgentProfile,
        ...
        """Test real clusterSession.run() with mock devices to identify bugs."""

        print("\n🔍 Starting REAL clusterSession execution test...")
        ...
        # Create real clusterSession (not mocked)
        ...
        print("🚀 Created real clusterSession instance")
        ...
        try:
            ...
            print(
                f"   Current network: {getattr(session, '_current_network', None)}"
            )
        ...
        network = getattr(session, "_current_network", None)
        if network:
            print(f"   network created: ✅")
            print(
                f"   network ID: {getattr(network, 'network_id', 'unknown')}"
            )
            print(f"   Task count: {len(getattr(network, 'tasks', []))}")
            print(
                f"   Dependency count: {len(getattr(network, 'dependencies', []))}"
            )
        else:
            print(f"   network created: ❌")
        ...
        assign_task_calls = (
            mock_network_client.device_manager.assign_task_to_device.call_count
        )
        ...
        if assign_task_calls > 0:
            ...
            for (
                ...
                mock_network_client.device_manager.assign_task_to_device.call_args_list
            ):
                args, kwargs = call
                device_id = (
        ...
        # Check 2: network was created
        if not network:
            issues_found.append("No network was created")
        ...
        return {
            ...
            "network_created": network is not None,
    ...
    async def test_session_error_handling(
        self, mock_network_client, temp_output_dir: Path
    ):
        ...
        try:
            ...
            original_assign_task = (
                mock_network_client.device_manager.assign_task_to_device
            )
            ...
            mock_network_client.device_manager.assign_task_to_device = AsyncMock(
