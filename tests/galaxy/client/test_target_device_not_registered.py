...
This test suite verifies the fix for the issue where network client
would incorrectly report success when the target device is not connected to
the Alien server.

Scenario:
1. network client attempts to register with server
...
Before fix:
- _register_network_client() always returned True
...
After fix:
- _register_network_client() waits for server response
...
from cluster.client.components import DeviceStatus, AgentProfile
