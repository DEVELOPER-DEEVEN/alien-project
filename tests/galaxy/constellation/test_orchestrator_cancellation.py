...
"""
Unit tests for TasknetworkOrchestrator cancellation mechanism.
...
from cluster.network.enums import TaskStatus, networkState, TaskPriority


@pytest_asyncio.fixture
def mock_orchestrator():
    """Create a mock TasknetworkOrchestrator for testing."""
...
def simple_network():
    ...
    network = Tasknetwork(
        network_id="test_network", name="Test network"
    )
...
async def test_cancel_execution_sets_flags(mock_orchestrator):
    ...
    # Act
    result = await mock_orchestrator.cancel_execution(network_id)

    # Assert
    assert result is True
    assert mock_orchestrator._cancellation_requested is True
    assert mock_orchestrator._cancelled_networks[network_id] is True
...
async def test_execution_loop_checks_cancellation_flag(
    mock_orchestrator, simple_network
):
...
@pytest.mark.asyncio
async def test_execution_loop_checks_network_specific_cancellation(
    mock_orchestrator, simple_network
):
...
async def test_cancel_execution_with_no_tasks(mock_orchestrator):
    ...
    # Act
    result = await mock_orchestrator.cancel_execution("test_network")
