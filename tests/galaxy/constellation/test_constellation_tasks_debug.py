...
def debug_tasks_field(log_file_path: str):
    """Debug the tasks field in network_before"""
    ...
    if "network_before" in log_entry and log_entry["network_before"]:
        const_before_str = log_entry["network_before"]
        const_before = json.loads(const_before_str)

        print("network_before structure:")
        print(f"  network_id: {const_before.get('network_id')}")
    ...
    if "network_after" in log_entry and log_entry["network_after"]:
        const_after_str = log_entry["network_after"]
        const_after = json.loads(const_after_str)

        print("network_after (line 1) structure:")
        print(f"  network_id: {const_after.get('network_id')}")
