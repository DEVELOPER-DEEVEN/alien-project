...
"""
Test clusterClient with Mock AgentProfile for Log Collection Scenario

This test demonstrates using clusterClient with the mock AgentProfile objects
...
from cluster.client.components.types import AgentProfile, DeviceStatus
from cluster.client.config_loader import networkConfig, DeviceConfig


class TestclusterClientLogCollectionSession:
    """Test clusterClient with mock AgentProfile for log collection scenario."""
    ...
    def mock_network_config(
        ...
        """Create mock networkConfig with our test devices."""
        device_configs = [
            DeviceConfig(
        ...
        return networkConfig(
            network_id="log_collection_test_network",
    ...
    def mock_cluster_session(self):
        ...
        # Mock network
        mock_network = Mock()
        mock_network.network_id = "test_network_001"
        mock_network.name = "Log Collection Test network"
        mock_network.tasks = [
            "collect_logs_linux1",
            "collect_logs_linux2",
            "generate_excel",
        ]
        mock_network.dependencies = ["collect_logs -> generate_excel"]
        mock_network.state = Mock()
    ...
    @pytest.mark.asyncio
    async def test_cluster_client_initialization_with_mock_devices(
        self, mock_network_config: networkConfig, temp_output_dir: Path
    ):
        """Test clusterClient initialization with mock devices."""
        with patch(
            "Alien.cluster.cluster_client.networkConfig.from_yaml"
        ) as mock_from_yaml:
            mock_from_yaml.return_value = mock_network_config
            ...
            assert client._device_config == mock_network_config
    ...
    @pytest.mark.asyncio
    async def test_cluster_client_full_initialization(
        self,
        mock_network_config: networkConfig,
        mock_network_client,
        temp_output_dir: Path,
    ):
        """Test clusterClient full initialization with mocked components."""
        with patch(
            "Alien.cluster.cluster_client.networkConfig.from_yaml"
            ...
            mock_from_yaml.return_value = mock_network_config
            mock_client_class.return_value = mock_network_client
            ...
            # Verify networkClient was created and initialized
            mock_client_class.assert_called_once_with(config=mock_network_config)
    ...
    async def test_process_log_collection_request(
        ...
        with patch(
            ...
            mock_from_yaml.return_value = mock_network_config
            mock_client_class.return_value = mock_network_client
            mock_session_class.return_value = mock_cluster_session
            ...
            # Verify clusterSession was created with correct parameters
            mock_session_class.assert_called_once()
            call_args = mock_session_class.call_args
            assert call_args[1]["task"] == "log_collection_and_excel_generation"
            assert call_args[1]["client"] == mock_network_client
            ...
            # Verify network info in result
            network_info = result["network"]
            assert network_info["id"] == "test_network_001"
            assert network_info["name"] == "Log Collection Test network"
            assert network_info["task_count"] == 3
            assert network_info["state"] == "completed"

    @pytest.mark.asyncio
    async def test_cluster_client_device_availability_check(
        self,
        mock_network_config: networkConfig,
        mock_network_client,
        temp_output_dir: Path,
    ):
        """Test that clusterClient can access all required devices for log collection."""
        with patch(
            "Alien.cluster.cluster_client.networkConfig.from_yaml"
        ) as mock_from_yaml, patch(
            "Alien.cluster.cluster_client.networkClient"
            ...
            # Initialize client
            client = clusterClient(output_dir=str(temp_output_dir))
            await client.initialize()

            # Verify client has access to network client
            assert client._client == mock_network_client
    ...
    @pytest.mark.asyncio
    async def test_cluster_client_error_handling(
        self,
        mock_network_config: networkConfig,
        mock_network_client,
        temp_output_dir: Path,
    ):
        """Test clusterClient error handling during request processing."""
        with patch(
            "Alien.cluster.cluster_client.networkConfig.from_yaml"
        ) as mock_from_yaml, patch(
            "Alien.cluster.cluster_client.networkClient"
    ...
    @pytest.mark.asyncio
    async def test_cluster_client_shutdown(
        self,
        mock_network_config: networkConfig,
        mock_network_client,
        mock_cluster_session,
        ...
        """Test clusterClient proper shutdown."""
        with patch(
            "Alien.cluster.cluster_client.networkConfig.from_yaml"
        ) as mock_from_yaml, patch(
            "Alien.cluster.cluster_client.networkClient"
    ...
    def test_cluster_client_session_name_generation(self):
        """Test automatic session name generation."""
        # Test with custom session name
        client1 = clusterClient(session_name="custom_session")
        assert client1.session_name == "custom_session"

        # Test with auto-generated session name
        client2 = clusterClient()
        assert client2.session_name.startswith("cluster_session_")
        assert len(client2.session_name) > len("cluster_session_")
    ...
    async def test_request_without_initialization_error(self):
        ...
        with pytest.raises(RuntimeError, match="cluster client not initialized"):
