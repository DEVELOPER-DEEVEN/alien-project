...
# Import real enums from cluster framework
...
# Simple Tasknetwork for testing
class SimpleTasknetwork:
    def __init__(
        self,
        network_id: str = None,
        name: str = None,
        ...
        self.network_id = (
            network_id
            or f"test_network_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
        self.name = name or self.network_id
        self.state = networkState.CREATED
    ...
    def _visualize_dag(self, action: str = "update"):
        ...
        if self._enable_visualization and self._visualizer:
            try:
                ...
                self._visualizer.display_network_overview(self)
                return True
    ...
    def start_execution(self):
        self.state = networkState.EXECUTING
        self._execution_start_time = datetime.now()
        print(f"🚀 Starting execution of network: {self.name}")
        self._visualize_dag("start_execution")

    def complete_execution(self, success: bool = True):
        self.state = networkState.COMPLETED
    ...
    def get_all_tasks(self):
        """Return all tasks in the network."""
        return list(self._tasks.values())

    def get_all_dependencies(self):
        """Return all dependencies in the network."""
...
def test_dag_visualization():
    ...
    for task_id in [
        ...
        network.mark_task_completed(task_id, success=True)

    # Complete the pipeline
    network.mark_task_completed("load_prod", success=True)
    network.complete_execution(success=True)
