...
a sample network with tasks and dependencies, then displaying
...
from cluster.visualization.dag_visualizer import DAGVisualizer, visualize_dag


def create_sample_network() -> Tasknetwork:
    """Create a sample network for demonstration."""

    # Create network with visualization enabled
    network = Tasknetwork(name="Sample DAG Demo", enable_visualization=True)
    ...
    # Add tasks to network (this will trigger visualization for each task)
    print("📊 Adding tasks to network...")
    for task in tasks:
...
def simulate_execution(network: Tasknetwork):
    """Simulate task execution with progress updates."""
    print("\n🚀 Starting network execution simulation...")
...
def demonstrate_visualization_modes(network: Tasknetwork):
    ...
    visualizer.display_dependency_summary(network)
...
def main():
    ...
    try:
        ...
        demonstrate_visualization_modes(network)

        # Simulate execution
        print("\nSimulating task execution...")
        simulate_execution(network)

        print("\n🎉 Demo completed!")
        print(f"Final network state: {network.state.value}")
