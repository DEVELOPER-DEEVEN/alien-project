#!/usr/bin/env python3
"""
Test script to verify network events are properly published.
"""
...
from cluster.core.events import get_event_bus, Event, EventType, IEventObserver
...
async def test_network_events():
    """Test network event publishing."""
    print("🧪 Testing network Events...")
    ...
    # Execute network with manual device assignments
    try:
        ...
        print(f"✅ network execution completed: {result['status']}")
        ...
        for i, event in enumerate(event_collector.events, 1):
            ...
            print(f"     network ID: {getattr(event, 'network_id', 'N/A')}")
        ...
        if missing_events:
            ...
            print("✅ All expected network events were published!")
            return True
...
async def main():
    """Run the test."""
    print("🚀 Starting network Events Test...")
    success = await test_network_events()
