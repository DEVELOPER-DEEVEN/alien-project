...
from alien.network.orion.task_orion import TaskOrion
from alien.network.orion.task_star import TaskStar
from alien.network.orion.enums import TaskStatus, TaskPriority, DeviceType


class TestOrionUpdateLock:
    """Test cases for orion update lock mechanism."""
    ...
    def test_update_lock_exists(self, orion):
        """Test that the orion has an update lock."""
        assert hasattr(orion, "_update_lock")
        assert isinstance(orion._update_lock, asyncio.Lock)

    @pytest.mark.asyncio
    async def test_lock_basic_functionality(self, orion):
        """Test basic lock acquire and release."""
        # Acquire lock
        await orion._update_lock.acquire()
        assert orion._update_lock.locked()

        # Release lock
        orion._update_lock.release()
        assert not orion._update_lock.locked()

    @pytest.mark.asyncio
    async def test_lock_context_manager(self, orion):
        """Test lock usage with async context manager."""
        assert not orion._update_lock.locked()

        async with orion._update_lock:
            assert orion._update_lock.locked()

        assert not orion._update_lock.locked()

    @pytest.mark.asyncio
    async def test_concurrent_access_blocked(self, orion, sample_tasks):
    ...
    @pytest.mark.asyncio
    async def test_race_condition_scenario(self, orion, sample_tasks):
        """
        ...
        4. At the same time, agent tries to edit orion
        """
        ...
        from alien.network.orion.task_star_line import TaskStarLine
    ...
    @pytest.mark.asyncio
    async def test_multiple_concurrent_readers_blocked(self, orion):
    ...
    @pytest.mark.asyncio
    async def test_lock_prevents_stale_ready_tasks(self, orion, sample_tasks):
        """
        ...
        async def agent_modify_tasks():
            ...
            async with orion._update_lock:
                ...
                if task1.task_id in orion._tasks:
                    orion.remove_task(task1.task_id)
...
class TestLockPerformance:
    ...
    async def test_lock_overhead(self):
        ...
        orion = TaskOrion(enable_visualization=False)

        # Measure time without lock
        start = asyncio.get_event_loop().time()
        for _ in range(100):
            orion.get_ready_tasks()
