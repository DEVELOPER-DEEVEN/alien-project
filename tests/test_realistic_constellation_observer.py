...
"""
Integration test to debug why networkAgent logging doesn't work in real cluster session.
"""
...
from cluster.core.events import TaskEvent, EventType, get_event_bus
from cluster.network.orchestrator.orchestrator import (
    TasknetworkOrchestrator,
)


class TestRealisticsnetworkObserverLogger:
    ...
    async def test_realistic_scenario_with_logging_levels(
        ...
        network_agent.logger.setLevel(logging.INFO)
    ...
    async def test_with_method_wrapping_to_check_calls(
        ...
        original_method = network_agent.add_task_completion_event
        call_count = 0
        ...
        network_agent.add_task_completion_event = (
    ...
    async def test_exception_handling_in_observer(
        ...
        network_agent.add_task_completion_event = AsyncMock(
    ...
    async def test_event_type_filtering(self, mock_orchestrator, caplog):
        ...
        # Create network agent with mock
        network_agent = networkAgent(orchestrator=mock_orchestrator)
        network_agent.add_task_completion_event = AsyncMock()
        ...
        network_agent.add_task_completion_event.assert_not_called()
