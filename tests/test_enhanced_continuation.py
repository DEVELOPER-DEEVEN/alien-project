"""
测试改进的orion continuation机制
"""
import asyncio
import pytest
from unittest.mock import MagicMock, AsyncMock
from network.agents.network_agent_state import MonitoringNetworkAgentState, NetworkAgentStatus
from network.core.events import EventType
from alien.module.context import Context


class EnhancedMockNetworkWeaverAgent:
    """Enhanced Mock NetworkWeaverAgent for testing continuation with handle_continuation"""
    
    def __init__(self):
        self._status = NetworkAgentStatus.MONITORING.value
...
class TestEnhancedOrionContinuation:
    """测试增强的orion continuation机制"""
    ...
    async def test_continuation_with_handle_continuation(self):
        ...
        agent = EnhancedMockNetworkWeaverAgent()
        context = Context()
        
        # 设置mock orion
        mock_orion = MagicMock()
        mock_orion.is_complete.return_value = True
        agent.current_orion = mock_orion
    ...
    async def test_multiple_continuation_cycles(self):
        ...
        async def multi_round_should_continue(orion, context=None):
            result = await original_should_continue(orion, context)
