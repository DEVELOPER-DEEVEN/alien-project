#!/usr/bin/env python3
"""
Test script for updated orion editor with:
...
from network.orion.editor.command_registry import command_registry
from network.orion.task_orion import TaskOrion
from network.orion.task_star import TaskStar
from network.orion.task_star_line import TaskStarLine, DependencyType
...
def test_serializable_parameters():
    ...
    print(
        f"\nFinal orion has {len(orion.tasks)} tasks and {len(orion.dependencies)} dependencies"
    )
...
def test_validation_rollback():
    """Test that commands validate orion and rollback on failure."""
    ...
    print(
        f"Initial state: {len(orion.tasks)} tasks, {len(orion.dependencies)} dependencies"
    )
    is_valid, errors = orion.validate_dag()
    print(f"Orion is valid: {is_valid}")
    if not is_valid:
    ...
    is_valid, errors = orion.validate_dag()
    print(f"Orion is still valid: {is_valid}")
    if not is_valid:
        print(f"Validation errors: {errors}")
...
def test_undo_redo_with_validation():
    ...
    editor.undo()
    print(f"   Tasks: {len(orion.tasks)}, Can redo: {editor.can_redo()}")
...
def main():
    """Run all tests."""
    print("Testing Updated Orion Editor")
    print("=" * 50)
