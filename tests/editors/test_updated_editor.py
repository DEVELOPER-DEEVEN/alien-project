#!/usr/bin/env python3
"""
Test script for updated network editor with:
...
from cluster.network.task_star_line import TaskStarLine, DependencyType
...
def test_serializable_parameters():
    ...
    print(
        f"\nFinal network has {len(network.tasks)} tasks and {len(network.dependencies)} dependencies"
    )
...
def test_validation_rollback():
    """Test that commands validate network and rollback on failure."""
    ...
    print(
        f"Initial state: {len(network.tasks)} tasks, {len(network.dependencies)} dependencies"
    )
    is_valid, errors = network.validate_dag()
    print(f"network is valid: {is_valid}")
    if not is_valid:
    ...
    print(
        f"After failed operation: {len(network.tasks)} tasks, {len(network.dependencies)} dependencies"
    )
    is_valid, errors = network.validate_dag()
    print(f"network is still valid: {is_valid}")
    if not is_valid:
        print(f"Validation errors: {errors}")
    ...
    print(
        f"Final state: {len(network.tasks)} tasks, {len(network.dependencies)} dependencies"
    )
    is_valid, errors = network.validate_dag()
    print(f"network is valid: {is_valid}")
    if not is_valid:
...
def test_undo_redo_with_validation():
    ...
    print(f"   Tasks: {len(network.tasks)}, Can undo: {editor.can_undo()}")

    print("2. Undoing add task...")
    editor.undo()
    print(f"   Tasks: {len(network.tasks)}, Can redo: {editor.can_redo()}")
...
def main():
    """Run all tests."""
    print("Testing Updated network Editor")
    print("=" * 50)
