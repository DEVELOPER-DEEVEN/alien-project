...
def test_mcp_server():
    ...
    try:
        # Get current orion to find valid dependency IDs
        orion_result = call_tool(
            "build_orion", {"tasks": [], "dependencies": [], "metadata": {}}
        )
        orion = json.loads(orion_result)

        # Find a dependency to remove
        valid_dep_id = None
        for dep_key in orion.get("dependencies", {}):
...
def main():
    ...
    print("Testing Orion Editor MCP Server (Simplified)")
    print("=" * 60)
