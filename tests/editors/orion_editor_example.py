...
from network.orion.enums import TaskPriority, DependencyType
...
def example_basic_operations():
    ...
    print(f"[OK] 创建了 {len(editor.list_tasks())} 个任务")
    ...
    print(f"[OK] 创建了 {len(editor.list_dependencies())} 个依赖关系")
    ...
    if is_valid:
        ...
        print(f"[TASK] 执行顺序: {' -> '.join(topo_order)}")
    else:
        print(f"[FAIL] 星座结构无效: {errors}")
...
def example_bulk_operations():
    ...
    print(
        f"[OK] 批量创建: {len(editor.list_tasks())} 个任务, {len(editor.list_dependencies())} 个依赖"
    )
...
def example_file_operations():
    ...
    editor1.save_orion(filename)
    print("[OK] 保存成功")

    # 从文件加载
    print(f" 从 {filename} 加载星座...")
    editor2 = OrionEditor()
    editor2.load_orion(filename)
    print(
        f"[OK] 加载成功: {len(editor2.list_tasks())} 个任务, {len(editor2.list_dependencies())} 个依赖"
    )
...
def example_advanced_features():
    ...
    print(f"[OK] 创建了包含 {len(editor.list_tasks())} 个任务的复杂星座")

    # 创建子图
    print("\n 提取子图...")
    subgraph = editor.create_subgraph(["A", "B", "D"])
    print(
        f"[OK] 子图包含 {len(subgraph.list_tasks())} 个任务, {len(subgraph.list_dependencies())} 个依赖"
    )
...
def main():
    ...
    try:
        ...
        print("\n 所有示例运行完成！")
        print("[OK] TaskOrion Editor 命令模式功能验证成功")
