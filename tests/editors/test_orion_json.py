...
"""
Test script for TaskOrion JSON serialization/deserialization.

This script tests the to_json() and from_json() methods of TaskOrion
...
class MinimalTaskOrion:
    def __init__(
        ...
        **kwargs,
        ...
        self._orion_id = (
            orion_id or f"orion_{str(uuid.uuid4())[:8]}"
        )
        self._name = name or self._orion_id
    ...
    def to_json(self, save_path: Optional[str] = None) -> str:
        ...
        json_str = json.dumps(orion_dict, indent=2, ensure_ascii=False)
...
def test_task_orion_json():
    """Test TaskOrion JSON functionality."""
    print("=" * 70)
    print("Testing TaskOrion JSON Serialization/Deserialization")
    print("=" * 70)

    try:
        # Create a orion with tasks and dependencies
        orion = MinimalTaskOrion(
            name="Test Orion", enable_visualization=False
        )
        ...
        orion._metadata = {
            ...
            "version": "1.0",
            "description": "Test orion for JSON operations",
        }
        ...
        # Add dependencies to orion
        orion.add_dependency(dep1)
        orion.add_dependency(dep2)

        print(f"Created TaskOrion:")
        print(f"  ID: {orion._orion_id}")
        print(f"  Name: {orion._name}")
        print(f"  State: {orion._state}")
        print(f"  Tasks: {len(orion._tasks)}")
        print(f"  Dependencies: {len(orion._dependencies)}")
        ...
        assert (
            restored_orion._orion_id == orion._orion_id
        )
        assert restored_orion._name == orion._name
        assert restored_orion._state == orion._state
        assert len(restored_orion._tasks) == len(orion._tasks)
        ...
        for task_id, original_task in orion._tasks.items():
            restored_task = restored_orion._tasks[task_id]
        ...
        for dep_id, original_dep in orion._dependencies.items():
            restored_dep = restored_orion._dependencies[dep_id]
        ...
        complex_json = orion.to_json()
        complex_restored = MinimalTaskOrion.from_json(json_data=complex_json)
...
def test_error_handling():
    """Test error handling in TaskOrion JSON operations."""
    print("\n" + "=" * 70)
...
def main():
    """Run all tests."""
    print("Starting TaskOrion JSON Tests")
    print("=" * 70)

    all_passed = True

    if not test_task_orion_json():
        all_passed = False
