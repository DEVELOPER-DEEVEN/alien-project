#!/usr/bin/env python3
"""
Simplified test script for the network Editor MCP Server.
...
def test_basic_operations():
    ...
    # Test 6: Build network
    print("\n6. Testing build_network...")
    total_tests += 1
    try:
        config = {
            "tasks": [
        ...
        result = call_tool("build_network", config)
        network = json.loads(result)
        print(
            f"   ✓ Built network with {len(network['tasks'])} total tasks"
        )
        success_count += 1
    except Exception as e:
...
def main():
    ...
    print("Testing network Editor MCP Server (Basic Operations)")
    print("=" * 70)
