...
def demo_validation_rollback():
    ...
    print(
        f"\n当前状态: {len(network.tasks)} 个任务, {len(network.dependencies)} 个依赖"
    )
    ...
    print(
        f"\n失败操作后: {len(network.tasks)} 个任务, {len(network.dependencies)} 个依赖"
    )
    is_valid, errors = network.validate_dag()
    ...
    print(
        f"\n最终状态: {len(network.tasks)} 个任务, {len(network.dependencies)} 个依赖"
    )
    is_valid, errors = network.validate_dag()
    print(f"   状态: {'有效' if is_valid else '无效'}")
...
def demo_advanced_features():
    ...
    print("\n测试撤销/重做:")
    print(f"   执行前: {len(network.tasks)} 个任务")
    print(f"   可撤销: {editor.can_undo()}")

    if editor.can_undo():
        editor.undo()
        print(f"   撤销后: {len(network.tasks)} 个任务")
        print(f"   可重做: {editor.can_redo()}")

        if editor.can_redo():
            editor.redo()
            print(f"   重做后: {len(network.tasks)} 个任务")
...
def main():
    """主程序演示所有功能"""
    print("Alien network Editor 更新功能演示")
    print("=" * 60)
