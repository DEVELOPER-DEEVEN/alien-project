#!/usr/bin/env python3
"""
Test script for the network Editor MCP Server.
...
from Alien.client.mcp.local_servers.network_mcp_server import (
    create_network_mcp_server,
)


def test_mcp_server():
    """Test all MCP server tools."""
    print("=== Testing network Editor MCP Server ===")

    # Create the MCP server instance
    mcp_server = create_network_mcp_server()
    ...
    # Test 7: Get network status
    print("\n7. Testing get_network_status...")
    try:
        status_result = call_tool("get_network_status")
    ...
    # Test 8: Build network
    print("\n8. Testing build_network...")
    config = {
        "tasks": [
            {
                "task_id": "batch_task1",
        ...
        "metadata": {"batch_created": True, "test_network": True},
    }

    try:
        build_result = call_tool("build_network", config, False)
        built = json.loads(build_result)
        print(f"   ✓ Built network with {len(built['tasks'])} total tasks")
    except Exception as e:
    ...
    try:
        ...
        status_after_undo = json.loads(call_tool("get_network_status"))
    ...
    try:
        ...
        status_after_redo = json.loads(call_tool("get_network_status"))
    ...
    # Test 11: Clear network
    print("\n11. Testing clear_network...")
    try:
        clear_result = call_tool("clear_network")
        cleared = json.loads(clear_result)
        print(f"   ✓ Cleared network, remaining tasks: {len(cleared['tasks'])}")
    except Exception as e:
        print(f"   ✗ Failed to clear network: {e}")
        return False
