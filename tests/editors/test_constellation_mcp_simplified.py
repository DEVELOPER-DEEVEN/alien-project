...
def test_mcp_server():
    ...
    # Test 5: Build network (batch operations)
    print("\n5. Testing build_network...")
    try:
        config = {
            "tasks": [
                {
        ...
        build_result = call_tool("build_network", config)
        built = json.loads(build_result)
        print(f"   ✓ Built network with {len(built['tasks'])} tasks")
    except Exception as e:
    ...
    try:
        # Get current network to find valid dependency IDs
        network_result = call_tool(
            "build_network", {"tasks": [], "dependencies": [], "metadata": {}}
        )
        network = json.loads(network_result)

        # Find a dependency to remove
        valid_dep_id = None
        for dep_key in network.get("dependencies", {}):
...
def main():
    ...
    print("Testing network Editor MCP Server (Simplified)")
    print("=" * 60)
