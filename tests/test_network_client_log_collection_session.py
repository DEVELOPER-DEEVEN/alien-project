...
"""
Test NetworkClient with Mock AgentProfile for Log Collection Scenario

This test demonstrates using NetworkClient with the mock AgentProfile objects
...
class TestNetworkClientLogCollectionSession:
    """Test NetworkClient with mock AgentProfile for log collection scenario."""
    ...
    def mock_orion_config(
        ...
        return OrionConfig(
            orion_id="log_collection_test_orion",
    ...
    def mock_network_session(self):
        ...
        # Mock orion
        mock_orion = Mock()
        mock_orion.orion_id = "test_orion_001"
        mock_orion.name = "Log Collection Test Orion"
        mock_orion.tasks = [
            "collect_logs_linux1",
            "collect_logs_linux2",
            "generate_excel",
        ]
        mock_orion.dependencies = ["collect_logs -> generate_excel"]
    ...
    @pytest.mark.asyncio
    async def test_network_client_initialization_with_mock_devices(
        self, mock_orion_config: OrionConfig, temp_output_dir: Path
    ):
        """Test NetworkClient initialization with mock devices."""
        with patch(
            "alien.network.network_client.OrionConfig.from_yaml"
        ) as mock_from_yaml:
            mock_from_yaml.return_value = mock_orion_config
            ...
            assert client._device_config == mock_orion_config
    ...
    @pytest.mark.asyncio
    async def test_network_client_full_initialization(
        self,
        mock_orion_config: OrionConfig,
        mock_orion_client,
        temp_output_dir: Path,
    ):
        """Test NetworkClient full initialization with mocked components."""
        with patch(
            "alien.network.network_client.OrionConfig.from_yaml"
            ...
            # Verify OrionClient was created and initialized
            mock_client_class.assert_called_once_with(config=mock_orion_config)
    ...
    async def test_process_log_collection_request(
        ...
        with patch(
            ...
            # Verify NetworkSession was created with correct parameters
            mock_session_class.assert_called_once()
            call_args = mock_session_class.call_args
            assert call_args[1]["task"] == "log_collection_and_excel_generation"
            assert call_args[1]["client"] == mock_orion_client
            ...
            # Verify orion info in result
            orion_info = result["orion"]
            assert orion_info["id"] == "test_orion_001"
            assert orion_info["name"] == "Log Collection Test Orion"
            assert orion_info["task_count"] == 3
            assert orion_info["state"] == "completed"

    @pytest.mark.asyncio
    async def test_network_client_device_availability_check(
        self,
        mock_orion_config: OrionConfig,
        mock_orion_client,
        temp_output_dir: Path,
    ):
        """Test that NetworkClient can access all required devices for log collection."""
        with patch(
            "alien.network.network_client.OrionConfig.from_yaml"
        ) as mock_from_yaml, patch(
            "alien.network.network_client.OrionClient"
            ...
            # Initialize client
            client = NetworkClient(output_dir=str(temp_output_dir))
            await client.initialize()

            # Verify client has access to orion client
            assert client._client == mock_orion_client
    ...
    @pytest.mark.asyncio
    async def test_network_client_error_handling(
        self,
        mock_orion_config: OrionConfig,
        mock_orion_client,
        temp_output_dir: Path,
    ):
        """Test NetworkClient error handling during request processing."""
        with patch(
            "alien.network.network_client.OrionConfig.from_yaml"
        ) as mock_from_yaml, patch(
            "alien.network.network_client.OrionClient"
    ...
    @pytest.mark.asyncio
    async def test_network_client_shutdown(
        self,
        mock_orion_config: OrionConfig,
        mock_orion_client,
        mock_network_session,
        ...
        """Test NetworkClient proper shutdown."""
        with patch(
            "alien.network.network_client.OrionConfig.from_yaml"
        ) as mock_from_yaml, patch(
            "alien.network.network_client.OrionClient"
    ...
    def test_network_client_session_name_generation(self):
        """Test automatic session name generation."""
        # Test with custom session name
        client1 = NetworkClient(session_name="custom_session")
        assert client1.session_name == "custom_session"

        # Test with auto-generated session name
        client2 = NetworkClient()
        assert client2.session_name.startswith("network_session_")
        assert len(client2.session_name) > len("network_session_")
    ...
    async def test_request_without_initialization_error(self):
        ...
        with pytest.raises(RuntimeError, match="Network client not initialized"):
