...
This test suite verifies the fix for the issue where orion client
would incorrectly report success when the target device is not connected to
the ALIEN server.

Scenario:
1. Orion client attempts to register with server
...
Before fix:
- _register_orion_client() always returned True
...
After fix:
- _register_orion_client() waits for server response
...
from network.client.components import DeviceStatus, AgentProfile
