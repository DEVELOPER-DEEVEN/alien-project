...
"""
Unit tests for NetworkClient task cancellation mechanism.
...
async def mock_client():
    """Create a mock NetworkClient for testing."""
...
async def test_process_request_saves_task_reference(mock_client):
    ...
    with patch.object(NetworkSession, "__init__", return_value=None), patch.object(
        NetworkSession, "run", new_callable=AsyncMock
    ), patch.object(NetworkSession, "_cleanup_observers", return_value=None):
        ...
        with patch.object(mock_client, "_session", mock_session):
            with patch("network.network_client.NetworkSession", return_value=mock_session):
...
async def test_process_request_clears_task_reference_on_completion(mock_client):
    ...
    with patch.object(NetworkSession, "__init__", return_value=None), patch.object(
        ...
        mock_session._rounds = []
        mock_session.log_path = "/tmp/test"
        mock_session.current_orion = None
...
async def test_process_request_clears_task_reference_on_error(mock_client):
    ...
    with patch.object(NetworkSession, "__init__", return_value=None), patch.object(
        NetworkSession, "run", side_effect=RuntimeError("Test error")
    ), patch.object(NetworkSession, "_cleanup_observers", return_value=None):
