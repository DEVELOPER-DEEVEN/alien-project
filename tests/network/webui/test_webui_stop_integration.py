...
Tests the complete flow from WebUI Stop button through to Network client restart.
"""
...
async def mock_app_state():
    """Create a mock AppState with NetworkClient for testing."""
    app_state = AppState()

    with patch("network.network_client.get_network_config"), patch(
        "network.network_client.OrionConfig"
    ), patch("network.network_client.setup_logger"):
        ...
        app_state.network_client = client
...
@pytest_asyncio.fixture
def network_service(mock_app_state):
    """Create NetworkService with mocked app state."""
    service = NetworkService(app_state=mock_app_state)
    return service


@pytest.mark.asyncio
async def test_stop_task_and_restart_full_flow(network_service, mock_app_state):
    ...
    with patch.object(
        mock_app_state.network_client, "create_next_session", new_callable=AsyncMock
        ...
        # Act
        result = await network_service.stop_task_and_restart()

        # Assert
        # 1. shutdown(force=True) 应该被调用
        mock_app_state.network_client._client.shutdown.assert_called_once()
        mock_app_state.network_client._session.force_finish.assert_called()

        # 2. initialize 应该被调用
        mock_app_state.network_client._client.initialize.assert_called_once()
...
@pytest.mark.asyncio
async def test_stop_task_cancels_running_task(network_service, mock_app_state):
    ...
    mock_app_state.network_client._current_request_task = mock_task
...
@pytest.mark.asyncio
async def test_stop_task_without_active_client(network_service, mock_app_state):
    """Test stop_task_and_restart when no Network client is active."""
    # Arrange
    mock_app_state.network_client = None

    # Act & Assert
    with pytest.raises(ValueError, match="Network client not initialized"):
        await network_service.stop_task_and_restart()


@pytest.mark.asyncio
async def test_stop_task_handles_shutdown_error(network_service, mock_app_state):
    """Test that stop_task_and_restart handles shutdown errors gracefully."""
    # Arrange
    mock_app_state.network_client._session.force_finish.side_effect = RuntimeError(
        "Shutdown error"
    )
...
@pytest.mark.asyncio
async def test_stop_task_handles_initialization_error(network_service, mock_app_state):
    """Test that stop_task_and_restart handles initialization errors."""
    # Arrange
    mock_app_state.network_client._client.initialize.side_effect = RuntimeError(
        "Init error"
    )
...
@pytest.mark.asyncio
async def test_stop_task_resets_counter_even_on_error(network_service, mock_app_state):
...
@pytest.mark.asyncio
async def test_stop_task_with_no_running_task(network_service, mock_app_state):
    """Test stop_task_and_restart when no task is currently running."""
    # Arrange
    mock_app_state.network_client._current_request_task = None
...
@pytest.mark.asyncio
async def test_stop_task_shutdown_uses_force_true(network_service, mock_app_state):
    """Test that stop_task_and_restart calls shutdown with force=True."""
    # Arrange
    original_shutdown = mock_app_state.network_client.shutdown
    ...
    with patch.object(
        mock_app_state.network_client, "shutdown", side_effect=track_shutdown_call
    ):
        with patch.object(
            mock_app_state.network_client, "create_next_session", new_callable=AsyncMock
        ) as mock_create_session:
...
async def test_is_client_available_returns_correct_status(
    network_service, mock_app_state
):
    """Test is_client_available method."""
    # When client exists
    assert network_service.is_client_available() is True

    # When client is None
    mock_app_state.network_client = None
    assert network_service.is_client_available() is False
