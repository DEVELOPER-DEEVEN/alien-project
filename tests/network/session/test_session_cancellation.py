...
"""
Unit tests for NetworkSession cancellation mechanism.
...
def mock_session():
    """Create a mock NetworkSession for testing."""
    with patch("network.session.network_session.get_network_config"), patch(
        "network.session.network_session.utils"
    ), patch("network.session.network_session.get_event_bus"):
...
async def test_request_cancellation_sets_flags(mock_session):
    ...
    # Arrange
    mock_orion = MagicMock(spec=TaskOrion)
    mock_orion.orion_id = "test_orion_123"
...
@pytest.mark.asyncio
async def test_request_cancellation_without_orion(mock_session):
    """Test request_cancellation when no orion is active."""
    # Arrange
    ...
    assert mock_session._cancellation_requested is True
    assert mock_session._finish is True
    # 不应该调用 cancel_execution（因为没有 orion）
    mock_session._orchestrator.cancel_execution.assert_not_called()


@pytest.mark.asyncio
async def test_round_checks_cancellation_flag():
    """Test that NetworkRound checks cancellation flag during execution."""
    # Arrange
    with patch("network.session.network_session.get_network_config"):
        mock_agent = MagicMock(spec=OrionAgent)
...
async def test_create_new_round_passes_session_reference(mock_session):
    """Test that create_new_round properly passes session reference to NetworkRound.

    This is a regression test for the bug where NetworkRound didn't receive
    ...
    ), "NetworkRound._session should reference parent session"
