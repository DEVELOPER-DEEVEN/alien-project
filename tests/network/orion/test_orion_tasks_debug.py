...
def debug_tasks_field(log_file_path: str):
    """Debug the tasks field in orion_before"""
    ...
    if "orion_before" in log_entry and log_entry["orion_before"]:
        const_before_str = log_entry["orion_before"]
        const_before = json.loads(const_before_str)

        print("orion_before structure:")
        print(f"  orion_id: {const_before.get('orion_id')}")
    ...
    if "orion_after" in log_entry and log_entry["orion_after"]:
        const_after_str = log_entry["orion_after"]
        const_after = json.loads(const_after_str)

        print("orion_after (line 1) structure:")
        print(f"  orion_id: {const_after.get('orion_id')}")
