...
"""
Test script to validate the refactored network Agent strategies and prompters.
...
def test_strategy_factory():
    """Test the networkStrategyFactory."""
    print("Testing networkStrategyFactory...")

    # Test creation mode strategies
    print("  Testing CREATION mode strategies...")
    creation_strategies = networkStrategyFactory.create_all_strategies(
    ...
    editing_strategies = networkStrategyFactory.create_all_strategies(
    ...
    supported_modes = networkStrategyFactory.get_supported_weaving_modes()
    ...
    print("  ✓ networkStrategyFactory tests passed!")


def test_prompter_factory():
    """Test the networkPrompterFactory."""
    print("Testing networkPrompterFactory...")
    ...
    # Test supported modes
    supported_modes = networkPrompterFactory.get_supported_weaving_modes()
    assert WeavingMode.CREATION in supported_modes
    assert WeavingMode.EDITING in supported_modes
    print(f"    ✓ Supported modes: {[mode.value for mode in supported_modes]}")

    print("  ✓ networkPrompterFactory tests passed!")
...
def test_strategy_types():
    ...
    creation_llm = networkStrategyFactory.create_llm_interaction_strategy(
        WeavingMode.CREATION
    )
    editing_llm = networkStrategyFactory.create_llm_interaction_strategy(
        WeavingMode.EDITING
    )

    creation_action = networkStrategyFactory.create_action_execution_strategy(
        WeavingMode.CREATION
    )
    editing_action = networkStrategyFactory.create_action_execution_strategy(
        WeavingMode.EDITING
    )
...
def main():
    """Run all tests."""
    print("=" * 60)
    print("Testing Refactored network Agent Architecture")
    print("=" * 60)
