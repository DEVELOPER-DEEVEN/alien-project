...
"""
Test cases for the refactored network Agent factory and strategy pattern implementation.
...
from cluster.agents.processors.strategies.network_factory import (
    networkStrategyFactory,
    networkPrompterFactory,
)
from cluster.agents.processors.strategies.base_network_strategy import (
    BasenetworkLLMInteractionStrategy,
    BasenetworkActionExecutionStrategy,
)
from cluster.agents.processors.strategies.network_creation_strategy import (
    networkCreationLLMInteractionStrategy,
    networkCreationActionExecutionStrategy,
)
from cluster.agents.processors.strategies.network_editing_strategy import (
    networkEditingLLMInteractionStrategy,
    networkEditingActionExecutionStrategy,
)
from cluster.agents.prompters.base_network_prompter import (
    BasenetworkPrompter,
)
from cluster.agents.prompters.network_creation_prompter import (
    networkCreationPrompter,
)
from cluster.agents.prompters.network_editing_prompter import (
...
class TestnetworkStrategyFactory:
    """Test cases for networkStrategyFactory."""

    def test_create_llm_interaction_strategy_creation_mode(self):
        """Test creating LLM interaction strategy for CREATION mode."""
        strategy = networkStrategyFactory.create_llm_interaction_strategy(
            WeavingMode.CREATION
        )

        assert isinstance(strategy, networkCreationLLMInteractionStrategy)
        assert isinstance(strategy, BasenetworkLLMInteractionStrategy)
        assert strategy.name == "network_llm_interaction_creation"
    ...
    def test_create_llm_interaction_strategy_editing_mode(self):
        ...
        assert isinstance(strategy, networkEditingLLMInteractionStrategy)
        assert isinstance(strategy, BasenetworkLLMInteractionStrategy)
        assert strategy.name == "network_llm_interaction_editing"

    def test_create_action_execution_strategy_creation_mode(self):
        """Test creating action execution strategy for CREATION mode."""
        strategy = networkStrategyFactory.create_action_execution_strategy(
            WeavingMode.CREATION
        )

        assert isinstance(strategy, networkCreationActionExecutionStrategy)
        assert isinstance(strategy, BasenetworkActionExecutionStrategy)
        assert strategy.name == "network_action_execution_creation"
    ...
    def test_create_action_execution_strategy_editing_mode(self):
        ...
        assert isinstance(strategy, networkEditingActionExecutionStrategy)
        assert isinstance(strategy, BasenetworkActionExecutionStrategy)
        assert strategy.name == "network_action_execution_editing"
    ...
    def test_unsupported_weaving_mode_llm_interaction(self):
        ...
        with pytest.raises(ValueError, match="Unsupported weaving mode"):
            networkStrategyFactory.create_llm_interaction_strategy("INVALID_MODE")
    ...
    def test_get_all_strategies_creation_mode(self):
        ...
        strategies = networkStrategyFactory.create_all_strategies(
            WeavingMode.CREATION
        )
...
class TestnetworkPrompterFactory:
    """Test cases for networkPrompterFactory."""
    ...
    def test_create_prompter_creation_mode(self):
        ...
        prompter = networkPrompterFactory.create_prompter(
            WeavingMode.CREATION,
            main_prompt,
            example_prompt,
            example_prompt,
        ...
        assert isinstance(prompter, networkCreationPrompter)
        assert isinstance(prompter, BasenetworkPrompter)
    ...
    def test_create_prompter_editing_mode(self):
        ...
        assert isinstance(prompter, networkEditingPrompter)
        assert isinstance(prompter, BasenetworkPrompter)

    def test_unsupported_weaving_mode_prompter(self):
        """Test that unsupported weaving mode raises ValueError for prompter."""
        with pytest.raises(ValueError, match="Unsupported weaving mode"):
            networkPrompterFactory.create_prompter(
                "INVALID_MODE", "prompt", "example", "example", "example"
            )


class TestBasenetworkStrategy:
    """Test cases for base network strategies."""

    @pytest.fixture
    def mock_agent(self):
        """Create a mock networkAgent."""
        agent = Mock()
    ...
    def test_base_llm_interaction_strategy_inheritance(self):
        ...
        strategy = networkCreationLLMInteractionStrategy()
        assert isinstance(strategy, BasenetworkLLMInteractionStrategy)
...
class TestStrategyBehaviorDifferentiation:
    ...
    @pytest.mark.asyncio
    @patch("Alien.cluster.agents.processors.strategies.base_network_strategy.json")
    async def test_creation_vs_editing_llm_strategies_different_behavior(
        ...
        editing_strategy = networkStrategyFactory.create_llm_interaction_strategy(
            WeavingMode.EDITING
        )
...
class TestPrompterBehaviorDifferentiation:
    ...
    def test_creation_vs_editing_prompters_different_behavior(self):
        ...
        # Create prompters
        creation_prompter = networkPrompterFactory.create_prompter(
            WeavingMode.CREATION,
            main_prompt,
            example_prompt,
            example_prompt,
        ...
        editing_prompter = networkPrompterFactory.create_prompter(
            WeavingMode.EDITING,
            main_prompt,
            example_prompt,
            example_prompt,
