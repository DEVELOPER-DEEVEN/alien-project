...
"""
Unit tests for updated networkProgressObserver
...
from cluster.core.events import TaskEvent, networkEvent, EventType
from Alien.module.context import Context


class TestnetworkProgressObserver:
    """Test the refactored networkProgressObserver."""
    ...
    def observer(self, mock_agent, mock_context):
        ...
        return networkProgressObserver(mock_agent, mock_context)
    ...
    async def test_agent_without_queue(self, mock_context):
        ...
        agent_no_queue = MockclusterWeaverAgent()
        delattr(agent_no_queue, "task_completion_queue")  # Remove queue
        agent_no_queue.logger = Mock()

        observer = networkProgressObserver(agent_no_queue, mock_context)
    ...
    @pytest.mark.asyncio
    async def test_network_event_handling(self, observer):
        ...
        # Handle the event
        await observer._handle_network_event(network_event)

        # Test that no exception was raised (the main goal of network event handling)
    ...
    @pytest.mark.asyncio
    async def test_network_event_exception_handling(self, observer):
        """Test exception handling in network event processing."""
        ...
        try:
            ...
            pytest.fail("network event handling should not raise exceptions")
    ...
    async def test_on_event_routing(self, observer, mock_agent):
        ...
        await observer.on_event(network_event)
...
class TestObserverIntegrationWithAgent:
    ...
    async def test_end_to_end_event_flow(self, integrated_setup):
        ...
        from cluster.agents.cluster_agent_states import MonitorclusterAgentState

        state = MonitorclusterAgentState()
        await state.handle(agent, context)

        # Verify agent processed the event
        agent.update_network_with_lock.assert_called_once()
        call_args = agent.update_network_with_lock.call_args[1]
