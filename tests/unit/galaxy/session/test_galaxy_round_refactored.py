...
"""
Unit tests for refactored clusterRound with state machine integration.

Tests cover the integration between clusterRound and the agent state machine,
...
from cluster.session.cluster_session import clusterRound
...
def mock_agent():
    ...
    agent = Mock()  # Use plain Mock instead of MockclusterWeaverAgent
...
class TestclusterRoundStateMachine:
    """Test clusterRound integration with agent state machine."""

    @pytest.fixture
    def cluster_round(self, mock_agent, mock_orchestrator, mock_context):
        """Create clusterRound for testing."""
        return clusterRound(
    ...
    async def test_round_initialization(
        self, cluster_round, mock_agent, mock_orchestrator
    ):
        """Test clusterRound initialization."""
        assert cluster_round._agent == mock_agent
        assert cluster_round._orchestrator == mock_orchestrator
        assert cluster_round._request == "Test request"
        assert cluster_round._id == 1

    @pytest.mark.asyncio
    async def test_successful_round_execution(
        self, cluster_round, mock_agent, simple_network
    ):
    ...
    @pytest.mark.asyncio
    async def test_round_execution_with_error(self, cluster_round, mock_agent):
    ...
    @pytest.mark.asyncio
    async def test_round_no_final_network(self, cluster_round, mock_agent):
        """Test round execution when no network is created."""
        # Setup no network scenario
        mock_agent._current_network = None
    ...
    @pytest.mark.asyncio
    async def test_round_properties(self, cluster_round, simple_network):
        """Test clusterRound properties."""
        # Test initial state
        assert cluster_round.network is None
        assert cluster_round.task_results == {}

        # Set network
        cluster_round._network = simple_network
        assert cluster_round.network == simple_network

    @pytest.mark.asyncio
    async def test_check_for_new_tasks(self, cluster_round, simple_network):
        """Test _check_for_new_tasks method."""
        # Test with no network
        await cluster_round._check_for_new_tasks()
        ...
        simple_network.get_ready_tasks = Mock(return_value=[ready_task])

        with patch.object(cluster_round, "logger") as mock_logger:
            await cluster_round._check_for_new_tasks()
            mock_logger.info.assert_called_with("New ready task detected: new_task")


class TestclusterRoundObserverIntegration:
    """Test clusterRound integration with observers."""

    @pytest.fixture
    def cluster_round_with_observers(self, mock_agent, mock_orchestrator, mock_context):
        """Create clusterRound with observers setup."""
    ...
    @pytest.mark.asyncio
    async def test_observer_setup(self, cluster_round_with_observers):
        """Test that observers are properly set up."""
        # Verify observers were created
        assert len(cluster_round_with_observers._observers) == 3

        # Verify observer types
        observer_types = [
            type(obs).__name__ for obs in cluster_round_with_observers._observers
        ]
        assert "networkProgressObserver" in observer_types
        assert "SessionMetricsObserver" in observer_types
        assert "DAGVisualizationObserver" in observer_types

    @pytest.mark.asyncio
    async def test_observer_subscription(self, cluster_round_with_observers):
        """Test that observers are subscribed to event bus."""
        # Mock event bus
        with patch.object(
            cluster_round_with_observers._event_bus, "subscribe"
        ) as mock_subscribe:
            # Re-setup observers to test subscription
            cluster_round_with_observers._setup_observers()

            # Verify subscription calls
            assert mock_subscribe.call_count == len(
                cluster_round_with_observers._observers
            )


class TestclusterRoundErrorScenarios:
    """Test error scenarios in clusterRound."""

    @pytest.fixture
    def error_round(self, mock_agent, mock_orchestrator, mock_context):
        """Create clusterRound for error testing."""
        return clusterRound(
    ...
    async def test_context_update_exception(
        self, error_round, mock_agent, simple_network
    ):
...
class TestclusterRoundAsyncBehavior:
    """Test async behavior and timing in clusterRound."""
