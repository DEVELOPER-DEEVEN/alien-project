...
"""
Unit tests for the event system in cluster framework.

This module tests the event bus, observers, and event-driven orchestration
functionality to ensure proper        # Create generic event (not TaskEvent or networkEvent)
        generic_event = Event(
...
class TestEventTypes:
    ...
    def test_task_event_creation(self):
        ...
        assert event.data["network_id"] == "test_network"

    def test_network_event_creation(self):
        """Test networkEvent creation and properties."""
        event = networkEvent(
        ...
        assert event.event_type == EventType.network_COMPLETED
        assert event.network_id == "network_789"
        assert event.network_state == "completed"
        assert event.data["total_tasks"] == 5


class TestnetworkProgressObserver:
    """Test cases for networkProgressObserver."""
    ...
    async def test_task_progress_handling(self):
        ...
        observer = networkProgressObserver(
            agent=mock_agent,
    ...
    @pytest.mark.asyncio
    async def test_network_event_handling(self):
        """Test handling of network events."""
        # Create mock agent and context
        mock_agent = Mock()
        mock_agent.task_completion_queue = AsyncMock()
        mock_context = Mock()
        ...
        # Verify network event was handled (no errors should occur)
        # The current implementation logs but doesn't queue network events
    ...
    async def test_irrelevant_event_filtering(self):
        ...
        # Create generic event (not task or network specific)
        generic_event = Event(
            event_type=EventType.TASK_STARTED,  # This should not match
            source_id="other_source",
            timestamp=time.time(),
...
class TestEventSystemIntegration:
    ...
    async def test_end_to_end_event_flow(self):
        ...
        progress_observer = networkProgressObserver(
            agent=mock_agent,
            context=mock_context,
        )
