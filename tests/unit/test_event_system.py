...
"""
Unit tests for the event system in Network framework.

This module tests the event bus, observers, and event-driven orchestration
functionality to ensure proper        # Create generic event (not TaskEvent or OrionEvent)
        generic_event = Event(
...
class TestEventTypes:
    ...
    def test_task_event_creation(self):
        ...
        assert event.data["orion_id"] == "test_orion"

    def test_orion_event_creation(self):
        """Test OrionEvent creation and properties."""
        event = OrionEvent(
        ...
        assert event.event_type == EventType.ORION_COMPLETED
        assert event.orion_id == "orion_789"
        assert event.orion_state == "completed"
        assert event.data["total_tasks"] == 5


class TestOrionProgressObserver:
    """Test cases for OrionProgressObserver."""
    ...
    @pytest.mark.asyncio
    async def test_orion_event_handling(self):
        """Test handling of orion events."""
        # Create mock agent and context
        mock_agent = Mock()
        mock_agent.task_completion_queue = AsyncMock()
        mock_context = Mock()
        ...
        # Verify orion event was handled (no errors should occur)
        # The current implementation logs but doesn't queue orion events
    ...
    async def test_irrelevant_event_filtering(self):
        ...
        # Create generic event (not task or orion specific)
        generic_event = Event(
            event_type=EventType.TASK_STARTED,  # This should not match
            source_id="other_source",
            timestamp=time.time(),
...
class TestEventSystemIntegration:
    ...
    async def test_end_to_end_event_flow(self):
        ...
        progress_observer = OrionProgressObserver(
            agent=mock_agent,
            context=mock_context,
        )
