...
class TestnetworkAgentPresentation(unittest.TestCase):
    """Tests for networkAgent-specific presentation"""

    @patch('Alien.agents.presenters.rich_presenter.Console')
    def test_present_network_agent_response(self, mock_console_class):
        """Test presenting networkAgent response"""
        mock_console = MagicMock()
        mock_console_class.return_value = mock_console
        ...
        mock_response.thought = "Test thought"
        mock_response.status = "CONTINUE"
        mock_response.network = None
        mock_response.action = None
        mock_response.results = None
        
        presenter.present_network_agent_response(mock_response, print_action=False)
    ...
    @patch('Alien.agents.presenters.rich_presenter.Console')
    def test_present_network_with_tasks(self, mock_console_class):
        """Test presenting network with tasks and dependencies"""
        mock_console = MagicMock()
        mock_console_class.return_value = mock_console
        ...
        # Create mock network
        mock_network = Mock()
        mock_network.network_id = "test-network-123"
        mock_network.name = "Test network"
        mock_network.state = "PENDING"
        ...
        mock_network.tasks = {"task-1": mock_task}
        ...
        mock_network.dependencies = {"dep-1": mock_dep}
        
        # Create mock response
        mock_response = Mock()
        mock_response.thought = "Creating network"
        mock_response.status = "START"
        mock_response.network = mock_network
        mock_response.action = None
        mock_response.results = None
        ...
        # Verify console.print was called multiple times
        self.assertTrue(mock_console.print.called)
        # Should print: thought, status, network info, task details, dependencies
        self.assertGreaterEqual(mock_console.print.call_count, 5)
...
class TestnetworkEditingActionsPresentation(unittest.TestCase):
    """Tests for network editing actions presentation"""

    @patch('Alien.agents.presenters.rich_presenter.Console')
    def test_present_network_editing_actions(self, mock_console_class):
        """Test presenting network editing actions"""
        mock_console = MagicMock()
        mock_console_class.return_value = mock_console
        ...
        presenter.present_network_editing_actions(mock_action_list)
        
        # Verify console.print was called
        self.assertTrue(mock_console.print.called)

    @patch('Alien.agents.presenters.rich_presenter.Console')
    def test_present_network_editing_actions_empty(self, mock_console_class):
        """Test presenting empty network editing actions"""
        mock_console = MagicMock()
        mock_console_class.return_value = mock_console
    ...
    def test_format_network_operation_add_task(self):
        ...
        result = presenter._format_network_operation(mock_action)
        self.assertIn("Add Task", result)
        self.assertIn("task-1", result)

    def test_format_network_operation_remove_task(self):
    ...
    def test_format_network_operation_add_dependency(self):
