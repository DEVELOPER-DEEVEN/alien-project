...
"""
Unit tests for updated OrionProgressObserver
...
from network.core.events import TaskEvent, OrionEvent, EventType
from alien.module.context import Context


class TestOrionProgressObserver:
    """Test the refactored OrionProgressObserver."""
    ...
    def observer(self, mock_agent, mock_context):
        ...
        return OrionProgressObserver(mock_agent, mock_context)
    ...
    async def test_agent_without_queue(self, mock_context):
        ...
        agent_no_queue = MockNetworkWeaverAgent()
        delattr(agent_no_queue, "task_completion_queue")  # Remove queue
        agent_no_queue.logger = Mock()

        observer = OrionProgressObserver(agent_no_queue, mock_context)
    ...
    @pytest.mark.asyncio
    async def test_orion_event_handling(self, observer):
        """Test orion event handling."""
        # Create orion event
        ...
        # Handle the event
        await observer._handle_orion_event(orion_event)

        # Test that no exception was raised (the main goal of orion event handling)
    ...
    @pytest.mark.asyncio
    async def test_orion_event_exception_handling(self, observer):
        """Test exception handling in orion event processing."""
        ...
        try:
            ...
            pytest.fail("Orion event handling should not raise exceptions")
    ...
    async def test_on_event_routing(self, observer, mock_agent):
        ...
        await observer.on_event(orion_event)
...
class TestObserverIntegrationWithAgent:
    ...
    def integrated_setup(self):
        ...
        observer = OrionProgressObserver(agent, context)
    ...
    async def test_end_to_end_event_flow(self, integrated_setup):
        ...
        from network.agents.network_agent_states import MonitorNetworkAgentState

        state = MonitorNetworkAgentState()
        await state.handle(agent, context)

        # Verify agent processed the event
        agent.update_orion_with_lock.assert_called_once()
        call_args = agent.update_orion_with_lock.call_args[1]
