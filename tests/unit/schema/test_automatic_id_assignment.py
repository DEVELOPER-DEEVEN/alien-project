...
This script tests the automatic generation of network_id, task_id, and line_id,
as well as the uniqueness validation within network contexts.
"""
...
def test_automatic_id_generation():
    ...
    # Test TasknetworkSchema automatic network_id generation
    network_data = {
        "name": "Auto network"
    }
    
    network_schema = TasknetworkSchema(**network_data)
    print(f"✅ TasknetworkSchema auto network_id: {network_schema.network_id}")
    assert network_schema.network_id is not None
    assert network_schema.network_id.startswith("network_")
...
def test_explicit_id_preservation():
    ...
    network_schema = TasknetworkSchema(
        network_id="explicit_network_001",
        name="Explicit network"
    )
    print(f"✅ Explicit network_id preserved: {network_schema.network_id}")
    assert network_schema.network_id == "explicit_network_001"
    
    return True


def test_uniqueness_validation():
    """Test uniqueness validation within network context."""
    ...
    # Create network with unique IDs
    try:
        ...
        print("✅ network with unique IDs created successfully")
    except Exception as e:
        print(f"❌ Failed to create network with unique IDs: {e}")
        return False
    ...
    try:
        ...
        bad_network = TasknetworkSchema(
            network_id="test_network_bad",
            name="Bad network",
            tasks={
                "unique_task_001": task1,
...
def test_id_manager_context():
    ...
    # Generate task IDs for network A
    task_id_1a = id_manager.generate_task_id("network_a")
    task_id_2a = id_manager.generate_task_id("network_a")
    
    # Generate task IDs for network B
    task_id_1b = id_manager.generate_task_id("network_b")
    task_id_2b = id_manager.generate_task_id("network_b")
    
    print(f"✅ network A task IDs: {task_id_1a}, {task_id_2a}")
    print(f"✅ network B task IDs: {task_id_1b}, {task_id_2b}")
    
    # Verify uniqueness within each network
    assert task_id_1a != task_id_2a
    assert task_id_1b != task_id_2b
    
    # Verify IDs can be same across different networks (they use different counters)
    print("✅ ID context separation working correctly")
    
    # Test availability check
    assert not id_manager.is_task_id_available("network_a", task_id_1a)
    assert id_manager.is_task_id_available("network_a", "unused_task_id")
...
def test_sequential_id_generation():
    """Test that IDs are generated sequentially within network context."""
    print("\n🧪 Testing sequential ID generation...")
    
    id_manager = IDManager()
    network_id = "seq_test_network"
    
    # Generate multiple task IDs
    task_ids = []
    for i in range(5):
        task_id = id_manager.generate_task_id(network_id)
    ...
    line_ids = []
    for i in range(3):
        line_id = id_manager.generate_line_id(network_id)
