...
"""
Test script for BaseModel integration with TaskStar, TaskStarLine, and Tasknetwork.

This script tests the serialization and deserialization functionality between
the network classes and their corresponding Pydantic BaseModel schemas.
"""
...
def test_task_network_basemodel():
    """Test Tasknetwork BaseModel integration."""
    print("\n🧪 Testing Tasknetwork BaseModel integration...")

    # Create a Tasknetwork with tasks and dependencies
    network = Tasknetwork(
    ...
    task1 = TaskStar(
        task_id="task_001",
        name="First Task",
        description="First task in network",
        device_type=DeviceType.WINDOWS,
    )
    ...
    task2 = TaskStar(
        task_id="task_002",
        name="Second Task",
        description="Second task in network",
        device_type=DeviceType.WINDOWS,
    )
    ...
    # Add metadata
    network.update_metadata(
        {"author": "test_user", "version": "1.0.0", "tags": ["test", "automation"]}
    )

    # Test to_basemodel
    schema = network.to_basemodel()
    print(f"✅ Tasknetwork to BaseModel: {type(schema).__name__}")
    assert isinstance(schema, TasknetworkSchema)
    assert schema.network_id == "test_network_001"
    assert schema.name == "Test network"
    assert len(schema.tasks) == 2
    assert len(schema.dependencies) == 1

    # Test from_basemodel
    network_restored = Tasknetwork.from_basemodel(schema)
    print(
        f"✅ Tasknetwork from BaseModel: {network_restored.network_id}"
    )
    assert network_restored.network_id == network.network_id
    assert network_restored.name == network.name
    assert len(network_restored.tasks) == 2
    assert len(network_restored.dependencies) == 1

    # Test JSON serialization roundtrip through BaseModel
    json_data = schema.model_dump_json()
    schema_restored = TasknetworkSchema.model_validate_json(json_data)
    network_final = Tasknetwork.from_basemodel(schema_restored)

    assert network_final.network_id == network.network_id
    assert network_final.name == network.name
    assert len(network_final.tasks) == len(network.tasks)
    print("✅ Tasknetwork JSON roundtrip successful")
...
def test_complex_scenario():
    ...
    network = Tasknetwork(
        network_id="complex_test_001", name="Complex Test network"
    )
    ...
    # Validate structure
    assert len(network_restored.tasks) == 3
    assert len(network_restored.dependencies) == 2

    # Check task details
    extraction_task = network_restored.get_task("data_extraction")
    ...
    processing_task = network_restored.get_task("data_processing")
...
def test_validation_and_error_handling():
    ...
    try:
        Tasknetwork.from_basemodel({"invalid": "dict"})
        assert False, "Should have raised ValueError"
    except ValueError as e:
        print("✅ Correctly caught invalid schema type for Tasknetwork")
...
def main():
    ...
    try:
        ...
        print("   ✅ Tasknetwork BaseModel integration")
