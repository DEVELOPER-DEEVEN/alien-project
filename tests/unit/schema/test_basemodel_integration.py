...
"""
Test script for BaseModel integration with TaskStar, TaskStarLine, and TaskOrion.

This script tests the serialization and deserialization functionality between
the orion classes and their corresponding Pydantic BaseModel schemas.
"""
...
def test_task_orion_basemodel():
    ...
    task1 = TaskStar(
        task_id="task_001",
        name="First Task",
        description="First task in orion",
        device_type=DeviceType.WINDOWS,
    )
    ...
    task2 = TaskStar(
        task_id="task_002",
        name="Second Task",
        description="Second task in orion",
        device_type=DeviceType.WINDOWS,
    )
    ...
    assert len(orion_restored.tasks) == 2
    assert len(orion_restored.dependencies) == 1

    # Test JSON serialization roundtrip through BaseModel
    json_data = schema.model_dump_json()
    schema_restored = TaskOrionSchema.model_validate_json(json_data)
    orion_final = TaskOrion.from_basemodel(schema_restored)
...
def test_complex_scenario():
    ...
    orion = TaskOrion(
        orion_id="complex_test_001", name="Complex Test Orion"
    )
    ...
    # Validate structure
    assert len(orion_restored.tasks) == 3
    assert len(orion_restored.dependencies) == 2

    # Check task details
    extraction_task = orion_restored.get_task("data_extraction")
    ...
    processing_task = orion_restored.get_task("data_processing")
