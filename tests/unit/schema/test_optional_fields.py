...
def test_optional_fields():
    ...
    # Test TasknetworkSchema with minimal required fields
    minimal_network_data = {
        "network_id": "minimal_network",
        "name": "Minimal network",
    }

    try:
        network_schema = TasknetworkSchema(**minimal_network_data)
        print("✅ TasknetworkSchema with minimal fields created successfully")
        print(f"   - created_at: {network_schema.created_at}")
        print(f"   - updated_at: {network_schema.updated_at}")
        assert network_schema.created_at is None
        assert network_schema.updated_at is None
    except Exception as e:
        print(f"❌ Failed to create TasknetworkSchema with minimal fields: {e}")
        return False
