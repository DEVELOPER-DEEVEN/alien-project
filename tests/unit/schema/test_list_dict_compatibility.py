...
"""
Test for List/Dict compatibility in TaskOrionSchema.
...
def test_tasks_and_dependencies_as_lists():
    ...
    assert isinstance(orion.tasks, dict), "Tasks 应该被转换为 Dict 格式"

    # 验证 dependencies 被转换为 Dict 格式
    assert isinstance(
        orion.dependencies, dict
    ...
    # 检查依赖 ID
    dep_ids = list(orion.dependencies.keys())
    print(f"   - 依赖 IDs: {dep_ids}")

    # 验证自动生成的 ID
    auto_generated_tasks = [
        task for task in orion.tasks.values() if task.name == "第三个任务"
    ]
...
def test_tasks_and_dependencies_as_dicts():
    ...
    # 验证格式保持为 Dict
    assert isinstance(orion.tasks, dict), "Tasks 应该保持 Dict 格式"
    assert isinstance(
        orion.dependencies, dict
...
def main():
    ...
    try:
        # 运行各项测试
        orion1 = test_tasks_and_dependencies_as_lists()
        orion2 = test_tasks_and_dependencies_as_dicts()
        mixed1, mixed2 = test_mixed_format_compatibility()
        orion3 = test_conversion_methods()
