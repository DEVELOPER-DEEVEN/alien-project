...
def test_tasks_and_dependencies_as_lists():
    ...
    print(f"✅ 星座创建成功: {network.name}")
    print(f"   - 星座 ID: {network.network_id}")
    print(f"   - 任务数量: {len(network.tasks)}")
    print(f"   - 依赖数量: {len(network.dependencies)}")

    # 验证 tasks 被转换为 Dict 格式
    assert isinstance(network.tasks, dict), "Tasks 应该被转换为 Dict 格式"
    ...
    auto_generated_tasks = [
        task for task in network.tasks.values() if task.name == "第三个任务"
    ]
...
def test_tasks_and_dependencies_as_dicts():
    ...
    # 验证格式保持为 Dict
    assert isinstance(network.tasks, dict), "Tasks 应该保持 Dict 格式"
    assert isinstance(
        network.dependencies, dict
...
def test_mixed_format_compatibility():
    ...
    print(
        f"✅ 混合格式1创建成功: tasks={type(network1.tasks).__name__}, dependencies={type(network1.dependencies).__name__}"
    )
    ...
    print(
        f"✅ 混合格式2创建成功: tasks={type(network2.tasks).__name__}, dependencies={type(network2.dependencies).__name__}"
    )
...
def main():
    """运行所有测试"""
    print("🎯 TasknetworkSchema List/Dict 兼容性测试")
    print("=" * 60)
    ...
    try:
        ...
        print("\n📊 测试统计:")
        print(
            f"   • 创建了 {len([network1, network2, mixed1, mixed2, network3, network4])} 个测试星座"
        )
