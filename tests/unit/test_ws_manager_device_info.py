...
from Alien.server.services.ws_manager import WSManager, ClientInfo
...
class TestWSManagerAgentProfile:
    ...
    def test_add_network_client_no_system_info(self):
        """Test that network clients don't get system info processing"""
        ws_manager = WSManager()
        mock_ws = Mock(spec=WebSocket)

        metadata = {
            "type": "network_client",
            "network_id": "const_001",
        }

        ws_manager.add_client(
            client_id="network_001",
            ws=mock_ws,
            client_type=ClientType.network,
            metadata=metadata,
        )

        client_info = ws_manager.online_clients["network_001"]
    ...
    def test_get_all_devices_info(self):
        ...
        # Add a network (should not be included)
        ws_manager.add_client(
            "network_001",
            mock_ws3,
            ClientType.network,
        ...
        assert "network_001" not in all_info
