...
"""
Real NetworkSession Integration Test with Mock AgentProfile

This test uses REAL NetworkSession.run() (not mocked) to test the complete
...
from network.core.types import ExecutionResult, TaskStatus


class TestRealNetworkSessionWithMockDevices:
    """Test real NetworkSession execution with mock AgentProfile to find bugs."""
    ...
    def _setup_comprehensive_logging(self):
        """Setup comprehensive logging for all network components."""
        ...
        loggers_to_configure = [
            "alien.network.session",
            "alien.network.agents",
            "alien.network.orion",
            "alien.network.client",
            "alien.network.core",
            "network.session",
            "network.agents",
            "network.orion",
    ...
    def mock_orion_client(
        ...
        """Create real OrionClient with mocked device manager."""
        # Create a real OrionClient but mock its device manager
        mock_client = Mock(spec=OrionClient)
        ...
        mock_client.get_orion_info.return_value = {
            "orion_id": "test_orion",
            "connected_devices": 3,
            "total_devices": 3,
        }
    ...
    @pytest.mark.asyncio
    async def test_real_network_session_execution_with_mock_devices(
        self,
        mock_orion_client,
        mock_linux_server_1: AgentProfile,
        mock_linux_server_2: AgentProfile,
        ...
        if orion:
            ...
            print(f"   Task count: {len(getattr(orion, 'tasks', []))}")
            print(
        ...
        if assign_task_calls > 0:
            ...
            for (
                ...
                mock_orion_client.device_manager.assign_task_to_device.call_args_list
            ):
                args, kwargs = call
                device_id = (
        ...
        # Check 2: Orion was created
        if not orion:
            issues_found.append("No orion was created")
        ...
        return {
            ...
            "orion_created": orion is not None,
    ...
    async def test_session_error_handling(
        self, mock_orion_client, temp_output_dir: Path
    ):
        ...
        try:
            ...
            original_assign_task = (
                mock_orion_client.device_manager.assign_task_to_device
            )
            ...
            mock_orion_client.device_manager.assign_task_to_device = AsyncMock(
