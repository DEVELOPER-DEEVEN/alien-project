...
"""
Comprehensive tests for OrionModificationSynchronizer.
...
def create_task_event(
    ...
    **extra_data
    ...
    return TaskEvent(
        ...
        task_id=task_id,
        status=status,
        data={"orion_id": orion_id, **extra_data},
    )
...
def create_orion_event(
    ...
    **extra_data
    ...
    """Helper function to create OrionEvent with correct parameters."""
    return OrionEvent(
...
def synchronizer(mock_orchestrator):
    ...
    return OrionModificationSynchronizer(
...
@pytest.fixture
def orion_modified_event():
    """Create a ORION_MODIFIED event."""
    return create_orion_event(
        task_id="task_1",
        orion_id="orion_123",
...
class TestBasicSynchronization:
    ...
    async def test_complete_modification(
        self, synchronizer, task_completed_event, orion_modified_event
    ):
        """Test that orion modified events complete pending modifications."""
        # Register pending modification
        await synchronizer.on_event(task_completed_event)
        assert synchronizer.has_pending_modifications()

        # Complete modification
        await synchronizer.on_event(orion_modified_event)
...
class TestRaceConditionPrevention:
    ...
    async def test_orchestrator_waits_for_modification(self, synchronizer):
        ...
        async def agent_flow():
            """Simulate agent modifying orion."""
...
class TestErrorRecovery:
    ...
    @pytest.mark.asyncio
    async def test_handle_missing_orion_id(self, synchronizer):
        """Test handling events with missing orion_id."""
        # Event without orion_id
        event = TaskEvent(
            event_type=EventType.TASK_COMPLETED,
            source_id="test",
            timestamp=time.time(),
    ...
    async def test_handle_missing_task_id(self, synchronizer):
        """Test handling orion events with missing on_task_id."""
        ...
        # Orion event without on_task_id
        mod_event = OrionEvent(
            event_type=EventType.ORION_MODIFIED,
            source_id="agent",
            timestamp=time.time(),
    ...
    async def test_sequential_task_execution(self, synchronizer):
        ...
        for task_id in tasks:
            ...
            task_event = TaskEvent(
                ...
                task_id=task_id,
                task_name=f"Task {task_id}",
                data={"orion_id": "sequential_orion"},
            )
            await synchronizer.on_event(task_event)
