...
"""
集成测试：Network状态机与观察者系统

测试范围：
1. NetworkRound与状态机集成
2. Observer与状态机的事件流
3. 完整的任务执行生命周期
4. 竞态条件解决验证
"""
...
from network.orion import TaskOrion, TaskStar, create_simple_orion
from network.core.events import EventType, TaskEvent, get_event_bus
...
class IntegrationTestHelper:
    ...
    @staticmethod
    def create_test_orion() -> TaskOrion:
        """创建测试用orion"""
        return create_simple_orion(
            task_descriptions=["Task 1", "Task 2", "Task 3"],
            orion_name="test_orion",
            sequential=True
        )
    
    @staticmethod
    def create_mock_client() -> OrionClient:
        """创建mock客户端"""
        client = MagicMock(spec=OrionClient)
        client.device_manager = MagicMock()
        return client
    
    @staticmethod
    def create_mock_orchestrator() -> TaskOrionOrchestrator:
        """创建mock编排器"""
        orchestrator = MagicMock(spec=TaskOrionOrchestrator)
        orchestrator.assign_devices_automatically = AsyncMock()
        orchestrator.orchestrate_orion = AsyncMock()
        return orchestrator


class TestNetworkRoundStateMachineIntegration:
    """测试NetworkRound与状态机集成"""
    ...
    def setup_method(self):
        ...
        # 创建测试orion
        self.test_orion = IntegrationTestHelper.create_test_orion()
        self.agent.process_initial_request = AsyncMock(return_value=self.test_orion)
    ...
    async def test_round_state_machine_execution(self):
        ...
        with patch.object(round_instance, 'is_finished') as mock_finished:
            ...
            self.orchestrator.orchestrate_orion.assert_called_once()
    ...
    async def test_round_state_transitions(self):
        ...
        assert round_instance.agent.current_orion is not None
...
class TestObserverStateMachineIntegration:
    ...
    def setup_method(self):
        ...
        # 创建observer
        self.observer = OrionProgressObserver(
            agent=self.agent,
            context=self.context
        )
        ...
        # 创建测试orion
        self.test_orion = IntegrationTestHelper.create_test_orion()
        self.agent._current_orion = self.test_orion
...
class TestEndToEndExecution:
    ...
    async def test_complete_execution_flow(self):
        """测试完整执行流程"""
        # 设置orion
        test_orion = IntegrationTestHelper.create_test_orion()
        self.agent.process_initial_request = AsyncMock(return_value=test_orion)
        
        # Mock orchestrator
        with patch.object(self.session._orchestrator, 'assign_devices_automatically') as mock_assign, \
             patch.object(self.session._orchestrator, 'orchestrate_orion') as mock_orchestrate:
            ...
            # 验证流程执行
            assert self.agent.current_orion is not None
    ...
    async def test_race_condition_resolution(self):
        ...
        # 创建真实的监控状态
        from network.agents.network_agent_state import MonitoringNetworkAgentState
        ...
        # 设置orion
        test_orion = IntegrationTestHelper.create_test_orion()
        test_orion.is_complete = MagicMock(return_value=False)
        self.agent._current_orion = test_orion
...
class TestErrorScenarios:
    ...
    @pytest.mark.asyncio
    async def test_orion_creation_failure_handling(self):
        """测试orion创建失败处理"""
        agent = MockNetworkWeaverAgent()
        agent.process_initial_request = AsyncMock(side_effect=Exception("Creation failed"))
        
        from network.agents.network_agent_state import CreatingNetworkAgentState
    ...
    @pytest.mark.asyncio
    async def test_monitoring_state_with_no_orion(self):
        """测试监控状态但没有orion"""
        agent = MockNetworkWeaverAgent()
    ...
    async def test_invalid_task_update_handling(self):
        ...
        monitoring_state = MonitoringNetworkAgentState()
        agent = MockNetworkWeaverAgent()
        agent.update_orion_with_lock = AsyncMock(side_effect=Exception("Update failed"))
