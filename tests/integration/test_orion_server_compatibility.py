"""
Integration Tests for Orion-Server Communication via AIP

Tests verify that:
1. Orion client sends messages in correct format
...
class MockServer:
    """Mock ALIEN server for testing orion communication."""
    ...
    async def handle_client_message(self, message: ClientMessage) -> ServerMessage:
        ...
        if message.type == ClientMessageType.REGISTER:
            if message.client_type == ClientType.ORION:
...
class TestOrionRegistrationCompatibility:
    """Test orion registration message compatibility."""

    @pytest.mark.asyncio
    async def test_registration_message_can_be_parsed_by_server(self, mock_server):
        """Test that server can parse orion registration message."""
        # Create registration message
        orion_id = "test_orion@device_001"
        ...
        assert orion_id in mock_server.client_registry
        assert mock_server.client_registry[orion_id]["target_device"] == target_device
    ...
    async def test_registration_via_protocol(self, mock_server):
        ...
        success = await protocol.register_as_orion(
            orion_id="test@device_001",
            target_device="device_001",
        ...
        assert sent_messages[0].client_type == ClientType.ORION


class TestOrionHeartbeatCompatibility:
    ...
    async def test_heartbeat_message_format(self, mock_server):
        ...
        heartbeat_msg = ClientMessage(
            ...
            client_id="test_orion@device_001",
            status=TaskStatus.OK,
            timestamp=datetime.now(timezone.utc).isoformat(),
...
class TestOrionTaskCompatibility:
    ...
    async def test_task_message_format(self, mock_server):
        ...
        task_msg = ClientMessage(
            ...
            task_name="network/test/excel_task",
            request="Open Excel",
...
class TestOrionDeviceInfoCompatibility:
    ...
    async def test_device_info_via_protocol(self, mock_server):
        ...
        # Request device info (orion-side method)
...
class TestEndToEndMessageFlow:
    ...
    async def test_complete_registration_flow(self, mock_server):
        ...
        assert messages_sent[0].client_type == ClientType.ORION
        assert "orion_123@device_001" in mock_server.client_registry
