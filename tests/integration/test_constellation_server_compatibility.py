"""
Integration Tests for network-Server Communication via AIP

Tests verify that:
1. network client sends messages in correct format
...
class MockServer:
    """Mock Alien server for testing network communication."""
    ...
    async def handle_client_message(self, message: ClientMessage) -> ServerMessage:
        ...
        if message.type == ClientMessageType.REGISTER:
            if message.client_type == ClientType.network:
...
class TestnetworkRegistrationCompatibility:
    """Test network registration message compatibility."""

    @pytest.mark.asyncio
    async def test_registration_message_can_be_parsed_by_server(self, mock_server):
        """Test that server can parse network registration message."""
        # Create registration message
        network_id = "test_network@device_001"
        ...
        assert network_id in mock_server.client_registry
        assert mock_server.client_registry[network_id]["target_device"] == target_device
    ...
    async def test_registration_via_protocol(self, mock_server):
        ...
        success = await protocol.register_as_network(
            network_id="test@device_001",
            target_device="device_001",
        ...
        assert sent_messages[0].client_type == ClientType.network


class TestnetworkHeartbeatCompatibility:
    ...
    async def test_heartbeat_message_format(self, mock_server):
        ...
        heartbeat_msg = ClientMessage(
            ...
            client_id="test_network@device_001",
            status=TaskStatus.OK,
            timestamp=datetime.now(timezone.utc).isoformat(),
...
class TestnetworkTaskCompatibility:
    ...
    async def test_task_message_format(self, mock_server):
        ...
        task_msg = ClientMessage(
            ...
            task_name="cluster/test/excel_task",
            request="Open Excel",
    ...
    async def test_task_via_protocol(self, mock_server):
        ...
        # Send task request (network role)
...
class TestnetworkDeviceInfoCompatibility:
    ...
    async def test_device_info_via_protocol(self, mock_server):
        ...
        # Request device info (network-side method)
...
class TestEndToEndMessageFlow:
    ...
    async def test_complete_registration_flow(self, mock_server):
        ...
        success = await protocol.register_as_network(
            network_id="network_123@device_001",
            target_device="device_001",
        ...
        assert messages_sent[0].client_type == ClientType.network
        assert "network_123@device_001" in mock_server.client_registry
