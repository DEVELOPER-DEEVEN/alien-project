"""
Integration tests for cluster network Client communication using AIP.

Tests the complete message flow between network Client and Alien Server using
...
from cluster.client.components.types import TaskRequest, AgentProfile
...
async def test_registration_with_aip_protocol(connection_manager):
    """Test network registration using AIP RegistrationProtocol."""
    device_id = "test_device"
    ws_url = "ws://localhost:5005/ws"
    ...
    with patch("websockets.connect", return_value=mock_ws):
        ...
        assert reg_msg.client_id == f"test_network@{device_id}"
...
async def test_heartbeat_with_aip_protocol(connection_manager, heartbeat_manager):
    ...
    with patch("websockets.connect", return_value=mock_ws):
        ...
        assert heartbeat_msg.client_id == f"test_network@{device_id}"
...
async def test_concurrent_operations_with_aip(connection_manager):
    ...
    with patch("websockets.connect", side_effect=mock_connect):
        ...
        for device_id in devices:
            ...
            reg_response = ServerMessage(
                type=ServerMessageType.HEARTBEAT,
                client_id=f"test_network@{device_id}",
