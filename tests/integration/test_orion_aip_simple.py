"""
Simplified integration tests for Orion Client AIP migration.

This test suite verifies that the orion client components correctly use
...
async def test_heartbeat_protocol_integration():
    ...
    assert msg.client_id == "test_orion@device_001"
...
async def test_registration_protocol_integration():
    """Test that RegistrationProtocol works correctly for orion."""
    transport = MockTransport()
...
async def test_registration_protocol_error_handling():
    ...
    # Register as orion - should fail
    success = await protocol.register_as_orion(
        orion_id="test_orion@device_999",
...
async def test_heartbeat_manager_creates_protocol():
    ...
    connection_manager.task_name = "test_orion"
...
async def test_connection_manager_has_aip_components():
    ...
    from network.client.components.connection_manager import (
        WebSocketConnectionManager,
    )

    manager = WebSocketConnectionManager(task_name="test_orion")
