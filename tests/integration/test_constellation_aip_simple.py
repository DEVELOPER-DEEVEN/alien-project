"""
Simplified integration tests for network Client AIP migration.

This test suite verifies that the network client components correctly use
...
async def test_heartbeat_protocol_integration():
    ...
    assert msg.client_id == "test_network@device_001"
...
async def test_registration_protocol_integration():
    """Test that RegistrationProtocol works correctly for network."""
    transport = MockTransport()
...
async def test_registration_protocol_error_handling():
    ...
    # Register as network - should fail
    success = await protocol.register_as_network(
        network_id="test_network@device_999",
...
async def test_heartbeat_manager_creates_protocol():
    ...
    connection_manager.task_name = "test_network"
...
async def test_connection_manager_has_aip_components():
    ...
    manager = WebSocketConnectionManager(task_name="test_network")
