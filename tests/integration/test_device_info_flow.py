...
Tests the complete flow of device info from device client to orion client.
"""
...
class TestAgentProfileIntegration:
    ...
    @pytest.mark.asyncio
    async def test_orion_request_device_info(self):
        """Test orion requesting device info via handler"""
        ws_manager = WSManager()
        session_manager = SessionManager()
        handler = ALIENWebSocketHandler(ws_manager, session_manager)
        ...
        # Now orion requests device info
        mock_orion_ws = AsyncMock()

        orion_request = ClientMessage(
            type=ClientMessageType.DEVICE_INFO_REQUEST,
            client_type=ClientType.ORION,
            client_id="orion_001@device_001",
        ...
        await handler.handle_device_info_request(
            orion_request, mock_orion_ws
        )

        # Verify response was sent
        mock_orion_ws.send_text.assert_called_once()

        # Parse the response
        response_json = mock_orion_ws.send_text.call_args[0][0]
    ...
    async def test_request_device_info_not_found(self):
        ...
        orion_request = ClientMessage(
            type=ClientMessageType.DEVICE_INFO_REQUEST,
            client_type=ClientType.ORION,
            client_id="orion_001@device_999",
