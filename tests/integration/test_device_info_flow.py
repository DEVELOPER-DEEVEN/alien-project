...
Tests the complete flow of device info from device client to network client.
"""
...
class TestAgentProfileIntegration:
    ...
    @pytest.mark.asyncio
    async def test_network_request_device_info(self):
        """Test network requesting device info via handler"""
        ws_manager = WSManager()
        session_manager = SessionManager()
        handler = AlienWebSocketHandler(ws_manager, session_manager)
        ...
        # Now network requests device info
        mock_network_ws = AsyncMock()

        network_request = ClientMessage(
            type=ClientMessageType.DEVICE_INFO_REQUEST,
            client_type=ClientType.network,
            client_id="network_001@device_001",
        ...
        await handler.handle_device_info_request(
            network_request, mock_network_ws
        )

        # Verify response was sent
        mock_network_ws.send_text.assert_called_once()

        # Parse the response
        response_json = mock_network_ws.send_text.call_args[0][0]
    ...
    async def test_request_device_info_not_found(self):
        ...
        network_request = ClientMessage(
            type=ClientMessageType.DEVICE_INFO_REQUEST,
            client_type=ClientType.network,
            client_id="network_001@device_999",
