...
"""
Test to verify why networkProgressObserver logs but networkAgent add_task_completion_event doesn't log.
"""
...
class TestnetworkObserverLogger:
    ...
    @pytest.fixture
    def network_agent(self, mock_orchestrator):
    ...
    @pytest.fixture
    def observer(self, network_agent):
        """Create a networkProgressObserver instance."""
        return networkProgressObserver(agent=network_agent)

    @pytest.mark.asyncio
    async def test_observer_calls_agent_add_task_completion_event(
        self, observer, network_agent, task_event, caplog
    ):
        ...
        network_agent.add_task_completion_event.assert_called_once()

        # Get the actual call arguments
        call_args = network_agent.add_task_completion_event.call_args
    ...
    async def test_direct_agent_add_task_completion_event_logging(
        self, network_agent, task_event, caplog
    ):
        ...
        print(f"Agent logger level: {network_agent.logger.level}")
        print(
            f"Agent logger effective level: {network_agent.logger.getEffectiveLevel()}"
        )
        print(f"Agent logger handlers: {network_agent.logger.handlers}")
        print(f"Agent logger propagate: {network_agent.logger.propagate}")

        # Call the method directly
        await network_agent.add_task_completion_event(task_event)
        ...
        if len(agent_logs) == 0:
            ...
            print(
                "This suggests the issue is with the logger configuration in networkAgent"
            )
        else:
    ...
    async def test_logger_configuration_comparison(
        self, observer, network_agent, caplog
    ):
        ...
        print(f"\nAgent logger:")
        print(f"  Name: {network_agent.logger.name}")
        print(f"  Level: {network_agent.logger.level}")
        print(f"  Effective level: {network_agent.logger.getEffectiveLevel()}")
        print(f"  Handlers: {network_agent.logger.handlers}")
        print(f"  Propagate: {network_agent.logger.propagate}")
        ...
        network_agent.logger.info("TEST: Agent logger test message")
