...
async def test_orion_client():
    """测试 OrionClient 连接功能"""

    print("=" * 80)
    print("[*] OrionClient 真实设备连接测试")
    print("=" * 80)

    try:
        # 1. 加载配置
        print("\n[1] 1. 加载配置文件...")
        config_path = "config/orion_sample.yaml"
        config = OrionConfig.from_yaml(config_path)

        print(f"[+] 配置加载成功!")
        print(f"   星群ID: {config.orion_id}")
        ...
        # 5. 获取星群信息
        print("\n[5] 5. 星群信息总结:")
        orion_info = client.get_orion_info()
        print(f"   星群ID: {orion_info['orion_id']}")
        print(f"   已连接设备: {orion_info['connected_devices']}")
        print(f"   总设备数: {orion_info['total_devices']}")
        print(
            f"   心跳间隔: {orion_info['configuration']['heartbeat_interval']}s"
        )
        print(
            f"   最大并发任务: {orion_info['configuration']['max_concurrent_tasks']}"
        )
...
async def test_device_operations():
    ...
    try:
        # 使用配置创建客户端
        config = OrionConfig.from_yaml("config/orion_sample.yaml")
        client = OrionClient(config=config)
...
async def main():
    ...
    print("[*] 开始 OrionClient 测试套件")

    # 测试1: 基础连接测试
    connection_test_passed = await test_orion_client()
    ...
    if passed_count == len(tests):
        print("[+] 所有测试都通过了！OrionClient 工作正常。")
    else:
