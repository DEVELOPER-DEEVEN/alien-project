...
This test compares config values accessed through the old Config class vs the new get_alien_config().
"""
...
def test_system_config_fields():
    ...
    for old_key, new_attr in test_fields.items():
        try:
            ...
            new_value = getattr(alien_config.system, new_attr)
...
def test_rag_config_fields():
    ...
    for old_key, new_attr in test_fields.items():
        try:
            old_value = old_configs.get(old_key)
            new_value = getattr(alien_config.rag, new_attr)
...
def test_agent_config_fields():
    ...
    agents_to_test = [
        ("HOST_AGENT", alien_config.host_agent, "host_agent"),
        ("APP_AGENT", alien_config.app_agent, "app_agent"),
        ("BACKUP_AGENT", alien_config.backup_agent, "backup_agent"),
        ("EVALUATION_AGENT", alien_config.evaluation_agent, "evaluation_agent"),
    ]
...
def main():
    ...
    print("Testing: Old Config.get_instance().config_data vs New get_alien_config()")
    print("=" * 80 + "\n")
