...
from network.orion import create_simple_orion
from alien.module.context import Context


class NetworkStateMachineTestRunner:
    """Network状态机测试运行器"""
    ...
    async def test_state_manager(self):
        ...
        try:
            ...
            # 测试状态缓存
            same_state = manager.get_state(NetworkAgentStatus.CREATING.value)
    ...
    async def test_monitoring_state_task_tracking(self):
        ...
        try:
            ...
            # 创建orion
            orion = create_simple_orion(
                task_descriptions=["Task 1", "Task 2"],
                orion_name="test",
                sequential=True
            )
            orion.is_complete = lambda: False
            agent._current_orion = orion
            agent.update_orion_with_lock = lambda *args: asyncio.coroutine(lambda: orion)()
    ...
    async def test_complete_workflow(self):
        ...
        try:
            ...
            # 模拟完整状态机循环
            state_manager = NetworkAgentStateManager()
            
            # 1. 创建状态
            agent._status = NetworkAgentStatus.CREATING.value
            ...
            # 设置orion完成
            agent.current_orion.is_complete = lambda: True
            agent.should_continue = lambda *args: asyncio.coroutine(lambda: False)()
            
            await state.handle(agent, context)
            
            assert agent._status == NetworkAgentStatus.FINISHED.value
    ...
    async def test_race_condition_resolution(self):
        ...
        try:
            ...
            orion.is_complete = mock_is_complete
            agent.should_continue = lambda *args: asyncio.coroutine(lambda: False)()
    ...
    async def test_concurrent_operations(self):
        ...
        try:
            monitoring_state = MonitoringNetworkAgentState()
            agent = MockNetworkWeaverAgent()
            agent.update_orion_with_lock = lambda *args: asyncio.coroutine(lambda: None)()
...
async def main():
    """主函数"""
    runner = NetworkStateMachineTestRunner()
