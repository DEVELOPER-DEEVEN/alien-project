...
"""
Tests for BasenetworkPrompter formatting functions.
...
class TestBasenetworkPrompter:
    """Test cases for BasenetworkPrompter formatting methods."""

    def setup_method(self):
        """Set up test fixtures."""
        # Mock the parent class initialization to avoid file loading
        with patch.object(BasenetworkPrompter, "__init__", lambda x, y, z: None):
            self.prompter = BasenetworkPrompter("mock_template", "mock_example")
    ...
    def test_format_network_none(self):
        """Test formatting None network."""
        result = self.prompter._format_network(None)
        assert result == "No network information available."

    def test_format_network_basic(self):
        """Test formatting basic network structure."""
        # Create a mock network with to_dict method
        mock_network = Mock()
        ...
        result = self.prompter._format_network(mock_network)

        # Check header information
        assert "Task network: Test network" in result
    ...
    def test_format_network_with_completed_task(self):
        """Test formatting network with completed task and result."""
        mock_network = Mock()
        mock_network.to_dict.return_value = {
            "name": "Completed Task network",
            "state": "executing",
            "tasks": {
                "task_001": {
                    "name": "Data Analysis",
    ...
    def test_format_network_with_failed_task(self):
        """Test formatting network with failed task."""
        mock_network = Mock()
        mock_network.to_dict.return_value = {
            "name": "Failed Task network",
            "state": "failed",
            "tasks": {
                "task_001": {
                    "name": "Database Query",
    ...
    def test_format_network_complex_dependencies(self):
        """Test formatting network with complex dependencies."""
        mock_network = Mock()
        mock_network.to_dict.return_value = {
    ...
    def test_format_network_exception_handling(self):
        """Test handling of network formatting exceptions."""
        mock_network = Mock()
        mock_network.to_dict.side_effect = Exception("Mock exception")

        result = self.prompter._format_network(mock_network)

        assert (
            result == "network information unavailable due to formatting error."
        )

    def test_format_network_empty_tasks_and_dependencies(self):
        """Test formatting network with no tasks or dependencies."""
        mock_network = Mock()
        mock_network.to_dict.return_value = {
            "name": "Empty network",
            "state": "created",
        ...
        assert "Task network: Empty network" in result
