...
展示 TaskOrionSchema 如何支持 tasks 和 dependencies 的 List 和 Dict 格式。
"""
...
def example_list_format():
    ...
    print("\n[DEP] 自动生成的依赖 ID:")
    for dep_id, dep in orion.dependencies.items():
...
def example_dict_format():
    ...
    print(f"[OK] 星座创建成功: {orion.name}")
    print(f"   - 星座 ID: {orion.orion_id}")
    print(f"   - 任务 IDs: {list(orion.tasks.keys())}")
    print(f"   - 依赖 IDs: {list(orion.dependencies.keys())}")
...
def example_mixed_format():
    ...
    print(f"[OK] 混合格式星座: {orion.name}")
    print(f"   - Tasks 类型: {type(orion.tasks).__name__}")
    print(f"   - Dependencies 类型: {type(orion.dependencies).__name__}")
...
def example_format_conversion():
    ...
    print(f"[OK] 原始格式（内部存储为 Dict）:")
    print(f"   - 任务数: {len(orion.tasks)}")
    print(f"   - 依赖数: {len(orion.dependencies)}")

    # 转换为 List 格式
    tasks_as_list = orion.get_tasks_as_list()
    deps_as_list = orion.get_dependencies_as_list()
...
def example_json_compatibility():
    ...
    # 方式1: 默认 Dict 格式 JSON
    dict_json = orion.model_dump_json(indent=2)
    print(f"[OK] Dict 格式 JSON 长度: {len(dict_json)} 字符")

    # 方式2: List 格式 JSON
    list_format = orion.to_dict_with_lists()
    list_json = json.dumps(list_format, indent=2)
    print(f"[OK] List 格式 JSON 长度: {len(list_json)} 字符")

    # 验证两种格式都能正确加载
    restored_from_dict = TaskOrionSchema.model_validate_json(dict_json)
    restored_from_list = TaskOrionSchema(**json.loads(list_json))

    print(f"\n[CONTINUE] 恢复验证:")
    print(f"   - 从 Dict JSON 恢复: [OK] {restored_from_dict.name}")
    print(f"   - 从 List JSON 恢复: [OK] {restored_from_list.name}")
    print(
        f"   - 任务数量一致: [OK] {len(restored_from_dict.tasks) == len(restored_from_list.tasks)}"
    )
...
def main():
    """运行所有示例"""
    print(" TaskOrionSchema List/Dict 兼容性演示")
    print("=" * 60)

    try:
        # 运行各种示例
        orion1 = example_list_format()
        orion2 = example_dict_format()
        orion3 = example_mixed_format()
        orion4, dict_with_lists = example_format_conversion()
        orion5 = example_json_compatibility()

        print("\n" + "=" * 60)
        print(" 所有示例运行完成！")

        print("\n[THOUGHT] 主要特性:")
        print("   [TASK] 支持 List 格式输入，自动转换为 Dict 存储")
        print("   [PLAN] 保持传统 Dict 格式的完全兼容性")
        print("   [CONTINUE] 提供 List ↔ Dict 格式转换方法")
        print("   [WEB] 支持两种格式的 JSON 序列化/反序列化")
