...
def example_json_persistence():
    ...
    # 创建复杂的星座
    network = Tasknetwork(
        network_id="example_network", name="示例任务星座"
    )
    ...
    print(f"✅ 从文件加载星座: {loaded_network.name}")
    print(f"   - 任务数量: {len(loaded_network.tasks)}")
    print(f"   - 依赖关系数量: {len(loaded_network.dependencies)}")
...
def example_api_integration():
    ...
    try:
        ...
        print(f"✅ 成功创建星座: {network.name}")
        print(f"   - 状态: {network.state.value}")
        print(f"   - 任务数: {len(network.tasks)}")
