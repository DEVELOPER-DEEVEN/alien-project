...
"""
Comprehensive tests for networkModificationSynchronizer.
...
def create_task_event(
    ...
    **extra_data
    ...
    return TaskEvent(
        ...
        task_id=task_id,
        status=status,
        data={"network_id": network_id, **extra_data},
    )
...
def create_network_event(
    ...
    **extra_data
    ...
    """Helper function to create networkEvent with correct parameters."""
    return networkEvent(
...
def synchronizer(mock_orchestrator):
    ...
    return networkModificationSynchronizer(
...
@pytest.fixture
def network_modified_event():
    """Create a network_MODIFIED event."""
    return create_network_event(
        task_id="task_1",
        network_id="network_123",
...
class TestBasicSynchronization:
    ...
    async def test_complete_modification(
        self, synchronizer, task_completed_event, network_modified_event
    ):
        """Test that network modified events complete pending modifications."""
        # Register pending modification
        await synchronizer.on_event(task_completed_event)
        assert synchronizer.has_pending_modifications()

        # Complete modification
        await synchronizer.on_event(network_modified_event)
...
class TestRaceConditionPrevention:
    ...
    async def test_orchestrator_waits_for_modification(self, synchronizer):
        ...
        async def agent_flow():
            """Simulate agent modifying network."""
...
class TestErrorRecovery:
    ...
    @pytest.mark.asyncio
    async def test_handle_missing_network_id(self, synchronizer):
        """Test handling events with missing network_id."""
        # Event without network_id
        event = TaskEvent(
            event_type=EventType.TASK_COMPLETED,
            source_id="test",
            timestamp=time.time(),
    ...
    async def test_handle_missing_task_id(self, synchronizer):
        """Test handling network events with missing on_task_id."""
        ...
        # network event without on_task_id
        mod_event = networkEvent(
            event_type=EventType.network_MODIFIED,
            source_id="agent",
            timestamp=time.time(),
    ...
    async def test_sequential_task_execution(self, synchronizer):
        ...
        for task_id in tasks:
            ...
            task_event = TaskEvent(
                ...
                task_id=task_id,
                task_name=f"Task {task_id}",
                data={"network_id": "sequential_network"},
            )
            await synchronizer.on_event(task_event)
    ...
    async def test_parallel_task_execution(self, synchronizer):
        ...
        for task_id in task_ids:
            event = TaskEvent(
                ...
                task_id=task_id,
                task_name=task_id,
                data={"network_id": "parallel_network"},
            )
            await synchronizer.on_event(event)
