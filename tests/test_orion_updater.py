"""
Tests for OrionUpdater class.
Validates orion update and modification functionality.
"""
...
class TestOrionUpdater:
    """Test OrionUpdater functionality."""

    @pytest.fixture
    def updater(self):
        """Create a OrionUpdater instance."""
        return OrionUpdater()
    ...
    def test_add_tasks(self, updater, sample_orion):
        """Test adding tasks to a orion."""
        descriptions = ["Third task", "Fourth task"]

        created_tasks = updater.add_tasks(
            sample_orion, descriptions, priority=TaskPriority.HIGH
        )

        assert len(created_tasks) == 2
        assert len(sample_orion.tasks) == 4

        # Check that tasks were added with correct properties
        for task in created_tasks:
            assert task.priority == TaskPriority.HIGH
            assert task.task_id in sample_orion.tasks

    def test_remove_tasks(self, updater, sample_orion):
        """Test removing tasks from a orion."""
        original_count = len(sample_orion.tasks)

        updater.remove_tasks(sample_orion, ["task_1"])

        assert len(sample_orion.tasks) == original_count - 1
        assert "task_1" not in sample_orion.tasks
        assert "task_2" in sample_orion.tasks

    def test_remove_tasks_with_dependencies(self, updater, sample_orion):
        """Test removing tasks also removes related dependencies."""
        # Add a dependency
        dep = TaskStarLine.create_unconditional("task_1", "task_2", "Test dependency")
        sample_orion.add_dependency(dep)

        assert len(sample_orion.dependencies) == 1

        # Remove task_1, which should also remove the dependency
        updater.remove_tasks(sample_orion, ["task_1"], remove_dependencies=True)

        assert "task_1" not in sample_orion.tasks
        assert len(sample_orion.dependencies) == 0

    def test_add_dependencies(self, updater, sample_orion):
        """Test adding dependencies to a orion."""
        dep_specs = [
            {
                "from_task_id": "task_1",
                "to_task_id": "task_2",
        ...
        created_deps = updater.add_dependencies(sample_orion, dep_specs)
    ...
    def test_add_dependencies_invalid_tasks(self, updater, sample_orion):
    ...
    def test_update_from_llm_output_add_task(self, updater, sample_orion):
        """Test updating orion from LLM output with add task instruction."""
        llm_output = """
        ADD TASK: New task from LLM
        """

        original_count = len(sample_orion.tasks)
        updater.update_from_llm_output(sample_orion, llm_output)

        assert len(sample_orion.tasks) == original_count + 1

    def test_update_from_llm_output_remove_task(self, updater, sample_orion):
        """Test updating orion from LLM output with remove task instruction."""
        llm_output = """
        REMOVE TASK: task_1
        """
        ...
        updater.update_from_llm_output(
            sample_orion, llm_output, preserve_existing=False
        )

        assert "task_1" not in sample_orion.tasks

    def test_update_from_llm_output_add_dependency(self, updater, sample_orion):
        """Test updating orion from LLM output with add dependency instruction."""
        llm_output = """
    ...
    def test_create_dependency_from_spec(self, updater, sample_orion):
        ...
        dep = updater._create_dependency_from_spec(sample_orion, dep_spec)

        assert dep is not None
        assert dep.from_task_id == "task_1"
        assert dep.to_task_id == "task_2"

    def test_create_dependency_from_spec_invalid(self, updater, sample_orion):
    ...
    def test_updater_with_logger(self):
        ...
        mock_logger = Mock()
        updater = OrionUpdater(logger=mock_logger)
        orion = TaskOrion(name="Logger Test")

        updater.add_tasks(orion, ["Test task"])

        # Verify logger was called
        mock_logger.info.assert_called()

    def test_preserve_existing_tasks(self, updater, sample_orion):
        ...
        updater.update_from_llm_output(
            sample_orion, llm_output, preserve_existing=True
        )

        assert "task_1" in sample_orion.tasks

    def test_alternative_dependency_spec_format(self, updater, sample_orion):
