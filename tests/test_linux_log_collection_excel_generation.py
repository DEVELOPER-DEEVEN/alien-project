...
It includes mock AgentProfile objects for testing cross-platform operations in a network environment.
"""
...
class TestLinuxLogCollectionExcelGeneration:
    ...
    def device_network(
        ...
        """Create a network of devices for testing."""
        return {
    ...
    def test_device_capabilities_for_log_collection_scenario(
        self, device_network: Dict[str, AgentProfile]
    ):
        """Test that devices have the required capabilities for the log collection scenario."""
        linux_servers = [
            dev for dev in device_network.values() if dev.os == "linux"
        ]
        windows_machines = [
            dev for dev in device_network.values() if dev.os == "windows"
        ]
    ...
    def test_device_metadata_validation(
        ...
        for device_id, device in device_network.items():
    ...
    def test_device_formatting_for_prompt(
        ...
        # This simulates how devices would be formatted for network prompts
        formatted_devices = []
