...
"""
Real-world race condition tests for OrionModificationSynchronizer.

This test suite simulates the actual race condition scenario:
- Orchestrator executes tasks and gets ready tasks
- Agent modifies orion based on task completion
...
class MockOrion:
    """Mock orion for testing."""
    
    def __init__(self):
        self.orion_id = "test_orion"
        self.tasks = {}
    ...
    def mark_task_completed(self, task_id: str):
        ...
        logging.info(f"[Orion] Task {task_id} marked as completed")
        
    def get_ready_tasks(self):
        """Get ready tasks."""
        logging.info(f"[Orion] Getting ready tasks: {self.ready_tasks}")
    ...
    def modify_task(self, task_id: str, modification: str):
        ...
        logging.info(f"[Orion] Modified task {task_id}: {modification}")
...
class MockOrchestrator:
    ...
    def __init__(self, orion: MockOrion, event_bus, synchronizer):
        self.orion = orion
    ...
    async def orchestrate(self):
        ...
        while not self.orion.is_complete():
            ...
            # Get ready tasks
            ready_tasks = self.orion.get_ready_tasks()
    ...
    async def _execute_task(self, task_id: str):
        ...
        # Mark task completed in orion
        self.orion.mark_task_completed(task_id)
        ...
        event = TaskEvent(
            ...
            task_id=task_id,
            status="completed",
            data={"orion_id": self.orion.orion_id}
        )
...
class MockAgent:
    """Mock agent that modifies orion based on task completion."""
    
    def __init__(self, orion: MockOrion, event_bus, modification_delay: float = 0.2):
        self.orion = orion
        self.event_bus = event_bus
    ...
    async def handle_task_completion(self, event: TaskEvent):
        """Handle task completion by modifying orion."""
        ...
        self.orion.modify_task(task_id, modification)
        ...
        # Publish ORION_MODIFIED event
        mod_event = OrionEvent(
            event_type=EventType.ORION_MODIFIED,
            source_id="agent",
            timestamp=time.time(),
            data={"on_task_id": task_id},
            orion_id=self.orion.orion_id,
            orion_state="executing"
        )
        
        self.logger.info(f"Agent publishing ORION_MODIFIED for {task_id}")
...
class TestRaceConditionWithSynchronizer:
    ...
    @pytest.mark.asyncio
    async def test_orchestrator_waits_for_agent_modification(self, event_bus, orion):
        """
        ...
        4. Agent starts modifying orion (slow, 0.3s)
        ...
        agent = MockAgent(orion, event_bus, modification_delay=0.3)
        ...
        # Create orchestrator
        orchestrator = MockOrchestrator(orion, event_bus, synchronizer)
        
        # Track timing
        timing_log = []
        
        async def track_orion_modifications():
            """Track when modifications happen."""
            while len(orion.modification_log) < 1:
    ...
    @pytest.mark.asyncio
    async def test_race_condition_prevented(self, event_bus, orion):
        """
        ...
        # Create agent
        agent = MockAgent(orion, event_bus, modification_delay=0.2)
        ...
        for i, task_id in enumerate(tasks_to_execute):
            ...
            orion.ready_tasks = [task_id]
            
            # Execute task
            await asyncio.sleep(0.1)
            orion.mark_task_completed(task_id)
...
class TestRaceConditionWithoutSynchronizer:
    ...
    @pytest.mark.asyncio
    async def test_race_condition_occurs_without_sync(self, event_bus, orion):
        """
        ...
        orchestrator = MockOrchestrator(orion, event_bus, synchronizer=None)
        ...
        async def execute_and_track():
            ...
            orion.ready_tasks = ["task_B"]
            await asyncio.sleep(0.05)
            ready = orion.get_ready_tasks()
