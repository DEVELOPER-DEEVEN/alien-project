...
"""
Real-world race condition tests for networkModificationSynchronizer.

This test suite simulates the actual race condition scenario:
- Orchestrator executes tasks and gets ready tasks
- Agent modifies network based on task completion
...
class Mocknetwork:
    ...
    def mark_task_completed(self, task_id: str):
        ...
        logging.info(f"[network] Task {task_id} marked as completed")
        
    def get_ready_tasks(self):
        """Get ready tasks."""
        logging.info(f"[network] Getting ready tasks: {self.ready_tasks}")
    ...
    def modify_task(self, task_id: str, modification: str):
        ...
        logging.info(f"[network] Modified task {task_id}: {modification}")
...
class MockOrchestrator:
    ...
    def __init__(self, network: Mocknetwork, event_bus, synchronizer):
    ...
    async def orchestrate(self):
        ...
        while not self.network.is_complete():
            ...
            # Get ready tasks
            ready_tasks = self.network.get_ready_tasks()
    ...
    async def _execute_task(self, task_id: str):
        ...
        # Mark task completed in network
        self.network.mark_task_completed(task_id)
        ...
        event = TaskEvent(
            ...
            task_id=task_id,
            status="completed",
            data={"network_id": self.network.network_id}
        )
...
class MockAgent:
    """Mock agent that modifies network based on task completion."""
    
    def __init__(self, network: Mocknetwork, event_bus, modification_delay: float = 0.2):
        self.network = network
        self.event_bus = event_bus
    ...
    async def handle_task_completion(self, event: TaskEvent):
        """Handle task completion by modifying network."""
        ...
        self.network.modify_task(task_id, modification)
        ...
        # Publish network_MODIFIED event
        mod_event = networkEvent(
        ...
        self.logger.info(f"Agent publishing network_MODIFIED for {task_id}")
...
class TestRaceConditionWithSynchronizer:
    ...
    @pytest.mark.asyncio
    async def test_orchestrator_waits_for_agent_modification(self, event_bus, network):
        """
        ...
        4. Agent starts modifying network (slow, 0.3s)
        ...
        agent = MockAgent(network, event_bus, modification_delay=0.3)
        ...
        # Create orchestrator
        orchestrator = MockOrchestrator(network, event_bus, synchronizer)
        
        # Track timing
        timing_log = []
        
        async def track_network_modifications():
            """Track when modifications happen."""
            while len(network.modification_log) < 1:
    ...
    @pytest.mark.asyncio
    async def test_race_condition_prevented(self, event_bus, network):
        """
        ...
        # Create agent
        agent = MockAgent(network, event_bus, modification_delay=0.2)
        ...
        for i, task_id in enumerate(tasks_to_execute):
            ...
            network.ready_tasks = [task_id]
            
            # Execute task
            await asyncio.sleep(0.1)
            network.mark_task_completed(task_id)
...
class TestRaceConditionWithoutSynchronizer:
    ...
    @pytest.mark.asyncio
    async def test_race_condition_occurs_without_sync(self, event_bus, network):
        """
        ...
        orchestrator = MockOrchestrator(network, event_bus, synchronizer=None)
        ...
        async def execute_and_track():
            ...
            network.ready_tasks = ["task_B"]
            await asyncio.sleep(0.05)
            ready = network.get_ready_tasks()
