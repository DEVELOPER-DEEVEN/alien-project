...
"""
Test for network update lock mechanism to prevent race conditions.

This test verifies that the update lock prevents race conditions between:
1. Orchestrator executing ready tasks
2. Agent's process_editing updating the network
"""
...
from Alien.cluster.network.task_network import Tasknetwork
from Alien.cluster.network.task_star import TaskStar
from Alien.cluster.network.enums import TaskStatus, TaskPriority, DeviceType


class TestnetworkUpdateLock:
    """Test cases for network update lock mechanism."""
    ...
    def test_update_lock_exists(self, network):
        """Test that the network has an update lock."""
        assert hasattr(network, "_update_lock")
        assert isinstance(network._update_lock, asyncio.Lock)

    @pytest.mark.asyncio
    async def test_lock_basic_functionality(self, network):
        """Test basic lock acquire and release."""
        # Acquire lock
        await network._update_lock.acquire()
        assert network._update_lock.locked()

        # Release lock
        network._update_lock.release()
        assert not network._update_lock.locked()

    @pytest.mark.asyncio
    async def test_lock_context_manager(self, network):
        """Test lock usage with async context manager."""
        assert not network._update_lock.locked()

        async with network._update_lock:
            assert network._update_lock.locked()

        assert not network._update_lock.locked()

    @pytest.mark.asyncio
    async def test_concurrent_access_blocked(self, network, sample_tasks):
        ...
        async def mock_agent_process_editing():
            """Simulates agent editing network."""
            async with network._update_lock:
    ...
    @pytest.mark.asyncio
    async def test_race_condition_scenario(self, network, sample_tasks):
        """
        ...
        4. At the same time, agent tries to edit network
        """
        ...
        from Alien.cluster.network.task_star_line import TaskStarLine
    ...
    @pytest.mark.asyncio
    async def test_multiple_concurrent_readers_blocked(self, network):
    ...
    @pytest.mark.asyncio
    async def test_lock_prevents_stale_ready_tasks(self, network, sample_tasks):
        """
        ...
        async def agent_modify_tasks():
            ...
            async with network._update_lock:
                ...
                if task1.task_id in network._tasks:
                    network.remove_task(task1.task_id)
...
class TestLockPerformance:
    ...
    async def test_lock_overhead(self):
        ...
        network = Tasknetwork(enable_visualization=False)

        # Measure time without lock
        start = asyncio.get_event_loop().time()
        for _ in range(100):
            network.get_ready_tasks()
