...
from alien.config import Config as ALIENConfig
from alien.module.basic import BaseSession, Context, ContextNames

_configs = InstantiationConfig.get_instance().config_data
_alien_configs = ALIENConfig.get_instance().config_data
    ...
    def _get_or_create_evaluation_agent(self) -> ExecuteEvalAgent:
        ...
        if self._app_name not in ExecuteFlow._app_eval_agent_dict:
            ExecuteFlow._app_eval_agent_dict[self._app_name] = ExecuteEvalAgent(
                ...
                self._app_env.app_root_name,
                is_visual=True,
                main_prompt=_alien_configs["EVALUATION_PROMPT"],
                example_prompt="",
                api_prompt=_alien_configs["API_PROMPT"],
            )
    ...
    def execute_action(self) -> None:
        ...
        try:
            ...
            if control_selected:
                ...
                if _alien_configs.get("SHOW_VISUAL_OUTLINE_ON_SCREEN", True):
                    control_selected.draw_outline(colour="red", thickness=3)
                    time.sleep(_alien_configs.get("RECTANGLE_TIME", 0))
