...
"""
Event system for Network framework using Observer pattern.
"""
...
class EventType(Enum):
    """
    Types of events in the Network system.

    Defines enumeration for different event types that can occur
    during Network framework execution.
    """
    ...
    # Orion lifecycle events (macro-level state changes)
    ORION_STARTED = "orion_started"
    ORION_COMPLETED = "orion_completed"
    ORION_FAILED = "orion_failed"

    # Structure modification events (for dynamic orion changes)
    ...
    DEVICE_CONNECTED = "device_connected"  # Device connected to orion
    DEVICE_DISCONNECTED = (
        "device_disconnected"  # Device disconnected from orion
    )
...
class Event:
    ...
    Represents the fundamental structure of all events in the Network system
...
class OrionEvent(Event):
    ...
    Extends base Event class with orion-specific information including
    orion ID, state, and list of newly ready tasks.
    """
...
class AgentEvent(Event):
    ...
    agent_name: str
    agent_type: str  # "orion", "app", "host", etc.
...
class IEventObserver(ABC):
    ...
    and handle events from the Network event system.
    """
...
class IEventPublisher(ABC):
    ...
    and manage observer subscriptions in the Network framework.
    """
...
class EventBus(IEventPublisher):
    ...
    subscriptions and distributes events throughout the Network system.
    """
...
def get_event_bus() -> EventBus:
    ...
    the Network framework for event publishing and subscription.
