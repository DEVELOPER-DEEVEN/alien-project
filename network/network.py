...
Primary command-line interface for Network Framework with comprehensive functionality.
This script provides both simple and advanced interfaces for Network sessions.

Simple Usage:
    python -m network "Create a machine learning pipeline"
    python -m network --interactive
    python -m network --demo

Advanced Usage:
    python -m network --request "Task description" --session-name "my_session"
    python -m network --request "Task" --output-dir "./results" --log-level DEBUG
...
# Add ALIEN2 to path to enable imports
ALIEN_ROOT = Path(__file__).parent.parent
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Network Framework - AI-powered DAG workflow orchestration",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  Simple Usage:
    python -m network "Create a data analysis pipeline"
    python -m network --demo
    python -m network --interactive

  Advanced Usage:
    python -m network --request "Build ML pipeline" --session-name "ml_session"
    python -m network --interactive --max-rounds 20 --log-level DEBUG
    python -m network --request "Task" --output-dir "./results" --mock
        """,
    )
    ...
    parser.add_argument(
        "--session-name", dest="session_name", help="Custom name for the Network session"
    )
...
# Parse args and setup logger BEFORE importing NetworkClient
# This ensures config warnings are displayed with correct color
args = parse_args()
setup_logger(args.log_level)

# Now import NetworkClient after logger is configured
...
async def network_quick_start(
    request: str,
    session_name: str = "network_quick",
    ...
    :param session_name: Name for the Network session (default: "network_quick")
...
async def network_interactive(
    session_name: str = "network_interactive",
    log_level: str = "WARNING",
    max_rounds: int = 10,
    output_dir: str = "./logs",
):
    ...
    :param session_name: Name for the Network session (default: "network_interactive")
    :param log_level: Logging level (default: "WARNING")
...
async def main():
    ...
    try:
        ...
        if "client" in locals():
            client.display.print_error(f"[FAIL] Network Framework error: {e}")
        else:
            # Fallback display for when client is not yet initialized
            from network.visualization.client_display import ClientDisplay

            display = ClientDisplay(console=console)
            display.print_error(f"[FAIL] Network Framework error: {e}")
        logging.error(f"Network Framework error: {e}", exc_info=True)
        sys.exit(1)
    finally:
...
async def run_demo_with_client(client: NetworkClient):
    """
    Run demo mode with initialized client.

    :param client: Initialized NetworkClient instance
    """
...
async def run_webui_mode(client: NetworkClient):
    """
    ...
    import webbrowser
    import uvicorn
    from network.webui.server import app, set_network_client

    # Set the Network client for the WebUI server
    set_network_client(client)
    ...
    if port is None:
        client.display.print_error(
            "[FAIL] Could not find an available port (tried 8000-8009)"
        )
        return
    ...
    if frontend_dir.exists():
        ...
        try:
            with open(env_file, "w", encoding="utf-8") as f:
                f.write(f"# Auto-generated by Network backend\n")
    ...
    # Display banner
    client.display.print_info("[ORION] Network WebUI Starting...")
    ...
    client.display.print_info("\n[THOUGHT] Press Ctrl+C to stop the server\n")
    ...
    async def open_browser_delayed():
        ...
        client.display.print_info(f"[WEB] Opening browser: {url}")
        webbrowser.open(url)
