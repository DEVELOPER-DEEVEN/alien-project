"""
Network Trajectory Report Generator

Quick script to generate markdown reports for Network task execution logs.
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate Markdown report for Network task execution logs"
    )
    parser.add_argument(
        "log_dir",
        type=str,
        help="Path to Network log directory (e.g., logs/network/task_1)",
    )
    parser.add_argument(
        "-o",
        "--output",
    ...
    parser.add_argument(
        "--no-orion",
        action="store_true",
        help="Exclude orion evolution details",
    )
    parser.add_argument(
        "--no-tasks",
        action="store_true",
    ...
    try:
        trajectory = NetworkTrajectory(args.log_dir)
    ...
    try:
        trajectory.to_markdown(
            ...
            include_orion_details=not args.no_orion,
