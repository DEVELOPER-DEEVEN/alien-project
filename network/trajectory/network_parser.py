...
"""
Network Trajectory Parser

Optimized parser for Network agent logs with orion visualization support.
"""
...
class NetworkTrajectory:
    """
    A class to structure and visualize Network trajectory data with orion support.

    This class parses Network response logs (JSONL format) and generates comprehensive
    Markdown documentation including:
    - Orion evolution (before/after states)
    ...
    def __init__(self, folder_path: str) -> None:
        """
        Initialize Network trajectory parser.

        :param folder_path: Path to the Network log directory (e.g., logs/network/task_1)
        """
    ...
    def _parse_orion(
        self, orion_json: Optional[str]
    ) -> Optional[Dict[str, Any]]:
        """
        Safely parse orion JSON string with compatibility for string-serialized tasks.

        This method handles both:
        1. Properly formatted orion JSON (new format after fix)
        2. Legacy format where tasks/dependencies are Python string representations

        :param orion_json: JSON string of orion data or dict
        :return: Parsed orion dict (may include 'parse_error' key) or None
        """
        if not orion_json:
            return None

        try:
            # Handle case where orion_json might already be a dict
            # (happens when reading from logs created after the fix)
            if isinstance(orion_json, dict):
                orion = orion_json
            else:
                orion = json.loads(orion_json)

            # Compatibility fix: Handle tasks field as string (legacy format)
            if "tasks" in orion and isinstance(orion["tasks"], str):
            ...
            logger.warning(f"Failed to parse orion JSON: {e}")
            # Return error info instead of None
            return {
                "parse_error": {
                    "field": "orion",
                    "error_type": "json_decode_error",
                    "message": f"Failed to parse orion JSON: {str(e)}",
                    "raw_preview": (
                        str(orion_json)[:200] + "..."
                        if len(str(orion_json)) > 200
                        else str(orion_json)
                    ),
                }
            }
        except Exception as e:
            logger.warning(f"Unexpected error parsing orion: {e}")
            return {
                "parse_error": {
                    "field": "orion",
    ...
    def _generate_topology_image(
        ...
        :param tasks: Dictionary of task_id -> task_data
        :param orion_id: Orion ID for unique filename
        ...
        # Clean orion_id for filename
        clean_id = orion_id.replace(":", "_").replace("/", "_")
    ...
    def _format_dependency_graph(
        ...
        :param dependencies: Dictionary of line_id -> dependency_data
        :param tasks: Dictionary of task_id -> task_data
        :param orion_id: Orion ID for image filename
        ...
        image_path = self._generate_topology_image(
            dependencies, tasks, orion_id, step_number, state
        )
    ...
    def to_markdown(
        ...
        :param include_orion_details: Include detailed orion evolution
        ...
        if len(self.step_log) == 0:
            ...
            with open(output_path, "w", encoding="utf-8") as file:
                file.write("# Network Trajectory Report\n\n")
        ...
        with open(output_path, "w", encoding="utf-8") as file:
            ...
            for idx, step in enumerate(self.step_log, 1):
                ...
                # Orion Evolution
                if include_orion_details:
                    orion_before = self._parse_orion(
                        step.get("orion_before")
                    )
                    orion_after = self._parse_orion(
                        step.get("orion_after")
                    )

                    if orion_before or orion_after:
                        file.write("#### Orion Evolution\n\n")

                        # Before state
                        if orion_before:
                            file.write(
                                "<details>\n<summary>Orion BEFORE (click to expand)</summary>\n\n"
                            )
                            file.write(
                                f"**Orion ID**: {orion_before.get('orion_id', 'N/A')}\n"
                            )
                            file.write(
                                f"**State**: {orion_before.get('state', 'N/A')}\n\n"
                            )
                            ...
                            if "parse_error" in orion_before:
                                ...
                                file.write(
                                    "**Note**: This orion cannot be fully parsed. "
                                    "Basic metadata shown above. "
                                    "This issue is fixed in newer versions.\n\n"
                                )
                            else:
                                # Normal parsing - show full details
                                tasks = orion_before.get("tasks", {})
                                deps = orion_before.get("dependencies", {})
                        ...
                        if orion_after:
                            ...
                            file.write(
                                f"**Orion ID**: {orion_after.get('orion_id', 'N/A')}\n"
                            )
                            file.write(
                                f"**State**: {orion_after.get('state', 'N/A')}\n\n"
                            )
                            ...
                            if "parse_error" in orion_after:
                                ...
                                # Normal parsing - show full details
                                tasks = orion_after.get("tasks", {})
                                deps = orion_after.get("dependencies", {})
            ...
            # Final Orion State (if available)
            if self.step_log:
                last_step = self.step_log[-1]
                final_orion = self._parse_orion(
                    last_step.get("orion_after")
                )

                if final_orion and include_task_details:
                    file.write("## Final Orion State\n\n")
                    file.write(
                        f"**ID**: {final_orion.get('orion_id', 'N/A')}\n"
                    )
                    file.write(
                        f"**State**: {final_orion.get('state', 'N/A')}\n"
                    )
                    file.write(
                        f"**Created**: {final_orion.get('created_at', 'N/A')}\n"
                    )
                    file.write(
                        f"**Updated**: {final_orion.get('updated_at', 'N/A')}\n\n"
                    )
                    ...
                    if tasks and isinstance(tasks, dict):
                        ...
                        deps = final_orion.get("dependencies", {})
                        file.write("### Final Dependency Graph\n\n")
                        file.write(
                            self._format_dependency_graph(
                                deps,
                                tasks,
                                final_orion.get("orion_id", "final"),
...
if __name__ == "__main__":
    ...
    console.print(
        "[BOLD BLUE] Network Trajectory Parser - Batch Mode", style="blue bold"
    )
    ...
    if not network_logs_dir.exists():
        console.print(f"[FAIL] Directory not found: {network_logs_dir}", style="red")
        sys.exit(1)
    ...
    if not task_dirs:
        console.print(
            f"[FAIL] No task directories found in {network_logs_dir}", style="red"
        )
        sys.exit(1)
