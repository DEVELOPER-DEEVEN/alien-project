...
"""
ALIEN3 Framework Client Library

Network Framework client class providing programmatic interface
for starting Network sessions, executing DAG-based workflows, and managing
orion orchestration.

This module provides the NetworkClient class for integration into other applications.
For command-line usage, use network.py as the main entry point.
"""
...
from network.client.config_loader import OrionConfig
from alien.logging.setup import setup_logger

from .client.orion_client import OrionClient
from .session.network_session import NetworkSession
...
class NetworkClient:
    """
    Main Network Framework client for command-line interaction.

    Provides capabilities for:
    - Starting Network sessions
    - Processing user requests into DAG workflows
    - Managing orion execution
    ...
    def __init__(
        ...
        :param session_name: Name for the Network session (auto-generated if None)
        ...
        """
        self.session_name = (
            session_name or f"network_session_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
        ...
        # (network.py already calls setup_logger before importing NetworkClient)
        ...
        self._session: Optional[NetworkSession] = None
        self._current_request_task: Optional[asyncio.Task] = None
        self._is_shutting_down: bool = False

        # Load device configuration from new config system
        network_config = get_network_config()
        device_info_path = network_config.orion.DEVICE_INFO
        self._device_config = OrionConfig.from_yaml(device_info_path)
    ...
    async def initialize(self) -> None:
        """
        Initialize all Network framework components.

        Sets up agent, orion client, orchestration, context,
        and Network session with progress indication.
        """
        try:
            with self.display.show_initialization_progress() as progress:
                ...
                progress.update(
                    task, description="[cyan]Setting up Orion Client..."
                )
    ...
    async def process_request(self, request: str) -> Dict[str, Any]:
        ...
        :raises RuntimeError: If Network client is not initialized
        """
        if not self._client:
            raise RuntimeError(
                "Network client not initialized. Call initialize() first."
            )
        ...
        try:
            ...
            with self.display.show_initialization_progress() as progress:
                # progress.add_task("[cyan]Executing Network session...", total=None)
            ...
            # Add orion info if available
            if self._session and self._session.current_orion:
                orion = self._session.current_orion
                result["orion"] = {
                    "id": orion.orion_id,
                    "name": orion.name,
                    "task_count": len(orion.tasks),
                    "dependency_count": len(orion.dependencies),
    ...
    async def reset_session(self) -> Dict[str, Any]:
        ...
        Clears the current session's orion, tasks, and execution history
    ...
    async def create_next_session(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.session_name = (
                f"network_session_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            )
    ...
    async def shutdown(self, force: bool = False) -> None:
        ...
        Properly closes all components including the orion client
