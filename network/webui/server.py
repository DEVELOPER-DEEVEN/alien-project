...
"""
Network Web UI Server.

FastAPI-based server that provides WebSocket communication for the Network Web UI.
Integrates with the Network event system to provide real-time updates.
...
from network.webui.routers import health_router, devices_router, websocket_router
...
app = FastAPI(
    title="Network Web UI",
    description="Modern web interface for Network Framework",
    version="1.0.0",
    lifespan=lifespan,
)
...
async def root() -> HTMLResponse:
    ...
    return HTMLResponse(
        content="<h1>Network Web UI</h1><p>Server is running</p>", status_code=200
    )


def set_network_session(session: "NetworkSession") -> None:
    """
    Set the Network session for the web UI.

    :param session: The NetworkSession instance
    """
    app_state = get_app_state()
    app_state.network_session = session


def set_network_client(client: "NetworkClient") -> None:
    """
    Set the Network client for the web UI.

    :param client: The NetworkClient instance
    """
    app_state = get_app_state()
...
def start_server(host: str = "0.0.0.0", port: int = 8000) -> None:
    """
    Start the Network Web UI server.
    ...
    logger: logging.Logger = logging.getLogger(__name__)
    logger.info(f"Starting Network Web UI server on {host}:{port}")
