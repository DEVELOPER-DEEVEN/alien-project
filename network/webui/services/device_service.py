...
"""
Device management service for Network Web UI.
...
class DeviceService:
    """
    Service for managing devices in the Network framework.
    ...
    def build_device_snapshot(self) -> Optional[Dict[str, Dict[str, Any]]]:
        ...
        Retrieves device information from the Network client's device manager
        ...
        if not network_client:
            self.logger.warning("Network client not available for device snapshot")
            return None

        # Get orion client from Network client
        orion_client = getattr(network_client, "_client", None)
        if not orion_client:
            self.logger.warning("Orion client not available")
            return None

        # Get device manager from orion client
        device_manager = getattr(orion_client, "device_manager", None)
    ...
    def get_device_manager(self) -> Optional[Any]:
        """
        Get the device manager from the Network client.
        ...
        return getattr(orion_client, "device_manager", None)
