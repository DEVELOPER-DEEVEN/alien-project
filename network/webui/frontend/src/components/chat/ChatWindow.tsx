...
import { Message, useNetworkStore } from '../../store/networkStore';
...
const ChatWindow: React.FC = () => {
  const { messages, searchQuery, messageKind, isTaskStopped } = useNetworkStore(
    (state) => ({
          ...
          {filteredMessages.length === 0 ? (
            <div className="flex h-full flex-col items-center justify-center gap-3 text-center text-slate-400">
              ...
              <p className="max-w-sm text-sm">
                Ready to launch. Describe a mission for the Network Agent, or use quick commands below to explore diagnostics.
              </p>
            </div>
          ) : (
            <>
