...
import { Message } from '../../store/networkStore';
 ...
 * Mirrors the backend's _format_orion_operation logic.
 */
const formatOrionOperation = (action: any): string => {
  ...
  switch (func) {
    ...
    case 'build_orion': {
      ...
      if (args.task_count !== undefined || args.dependency_count !== undefined) {
        ...
        return `Build Orion (${taskCount} tasks, ${depCount} dependencies)`;
      }
    ...
    case 'load_orion': {
      ...
      return `Load Orion from '${filename}'`;
    }
...
const ActionTreeNode: React.FC<{
  ...
  const operation = formatOrionOperation(action);
...
const MessageBubble: React.FC<MessageBubbleProps> = ({ message, nextMessage, stepNumber }) => {
  ...
  const displayName = useMemo(() => {
    ...
    return message.agentName.toLowerCase().includes('orion') ? 'ALIEN' : message.agentName;
    ...
    >
      <div
        className={clsx(
          ...
          isUser
            ? 'rounded-br-xl border-network-blue/50 bg-gradient-to-br from-network-blue/25 via-network-purple/25 to-network-blue/15 text-slate-50 shadow-[0_0_30px_rgba(15,123,255,0.2),inset_0_1px_0_rgba(147,197,253,0.15)]'
                  ...
                  <ul className="space-y-1 text-sm">
                    {responsePayload.plan.map((step: string, index: number) => (
                      <li key={index} className="flex items-start gap-2 text-slate-200">
                        <span className="mt-[2px] h-2 w-2 rounded-full bg-network-blue" aria-hidden />
                        <span>{step}</span>
                      </li>
                    ))}
                  </ul>
