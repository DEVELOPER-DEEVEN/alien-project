...
import { createClientId, useNetworkStore } from '../../store/networkStore';
...
const Composer: React.FC = () => {
  ...
  const { connected, session, ui, toggleComposerShortcuts, resetSessionState, messages, setTaskRunning, stopCurrentTask } = useNetworkStore((state) => ({
    ...
    (command: string) => {
      switch (command) {
        case '/reset':
          ...
          resetSessionState({ clearHistory: true }); // Explicitly clear all history including orions
          return true;
        case '/replay': {
          ...
          resetSessionState({ clearHistory: false }); // Keep orion history
  ...
  const handleSubmit = useCallback(async () => {
    ...
    // Check if there are existing orions - if yes, create a placeholder for the new request
    const currentOrions = Object.keys(store.orions);
    if (currentOrions.length > 0) {
      // Create a temporary empty orion to provide immediate visual feedback
      const tempOrionId = `temp-${Date.now()}`;
      store.upsertOrion({
        id: tempOrionId,
        name: 'Loading...',
        status: 'pending',
        description: 'Waiting for orion to be created...',
        taskIds: [],
        dag: { nodes: [], edges: [] },
      ...
      // Switch to the new empty orion
      store.setActiveOrion(tempOrionId);
      console.log('📊 Created temporary orion for new request');
    }
    ...
    <div className="relative rounded-[30px] border border-white/10 bg-gradient-to-br from-[rgba(11,24,44,0.82)] to-[rgba(8,15,28,0.75)] p-4 shadow-[0_8px_32px_rgba(0,0,0,0.4),0_2px_8px_rgba(15,123,255,0.12),inset_0_1px_1px_rgba(255,255,255,0.06)] ring-1 ring-inset ring-white/5">{/* backdrop-blur-md removed for performance */}
      <div className="relative">
        <textarea
          ...
          placeholder={connected ? 'Ask Network to orchestrate a new mission…' : 'Waiting for connection…'}
          rows={3}
