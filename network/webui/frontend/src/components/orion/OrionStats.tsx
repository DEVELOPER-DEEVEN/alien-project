...
interface OrionStatsProps {
  orion: OrionSummary;
  onBack: () => void;
}

const OrionStats: React.FC<OrionStatsProps> = ({ orion, onBack }) => {
  // Get statistics from metadata (sent from backend via get_statistics())
  const stats = orion.metadata?.statistics || {};
  const statusCounts = stats.task_status_counts || {};
  
  const totalTasks = stats.total_tasks || orion.statistics.total;
  ...
  return (
    <div className="flex h-full flex-col gap-4 overflow-y-auto p-1">
      ...
      {/* Additional Metadata */}
      {orion.metadata && Object.keys(orion.metadata).length > 0 && (
        <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
          <div className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400 mb-3">
          ...
          <div className="space-y-2 text-xs">
            {orion.description && (
              <div>
                ...
                <div className="mt-1 text-slate-200">{orion.description}</div>
              </div>
            )}
            {orion.metadata.display_name && (
              <div className="flex justify-between">
                <span className="text-slate-400">Name:</span>
                <span className="text-slate-200">{orion.metadata.display_name}</span>
              </div>
            )}
          </div>
        </div>
