...
interface NetworkStore {
  ...
  addEventToLog: (event: NetworkEvent) => void;
  clearEventLog: () => void;

  orions: Record<string, OrionSummary>;
  upsertOrion: (orion: Partial<OrionSummary> & { id: string }) => void;
  removeOrion: (id: string) => void;
  setActiveOrion: (id: string | null) => void;

  tasks: Record<string, Task>;
  bulkUpsertTasks: (orionId: string, tasks: Array<Partial<Task> & { id: string }>, dependencies?: Record<string, string[]>) => void;
...
const defaultSessionState = (): SessionState => ({
  id: null,
  displayName: 'Network Session',
  welcomeText: 'Launch a request to orchestrate a new TaskOrion.',
  startedAt: null,
  debugMode: false,
  highContrast: false,
});
...
export const useNetworkStore = create<NetworkStore>()((set, get) => ({
  ...
  orions: mockData ? { [mockData.orion.id]: mockData.orion } : {},
  upsertOrion: (orion) => {
    set((state) => {
      const existing = state.orions[orion.id];
      const taskIds = orion.taskIds || existing?.taskIds || [];
      const updatedStats = computeOrionStats(
        orion.id,
        taskIds,
        state.tasks,
      );

      const updated: OrionSummary = {
        id: orion.id,
        name: orion.name || existing?.name || orion.id,
        status: (orion.status as TaskStatus) || existing?.status || 'pending',
        description: orion.description ?? existing?.description,
        metadata: orion.metadata ?? existing?.metadata,
        createdAt: orion.createdAt ?? existing?.createdAt ?? getNow(),
        updatedAt: getNow(),
        taskIds,
        dag: {
          nodes: orion.dag?.nodes ?? existing?.dag.nodes ?? [],
          edges: orion.dag?.edges ?? existing?.dag.edges ?? [],
        },
      ...
      return {
        ...
        ui: {
          ...state.ui,
          activeOrionId:
            state.ui.activeOrionId || orion.id,
        },
      };
    });
  },
  removeOrion: (id) =>
    set((state) => {
      const { [id]: removed, ...remaining } = state.orions;
      return {
        orions: remaining,
        ui: {
          ...state.ui,
          // If the removed orion was active, clear the active selection
          activeOrionId:
            state.ui.activeOrionId === id
              ? null
              : state.ui.activeOrionId,
        },
      };
    }),
  setActiveOrion: (id) =>
  ...
  bulkUpsertTasks: (orionId, tasks, dependencies = {}) => {
    set((state) => {
      ...
      const existingOrion = state.orions[orionId];

      return {
        tasks: nextTasks,
        orions: {
          ...state.orions,
          [orionId]: existingOrion
            ? {
                ...existingOrion,
  ...
  updateTask: (taskId, patch) => {
    set((state) => {
      ...
      const orion = state.orions[existing.orionId];

      const nextState: Partial<NetworkStore> = {
        tasks: {
          ...state.tasks,
          [taskId]: updatedTask,
        },
      } as Partial<NetworkStore>;
      ...
      return nextState as NetworkStore;
    });
  },
  ...
  ui: {
    ...defaultUIState(),
    activeOrionId: mockData?.orion.id || null,
  },
  ...
  resetSessionState: (options?: { clearHistory?: boolean }) =>
    set((state) => {
      ...
      return {
        messages: [],
        eventLog: [],
        orions: clearHistory ? {} : state.orions,
