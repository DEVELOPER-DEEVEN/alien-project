...
import App from './App';
import './index.css';
import { NetworkEvent, getWebSocketClient } from './services/websocket';
import {
  createClientId,
  NotificationItem,
  Task,
...
const safeTimestamp = (event: NetworkEvent) =>
...
const extractOrionPayload = (event: NetworkEvent) => {
  const data = event.data || {};
  return (
    data.orion ||
    data.updated_orion ||
    data.new_orion ||
    event.output_data?.orion ||
    null
  );
};

const updateOrionFromPayload = (event: NetworkEvent) => {
  const orion = extractOrionPayload(event);
  if (!orion) {
    return;
  }
  ...
  const store = useNetworkStore.getState();
  const orionId =
    orion.orion_id || event.orion_id || store.ensureSession();

  const dependencies = orion.dependencies || {};

  const tasks: Array<Partial<Task> & { id: string }> = [];
  if (orion.tasks) {
    Object.entries(orion.tasks).forEach(([taskId, raw]) => {
  ...
  store.bulkUpsertTasks(orionId, tasks, dependencies);
  ...
  store.upsertOrion({
    id: orionId,
    name: orion.name || orionId,
    status: orion.state || event.orion_state || 'running',
    description: orion.description,
    metadata: {
      ...(orion.metadata || {}),
      statistics: orion.statistics,  // Include statistics at top level of metadata
      execution_start_time: orion.metadata?.execution_start_time,
      execution_end_time: orion.metadata?.execution_end_time,
    },
    createdAt: parseIsoOrUndefined(orion.created_at),
...
const handleAgentResponse = (event: NetworkEvent) => {
  const store = useNetworkStore.getState();
  ...
  store.addMessage({
    ...
    role: 'assistant',
    kind: 'response',
    author: event.agent_name || 'Network Agent',
    content,
...
const handleAgentAction = (event: NetworkEvent) => {
  const store = useNetworkStore.getState();
...
const handleTaskEvent = (event: NetworkEvent) => {
  const store = useNetworkStore.getState();
  const orionId =
    event.orion_id || event.data?.orion_id || extractOrionPayload(event)?.orion_id;

  if (!event.task_id || !orionId) {
    return;
  }

  // Update orion from task event data FIRST if available
  // This ensures orion state (including tips) is populated before individual task updates
  if ((event.event_type === 'task_completed' || event.event_type === 'task_failed') && event.data?.orion) {
    updateOrionFromPayload(event);
  }
...
const handleOrionEvent = (event: NetworkEvent) => {
  updateOrionFromPayload(event);

  // Auto-switch to new orion when it starts
  if (event.event_type === 'orion_started') {
    const store = useNetworkStore.getState();
    const orionId = event.orion_id;
    if (orionId) {
      // Remove any temporary orions that were created as placeholders
      Object.keys(store.orions).forEach((id) => {
        if (id.startsWith('temp-')) {
          store.removeOrion(id);
          console.log(`🗑️ Removed temporary orion: ${id}`);
        }
      });
      
      store.setActiveOrion(orionId);
      console.log(`🌟 Auto-switched to new orion: ${orionId}`);
    }
  }

  if (event.event_type === 'orion_completed') {
    emitNotification({
      severity: 'success',
      title: 'Orion completed',
      description: `Orion ${event.orion_id || ''} finished execution successfully.`,
      source: event.orion_id,
    });
  }

  if (event.event_type === 'orion_failed') {
    emitNotification({
      severity: 'error',
      title: 'Orion failed',
      description: `Orion ${event.orion_id || ''} reported a failure.`,
      source: event.orion_id,
    });
  }
};

const handleDeviceEvent = (event: NetworkEvent) => {
  ...
  window.setTimeout(() => {
    useNetworkStore.getState().clearDeviceHighlight(deviceId);
  }, 4000);
...
const handleGenericEvent = (event: NetworkEvent) => {
  ...
  if (messageType === 'reset_acknowledged') {
    ...
    useNetworkStore.getState().pushNotification({
      id: `reset-${Date.now()}`,
      title: 'Session Reset',
  ...
  if (messageType === 'stop_acknowledged') {
    ...
    // Note: We don't clear orion/tasks/devices - they persist after stop
    return;
  }
  ...
  switch (event.event_type) {
    ...
    case 'orion_started':
    case 'orion_modified':
    case 'orion_completed':
    case 'orion_failed':
      handleOrionEvent(event);
      break;
    case 'task_started':
    case 'task_completed':
    case 'task_failed':
...
wsClient
  .connect()
  .catch((error) => {
    console.error('❌ Failed to connect to Network WebSocket server:', error);
