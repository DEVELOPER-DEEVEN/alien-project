...
"""
WebSocket Observer for Network Web UI.

This observer subscribes to all Network events and pushes them to connected WebSocket clients.
...
class EventSerializer:
    ...
    def _initialize_type_cache(self) -> None:
        ...
        # Try to import TaskOrion
        try:
            from network.orion import TaskOrion

            self._cached_types["TaskOrion"] = TaskOrion
        except ImportError:
            self._cached_types["TaskOrion"] = None
            self.logger.debug("TaskOrion not available for serialization")
    ...
    def _register_handlers(self) -> None:
        ...
        task_orion_type = self._cached_types.get("TaskOrion")
        if task_orion_type:
            self._type_handlers[task_orion_type] = self._serialize_orion
    ...
    def serialize_event(self, event: Event) -> Dict[str, Any]:
        ...
        if isinstance(event, TaskEvent):
            ...
            base_dict.update(self._serialize_orion_event_fields(event))
    ...
    def _serialize_orion_event_fields(
        self, event: OrionEvent
    ) -> Dict[str, Any]:
        """
        Extract orion-specific fields from a OrionEvent.

        :param event: The orion event to serialize
        :return: Dictionary of orion-specific fields
        """
        return {
            "orion_id": event.orion_id,
            "orion_state": event.orion_state,
    ...
    def serialize_value(self, value: Any) -> Any:
        ...
        and custom Network objects using a chain of serialization strategies.
    ...
    def _serialize_orion(self, value: Any) -> Dict[str, Any]:
        """
        Serialize a TaskOrion object.

        :param value: TaskOrion instance
        :return: Serialized dictionary with orion details
        """
        try:
            orion_dict = {
                "orion_id": value.orion_id,
                "name": value.name,
                "state": self._extract_enum_value(value.state),
                "tasks": self._serialize_orion_tasks(value.tasks),
            ...
            if hasattr(value, "get_statistics") and callable(value.get_statistics):
                try:
                    orion_dict["statistics"] = value.get_statistics()
                except Exception as e:
                    self.logger.warning(f"Failed to get orion statistics: {e}")

            return orion_dict
        except Exception as e:
            self.logger.warning(f"Failed to serialize TaskOrion: {e}")
            return str(value)

    def _serialize_orion_tasks(
        self, tasks: Dict[str, Any]
    ) -> Dict[str, Dict[str, Any]]:
        """
        Serialize all tasks in a orion.
...
class WebSocketObserver(IEventObserver):
    """
    Observer that forwards all Network events to WebSocket clients.
