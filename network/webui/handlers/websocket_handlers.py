...
"""
WebSocket message handlers for Network Web UI.
...
from network.webui.models.enums import WebSocketMessageType, RequestStatus
from network.webui.services import DeviceService, NetworkService
...
class WebSocketMessageHandler:
    ...
    def __init__(self, app_state: AppState) -> None:
        ...
        self.network_service = NetworkService(app_state)
    ...
    async def _handle_request(self, websocket: WebSocket, data: dict) -> None:
        ...
        if not self.network_service.is_client_available():
            await websocket.send_json(
                {
                    "type": WebSocketMessageType.ERROR,
                    "message": "Network client not initialized",
                }
            )
            return
        ...
        async def process_in_background() -> None:
            try:
                result = await self.network_service.process_request(request_text)
                await websocket.send_json(
                    {
    ...
    async def _handle_reset(self, websocket: WebSocket, data: dict) -> None:
        ...
        Resets the current Network session and clears state.
        ...
        try:
            result = await self.network_service.reset_session()
            await websocket.send_json(
                {
                    "type": WebSocketMessageType.RESET_ACKNOWLEDGED,
    ...
    async def _handle_next_session(self, websocket: WebSocket, data: dict) -> None:
        ...
        Creates a new Network session while potentially maintaining some context.
        ...
        try:
            result = await self.network_service.create_next_session()
            await websocket.send_json(
                {
    ...
    async def _handle_stop_task(self, websocket: WebSocket, data: dict) -> None:
        ...
        Shuts down the Network client to clean up device tasks, then reinitializes
        ...
        if not self.network_service.is_client_available():
            self.logger.warning("No active network client to stop")
            await websocket.send_json(
                {
        ...
        try:
            new_session_result = await self.network_service.stop_task_and_restart()
            await websocket.send_json(
                {
                    "type": WebSocketMessageType.STOP_ACKNOWLEDGED,
