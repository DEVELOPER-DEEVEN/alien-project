...
"""
Orion result formatter for beautiful and structured display.
"""
...
class OrionFormatter:
    """Formatter for displaying orion execution results in a structured way."""
    ...
    def display_orion_result(self, orion_data: Dict[str, Any]):
        """
        Display orion execution result in a beautiful structured format.

        Args:
            orion_data: Dictionary containing orion execution data
        """
        self.console.print("\n")
        ...
        # Overview table
        overview = self.create_overview_table(orion_data)
        self.console.print(overview)
        self.console.print()

        # Statistics table
        if "statistics" in orion_data:
            stats_table = self.create_statistics_table(orion_data["statistics"])
            self.console.print(stats_table)
            self.console.print()

            # Critical path panel
            critical_panel = self.create_critical_path_panel(
                orion_data["statistics"]
            )
            if critical_panel:
...
def format_orion_result(result_data: Dict[str, Any]):
    """
    Utility function to format and display orion result.

    Args:
        result_data: Dictionary containing orion execution data
    ...
    Example usage:
        ...
        ...     'statistics': {...}
        ... }
        >>> format_orion_result(data)
    """
    formatter = OrionFormatter()
    formatter.display_orion_result(result_data)
...
if __name__ == "__main__":
    ...
    format_orion_result(example_data)
