...
from ..orion.task_star_line import TaskStarLine


class VisualizationChangeDetector:
    """
    Utility class for detecting and analyzing changes between orion states.
    ...
    @staticmethod
    def calculate_orion_changes(
        old_orion: Optional["TaskOrion"],
        new_orion: "TaskOrion",
    ) -> Dict[str, Any]:
        """
        Calculate detailed changes between old and new orion by comparing their structure.

        :param old_orion: Previous orion state (can be None for new orion)
        :param new_orion: Current orion state
        :return: Dictionary containing detailed changes
        """
        changes = {
            "modification_type": "orion_created",
            "added_tasks": [],
            "removed_tasks": [],
            "modified_tasks": [],
            "added_dependencies": [],
        ...
        if not old_orion:
            # New orion - all tasks and dependencies are "added"
            changes["modification_type"] = "orion_created"
            changes["added_tasks"] = [
                task.task_id for task in new_orion.tasks.values()
            ]
            changes["added_dependencies"] = [
                f"{dep.from_task_id}->{dep.to_task_id}"
                for dep in new_orion.dependencies.values()
            ]
            return changes

        # Get task IDs for comparison
        old_task_ids = set(old_orion.tasks.keys())
        new_task_ids = set(new_orion.tasks.keys())
        ...
        for task_id in common_task_ids:
            old_task = old_orion.tasks[task_id]
            new_task = new_orion.tasks[task_id]
        ...
        for dep in old_orion.dependencies.values():
