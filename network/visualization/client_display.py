...
"""
Network Client Display Module

This module provides rich console display utilities for the Network client,
...
class ClientDisplay:
    """
    Rich console display manager for Network client.
    ...
    def show_network_banner(self) -> None:
        """
        Show the Network Framework banner.
...
╚══════════════════════════════════════╝"""
    ...
    def show_welcome_with_usage(self) -> None:
        ...
        welcome_panel = Panel(
            ...
            "  [cyan]python -m network --interactive[/cyan]\n"
            "  [cyan]python -m network --demo[/cyan]\n\n"
            "[bold yellow]Advanced Usage:[/bold yellow]\n"
            "  [cyan]python -m network --request 'Task' --session-name 'my_session'[/cyan]\n"
            "  [cyan]python -m network --interactive --max-rounds 20[/cyan]\n\n"
    ...
    def show_status(
        ...
        Displays a formatted table with current Network session
    ...
    def display_result(self, result: Dict[str, Any]) -> None:
        ...
        if result.get("orion"):
            ...
            result_table.add_row(
                "Orion",
                f"[bold]{orion['name']}[/bold] ({orion['task_count']} tasks)",
            )
            result_table.add_row("State", orion.get("state", "Unknown"))
        ...
        # Show orion details if available
        if result.get("orion"):
            orion = result["orion"]
            orion_panel = Panel(
                f"[bold cyan]Orion Details:[/bold cyan]\n"
                f"• ID: {orion.get('id', 'N/A')}\n"
                f"• Tasks: {orion.get('task_count', 0)}\n"
                f"• Dependencies: {orion.get('dependency_count', 0)}\n"
                f"• State: {orion.get('state', 'Unknown')}",
                title="DAG Information",
                border_style="cyan",
            )
            self.console.print(orion_panel)
