...
from network.visualization import DAGVisualizer, TaskDisplay
...
class TaskVisualizationHandler:
    ...
    async def handle_task_started(
        self, event: TaskEvent, orion: TaskOrion
    ) -> None:
        """
        ...
        try:
            ...
            task = orion.get_task(task_id) if task_id else None
            ...
            # Show topology for smaller orions
            # if orion.task_count <= 10:
    ...
    async def handle_task_completed(
        ...
        try:
            ...
            # Show execution progress for smaller orions
            if orion.task_count <= 10:
                self._visualizer.display_execution_flow(orion)
    ...
    async def handle_task_failed(
        ...
        try:
            ...
            # Always show failure status regardless of orion size
            # self._visualizer.display_execution_flow(orion)
    ...
    async def handle_task_event(
        ...
        if event.event_type == EventType.TASK_STARTED:
            await self.handle_task_started(event, orion)
        elif event.event_type == EventType.TASK_COMPLETED:
            await self.handle_task_completed(event, orion)
        elif event.event_type == EventType.TASK_FAILED:
            await self.handle_task_failed(event, orion)
