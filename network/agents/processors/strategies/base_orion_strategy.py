...
"""
Base strategies for Orion Agent processing.

This module provides base classes for different types of orion processing strategies,
...
class OrionLLMInteractionStrategy(BaseProcessingStrategy):
    """
    Base LLM interaction strategy for Orion Agent with shared logic.
    ...
    def __init__(self, fail_fast: bool = True) -> None:
        """
        Initialize base Orion LLM interaction strategy.
        :param fail_fast: Whether to raise exceptions immediately on errors
        """
        super().__init__(
            name=f"orion_llm_interaction",
            fail_fast=fail_fast,
        )

    async def execute(
        self, agent: "OrionAgent", context: ProcessingContext
    ) -> ProcessingResult:
        """
        Execute LLM interaction with comprehensive error handling and retry logic.
        :param agent: The Orion instance.
        ...
        try:
            ...
            orion: TaskOrion = context.get_global("ORION")
            ...
            self.logger.info(f"Orion LLM interaction completed successfully")
            ...
            error_msg = (
                f"orion LLM interaction failed: {str(traceback.format_exc())}"
            )
    ...
    async def _build_comprehensive_prompt(
        ...
        try:
            ...
            orion_json = orion.to_json() if orion else ""

            # Log request data for debugging
            self._log_request_data(
                session_step=session_step,
                device_info=device_info,
                orion_json=orion_json,
                request=request,
                prompt_message=prompt_message,
    ...
    async def _get_llm_response_with_retry(
        self, agent: "OrionAgent", prompt_message: Dict[str, Any]
    ) -> tuple[str, float]:
        """
    ...
    def _parse_and_validate_response(
        self, agent: "OrionAgent", response_text: str
    ) -> OrionAgentResponse:
        """
    ...
    def _validate_response_fields(self, response: OrionAgentResponse) -> None:
        """
...
class BaseOrionActionExecutionStrategy(BaseProcessingStrategy):
    """
    Base strategy for executing Orion actions with shared logic.
    ...
    def __init__(self, weaving_mode: WeavingMode, fail_fast: bool = False) -> None:
        """
        Initialize base Orion action execution strategy.
    ...
    async def execute(
        ...
        try:
            ...
            parsed_response: OrionAgentResponse = context.get_local(
                "parsed_response"
            )
            ...
            error_msg = f"Orion action execution ({self.weaving_mode.value}) failed: {str(traceback.format_exc())}"
            self.logger.error(error_msg)
    ...
    async def _execute_orion_action(
        ...
        try:
            ...
            raise Exception(f"Failed to execute orion action: {str(e)}")
...
class OrionMemoryUpdateStrategy(BaseProcessingStrategy):
    """
    Memory update strategy for Orion Agent - shared across all modes.
    ...
    def __init__(self, fail_fast: bool = False) -> None:
        """
        Initialize Orion Agent memory update strategy.
        :param fail_fast: Whether to raise exceptions immediately on errors
        """
        super().__init__(name="orion_memory_update", fail_fast=fail_fast)
    ...
    async def execute(
        ...
        try:
            ...
            error_msg = f"Orion Agent memory update failed: {str(e)}"
            self.logger.error(error_msg)
    ...
    def _create_and_populate_memory_item(
        ...
        additional_memory: "OrionProcessorContext",
    ) -> MemoryItem:
        """
