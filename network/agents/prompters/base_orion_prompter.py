...
"""
Base Orion Agent Prompter.

This module provides the base prompter class for Orion Agents with
...
class BaseOrionPrompter(BasicPrompter, ABC):
    """
    Base prompter for Orion Agent with shared functionality.
    ...
    def __init__(self, prompt_template: str, example_prompt_template: str):
        """
        Initialize base orion prompter.
    ...
    def _format_orion(self, orion: TaskOrion) -> str:
        """
        Format orion information for prompt inclusion with modification hints.

        :param orion: Task orion object
        :return: Formatted orion string with modification indicators
        """
        if orion is None:
            return "No orion information available."

        try:
            orion_dict = orion.to_dict()
        except Exception:
            return "Orion information unavailable due to formatting error."

        lines = []

        # Header information
        lines.append(f"Task Orion: {orion_dict.get('name', 'Unnamed')}")
        lines.append(f"Status: {orion_dict.get('state', 'unknown')}")
        lines.append(f"Total Tasks: {len(orion_dict.get('tasks', {}))}")
        lines.append("")

        # Get modifiable items for reference
        try:
            modifiable_task_ids = {
                task.task_id for task in orion.get_modifiable_tasks()
            }
            modifiable_dep_ids = {
                dep.line_id for dep in orion.get_modifiable_dependencies()
            }
        except Exception:
        ...
        dependencies = orion_dict.get("dependencies", {})
        if dependencies:
    ...
    def user_content_construction(
        ...
        :param orion: The task orion.
        return: The prompt for LLMs.
        """

        prompt_text = self.user_prompt_construction(request, device_info, orion)
    ...
    def user_prompt_construction(
        ...
        prompt = self.prompt_template["user"].format(
            ...
            orion=self._format_orion(orion),
        )
...
class OrionPrompterFactory:
    """
    Factory class for creating Orion prompters based on weaving mode.
    ...
    def create_prompter(
        ...
        if not cls._prompter_classes:
            ...
            cls._prompter_classes = {
                WeavingMode.CREATION: OrionCreationPrompter,
                WeavingMode.EDITING: OrionEditingPrompter,
            }
        ...
        if weaving_mode == WeavingMode.CREATION:
            prompt_template = agent_config.ORION_CREATION_PROMPT
            example_prompt_template = agent_config.ORION_CREATION_EXAMPLE_PROMPT
        elif weaving_mode == WeavingMode.EDITING:
            prompt_template = agent_config.ORION_EDITING_PROMPT
            example_prompt_template = agent_config.ORION_EDITING_EXAMPLE_PROMPT
