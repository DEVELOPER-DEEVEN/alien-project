...
"""
TaskOrion - DAG management system for Orion V2.
...
from network.orion.enums import OrionState
from network.visualization.dag_visualizer import DAGVisualizer

from ..core.interfaces import IOrion

# Use the orion-specific TaskStatus instead of contracts
from .enums import OrionState, TaskStatus
...
class TaskOrion(IOrion):
    """
    Manages a DAG of tasks (TaskOrion) with comprehensive orchestration capabilities.
    ...
    def __init__(
        ...
        """
        Initialize a TaskOrion.

        :param orion_id: Unique identifier (auto-generated if None)
        :param name: Human-readable name for the orion
        """
        self._orion_id: str = (
            orion_id
            or f"orion_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{str(uuid.uuid4())[:8]}"
        )
        self._name: str = name or self._orion_id
        self._state: OrionState = OrionState.CREATED
    ...
    @property
    def orion_id(self) -> str:
        """Get the orion ID."""
        return self._orion_id
    ...
    def add_task(self, task: TaskStar) -> None:
        ...
        # Update orion state as task composition changed
        self.update_state()

    def remove_task(self, task_id: str) -> None:
        """
        Remove a task from the orion.
    ...
    def add_dependency(self, dependency: TaskStarLine) -> None:
        ...
        # Update orion state as dependencies changed
        self.update_state()

    def remove_dependency(self, dependency_id: str) -> None:
        """
        Remove a dependency from the orion.
    ...
    def is_complete(self) -> bool:
        """Check if the entire orion has completed execution."""
        return all(task.is_terminal for task in self._tasks.values())

    def update_state(self) -> None:
        """Update the orion state based on task states."""
        if not self._tasks:
            self._state = OrionState.CREATED
            return
        ...
        if all_terminal:
            if has_failed and has_completed:
                self._state = OrionState.PARTIALLY_FAILED
            elif has_failed:
                self._state = OrionState.FAILED
            else:
                self._state = OrionState.COMPLETED
        elif has_running or has_completed:
            self._state = OrionState.EXECUTING
        else:
            self._state = OrionState.READY
    ...
    def start_task(self, task_id: str) -> None:
        ...
        # Update orion state as task status changed
        self.update_state()
    ...
    def get_statistics(self) -> Dict[str, Any]:
        """
        Get statistics about the orion.
    ...
    def to_dict(self) -> Dict[str, Any]:
        """
        Convert the TaskOrion to a dictionary representation.

        :return: Dictionary representation of the TaskOrion
        """
    ...
    @staticmethod
    def _parse_orion_state(state_value: Any) -> OrionState:
        """
        Parse orion state value (string or OrionState) into OrionState enum.

        :param state_value: State value to parse
        :return: OrionState enum instance
        """
        if isinstance(state_value, OrionState):
            return state_value
        elif isinstance(state_value, str):
            # Map string names to OrionState
            state_map = {
                "CREATED": OrionState.CREATED,
                "READY": OrionState.READY,
                "EXECUTING": OrionState.EXECUTING,
                "COMPLETED": OrionState.COMPLETED,
                "FAILED": OrionState.FAILED,
                "PARTIALLY_FAILED": OrionState.PARTIALLY_FAILED,
            }
            return state_map.get(state_value.upper(), OrionState.CREATED)
        else:
            return OrionState.CREATED

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "TaskOrion":
        """
        Create a TaskOrion from a dictionary representation.

        :param data: Dictionary representation
        :return: TaskOrion instance
        """
        # Create orion with basic properties
        orion = cls(
            orion_id=data.get("orion_id"), name=data.get("name")
        )

        # Restore state and metadata
        orion._state = cls._parse_orion_state(
            data.get("state", OrionState.CREATED.value)
        )
        orion._metadata = data.get("metadata", {})

        # Restore timestamps
        if data.get("created_at"):
            orion._created_at = datetime.fromisoformat(data["created_at"])
        if data.get("updated_at"):
            orion._updated_at = datetime.fromisoformat(data["updated_at"])
        if data.get("execution_start_time"):
            orion._execution_start_time = datetime.fromisoformat(
                data["execution_start_time"]
            )
        if data.get("execution_end_time"):
            orion._execution_end_time = datetime.fromisoformat(
        ...
        for dep_id, dep_data in data.get("dependencies", {}).items():
            ...
            orion._dependencies[dep_id] = dependency

        return orion

    def to_json(self, save_path: Optional[str] = None) -> str:
        """
        Convert the TaskOrion to a JSON string representation.

        :param save_path: Optional file path to save the JSON to disk
        :return: JSON string representation of the TaskOrion
        :raises IOError: If file writing fails when save_path is provided
        """
        import json

        # Get dictionary representation
        orion_dict = self.to_dict()

        # Handle potentially non-serializable attributes
        serializable_dict = self._ensure_json_serializable(orion_dict)
        ...
        if save_path:
            try:
                ...
                raise IOError(
                    f"OS error saving TaskOrion to {save_path}: {e}"
                ) from e
            except Exception as e:
                raise IOError(
                    f"Unexpected error saving TaskOrion to {save_path}: {e}"
                ) from e
    ...
    def from_json(
        ...
        """
        Create a TaskOrion from a JSON string or JSON file.

        :param json_data: JSON string representation of the TaskOrion
        :param file_path: Path to JSON file containing TaskOrion data
        :return: TaskOrion instance
        ...
        # Create TaskOrion instance from dictionary
        return cls.from_dict(data)

    @classmethod
    def from_basemodel(cls, schema: "TaskOrionSchema") -> "TaskOrion":
        """
        Create a TaskOrion from a Pydantic BaseModel schema.

        :param schema: TaskOrionSchema instance
        :return: TaskOrion instance
        """
        from network.agents.schema import TaskOrionSchema

        if not isinstance(schema, TaskOrionSchema):
            raise ValueError("Expected TaskOrionSchema instance")

        # Convert schema to dict and use existing from_dict method
        data = schema.model_dump()
        return cls.from_dict(data)

    def to_basemodel(self) -> "TaskOrionSchema":
        """
        Convert the TaskOrion to a Pydantic BaseModel schema.

        :return: TaskOrionSchema instance
        """
        from network.agents.schema import TaskOrionSchema

        # Get dictionary representation and create schema
        data = self.to_dict()
        return TaskOrionSchema(**data)
    ...
    def display_dag(self, mode: str = "overview") -> None:
        ...
        try:
            ...
            if mode == "overview":
                visualizer.display_orion_overview(self)
            elif mode == "topology":
    ...
    def __str__(self) -> str:
        """String representation of the TaskOrion."""
        return f"TaskOrion(id={self._orion_id}, tasks={len(self._tasks)}, state={self._state.value})"

    def __repr__(self) -> str:
        """Detailed representation of the TaskOrion."""
        return (
            f"TaskOrion(orion_id={self._orion_id!r}, "
            f"name={self._name!r}, "
