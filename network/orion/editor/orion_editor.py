...
Main editor class providing high-level interface for orion manipulation
...
class OrionEditor:
    """
    High-level editor for TaskOrion manipulation.

    Provides a command pattern-based interface for comprehensive
    orion editing operations with undo/redo support.
    """

    def __init__(
        self,
        orion: Optional[TaskOrion] = None,
    ...
    def add_observer(self, observer: callable) -> None:
        """
        Add an observer for orion changes.
    ...
    def add_task(self, task: Union[TaskStar, Dict[str, Any]]) -> TaskStar:
        ...
        command = AddTaskCommand(self._orion, task_data)
    ...
    def create_and_add_task(
        ...
        """
        Create and add a new task to the orion.
    ...
    def remove_task(self, task_id: str) -> str:
        ...
        :raises: CommandExecutionError if operation fails
        """
        command = RemoveTaskCommand(self._orion, task_id)
        result = self._invoker.execute(command)
    ...
    def update_task(self, task_id: str, **updates) -> TaskStar:
        ...
        :raises: CommandExecutionError if operation fails
        """
        command = UpdateTaskCommand(self._orion, task_id, updates)
        result = self._invoker.execute(command)
    ...
    def get_task(self, task_id: str) -> Optional[TaskStar]:
        ...
        """
        return self._orion.get_task(task_id)
    ...
    def list_tasks(self) -> List[TaskStar]:
        ...
        """
        return self._orion.get_all_tasks()
    ...
    def add_dependency(
        ...
        command = AddDependencyCommand(self._orion, dependency_data)
        result = self._invoker.execute(command)
    ...
    def create_and_add_dependency(
        ...
        **kwargs,
        ...
        """
        Create and add a new dependency to the orion.
    ...
    def remove_dependency(self, dependency_id: str) -> str:
        ...
        :raises: CommandExecutionError if operation fails
        """
        command = RemoveDependencyCommand(self._orion, dependency_id)
        result = self._invoker.execute(command)
    ...
    def update_dependency(self, dependency_id: str, **updates) -> TaskStarLine:
        """
        Update a dependency in the orion.
        ...
        :raises: CommandExecutionError if operation fails
        """
        command = UpdateDependencyCommand(self._orion, dependency_id, updates)
        result = self._invoker.execute(command)
    ...
    def get_dependency(self, dependency_id: str) -> Optional[TaskStarLine]:
        ...
        """
        return self._orion.get_dependency(dependency_id)
    ...
    def list_dependencies(self) -> List[TaskStarLine]:
        ...
        """
        return self._orion.get_all_dependencies()
    ...
    def get_task_dependencies(self, task_id: str) -> List[TaskStarLine]:
        ...
        """
        return self._orion.get_task_dependencies(task_id)

    # Bulk Operations

    def build_orion(
        self, config: TaskOrionSchema, clear_existing: bool = True
    ) -> TaskOrion:
        """
        Build orion from configuration.

        :param config: Configuration dictionary
        :param clear_existing: Whether to clear existing tasks/dependencies
        :return: The built orion
        :raises: CommandExecutionError if operation fails
        """
        command = BuildOrionCommand(self._orion, config, clear_existing)
        result = self._invoker.execute(command)
        self._notify_observers("build_orion", result)
        self._orion = result  # Update reference in case of new instance
        return result
    ...
    def build_from_tasks_and_dependencies(
        ...
        """
        Build orion from task and dependency lists.
        ...
        return self.build_orion(config, clear_existing)

    def clear_orion(self) -> TaskOrion:
        """
        Clear all tasks and dependencies from the orion.

        :return: The cleared orion
        :raises: CommandExecutionError if operation fails
        """
        command = ClearOrionCommand(self._orion)
        result = self._invoker.execute(command)
        self._notify_observers("clear_orion", result)
        return result

    # File Operations

    def load_orion(self, file_path: str) -> TaskOrion:
        """
        Load orion from JSON file.

        :param file_path: Path to JSON file
        :return: The loaded orion
        :raises: CommandExecutionError if operation fails
        """
        command = LoadOrionCommand(self._orion, file_path)
        result = self._invoker.execute(command)
        self._notify_observers("load_orion", result)
        return result

    def save_orion(self, file_path: str) -> str:
        """
        Save orion to JSON file.

        :param file_path: Path to save JSON file
        :return: The file path
        :raises: CommandExecutionError if operation fails
        """
        command = SaveOrionCommand(self._orion, file_path)
        result = self._invoker.execute(command)
        self._notify_observers("save_orion", result)
        return result

    def load_from_dict(self, data: Dict[str, Any]) -> TaskOrion:
        """
        Load orion from dictionary data.

        :param data: Dictionary representation of orion
        :return: The loaded orion
        """
        # Create temporary orion and copy state
        temp_orion = TaskOrion.from_dict(data)

        # Use build command to apply the state
        config = temp_orion.to_dict()
        return self.build_orion(config, clear_existing=True)

    def load_from_json_string(self, json_string: str) -> TaskOrion:
        """
        Load orion from JSON string.

        :param json_string: JSON string representation
        :return: The loaded orion
        """
        temp_orion = TaskOrion.from_json(json_data=json_string)
        config = temp_orion.to_dict()
        return self.build_orion(config, clear_existing=True)
    ...
    def validate_orion(self) -> tuple[bool, List[str]]:
        """
        Validate the orion structure.

        :return: Tuple of (is_valid, list_of_errors)
        """
        return self._orion.validate_dag()
    ...
    def get_topological_order(self) -> List[str]:
        ...
        :raises: ValueError if orion has cycles
        """
        return self._orion.get_topological_order()

    def has_cycles(self) -> bool:
        """Check if the orion has any cycles."""
        return self._orion.has_cycle()

    def get_ready_tasks(self) -> List[TaskStar]:
        """Get tasks that are ready to execute."""
        return self._orion.get_ready_tasks()

    def get_statistics(self) -> Dict[str, Any]:
        """Get orion statistics."""
        stats = self._orion.get_statistics()
        stats.update(
            {
    ...
    def create_subgraph(self, task_ids: List[str]) -> "OrionEditor":
        """
        Create a new editor with a subgraph containing specified tasks.

        :param task_ids: List of task IDs to include in subgraph
        :return: New OrionEditor with subgraph
        """
        subgraph_orion = TaskOrion(
            name=f"{self._orion.name}_subgraph"
        )
        subgraph_editor = OrionEditor(subgraph_orion)
    ...
    def merge_orion(
        self, other_editor: "OrionEditor", prefix: str = ""
    ) -> None:
        """
        Merge another orion into this one.

        :param other_editor: OrionEditor to merge from
    ...
    def display_orion(self, mode: str = "overview") -> None:
        """
        Display the orion using visualization.

        :param mode: Display mode ('overview', 'topology', 'details', 'execution')
        """
        self._orion.display_dag(mode)
    ...
    def execute_command_by_name(self, command_name: str, *args, **kwargs) -> Any:
        ...
        command = command_registry.create_command(
            command_name, self._orion, *args, **kwargs
        )
        if command is None:
    ...
    def __str__(self) -> str:
        ...
        return (
            f"OrionEditor("
            f"orion={self._orion.orion_id}, "
            f"tasks={len(self._orion.tasks)}, "
            f"dependencies={len(self._orion.dependencies)}, "
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"OrionEditor("
            f"orion_id={self._orion.orion_id!r}, "
            f"tasks={len(self._orion.tasks)}, "
            f"dependencies={len(self._orion.dependencies)}, "
