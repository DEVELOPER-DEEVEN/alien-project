...
from network.core.types import ExecutionResult
...
class WebSocketConnectionManager:
    ...
    async def connect_to_device(
        ...
        try:
            ...
            if not success:
                ...
                raise ConnectionError("Failed to register orion client")
    ...
    async def _register_orion_client(self, device_info: AgentProfile) -> bool:
        """
        Register this orion as a client using AIP RegistrationProtocol.

        :param device_info: Device information to register with
        :return: True if registration successful, False otherwise
        """
        try:
            orion_client_id = f"{self.task_name}@{device_info.device_id}"
            ...
            # (don't use register_as_orion which calls receive_message)
            from aip.messages import (
                ClientMessage,
                ClientMessageType,
                ClientType,
    ...
    async def send_task_to_device(
        ...
        try:
            ...
            orion_task_id = f"{self.task_name}@{task_request.task_id}"

            # Create client message for task execution
            # Note: Orion sends ClientMessage.TASK to server, which is different
            ...
            task_message = ClientMessage(
                ...
                target_id=device_id,
                task_name=f"network/{self.task_name}/{task_request.task_name}",
            ...
            response = await asyncio.wait_for(
                self._wait_for_task_response(device_id, orion_task_id),
    ...
    async def request_device_info(self, device_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Note: We still use manual ClientMessage construction because orion
