...
@dataclass
class OrionConfig:
    """Configuration for the orion system"""
    ...
    @classmethod
    def from_file(cls, config_path: str) -> "OrionConfig":
        """
        Load configuration from a JSON or YAML file.

        :param config_path: Path to the configuration file
        :return: OrionConfig instance
        """
    ...
    @classmethod
    def from_json(cls, config_path: str) -> "OrionConfig":
        """
    ...
    @classmethod
    def from_args(cls, args: argparse.Namespace) -> "OrionConfig":
        """
    ...
    def from_env(cls) -> "OrionConfig":
        ...
        config.heartbeat_interval = float(
            os.getenv("ORION_HEARTBEAT_INTERVAL", config.heartbeat_interval)
        )
        config.max_concurrent_tasks = int(
            os.getenv("ORION_MAX_CONCURRENT_TASKS", config.max_concurrent_tasks)
        )

        # Load devices from environment
        devices_json = os.getenv("ORION_DEVICES")
        if devices_json:
            try:
    ...
    def add_device(
        ...
        :param server_url: ALIEN server WebSocket URL
...
def setup_argument_parser() -> argparse.ArgumentParser:
    """
    Set up command line argument parser for orion configuration.
...
Examples:
  ...
  python -m alien.orion.client --config config/orion.json
  python -m alien.orion.client --config config/orion.yaml
  
  # Add devices via command line
  python -m alien.orion.client \\
    --device laptop_001:ws://192.168.1.100:5000/ws \\
    --device workstation_002:ws://192.168.1.101:5000/ws
  
  # Create sample config
  python -m alien.orion.client --create-sample-config config/sample.json
  python -m alien.orion.client --create-sample-config config/sample.yaml
        """,
    )

    parser.add_argument(
        "--config", "-c", type=str, help="Path to orion configuration file"
    )

    parser.add_argument(
        "--orion-id",
        type=str,
        default="orion_orchestrator",
        help="Unique identifier for this orion instance",
    )
...
if __name__ == "__main__":
    ...
    if args.create_sample_config:
        ...
        if args.config:
            ...
            config = OrionConfig.from_args(args)
