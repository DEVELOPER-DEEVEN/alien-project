...
class DeviceInfoProtocol(AIPProtocol):
    ...
    Handles:
    - Device info requests from network
    ...
    async def request_device_info(
        ...
        """
        Request device information (network-side).

        :param network_id: network client ID
        ...
        self.logger.info(
            f"Sent device info request: {network_id} → {target_device}"
        )
