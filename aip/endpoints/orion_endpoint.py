...
Wraps the existing Network orion client with AIP protocol abstractions.
"""
...
class OrionEndpoint(AIPEndpoint):
    """
    Orion Client endpoint for AIP.
    ...
    def __init__(
        ...
        """
        Initialize orion endpoint.

        :param task_name: Task name for this orion
        ...
        self.logger = logging.getLogger(f"{__name__}.OrionEndpoint")

    async def start(self) -> None:
        """Start the endpoint."""
        self.logger.info(f"Orion endpoint started for {self.task_name}")

    async def stop(self) -> None:
        """Stop the endpoint and disconnect all devices."""
        self.logger.info("Stopping orion endpoint")
    ...
    async def on_device_disconnected(self, device_id: str) -> None:
        ...
        :param device_id: Device ID
        """
        self.logger.warning(f"Device {device_id} disconnected from orion")
