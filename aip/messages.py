...
Key Concepts:
    - ClientType: Distinguishes between device agents and orion clients
...
class ClientType(str, Enum):
    ...
    ORION: An orchestrator that manages multiple devices
    """
...
class ClientMessage(BaseModel):
    ...
    Fields:
        ...
        status: Task status
        client_type: Type of client (DEVICE or ORION)
        session_id: Unique session identifier
        task_name: Human-readable task name
        client_id: Unique client identifier
        target_id: Target device ID (for orion clients)
    ...
    target_id: Optional[str] = Field(
        default=None, description="Target device ID (for orion)"
    )
...
class MessageValidator:
    ...
    def validate_registration(msg: ClientMessage) -> bool:
        ...
        if not msg.client_id:
            return False
        if msg.client_type == ClientType.ORION and not msg.target_id:
            # Orion clients should specify target device
