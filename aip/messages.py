...
Key Concepts:
    - ClientType: Distinguishes between device agents and network clients
...
class ClientType(str, Enum):
    ...
    network: An orchestrator that manages multiple devices
    """
...
class ClientMessage(BaseModel):
    ...
    Fields:
        ...
        status: Task status
        client_type: Type of client (DEVICE or network)
        session_id: Unique session identifier
        task_name: Human-readable task name
        client_id: Unique client identifier
        target_id: Target device ID (for network clients)
    ...
    target_id: Optional[str] = Field(
        default=None, description="Target device ID (for network)"
    )
...
class MessageValidator:
    ...
    def validate_registration(msg: ClientMessage) -> bool:
        ...
        if not msg.client_id:
            return False
        if msg.client_type == ClientType.network and not msg.target_id:
            # network clients should specify target device
